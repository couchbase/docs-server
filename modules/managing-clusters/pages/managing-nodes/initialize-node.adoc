...
node; using the
xref:managing-clusters:managing-nodes/initialize-node.adoc#initialize-a-node-with-the-cli[CLI],
and the
...
xref:cli:cbcli/node-init.adoc[node-init] command.
...
xref:managing-clusters:managing-nodes/provision-node.adoc[Provision
a Node]
for details.
