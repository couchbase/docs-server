...
xref:managing-clusters:managing-nodex/failover-graceful.adoc#graceful-failover-with-the-cli[Graceful
...
Perform full recovery of the failed over node by means of the
`recovery` command as follows:
...
xref:cli:cbcli/couchbase-cli-rebalance.adoc[rebalance]; on `recover`, that at
