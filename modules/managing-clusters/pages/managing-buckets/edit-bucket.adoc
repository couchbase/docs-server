...
image::managing-buckets/bucketsViewInitialEdit.png[,880,align=left]
...
image::managing-buckets/bucketsViewWithExpandedBucketRow.png[,820,align=left]
...
[#edit_bucket_button]
image::managing-buckets/editBucketButton.png[,260,align=left]
...
xref:managing-buckets/create-bucket.adoc[Create a Bucket]
...
* *Flush*: This setting enables or disables the
xref:managing-buckets/flush-bucket.adoc[Flush] command for the current bucket.
...
+
[#flush_bucket_button]
image::managing-buckets/flushBucketButton.png[,360,align=left]
+
If flushing is _disabled_, the *Flush* button does not appear.

== Changing Bucket-Settings with the CLI and REST API

You can change bucket-settings using the CLI command xref:cli:cbcli/couchbase-cli-bucket-edit.adoc[bucket-edit]; or the xref:rest-api:rest-bucket-parameters.adoc[Bucket REST API].
