...
+
An Ephemeral bucket handles running out of RAM quota in one of two ways, depending on how you configured it:

 ** Resident data items stay in RAM.
Couchbase Server does not allow you to add more data.
Instead, it returns an error.

 ** The system ejects data from RAM to make space for new data.
When an Ephemeral bucket removes data from memory, you cannot recover it.
