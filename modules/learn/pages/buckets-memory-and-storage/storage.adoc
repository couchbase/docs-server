...
At node-initialization, Couchbase Server allows up to four custom paths to be established, for the saving of data to the filesystem: the Data Service, the Index Service, the Analytics Service, and the Eventing Service can thereby each be assigned their own, separate data path. Note that these paths are node-specific: consequently, the data for any of these services may occupy a different filesystem-location, on each node.
