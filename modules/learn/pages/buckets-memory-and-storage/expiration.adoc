...
As described in the _Expiration Overview_ section of xref:java-sdk:howtos:kv-operations.adoc#document-expiration[Key Value Operations], an expiration time can be specified per item, by means of Couchbase SDK APIs: this is referred to as the item’s TTL.
