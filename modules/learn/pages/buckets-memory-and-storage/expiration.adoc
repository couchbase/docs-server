...
As described in the _Expiration Overview_ section of xref:3.0@jjava-sdk:howtos:kv-operations.adoc#document-expiration[Core Operations], an expiration time can be specified per item, by means of Couchbase SDK APIs: this is referred to as the item’s TTL.
