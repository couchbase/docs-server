...
This choice affects the minimum memory requirements for the bucket.
The 128 vBucket configuration is the default for new buckets in Couchbase Server Enterprise Edition 8.0 and later.
It has a minimum memory requirement of 100{nbsp}MB per node.
...
Magma using 1024 vBuckets requires a minimum of 1{nbsp}GiB of memory per node.

As your dataset grows, Magma has a minimum memory-to-data ratio of 1%.
For example, a node with 5{nbsp}TB  data in a Magma bucket must have 64{nbsp}GiB of RAM allocated to the bucket.
