...
The maximum thread-allocation that can be specified for each is _64_, the minimum _0_.
If _0_ (which is the default) is passed, Couchbase Server itself calculates and applies an appropriate number of threads.
...
Note also that number of threads can also be configured for the _NonIO_ and _AuxIO_ thread pools:
...
Again, the maximum thread-allocation that can be specified for each is _64_, the minimum _0_; with the default _0_ specifying that Couchbase Server itself should calculate and apply an appropriate number of threads.
