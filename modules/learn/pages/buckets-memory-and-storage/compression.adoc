...
The _compression ratio_ is a cluster-wide parameter that specifies the minimum required size of a document _before_ compression, in relation to its predicted size _after_ compression.
If the document to be compressed does not meet this requirement, compression is not performed, and the document is retained in uncompressed form: this ensures that server-resources are conserved, rather than being used in a compression-routine that would provide only minimal size-reduction.

The cluster-wide parameter-value is _1.2_: this _cannot_ be modified.

Therefore, if the size of a document before compression is `bc`, and its predicted size after compression is `ac`, compression is performed only if `bc >= (1.2 * ac)`.
In cases where `bc < (1.2 * ac)`, compression is not performed: instead, the document is retained in uncompressed form.
