...
Therefore, if the size of a document before compression is `bc`, and its size after compression is `ac`, the document is retained in compressed form only if `bc >= (1.2 * ac)`.
In cases where `bc < (1.2 * ac)`, the document is retained in uncompressed form.
