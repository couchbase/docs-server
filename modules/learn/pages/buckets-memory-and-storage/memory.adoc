...
The xref:services-and-indexes/services/data-service.adoc[Data Service] must run on at least one node in any cluster. Every xref:buckets-memory-and-storage/buckets.adoc[bucket] you create on a node has its own memory quota. 
