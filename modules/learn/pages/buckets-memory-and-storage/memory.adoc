...
For more information about Couchbase bucket ejection policies, the blog post https://blog.couchbase.com/a-tale-of-two-ejection-methods-value-only-vs-full/[A Tale of Two Ejection Methods: Value-only vs. Full^]
...
Memory can become available when users delete documents or documents expiring because of xref:learn:data/expiration.adoc[expiration settings]). 
...
Ejecting data from a Couchbase bucket does not remove the data from disk, so to it's still available. 
The only effect is that next access to the data is slower because the Data Service has to read it from disk instead of from memory.
====
...
* Perform a graceful failover followed by a full recovery and rebalance of each node running the Data Service
