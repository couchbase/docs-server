...
Value-only::
The bucket only ejects a document's data when it ejects it from memory. 
It keeps the document's keys and metadata in memory.
Retaining the keys and metadata help limkt the performance impact of ejecting the document from memory.
...
Full::
The bucket removes the entire document, including its metadata and keys, when it ejects a document from memory. 
...
No ejection::
If the bucket reaches its memory quota, the bucket does not eject data.
Instead, it refuses to load any new data until memory becomes available (for example, by users deleting documents or documents expiring due to xref:learn:data/expiration.adoc[expiration settings]). 
Eject data when RAM is full::
If the bucket approaches its memory quota, the bucket ejects documents to make space for new data. 
It chooses the documents to eject based on the Not Recently Used (NRU) algorithm, which uses metadata to determine which documents have not been accessed recently.
...
For example, suppose you're changing a bucket from using the xref:learn:buckets-memory-and-storage/storage-engines.adoc#storage-engine-couchstore[Couchstore storage engine] to the xref:learn:buckets-memory-and-storage/storage-engines.adoc#storage-engine-magma[Magma storage engine].
...
[#watermarks]
=== Memory Watermarks
For each bucket, Couchbase Server manages available memory using two watermarks: `mem_low_wat` and `mem_high_wat`.
The `mem_high_wat` watermark is the threshold where Couchbase Server takes action to prevent the bucket from exceeding its memory allocation.
...
It continues ejecting items until the bucket's memory use drops to the `mem_low_wat` watermark.
...
You can change the default settings using the `cbepctl` utility.
