...
:description: pass:q[Using this model as a template, it is possible to write the full implementation of the built-in functions `_sum` and `_count` when working with the sales data and the standard `map()` function below:]
:page-aliases: views:views-writing-rewriting,understanding-couchbase:views/views-writing-rewriting
