...
* On the target bucket, a collection is created that allows a new, _implicit_ mapping to occur between a source collection and the new target collection: a _backfill_ process is automatically started, to replicate documents between these collections, as appropriate.
For information, see xref:learn:clusters-and-availability/xdcr-with-scopes-and-collections.adoc#xdcr-with-scopes-and-collections[XDCR with Scopes and Collections].
