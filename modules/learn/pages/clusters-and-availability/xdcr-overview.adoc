...
Thus, XDCR can be used to _migrate_ the data from its bucket on the pre-7.0 cluster to a bucket the 7.0+ cluster, and thereby to assign the data to appropriate scopes and collections defined within the new bucket (again, optionally using XDCR Advancing Filtering).
