...
Thus, XDCR can replicate data from its current source, into a specified collection, within a target bucket on a 7.0+ cluster.
...
If, for a given document, one or both criteria are not satisfied, the document is _dropped_ from memory by XDCR, and therefore not replicated: however, the attempted replication of other documents is continued.
...
* On the target bucket, a collection is created that allows a new, _implicit_ mapping to occur between a source collection and the new target collection: a _backfill pipeline_ is automatically created, to replicate documents between these collections, as appropriate.
