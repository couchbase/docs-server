...
To manage system resources in these circumstances, each replication can be assigned a priority of _High_, _Low_, or _Auto_:

* _High_. No resource constraints are ever applied to the replication.
...
* _Auto_. Resource constraints are applied to the replication while its _initial process_ is underway.
Subsequently, it is made _High_ priority.
