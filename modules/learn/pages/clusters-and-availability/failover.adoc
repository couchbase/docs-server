...
Such a configuration is possible in Couchbase Enterprise Server 7.6+: one or more arbiter nodes can be added to the cluster, with the intention of ensuring that the Master Services will occupy such a node, and thereby not be co-located with any service.
