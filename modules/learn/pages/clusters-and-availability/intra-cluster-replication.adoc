...
[abstract]
Intra-cluster replication replicates data across the nodes of a cluster, by means of the _Data Change Protocol_.
...
See xref:learn:clusters-and-availability/failover.adoc[Failover].
...
For example, in a five node cluster with three replicas, a single node can be failed over without danger: if a second node fails, failover might result in data-loss, due to required replicas no longer being available.
...
See xref:manage:manage-nodes/fail-nodes-over.adoc[Fail a Node over and Rebalance].
