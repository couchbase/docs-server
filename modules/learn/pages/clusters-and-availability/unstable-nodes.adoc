...
The kernel may respond by refusing connections and dropping packets, which can cause nodes to become unreachable.
...
To track instability, you can monitor the xref:metrics-reference:ns-server-metrics.html[`cm_node_unreachable_total`] metric. 
It counts each time a node in the cluster is unable to connect to another node.
If you see a pattern of multiple nodes reporting the same node as unreachable, but that node recovers, it may be unstable.

[#what-metric-reports]
== What the Metric Reports

The `cm_node_unreachable_total` is a cross-node counter metric that reports how many times a node has been unable to reach another node. 
...
Each entry represents a node reporting another node as unreachable for a specific reason.
For example:
...
* The `instance` is the node which is reporting another node as being unreachable.
...
+
    ...
    ** `no_network`: The reporting node's network is available.
...
In the previous example, `node1.example.com` has reported that it was unable to reach `node4.example.com` 26 times due to an network distribution heartbeat timeout.
...
When a node is unstable, you see a patten of multiple nodes incrementing their `cm_node_unreachable_total` counts for a particular node. 
...
The lines in the graph show nodes incrementing their `cm_node_unreachable_total` metric counters to report `node4.example.com` was unreachable.
