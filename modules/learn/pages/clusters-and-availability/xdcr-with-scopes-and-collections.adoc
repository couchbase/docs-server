...
_Explicit_ mapping is administrator-specified, and allows replication to occur between keyspaces that are not identical.
...
However, multiple source collections can be mapped (by means of multiple rules) to a single target collection.
For example, the expression `{"inventory.airport":"MyInventory.MyAirport", "inventory.MyAirport":"MyInventory.MyAirport"}` is valid, and affirms replication from the collections `airport` and `MyAirport`, located within the source-scope `inventory`, to the collection `MyAirport`, located within the target-scope `MyInventory`.

If the replication of multiple source-collections to a single target collection results in conflicts, the procedure for xref:learn:clusters-and-availability/xdcr-conflict-resolution.adoc[XDCR Conflict Resolution] that has been established for the bucket is applied.
...
=== A Note on Source-Collection Removal
