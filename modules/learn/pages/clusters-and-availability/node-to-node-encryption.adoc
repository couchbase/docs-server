...
+
Note, however, that node-to-node encryption has _not_ been implemented for the Eventing Service in versions of Couchbase Server prior to 7.0.2.
Therefore, on such earlier versions, when _all_ is specified, whereas the data passed to and from all other Couchbase Services is passed in encrypted form, the data passed to and from the Eventing Service continues to be passed in _unencrypted_ form.
