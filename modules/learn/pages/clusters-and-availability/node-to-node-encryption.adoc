...
+
The address family is changed by means of the xref:cli:cbcli/couchbase-cli-change-ip-family.adoc[change-ip-family] CLI command.

. Enable cluster encryption.
Cluster encryption must be specifically enabled: by default, it is disabled.
To enable or disable cluster encryption, or to determine the current setting, use the xref:cli:cbcli/couchbase-cli-change-cluster-encryption.adoc[change-cluster-encryption] CLI command.
...
. Use xref:cli:cbcli/couchbase-cli-change-cluster-encryption.adoc[change-cluster-encryption], to disable node-to-node encryption.
...
. Re-enable node-to-node encryption, using xref:cli:cbcli/couchbase-cli-change-cluster-encryption.adoc[change-cluster-encryption], and establish the appropriate level of encryption.
