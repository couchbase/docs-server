...
Before the certificates for a cluster are created or rotated, node-to-node encryption should be disabled.
Subsequently, it can be re-enabled:
...
. Rotate certificates: see the procedures in xref:manage:manage-security/configure-server-certificates.adoc[Configure Server Certificates].
