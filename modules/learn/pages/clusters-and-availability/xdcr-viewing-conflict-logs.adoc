...
If the documents are large such that the combined 3 documents are larger than the maximum xref:learn:clusters-and-availability/size-limitations.adoc[size limit], you can copy only a subset of the fields from the `src_` and `tgt_` documents to the custom single document.
