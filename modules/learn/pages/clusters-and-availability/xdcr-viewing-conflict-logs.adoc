...
To use only KV, use Couchbase SDK with KV APIs including xref:java-sdk:howtos:kv-operations.adoc#kv-range-scan[KV Range Scan].

For example, to get a list of all `crd` prefix documents (CRD documents) using the KV Range Scan xref:java-sdk:howtos:kv-operations.adoc#kv-range-scan-prefix[PrefixScan], use KV `collection.get` to get the conflicting `src` and `tgt` docs.
...
For more information about KV Range Scan operations and data operations, see xref:java-sdk:howtos:kv-operations.adoc[Data Operations]. For more information about data operations on sub-documents, see xref:java-sdk:howtos:subdocument-operations.adoc[Sub-Document Operations]
