...
To ensure cluster-security, in Couchbase Server 7.1.1 and later versions, restrictions can be placed on addition and joining, based on the establishment of _node-naming conventions_.
...
NOTE: In Couchbase Enterprise Server 7.2 and later versions, when certificates are used for cluster authentication, each node certificate must be configured with the node-name correctly specified as a Subject Alternative Name (SAN).
...
NOTE: In Couchbase Enterprise Server 7.2 and later versions, the node-name _must_ be correctly identified in the node certificate as a Subject Alternative Name.
