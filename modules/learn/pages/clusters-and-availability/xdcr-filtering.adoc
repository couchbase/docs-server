...
Note that filter-evaluation is _not_ performed based on the measurement of elapsed time: therefore, the replication of a document cannot be triggered by establishing a filter on a time-value within the document.
For example, a filter that associates a future time, specified as a numeric value, with a key named `expiration_time` does _not_ trigger replication when the specified time is reached.
