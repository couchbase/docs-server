...
Therefore, to ensure that document-deletions are _not_ replicated, deletion filters must be appropriately configured.

=== Failure to Configure Deletion Filters

In certain circumstances, failure to configure deletion filters so as to prevent the replication of deletions may cause loss of data.
For example:
...
The expected outcome is that documents of both type _A_ and type _B_ continue to exist on the target.
...
To avoid this outcome, replication _2_ should be created with deletion filters configured to prevent the replication of deletions.
The prior deletions of documents of type _A_ from the source are thereby _not_ replicated to the target.

=== Deletion Filters and Migration

The appropriate configuring of deletion filters is critically important in cases of _migration_ where documents are being assigned to newly created collections, with their source collection (likely the default collection of a legacy bucket) intended subsequently to be dropped.
