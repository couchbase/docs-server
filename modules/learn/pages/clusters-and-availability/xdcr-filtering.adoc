...
* Document ID References Only: The filter expression must evaluate only the document key ID (META().id).
Expressions attempting to match JSON document fields (such as type = "user") or extended attributes will fail validation and will not be able to be used with `filterDeletionsWithExpression=true` or `filterExpirationsWithExpression=true`.
