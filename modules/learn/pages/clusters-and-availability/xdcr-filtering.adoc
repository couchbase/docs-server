...
These filters (which are fully described in xref:manage:manage-xdcr/filter-xdcr-replication.adoc#deletion-filters[Deletion Filters]) respectively control the replication of _deletions_, _expirations_, and document _TTL_ settings.
All three should be understood; but an understanding of _deletion_ and _expiration_ filtering is critical, for the successful and safe management of replication and migration.

In this section, the term _deletion filters_ is used to refer collectively to the filters for _deletions_ and for _expirations_; and all statements characterizing document _deletion_ apply equally to document _expiration_.
...
+
Note, however, that if read-write application-access continues to be granted to the source collection during the life of the migration, application-deletions that occur on the source are not replicated to the target collection; eventually rendering source and target collections inconsistent.
...
+
Note that if a new replication is subsequently created between the same source and target collections, with deletion filters configured to permit the replication of deletions, the deletions will be replicated to the target if the tombstones produced by the source-data deletions have not yet been purged.
