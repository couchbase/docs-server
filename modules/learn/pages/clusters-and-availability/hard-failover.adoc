...
Note, however, that since, in Couchbase Server 7.0+, topology-related metadata is managed with high consistency by means of a _consensus_ methodology, node-loss due to unsafe failover may result in loss of topology data, with significant negative consequences.
