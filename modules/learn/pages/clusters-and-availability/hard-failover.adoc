...
Hard failover must be performed according to one of the following, two constraints:

* _Safe_ (the default constraint).
When hard failover is initiated, it is continued only:

** In circumstances where data-loss cannot result.

** When a majority of nodes can still be contacted, following a node’s becoming unresponsive.

** When a majority of groups can still be contacted, following a group’s becoming unresponsive.

* _Unsafe_.
When hard failover is initiated, it is continued regardless of circumstances or consequences.
_Unsafe_ hard failover is therefore dangerous, and should only be used when absolutely no alternative is available.
