...
[#safe-and-unsafe]
== Hard-Failover Modes: Safe and Unsafe

Either of the following two _modes_ can be specified for hard failover:

* _Safe_ (the default mode).
When hard failover has been initiated, it is continued by Couchbase Server only in the following circumstances:

** Data-loss cannot result.

** A majority of nodes can still be contacted, following a node’s becoming unresponsive.

** A majority of groups can still be contacted, following a group’s becoming unresponsive.

* _Unsafe_.
When hard failover is initiated, it is continued by Couchbase Server regardless of the constraints that characterise _safe_ mode.
+
_Unsafe_ hard failover is therefore dangerous, and should only be used when no alternative is available.

Note that _safe_ hard failover is the only hard failover performed by xref:learn:clusters-and-availability/automatic-failover.adoc[Automatic Failover].

For information on the practical steps required to perform hard failover in both safe and unsafe mode, see xref:manage:manage-nodes/failover-hard.adoc[Perform Hard Failover].
