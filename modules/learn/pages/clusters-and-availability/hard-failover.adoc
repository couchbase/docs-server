...
The Eventing Service (like most other distributed systems) provides an _At-least-once_ guarantee.
The processing of duplicate mutations happens only on the occurrence of edge-conditions that involve failure; and can happen only within a limited time-period, following the last completed DCP checkpoint.
