...
If cluster contains a single node that hosts the Eventing Service, and this node undergoes hard failover, the Eventing Service on the node stops, and ongoing mutations on the node are interrupted: this potentially results in data loss.
...
If multiple cluster-nodes host the Eventing Service, responsibility for handling data-mutations is divided between these nodes; each node handling the data-mutations for a defined subset of vBuckets.
If a hard failover is performed on one of the Eventing-Service nodes:

* On Couchbase Server version 6.6 and earlier, mutations that were to be handled by the failed-over node are lost, until a subsequent rebalance is performed; at which point the failed-over node's former responsibilities are assigned to the surviving Eventing-Service nodes.

* On Couchbase Server version 6.6.1 and later, the failed-over node's former responsibilities are assigned to the surviving Eventing-Service nodes as part of the hard-failover process, thereby minimizing mutation-loss.
When hard failover is, in these circumstances, selected by means of the Couchbase Web Console UI, a notification such as the following is provided, when failover-confirmation is requested: _Failover of this node will trigger internal processing after failover for the following service: Eventing._
_This processing may take some time to complete._

Note that vBucket reallocations that occur due to failover may themselves lead to mutations being interrupted, and therefore potentially to data loss.
