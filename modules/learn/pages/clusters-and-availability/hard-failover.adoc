...
In the event of a cluster’s xref:learn:clusters-and-availability/cluster-manager.adoc#master-services[Master Services] not being able to contact a majority of the cluster’s nodes (sometimes referred to as a _quorum failure_), an attempted hard failover will, by default, _not_ be executed.
...
If, following a quorum failure, an administrator is confident that simultaneous hard failovers will not occur across the paritioned cluster, a single hard-failover can be performed in _unsafe_ mode; this is executed even when a majority of cluster-nodes cannot be contacted.

Note, however, that since, in Couchbase Server 7.0+, topology-related metadata is managed with high consistency by means of a _consensus-based_ protocol, node-loss due to unsafe failover may result in loss of topology data, with significant negative consequences.
For example, information on the recent creating or dropping of buckets, scopes, and collections may be lost.
In consequence, an unsafe failover should only be performed when it is determined either that no such loss will occur; or that the likely consequences are acceptable.
