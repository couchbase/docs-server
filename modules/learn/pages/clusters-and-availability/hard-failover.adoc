...
However, in the case one other node is running the Search service and a replica had been configured for the index, queries will continue to get full results as the replica will be promoted to active status immediately upon fail over.

When rebalance occurs:

* If replicas have been configured for the Full Text Indexes, the Search Service will generate new replica index partitions if the cluster size permits it.
