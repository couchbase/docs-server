...
NOTE: In Couchbase Server versions 7.6.0 and 7.6.1, when you enabled file-based rebalance you could not choose which Index Service node would contain an index.
...
* If you enable file-based index rebalance on a new cluster before you create any indexes, Couchbase Server uses  file-based index rebalance for all rebalances.
By enabling the setting before creating any index lets Couchbase Server configure the indexes to use file-based index rebalance as you create them.  

* If you enable file-based index rebalance after you have created indexes, Couchbase Server does not use the file-based rebalance method for the next rebelance. 
Instead, the next rebalance process still rebuilds the indexes. 
During this rebuild process, Couchbase Server adds metadata to the index files that enables file-based rebalance. 
After this initial rebalance, all later rebalance processes move the index files as long you as you keep file-base rebalance enabled.
