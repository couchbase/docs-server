...
During a rebalance process, indexes are relocated and rebuilt in batches.
The default batch size is 3 to set a limit on the amount of work attempted concurrently.
...
* Increases rebalance performance by increasing pipeline parallelism by starting the next batch when the previos batch is only partially completed.
...
You can change the default batch size of 3 by the Full Admin and Cluster Admin roles using the REST API. For information, see xref:rest-api:rest-modify-index-batch-size.adoc[Modify Index Batch Size].
