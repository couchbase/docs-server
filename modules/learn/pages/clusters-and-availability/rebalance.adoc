...
The addition or removal of Query Service nodes during rebalance is immediately effective: an added node is immediately available to serve queries. A removed node will complete all queries and in-flight transactions in progress before shutting down and completing the rebalancing. During the shutdown, the query service will not accept new requests.
