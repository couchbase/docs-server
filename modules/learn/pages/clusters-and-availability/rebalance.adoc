...
When rebalance occurs, indexes and relocated and rebuilt in _batches_, so as to place a limit on the amount of work that is attempted concurrently -- the default batch size being `3`.
...
* Increases rebalance performance by increasing pipeline parallelism.
