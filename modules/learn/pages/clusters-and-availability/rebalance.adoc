...
// Hiding the first for now because its metric is marked as internal.
// * xref:metrics-reference:data-service-metrics.adoc#kv_file_chunk_read_bytes[`kv_file_chunk_read_bytes`]: the number of bytes a node has sent to other nodes for FBR.
* xref:metrics-reference:data-service-metrics.adoc#kv_ep_snapshot_read_bytes[`kv_ep_snapshot_read_bytes`]: the number of bytes a node has read from other nodes files for FBR.
* xref:metrics-reference:data-service-metrics.adoc#kv_ep_dcp_cache_transfer_read_bytes[`kv_ep_dcp_cache_transfer_read_bytes`]: the number of bytes a node has read from other nodes caches for FBR.
