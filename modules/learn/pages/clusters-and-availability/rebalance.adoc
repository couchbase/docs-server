...
Couchbase Server either rebuilds indexes in their new locations or moves index files between nodes during a rebalance,
depending on the cluster's index storage mode and on configuration settings.
For more information about the rebalance operation on Index Service, see xref:learn:clusters-and-availability/rebalance-and-index-service.adoc[Rebalance and Index Service].
