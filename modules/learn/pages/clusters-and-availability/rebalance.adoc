...
When a node is removed and rebalanced, the Query Service allows existing queries and transactions to complete before shutting down.
...
To monitor the shutdown progress, you can check the Query Service diagnostic log on the node(s) being removed.
These logs contain messages indicating how many transactions and queries are still running.

During shutdown, any new connection attempts to the node will result in error 1180 (`E_SERVICE_SHUTTING_DOWN`), indicating the service is shutting down and cannot accept new requests. 
You may also encounter error 1181 (`E_SERVICE_SHUT_DOWN`), in the brief period between the completion of the last statement or transaction and the service exiting. 
