...
After this initial rebalance, all later rebalances use the file-base rebalance method as long you as you keep the file-base rebalance enabled.
...
You can have a rebalance redistribute indexes im more cases by enabling the **Optimize Index Placement On Rebalance** setting on the Couchbase Server Web Console's xref:manage:manage-settings/general-settings.adoc#index-storage-mode[Settings page]. 
You can also change this setting using the xref:manage:manage-settings/general-settings.adoc#index-settings-via-rest[Settings REST API] where it is named `redistributeIndexes`.
