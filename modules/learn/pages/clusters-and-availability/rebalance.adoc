...
It also adds an additional cache transfer phase beyond DCP's 2-step process for replica vBucket transfers:

image::clusters-and-availability/rebalance-stages-fbr-1.svg[align=left]

It also adds the cache transfer phase for active vBucket transfers:
...
In Couchbase Server {totoro}, FBR uses a separate concurrent-moves setting, `dataServiceFileBasedRebalanceMovesPerNode`, which is independent of the DCP `rebalanceMovesPerNode` setting.
