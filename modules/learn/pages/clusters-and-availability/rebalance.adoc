...
The following sections explain the rebalance operation effects on non-Data Services.
...
Couchbase Server either rebuilds indexes (using DCP-based method) in their new locations or
moves index files between nodes (#file-based-rebalance-fbr[File-Based Rebalance]) during a rebalance,
