...
The stages differ, depending on whether the vBucket is an _active_ or a _replica_ vBucket.
When rebalance occurs, Couchbase Web Console provides a dialog that shows rebalance-progress per bucket: the average amount of time required for each stage of a vBucket move is displayed, and continuously updated.
See xref:manage:manage-nodes/add-node-and-rebalance.adoc[Add a Node and Rebalance] for information.
...
The stages in which rebalance moves a replica vBucket are shown by the following illustration.
...
Phase 1, _Backfill_, and Phase 2, _Book-keeping_, are identical to those required for replica vBuckets; except for the presence of additional _Persistence Time_, which is required as part of _Book-keeping_ for an active-vBucket move.

Phase 3, _Active Takeover_, comprises the operations required to identify the relocated vBucket as the new active copy.
...
For information on failure-notifications, and options for cancelling rebalance-retries, see xref:manage:manage-nodes/add-node-and-rebalance.adoc[Add a Node and Rebalance].
