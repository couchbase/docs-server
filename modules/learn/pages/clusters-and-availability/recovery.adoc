...
(Note that in this case, a Swap Rebalance can be performed instead.)
...
However, in cases where the node’s memory-footprint is extremely large, and -size exceeds bucket memory-quotas, the memory-management overhead potentially entailed by Delta recovery might imply Full recovery’s taking less time overall.
...
To select a recovery option with the Couchbase REST API, see Setting Recovery Type.
To perform the subsequent rebalance, see Rebalancing Nodes
