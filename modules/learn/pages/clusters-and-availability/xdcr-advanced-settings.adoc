...
| Defines whether documents are to be compressed for XDCR, and if so, which  compression type is to be used.The default is `Auto`, which will allow compression if both source and target support it.
...
| Specifies the upper limit for network usage during replication, per source node, in MB per second.
