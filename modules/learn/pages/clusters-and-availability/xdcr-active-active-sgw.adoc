...
* Eventing Service cannot be used with Sync Gateway and bi-directional XDCR. If used with Sync Gateway in a bi-directional, active-active XDCR environment, the updates of Eventing Service metadata in the source and the target clusters causes XDCR to ping-pong and never stop replicating. One-way replication is possible.
====
...
Set the bucket property `enableCrossClusterVersioning` to use the setting `mobile=Active` during the processes xref:manage:manage-xdcr/create-xdcr-replication.adoc[Create a Replication] and xref:manage:manage-xdcr/xdcr-management-overview.adoc[Manage XDCR]. To enable the bucket property `enableCrossClusterVersioning` using REST API or from the UI, see xref:learn:clusters-and-availability/xdcr-enable-crossclusterversioning.adoc[XDCR enableCrossClusterVersioning].

For more information about the pre-requisites, see xref:learn:clusters-and-availability/xdcr-enable-crossclusterversioning.adoc/#bidirectional-replication-with-sgw-and-xdcr[Bi-directional replication with Sync Gateway and XDCR]
