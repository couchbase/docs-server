...
* In Couchbase Server versions earlier than 8.5, you could not use XDCR Active-Active with Sync Gateway if a document contained more than 10 user-created extended attributes (user xattrs).
This limitation no longer applies when both source and target clusters use Couchbase Server 8.5 and later.
