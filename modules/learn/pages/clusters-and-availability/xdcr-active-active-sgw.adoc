...
* Eventing Service cannot be used with Sync Gateway in bi-directional XDCR. If used with the _Sync Gateway in a bi-directional, active-active XDCR_ environment, the updates of Eventing Service metadata in the source and the target clusters causes XDCR to ping-pong and never stop replicating. One-way replication is possible.
====
