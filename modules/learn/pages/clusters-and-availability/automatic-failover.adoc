...
Specifically, the Cluster Manager autonomously detects and verifies that the node or group is unresponsive, and then initiates the _hard_ failover process, in _safe_ mode (see xref:learn:clusters-and-availability/hard-failover.adoc#safe-and-unsafe[Hard-Failover Modes: Safe and Unsafe].)
...
The final three constraints in the above list are those that characterise _safe_ hard failover; which is indeed the only hard failover that auto-failover performs.
