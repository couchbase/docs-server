...
* If the majority of nodes (latexmath:[\frac{nodes+1}{2}]) in the cluster can form a quorum to initiate failover following the failure of one or more nodes. Multiple concurrently failing nodes will be auto-failed over as long as the administrator specified limit is not exceeded.
* Sequentially, only up to an administrator-specified maximum number of nodes.
