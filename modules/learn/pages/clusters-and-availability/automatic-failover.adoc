...
Specifically, the Cluster Manager autonomously detects and verifies that the node or group is unresponsive, and then initiates the _hard_ failover process, with the _safe_ constraint applied (see xref:learn:clusters-and-availability/hard-failover.adoc#safe-and-unsafe[Safe and Unsafe].)
