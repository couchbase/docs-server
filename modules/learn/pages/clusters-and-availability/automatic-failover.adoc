...
The events that can trigger an auto-failover are:
...
+
See <<#auto-failover-and-ephemeral-buckets>> for more information bout auto-failover and ephemeral buckets.
...
If this setting is not enabled, there's a risk that the restarting node could cause data loss by rolling back asynchronous writes made to the replica vBuckets but the restarting node's vBuckets has missed. 
...
For example, suppose you use the unreplicated ephemeral bucket as a cache.
Then you should consider enabling this setting to allow Couchbase Server to auto-failover nodes containing its vBuckets.
...
If one of these nodes becomes unresponsive, you must manually fail it over.
