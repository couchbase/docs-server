...
[#automatic-failover-during-rebalance]
== Automatic Failover During Rebalance

Couchbase Server provides a setting to determine whether, once enabled, auto-failover can specifically be triggered during xref:learn:clusters-and-availability/rebalance.adoc[Rebalance], in the event of a node becoming unresponsive.
...
The rebalance may hang for an indeterminate period, before failure.
