...
See xref:learn:clusters-and-availability/groups.adoc[Server Group Awareness], for information abput server groups.

[#auto-failover-and-ephemeral-buckets]
== Auto-Failover and Ephemeral Buckets
Couchbase Server offers ephemeral buckets, which are buckets that are stored only in memory. 
...
To prevent this data loss, by default Couchbase Server does not allow auto-failover of a node that contains vBuckets for an unreplicated ephemeral.
In this case, you must manually fail over the node if it us unresponsive. 
...
When it's enabled, Couchbase Server automatically promotes the replicas of the ephemeral bucket on the restarting node.
If this setting is not enabled, there's a risk that the node could cause data loss by rolling back asynchronous writes in the replicas that were in progress when it restarted. 
This setting defaults to enabled.
When it's enabled, Couchbase Server automatically makes active the replicas of any active vBuckets on the restarting node.
If this setting is not enabled, there's a risk that the restarting node could cause data loss by rolling back asynchronous writes that the replica vBuckets have but the restarting node's vBuckets are missing. 
...
When Couchbase Server fails over a node with an unreplicated ephemeral bucket, the data in the active vBuckets on the node is lost.
...
For example, if you use the unreplicated ephemeral bucket for caching data, consider enabling this setting to allow Couchbase Server to auto-failover nodes containing its vBuckets.
...
+
If the data in the ephemeral bucket is critical for your application, enable one or more replicas for it. 
See xref:manage:manage-buckets/create-bucket.adoc#ephemeral-bucket-settings[Ephemeral Bucket Settings] for more information about adding replicas to an ephemeral bucket.
...
+
NOTE: If the unreplicated ephemeral bucket is indexed, Couchbase Server rebuilds the index after it auto-fails over the node even if the index is not on the failed node.
