...
Customers will notice this as the smaller server groups will "perform" worse than the larger server groups.
...
Note that in 7.1+, _automatic failover_ can fail over more than three nodes concurrently: this has permitted the removal of pre-7.1 interfaces that were specific to triggering _auto-failover for server groups_.
