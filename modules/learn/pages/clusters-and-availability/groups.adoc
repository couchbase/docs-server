...
[IMPORTANT]
====
The number of replicas placed in server groups depends on the number of configured server groups.
If there are at least two server groups, then we can broadly say that:

[stem]
++++
number\:of\:replicas = number\:of\:server\:groups - 1
++++

* If you have two server groups, then Couchbase will place one replica in each node of the other server group. +
* If you have three server groups then Couchbase will place two replicas of each bucket from one node of one server into each node of the other two servers.

Note that this is a change in behaviour from versions of Couchbase prior to 7.1x, where the number of replicas for the vBucket was, broadly speaking,  _equal_ to the number of groups in the rack.
This change of behaviour only comes into effect if the cluster is configured with more than one server group containing Data Service nodes.
====
