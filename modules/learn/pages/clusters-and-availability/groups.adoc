...
NOTE: In 7.1 and later versions, automatic failover can fail over more than three nodes concurrently: this has permitted the removal of pre-7.1 interfaces that were specific to triggering auto-failover for server groups.
...
Due to the removal of the pre-7.1 interfaces, applications that attempt to use the interfaces with 7.1 or a later version will _fail._
