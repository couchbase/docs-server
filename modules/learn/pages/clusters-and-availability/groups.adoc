...
Protection is optimal when groups are assigned _equal numbers of nodes_, and replica vBuckets and index replicas are therefore distributed such that none ever occupies the same group as its associated active vBucket or index.
By contrast, when groups are _not_ assigned equal numbers of nodes, rebalance can only produce a _best effort_ redistribution of replicas: this may result in one or more replica vBuckets and index replicas occupying the same group as their associated active vBucket and index; meaning that vBucket or index data may be lost if such a group becomes unavailable.
