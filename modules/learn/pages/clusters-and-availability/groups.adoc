...
*active balance*:: there should be approximately{empty}footnote:approximate-vbuckets[Note that by "approximately the same" we mean the number should vary by 5 vbuckets or less] the same number of active vbuckets on each node in the cluster.

*replica balance*:: there should be approximately{empty}footnote:approximate-vbuckets[] the same number of replica vbuckets on each node in the cluster.
...
when the vbucket map is generated, the aim is to ensure that there are the same number of active vbuckets on each cluster and that replicas of a given vbucket must reside in separate groups.
...
However, in order to ensure that replicas of a VBucket must reside in a separate group, then we may have a situation where there are vBuckets from Server 9 are replicated to Group 1, but there are no additional vBuckets in Group 1 to provide balance in Group 2.

NOTE: Since smaller server groups will carry more replicas on each node, their performance may not be the same as the nodes in the bigger server groups. 
For best performance, try to maintain equality of server groups.
