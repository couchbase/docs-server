...
Couchbase Server supports use of multiple CA (or _root_) certificates, for a single cluster.
...
[#multiple_root_certs_thumbnail]
xref:learn:security/using-multiple-cas.adoc#illustration[image:security/clusterWithCerts.png[,220]]

For detailed information, see xref:learn:security/using-multiple-cas.adoc[Using Multiple Root Certificates].
