...
Couchbase’s traditional strong consistency across single documents is now complemented by multi-document, distributed ACID transactions.
The flexible schema model inherent to JSON allows expressing data in a more natural way than would be typically found in an RDBMS, including the extension of the inherent schema over time.
...
Where a more immediate and guaranteed form of consistency may be required across documents, Couchbase offers _ACID Transactions_.
