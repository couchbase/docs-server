...
It also enables seamless integration between KV and xref:n1ql:n1ql-language-reference/begin-transaction.adoc#purpose[Query] operations within transactions.
...
// == Custom Metadata
include::3.2@java-sdk:howtos:distributed-acid-transactions-from-the-sdk.adoc[tag=custom-metadata]
...
//  as "I've actually made it through the API that you can't do an AT_PLUS query."
// commenting out, but keeping on hand as reminder that maybe this AT_PLUS limitation needs noting in another form!
////
...
// Commenting out for now - may need reinstating in some form if timing of releases means any customer confusion.
// or could be added in individual SDK's txn pages.
////
NOTE: SDK Transactions Version 1.0.x do not support N1QL operations, but can be used with Couchbase Server 6.6.1.
