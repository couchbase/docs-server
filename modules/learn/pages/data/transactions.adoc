...
`persistToMajority` provides the strongest protection from failures but is the least performant amongst the Durability levels. For more information, see xref:data/durability.adoc#durability-requirements[Durability Levels].
...
// * xref:ruby-sdk:howtos:distributed-acid-transactions-from-the-sdk.adoc[]
