...
_Transactions_ are provided by the Java SDK to support mutations across multiple documents.
Mutations are recorded per document as _meta data_.
When all documents in the transaction have had their intended mutations so recorded, durable writes are performed in sequence, one for each document; in order to commit the changes.

Note that transactions are supported by the Java SDK only: no other Couchbase SDK, service, or process recognizes these transactions in any way.
Therefore, services, processes, and non-Java-SDK applications continue to read, write, and replicate data without reference to the status of any ongoing transaction.
In consequence, the mutations durably written by a transaction across multiple documents are never guaranteed to reside in committed form on a Couchbase Server-cluster simultaneously; since any document may be further modified, immediately following the completion of its durable write.

Note also that if a document included in a transaction is modified by a service, a process, or a non-Java-SDK application _after_ the transaction's sequence of durable writes has started, but _before_ the document itself has been durably written, the Java-SDK application responsible for the transaction is informed of the anomaly; and the durable write for the document is then _performed notwithstanding_.
This means that non-transactionally based mutations may be lost, due to their being subsequently overwritten by a transaction's durable write.

For more details on Java SDK transactions, see xref:3.0@java-sdk:howtos:distributed-acid-transactions-from-the-sdk.adoc[Distributed Transactions from the Java SDK].
