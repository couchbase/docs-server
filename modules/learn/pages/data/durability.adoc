...
Couchbase Server informs the client of an _ambiguous result_ whenever it cannot confirm that an attempted commit was successful.
This situation may be caused by node-failure, network-failure, or timeout.
Note that an ambiguous result is only possible when an attempt to commit has been initiated by Couchbase Server.

If a client receives notification of an ambiguous result, and the attempted write is _idempotent_, the write can be re-attempted.
If the attempted write is _not_ idempotent, the current state of the saved data should be verified, and the write then re-attempted _if appropriate_.
