...
Mutations are recorded per document as _meta data_: when all documents in the transaction have had their intended mutations so recorded, durable writes are performed in sequence, one for each document; in order to commit the changes.
...
This means that non-transactionally based mutations may be lost, due to their being overwritten by a transaction.

For details on Java SDK transactions, see xref:3.0@java-sdk:howtos:distributed-acid-transactions-from-the-sdk.adoc[Distributed Transactions from the Java SDK].
