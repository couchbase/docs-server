...
The scenario assumes that the required durability-level is client-specified, with no level having been assigned to the bucket: however, the lifecycle is the same regardless of whether the prevailing durability level has been imposed by client or bucket.
