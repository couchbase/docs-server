...
In Couchbase Enterprise Server 7.2+, auto-failover can be configured not to occur if a node's failover would result in the loss of durably written data.
This means ensuring that at least one node containing the durably written data is preserved.

If auto-failover is configured in this way, consequences include the following:

* If data has been durably written to _one_ node (which is to say, no replica exists), the node cannot be automatically failed over.

* If data has been durably written to _two_ nodes, only _one_ of the nodes (either the active or the replica) can be automatically failed over.

* If data has been durably written to _three_ nodes, only two of the nodes (either both of the replicas, or one of the replicas and the active) can be automatically failed over.

* If data is being written to _three_ nodes, with the durable writes to the active and to one replica having been completed, and with the durable write to the other replica still pending, only one of the two nodes to which durable writes have been completed (either the active or the replica) can be failed over.
The node for which the durable write is still pending can also be failed over.
