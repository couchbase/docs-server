...
You may want Couchbase Server to automatically remove some types of documents after a period of time. For example, you often want documents that contain user session data to expire some time after the user's last interaction. Or you may use documents to store cached data. You can have these documents automatically expire so your application knows to refresh the data from its source. 

You can set an expiration value on an individual document either when you create it or when you mutate it. You set the expiration to the number of seconds the document should exist before Couchbase Server automatically removes it. You can change this value later, in case you want extend the life of the document. 

You can configure buckets and collections to automatically apply a default expiration 
 on the documents they contain. You set this default expiration by setting their `maxTTL` to the default number of seconds any new or mutated document should last before expiring. This setting also imposes an upper limit on the expiration time you can explicitly set on a document. If you explicitly set the document's expiration, Couchbase Server sets its expiration to ver value is smaller: the number of seconds you set or `maxTTL`.  

The 


TTL is an integer value, specified per bucket or per collection, that determines the maximum expiration time of individual items within the bucket or collection: if TTL is enabled, each newly created item lives for the number of seconds specified by TTL, following the item's creation.
