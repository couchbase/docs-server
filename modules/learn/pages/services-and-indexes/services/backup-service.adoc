...
You can use this tool directly to perform backups and merges.
Both allow incremental backups and let you merge incremental backups to deduplicate their data.
They use the same backup archive structure and allow you to list the contents of backup and search for specific documents.
...
It only performs incremental backups on a repository that already contains a backup.
...
You have several ways you can configure your cluster to avoid having backup tasks fail due to these conflicts.

One option to avoid task conflicts to have a single Backup Service node.
This configuration is useful if you have multiple backup tasks that target the same repository. 
If one task is scheduled to start while another task is running, the Backup Service adds the task to a queue instead of causing it to fail.
