...
If, due to an interval-specification that is too small, a scheduled task attempts to start while its prior repetition is still ongoing, the new task fails to start.
In such circumstances, all data continues to be backed up by the prior repetition; and will be backed up again by any subsequent repetition that starts after the completion of the previous backup.
