...
You do not have to restore backed up data to a bucket using the same  xref:learn:buckets-memory-and-storage/storage-engines.adoc[storage engine] as the original bucket. 
For example, you can restore data backed up from a bucket that used the Couchstore storage engine to a bucket using Magma.
 
[#retaining-and-pruning]
== Retention and Pruning

The Backup Service retains backups for a period of time you specify in the plan using the retention period.
The retention period is the time after which the backups are eligible for deletion.
You can set the retention period to a specific number of days.

You can also set the Backup Service to prune backups that have exceeded the retention period.
You can either prune backups automatically or manually.
You can use default Plan settings or create a custom pruning task to prune backups automatically.
You can prune on-demand from the Backup Service UI.
...
* xref:rest-api:backup-trigger-prune.adoc[Perform an Immediate Prune].

* xref:rest-api:backup-retention.adoc[Retain a Backup].
