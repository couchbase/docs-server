...
The Analytics Service further enhances efficiency by supporting parallel query-processing and bulk data-handling; and by allowing analytic queries to be run on its own form of _indexes_.
These are kept local to the data that has been _shadowed_ to the Analytics Service; and are thus partitioned in the same way that the data itself is partitioned.
