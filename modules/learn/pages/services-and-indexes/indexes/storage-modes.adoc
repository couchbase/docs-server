...
After Bloom-filter enablement, when a lookup occurs, and the correct memory page is located, the Bloom filter indicates either that the item is _not_ on the page, or that it _may be_ on the page.
...
Per page Bloom filters can be enabled by means of the REST API -- they are _disabled_ by default.
