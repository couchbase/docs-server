...
If Bloom filters are enabled (which is the default), when a lookup occurs, and the correct memory page is located, the Bloom filter indicates either that the item is _not_ on the page, or that it _may be_ on the page.
If the filter indicates that:

* The item is _not_ in the page, then the item is not on disk, and no disk I/O need be incurred.

* The item _may be_ on the page, then the item can continue to be searched for, and disk I/O must therefore be incurred.

The consequent reduction in disk I/O promotes the efficiency of searches and of system performance generally.

Per page Bloom filters can be enabled by means of the REST API -- they are _enabled_ by default.
For information, see xref:rest-api:rest-managing-bloom-filters.adoc[Managing Bloom Filters].
...
By keeping more items in memory, the need for disk I/O is reduced, as are corresponding latencies.

The selection of items to be compressed occurs periodically, with the period-duration being shorter than that used for xref:learn:buckets-memory-and-storage/memory.adoc#ejection[Ejection].
...
In consequence, items most likely to be accessed remain _uncompressed_ in memory, and are therefore accessible with the least latency; while items less likely to be accessed are retained in memory in _compressed_ form, until their ejection; beyond which, they must be accessed through disk I/O.
