...
====
In this keyspace, `schedule` is an array of objects with flight details.
...
[source,n1ql]
----
CREATE INDEX travel_info ON `travel-sample`(name,type,id,icao,iata);
----
...
[source,n1ql]
----
CREATE INDEX travel_info ON `travel-sample`(name, id, icao, iata)
WHERE type='airline';
----
...
[source,n1ql]
----
CREATE INDEX i1 ON `travel-sample`(LOWER(name),id, icao)
WHERE type = 'airline';

CREATE INDEX i2 ON `travel-sample`(LOWER(name),id, icao)
WHERE type = 'airline';

CREATE INDEX i3 ON `travel-sample`(LOWER(name),id, icao)
WHERE type = 'airline';
----
