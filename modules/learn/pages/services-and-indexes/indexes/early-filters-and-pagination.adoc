...
:description: When covering indexes and index pushdowns are not available, the Query Service may use early filtering, early order, and early pagination to improve the query response time.
...
If a query has a xref:n1ql:n1ql-language-reference/covering-indexes.adoc[covering index], then the index contains all the data needed to perform the query.
...
Couchbase Server therefore attempts to apply filters, order, and pagination before fetching data, wherever possible.
...
With this feature, the Query service may apply filtering, order, and pagination directly to the results returned by the Index service, before performing the fetch.
...
note bottom of Filter
  Early filter, order, and pagination
...
In addition, early filtering may be performed independently of early order and pagination, and vice versa.
...
<.> The `index_projection` section contains the `index_key` needed for evaluating the early index filters -- in this case position `0` for `"city"`.
====
...
== Early Ordering and Pagination

Early ordering and pagination may be applied independently of early filtering.
...
* When early order is in effect, parallelism is turned off.

=== Caveats

Early ordering is performed on the index keys.
It is possible for the document to be updated while the query is running, so that the early order may no longer be correct.
Note that this caveat applies to index order pushdowns also.

A similar caveat applies to early pagination.
It is possible for the document to be updated while the query is running, so that it no longer satisfies the index span; in which case, it may be discarded by the filter step _after_ the fetch.
In such situations, the query may return fewer documents than the limit specifies.
Note this caveat applies to index pagination pushdowns also.
...
| Covering index
| Index pushdown ^<<note>>^
| Early filter, order, or pagination ^<<note>>^
| Default filter, order, or pagination ^<<note>>^
...
| Supports grouping and aggregates?
| icon:question-circle[orange] Some
| icon:question-circle[orange] Some
| icon:times-circle[red] No
| icon:check-circle[green] Yes

| Filter, order, and pagination performed...
| After scan
| After scan, +
before fetch
| After scan, +
