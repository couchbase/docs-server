...
For example, when you xref:manage:monitor/monitoring-n1ql-query.adoc[enable monitoring] with the N1QL configuration parameter `profile = "timings"` for this query, you can see that indexer returns 165 documents which is same as the final result set of the query.
+
[source,json]
----
    "~children": [
