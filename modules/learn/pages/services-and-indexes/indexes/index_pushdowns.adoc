...
In <<example-comp-explain>> above, the query has a composite predicate on continuous leading index-keys, namely the index-keys `distance` and `sourceairport`, which occur together the start of the index definition.
...
USE INDEX (def_route_src_dst_dist)
WHERE type = "route"
AND distance < 2000 AND destinationairport = "LAX"; <1>
----

.Results
[source,json]
----
include::example$services-and-indexes/indexes/pushdown-skip.json[tag=excerpt]
...
----
====
