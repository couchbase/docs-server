...
* xref:learn:data/transactions.adoc[Transaction]: An operation that ensures that when multiple discrete data-elements need to be modified such that only the successful modification of all justifies the modification of any, _either_ all the modifications do occur successfully; or _none_ of them occurs.
