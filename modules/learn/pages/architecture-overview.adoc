...
Sub-millisecond data operations are supported by powerful Query and Index Services, and by a flexible, document-oriented query-language, _N1QL_.
...
Data can be retained either in memory only, or in memory and storage, as judged appropriate by the administrator.
...
Data items can also be selectively replicated across data centers; for the purpose either of backup only, or of simultaneous, multi-locational application-access.
...
* _Search_: Create indexes specially purposed for Full Text Search.
...
These services can be deployed, maintained, and provisioned independently of one another, so as to allow _Multi-Dimensional Scaling_; which ensures the most effective ongoing response to changing business conditions and emergent workload-requirements.
For example, a development environment running the Data, Index, Query, and Search Services might permit an instance of each on every node of a five-node cluster:
...
Such a deployment might be well-suited to a pure development context, in which little or no distinction need be made between the individual services, in terms of workloads and priorities. However, the same cluster, when moved to production, might require a more appropriately tuned service-configuration, such as the following:
...
A more detailed overview of Couchbase Server's technology high-points and business benefits can be found in xref:introduction:intro.adoc[Why Couchbase?].

[#about-this-section]
== About This Section

This section of the Couchbase documentation-set allows the administrator or developer to learn about the principal features of Couchbase Server.
