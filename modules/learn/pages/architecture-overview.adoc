...
Sub-millisecond data operations are provided by powerful services for querying and indexing, and by a feature-rich, document-oriented query-language, {sqlpp}.
...
* _Query_: Parses queries specified in the {sqlpp} query-language, executes the queries, and returns results.
...
* *Services and Indexes*: _Services_ can be deployed to support different forms of data-access: for example, the _Data Service_ allows items to be retrieved by _key_; while the _Query Service_ allows them to be retrieved by means of _queries_, designed in the {sqlpp} query-language.
