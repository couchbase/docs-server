...
* _Single Sign On_ (SSO) lets users avoid repeated username and password prompts for every service they sign into. Instead, the user authenticates once with the IdP when first connecting to a service. This initial authentication starts a session for the user with the IdP. While the session is active, when the user connects to a service that uses the IdP, it authenticates them without prompting for a username and password. 
...
* By centralizing user authentication, SAML reduces the workload of administrators. They have a single place to authorize new users and deauthorize users who should no longer have access. When an administrator deauthorizes a user, services automatically end all of the user's active sessions. This feature also benefits users by reducing the number of passwords they need to remember.

Enabling SAML authentication does not prevent users who have locally defined accounts from logging into Couchbase Server directly with their username and password. In addition, you can enable both SAML authentication and LDAP authentication. However, users may become confused if you do not synchronize user names and passwords between the LDAP server and the IdP. 

=== Mapping of Groups and Roles With SAML Authentication

When a user authenticates with an IdP, the IdP sends the user back to Couchbase Server with a SAML-formatted message containing their authentication information. Included within this message are attributes defined for the user by the IdP. You can choose to map two of these attributes to define the groups and roles for users in Couchbase Server. If you do not map attributes for the group and roles, Couchbase Server gets the groups and roles from user's Couchbase Server account. When no mapping exists between attributes and groups and roles, only users registered as external Couchbase Server users can log in via SAML. If you map attributes to groups and roles, both users registered as external users and users without a Couchbase Server account can log in. Couchbase Server grants them access to the Web Console if one of the groups or roles mapped from a SAML attribute gives them UI authorization.
