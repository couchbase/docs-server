...
* *X.509 client certificates*, which work for client applications, the CLI, the REST API, and the Web Console.
...
You can also sign in to the Web Console with one, using the *Sign In With Certificate* button on the sign-in page.
That button appears only on a TLS connection, and only when client certificate authentication is enabled on the cluster.

When client certificate authentication is set to mandatory, every connection has to present a trusted certificate and passwords are no longer accepted.
Single sign-on still works, as long as the browser presents a trusted certificate.

A request may carry both a certificate and a xref:learn:security/external-authentication.adoc#jwt-overview[JWT].
In that case the certificate secures the connection, but the *token* decides who the user is: Couchbase Server authenticates the token's subject and ignores the username in the certificate.
See xref:manage:manage-security/authenticate-using-jwt.adoc#jwt-and-client-certificates[Using a JWT with Client Certificate Authentication].
