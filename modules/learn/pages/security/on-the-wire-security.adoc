...
This means that a custom cipher-suite list configured for the _Search_, _Index_, _Query_, or _Backup_ service will always implicitly include _all_ the TLS 1.3 cipher-suites in the list of supported cipher-suites for that service, in the order specified: none of those TLS 1.3 cipher-suites can be omitted, nor can their order be changed with respect to one another.

If the minimum TLS version for one of these services is less than 1.3, and only selected non-1.3 cipher-suites are explicitly included in a custom list configured for the service, all 1.3 cipher-suites supported for the service are implicitly appended to the explicitly stated list, in the appropriate order for 1.3 cipher-suites.

If all 1.3 cipher-suites for one of these services are, in a custom list, explicitly specified in the order appropriate for 1.3 cipher-suites, non-1.3 cipher-suites can be explicitly positioned before or after those 1.3 cipher-suites, and can be interspersed with those 1.3 cipher-suites.
