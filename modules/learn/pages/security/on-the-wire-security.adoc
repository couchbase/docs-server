...
The cluster-wide default value for the minimum TLS version is `tlsv1`: this is used by a service if no value has been specified for that service, and no global value has been specified.
