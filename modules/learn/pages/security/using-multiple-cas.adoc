...
a node certificate references its CA by means of a _concatenation_ of all certificates -- node and intermediates -- whose signature-chain leads to the CA.
...
When client-access is attempted, Couchbase Server duly concatenates the node certificate and intermediate certificates, and presents the resulting file to the client.

The upload of intermediate certificates potentially increases the efficiency with which certificate deployment is performed; since it allows node certificates to be uploaded without manual concatenation being needed for each.

For an example, see xref:manage:manage-security/configure-server-certificates.adoc#intermediate-upload[Deploying an Intermediate Certificate via Upload].
