...
* Allows an individual node either to use a root certificate that is also used by one or more other nodes; or to use a root certificate that is used by no other node.
...
All certificates in the trust store are accessible to all nodes, and multiple root certificates may therefore be used in facilitating encrypted node-to-node communication.
