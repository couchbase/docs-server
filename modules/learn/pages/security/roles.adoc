...
[#roles-parameter-lists]
=== Role Parameter Lists

Some roles let you limit the privileges they grant.
For example, when granting a user the <<query-select>> role, you select which buckets, scopes, and collections they can query.
You specify these limits using a role parameter list.

Roles that support role parameter lists default to denying access to anything not in the list. 
For example, suppose you grant a user the ability to query data from the `travel-sample.inventory.airline` collection. 
That permission does not give them the ability to read data from any other collection in the `inventory` scope or any other scope in the `travel-sample` bucket.

When setting which buckets, scopes, and collections a role grants a user or group access to, you use a colon-delimited list. 
For example, to grant a user access to the `travel-sample.inventory.airline` collection, you use `[travel-sample:inventory:airlines]` in the roles list.

Role parameter lists support wildcard patterns that let you grant users access to multiple objects at once.
The wildcard syntax varies depending on a role.

To use a wildcard in a bucket-related role parameter, you omit it and every parameter after it from the list, rather than typing an asterisk.

For example, suppose you want a user to be able to read data from all collections in the `travel-sample.inventory` scope. 
Then you would use `[travel-sample:inventory]` in the role parameter list. 
If you want to grant the user access to all scopes and collections in the `travel-sample` bucket, set the role's parameter list to `[travel-sample]`.

For bucket-related role parameter lists, Couchbase Server displays the wildcard parameters as asterisks when you view the role afterwards.
For example, suppose you granted a user the Query Select role and set the role parameter list to `[travel-sample]`.
Then when you display the user's details, Couchbase Server displays  `[travel-sample:*:*]` to make clear that the user has access to all scopes and collections in the bucket.
You do not type these asterisks yourself when granting the role.

Other roles that allow query parameters (such as the <<credential-consumer>> role) have their own role parameter list syntax. 
See the documentation related to these roles for details.
...
| Cannot grant Full Admin, Read-Only Admin, Local User Admin, External User Admin, or Security Admin roles to users or groups.
...
The Credential Admin role lets the user create, read, update, and delete credentials in the credential store.
This role manages credentials only.
It does not let the user consume (use) any credential. 
To let a user consume credentials, grant them the <<credential-consumer>> role instead.

NOTE: No role, including Credential Admin, can retrieve a credential's sensitive values (such as passwords, keys, or tokens) after they are set.
The credential store always masks these values in read responses.
This is true even for users who created the credential.

For more information about the credential store, see xref:learn:security/credential-store.adoc[].

[#table_credential_admin_role,cols="1,2,2,hrows=2"]
|===
3+^| Role: Credential Admin (`credential_admin`)
...
| *Credential Store*
| Create, read, update, and delete credentials and their metadata.
| Cannot consume (use) any credential.
Cannot view sensitive credential values such as tokens.
...
The Credential Consumer role grants the user permission to use specific credentials.
You list the ID of the credentials the user can access in the <<roles-parameter-lists,role parameter list>> when granting the role.

You can use wildcards to grant the user access to multiple credentials that share a common prefix.
Credential IDs are commonly named as a path.
For example, you might name a credential used to store continuous backup data for the `travel-sample` bucket `backup/continuous/travel-sample`.
Granting access to `backup/*` lets the user use every backup-related credential matching that prefix.

NOTE: You cannot grant access to credentials using a wildcard pattern (such as `backup/*`) until at least one credential matches the wildcard.
This restriction does not apply to granting access to all credentials using the wildcard pattern `*`.

This role does not let the user view a credential's metadata or stored values.
It only allows a Couchbase Server service to use the credential on the user's behalf.
For more information, see xref:learn:security/credential-store.adoc[].
For details on granting users and groups this role, see xref:manage:manage-security/manage-credentials/manage-user-access-credentials.adoc[].

[#table_credential_consumer_role,cols="1,2,2,hrows=2"]
|===
3+^| Role: Credential Consumer (`credential_consumer[<pattern>]`)
...
| *Credential Store*
| Use (consume) credentials an administrator has granted them access to.
| Cannot view credential metadata or values. Cannot create, update, or delete credentials.
