...
The *Manage Scope Functions* role (a _Query & Index_ role) allows N1QL and _user defined_ functions to be managed for a given scope, given corresponding specification of _bucket_.
The user can access Couchbase Web Console, but cannot read data.
...
The *Execute Scope Functions* role (a _Query & Index_ role) allows N1QL and _user defined_ functions to be executed for a given scope, given corresponding specification of _bucket_.
The user can access Couchbase Web Console, but cannot read data.
...
[#table_mobile_sync_gateway_role,cols="15,8,8,8,8",hrows=3]
|===
