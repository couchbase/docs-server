...
Some roles let you grant users privileges to specific objects.
You specify these objects using a role parameter list.

For example, when granting roles that let a user or group read or manipulate data, you specify the buckets, scopes, and collections where they have that ability.
When you grant the Credential Consumer role that allows users to use credentials in the xref:learn:security/credential-store.adoc[credential store], you list which credentials they can use.  

Roles that support role parameter lists default to denying access to anything not in the list. 
For example, suppose you grant a user the ability to read data from the `travel-sample.inventory.airline` collection. 
...
To wildcard a parameter, omit it and every parameter after it from the list, rather than typing an asterisk.

For example, suppose you want a user to be able to read data from all scopes and collections in the `travel-sample` bucket using {sqlpp} queries.
In this case, you need to grant them the Query Select role and set the role's parameter list to `[travel-sample]`.
Couchbase Server displays the omitted parameters as asterisks when you view the role afterward, for example `[travel-sample:*:*]`, but you do not type the asterisks yourself when granting the role.
...
You list the ID of the credentials the user can access when granting the role, similar to granting access to buckets.
...
NOTE: 
You cannot grant access to credentials using a wildcard pattern (such as `backup/*`) until at least one credential matches the wildcard.
...
For more information, see xref:security/credential-store.adoc[].
