...
These roles grant users privileges to manage and query external catalogs and external collections, which let you use {sqlpp} to query Apache Iceberg tables in an external system without copying the data into Couchbase. These roles are only available in Couchbase Server Enterprise Edition.

[#external-catalog-admin]
=== External Catalog Admin

The External Catalog Admin role (`external_catalog_admin`) can read/update/delete the configured external catalogs.

[#external-catalog-reader]
=== External Catalog Read-Only Admin

The External Catalog Read-Only Admin role (`external_catalog_reader`) can read the configured external catalogs.

[#query-select-external-catalog]
=== Query External Catalog Select

The Query External Catalog Select role (`query_select_external_catalog`) can execute a SELECT statement on a given external catalog. This user can read external data, but not write it.
When granting this role, you choose the external catalog the user can query through.
To successfully query an external collection, a user also needs the Query Select role on the collection itself and the Credential Consumer role for any credentials the catalog and collection reference.

NOTE: Querying an external collection also requires the Credential Consumer role, parameterized with the credential or credentials the catalog and collection use. Couchbase defines the Credential Consumer role as part of the credential store feature, not as part of this role list.
