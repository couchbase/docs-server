...
All data within a bucket is contained within a collection, within a scope.
The permissions conveyed by bucket-related roles are restricted in the following ways:

* By _Bucket_: Permissions apply to all data in the specified bucket.

* By _Bucket_ and _Scope_: Permissions apply only to the collections within the specified scope, within the specified bucket.

* By _Bucket_, _Scope_, and _Collection_: Permissions apply only to the data within the specified collection, within the specified scope, within the specified bucket.
...
The *Manage Scope Functions* role (a _Query & Index_ role) allows N1QL functions to be managed for a given scope.
The user can access Couchbase Web Console, but cannot read data.
...
The *Execute Scope Functions* role (a _Query & Index_ role) allows N1QL functions to be executed for a given scope.
The user can access Couchbase Web Console, but cannot read data.
...
The *Manage Scope External Functions* role (a _Query & Index_ role) allows external language functions to be managed for a given scope.
The user can access Couchbase Web Console, but cannot read data.
...
The *Execute Scope External Functions* role (a _Query & Index_ role) allows external language functions to be executed for a given scope.
The user can access Couchbase Web Console, but cannot read data.
...
The *Manage Collections in Scope* role (a _Bucket_ role) allows the creation and deletion of collections within a given scope.
...
|===
5+^| Role: Manage Collections in Scope (`scope_admin`)
...
The *Sync Gateway* role (which is a _Mobile_ role) allows full access to bucket data, as required by Sync Gateway.
The role does not allow access to Couchbase Web Console.
...
The *Data Reader* role (which is a _Data_ role) allows data to be read, _per bucket_.
...
The *Data Writer* role (which is a _Data_ role) allows data to be written, _per bucket_.
The role does not allow access to Couchbase Web Console: it is intended to support applications, rather than users.
...
The *Data DCP Reader* role (which is a _Data_ role) allows DCP streams to be initiated, _per bucket_.
The role does not allow access to Couchbase Web Console: it is intended to support applications, rather than users.
...
The *Data Monitor* role (which is a _Data_ role) allows statistics to be read, _per bucket_.
It does not allow access to Couchbase Web Console, and does not permit the reading of data.
...
The *Views Reader* role (which is an _Administrative_ role) allows data to be read from the views, _per bucket_.
...
The *Query Select* role (which is a _Query & Index_ role) allows the SELECT statement to be executed, _perbucket_.
...
The *Query Update* role (which is a _Query & Index_ role) allows the UPDATE statement to be executed, _per bucket_.
...
The *Query Insert* role (which is a _Query & Index_ role) allows the INSERT statement to be executed, _per bucket_.
The role supports access to Couchbase Web Console, but does not allow the reading of data.
...
The *Query Delete* role (which is a _Query & Index_ role) allows the DELETE statement to be executed, _per bucket_.
The role supports access to Couchbase Web Console, but does not allow the reading of data.
...
The *Query Manage Index* role (which is a _Query & Index_ role) allows indexes to be managed, _per bucket_.
The role allows access to Couchbase Web Console, but does not permit the reading of data.
...
The role *Search Reader* (which is a _Search_ role) allows _Full Text Search_ indexes to be searched, _per bucket_.
...
The *Analytics Select* role (which is an _Analytics_ role) allows the querying of datasets created on the bucket.
...
The *Analytics Manager* role (which is an _Analytics_ role) allows the management and querying of datasets created on the bucket; and the management of Analytics Service local links.
The role allows access to Couchbase Web Console, and permits the reading of some data.
