...
The *Query Use Sequential Scan* role (which is a _Query & Index_ role) allows users to perform a sequential scan query.
This is a query of a keyspace that does not have a suitable index. 
Couchbase Server limits sequential scan queries to users with this role because querying a large unindexed keyspace can be expensive.  
Administrators automatically have permission to perform sequential scan queries.
