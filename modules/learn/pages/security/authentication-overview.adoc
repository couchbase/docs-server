...
LDAP_based authentication can be set up in one of the following ways;
...
In cases where the specified username specified does _not_ match against the list of locally defined users, if Native LDAP Support has been configured, Couchbase Server attempts to authenticate and authorize the user by means of LDAP.
...
(For information on mapping LDAP groups to Couchbase-Server user-groups, see xref:manage:manage-security/configure-ldap.adoc[Configure LDAP].)
+
