...
This is enforced through account locking, which prevents a user from logging in or continuing existing sessions once their account is marked as locked.
When an account is locked, all active UI sessions and long-running connections such as Memcached or streaming connections are terminated immediately. 
...
Administrators can block authentication manually through the UI, REST API, or CLI, or automatically through inactivity-based locking after a defined idle period (using TTL).
