...
* _LDAP Support Based on `saslauthd`_.
...
Migration to _Native LDAP_ is recommended; so that support for encrypted communication and LDAP groups become available.
...
Native LDAP supports _LDAP groups_.
...
_Native LDAP_ is recommended, for Couchbase Server Enterprise Edition 6.5+.
