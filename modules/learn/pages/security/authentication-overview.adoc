...
The specified username and password must match ones already stored and accessible, either on the Couchbase-Server cluster itself, or, if the Enterprise Edition of Couchbase Server is used, _externally_:
...
** Users that are not defined on Couchbase Server, but are defined entirely on LDAP.
...
LDAP-based authentication can be set up in one of the following ways;
...
In cases where the specified username specified does not match against the list of locally defined users, if Native LDAP Support has been configured, Couchbase Server attempts to authenticate and authorize the user by means of LDAP.
...
This makes use of the user-specified password and a _template_ or _query_, which has been preconfigured: for examples of how to configure templates and queries, see xref:manage:manage-security/configure-ldap.adoc#enable-ldap-user-authentication[Enable LDAP User Authentication].
