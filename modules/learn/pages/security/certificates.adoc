...
A check is performed when the certificate is loaded onto the node; and, after the certificate has been loaded, a check will also be performed whenever an attempt is made to add or join the node to a cluster.
If the node-name is either incorrectly specified as a SAN, or is missing:

* If the name of the node itself can be changed, a warning is generated, and the operation succeeds.

* If the name of the node itself cannot be changed, an error occurs, and the operation fails.

See xref:learn:clusters-and-availability/nodes.adoc#naming-clusters-and-nodes[Naming Clusters and Nodes], for more information on naming conventions and restrictions.
