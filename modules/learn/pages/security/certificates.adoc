...
Alternatively, if the node certificate has gained the CA's authority by means of a sequence of one or more intermediate certificates, `chain.pem` may be allowed to contain only the node certificate; under the assumption that all intermediates in the chain are already resident in the client's _trust store_.
