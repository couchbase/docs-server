...
All certificates, both on the server-side and on the client-side, should therefore be made available as a complete _trust chain_.
In 7.1+ either of the following procedures can be used:

* The entity's own certificate, and all intermediate certificates, are provided directly to the interlocutor.
The interlocutor is expected to find the CA -- the last and most essential certificate in the chain -- in its own trust store; and if it does, authentication may proceed.

* The entity's own certificate alone is provided directly to the interlocutor.
The interlocutor is expected to find all associated intermediates, and the CA, in its own trust store; and if it does, authentication may proceed.
...
However, when the node certificate has gained the CA's authority by means of a sequence of one or more intermediate certificates, `chain.pem` may be a correspondingly ordered _concatenation_ of all the certificates in the chain, except the cluster certificate.
...
Alternatively, if the node certificate has gained the CA's authority by means of a sequence of one or more intermediate certificates, `chain.pem` may be allowed to contain only the node certificate; under the assumption that all intermediates in the chain are already resident in the clients _trust store_.
