...
Use of the _Subject Common Name_ to identify either a server or a client is now deprecated (see https://tools.ietf.org/html/rfc6125#section-6.4.4[section 6.4.4 of RFC 6125]); meaning that in production, server and client should each specify at least one _Subject Alternative Name_, in order to be identified.
...
The examples of certificate-creation provided in xref:manage:manage-security/manage-certificates.adoc[Manage Certificates] continue to include definitions of Subject Common Name.
Optionally, the Subject Common Name can be omitted.
However, at least one element of the _Distinguished Name_ (or _DN_), of which the Subject Common Name constitutes a single field, _must_ be included: otherwise, certificate-creation fails.
(The other fields are _Country Name_, _State or Province Name_, _Locality Name_, _Organization Name_, _Organizational Unit Name_, and _Email Address_.)
