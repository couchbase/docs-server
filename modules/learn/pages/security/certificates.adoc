...
Once no node has a certificate that was signed by the self-signed CA you can delete the it.
You can learn which nodes have certificates signed by a root CA from entry in the Couchbase Server Web Console's *Certificates* tab on the *Security* page. 
...
Couchbase Server requires certificates for nodes to meet the following requirements:
