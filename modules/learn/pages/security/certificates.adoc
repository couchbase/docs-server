...
* If only a prefix or a delimiter is specified for a given path, parsing is attempted in accordance with whichever has been specified; and authentication is then attempted with the name that results.

* If both a prefix and a delimiter are specified for a given path, parsing is first attempted in accordance with the specified prefix.
The results of this initial parsing-attempt are then attemptedly parsed again, in accordance with the specified delimiter.
Authentication is then attempted with the string produced by the two, sequential parsing-attempts.
