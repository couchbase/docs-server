...
In this case, the certificate on the new node (always) and on each cluster node handling the join (sometimes, depending on the cluster's configuration) is checked for a node-name correctly specified as a SAN:
