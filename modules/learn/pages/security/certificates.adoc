...
In this case, the certificate on the new node (always) and on each cluster node (sometimes, depending on the cluster's configuration) is checked for a node-name correctly specified as a SAN:

** New node: If the node-name is not correctly specified as a SAN, an error is returned, and the add or join fails.

** Cluster node: If the cluster has two or more nodes prior to the add or join, the check is _not_ performed.
