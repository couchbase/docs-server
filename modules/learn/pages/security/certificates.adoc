...
* The node-certificate for each server in the cluster must specify its node-name as a _Subject Alternative Name_: see xref:learn:security/certificates.adoc#server-certificate-validation[Server Certificate Validation], immediately below, for details.
...
[#validation-checks-and-responses]
=== Checks and Responses

The following table lists scenarios in which node-certificate name-checks occur.
Note that checks are only performed on 7.2+ nodes.
When a 7.2+ node is being added to a cluster that contains 7.2+ nodes, both the new node and the 7.2+ cluster-nodes are checked.

In this table, when a certificate is referred to as _correct_, its node-name is correctly specified as a SAN.
When a certificate is referred to as _incorrect_, its node-name is either incorrectly specified as a SAN, or is not specified as a SAN.

[options="header", cols="9,4,4,4,4"]
|===
| Action | Number of Nodes in Cluster | Node Version | Cluster Version | Result

| Reload a _correct_ node-certificate onto a node that is already part of a cluster. | Any | 7.2+ | Any | Success

| Reload an _incorrect_ node-certificate onto a node that is _not_ part of a cluster. | -- | 7.2+ | -- | Warning

| Reload an _incorrect_ node-certificate onto a node that is already part of a cluster. | One or more | 7.2+ | Any | Failure

| Add or join a new node bearing a _correct_ node-certificate to a cluster all of whose 7.2+ nodes bear node-certificates that are _correct_. | Any | 7.2+ | Any | Success

| Add or join a new node bearing an _incorrect_ node-certificate to a cluster all of whose 7.2+ nodes bear node-certificates that are _correct_. | Any | 7.2+ | Any | Failure

| Add or join a new node bearing a _correct_ node-certificate to a one-node, 7.2+ cluster whose node-certificate is _incorrect_. | One | 7.2+ | 7.2+ | Failure

| Add or join a new node bearing a _correct_ node-certificate to a cluster at least one of whose 7.2+ nodes bears a node-certificate that is _incorrect_ .| Two or more | 7.2+ | 7.2+ | Success

| Add or join a new node bearing an _incorrect_ node-certificate to a cluster at least one of whose 7.2+ nodes bears a node-certificate that is _incorrect_. | Any | 7.2+ | Any | Failure
