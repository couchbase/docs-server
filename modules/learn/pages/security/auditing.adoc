...
== Administrative and Data Events

The events in the following tables support the auditing of administrative and configuration changes made to the cluster.
In each table, the first column (at the left) features the event-name; the second the event-description; the third the event-group heading under which the event is listed in the *Events* panel, accessed under the *Auditing* tab of the Couchbase Web Console *Security* screen; and the fourth the event _type_ &#8212; whether it relates to administration or data.

See xref:manage:manage-security/manage-auditing.adoc[Manage Auditing], for instructions on how to _filter_ these events by means of Couchbase Web Console.
...
| login success | Successful login to a Couchbase cluster | REST API | Admin
|===
