...
The metadata provides additional information about the index like where does the index reside ([.output]`datastore_id`), its state ([.output]`state`), and the indexing method used ([.output]`using`).
