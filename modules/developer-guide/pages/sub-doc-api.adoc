...
Consider a simple https://gist.github.com/ToddGreenstein/f43559b9ca5fc476f18c[Java] example that uses the sub-document API to connect to the [.param]`travel-sample` bucket, fetch the [.param]`name` field from the document "[.input]`airline_13633`", and then print it.
...
For more information on CAS, see xref:sdk:concurrent-mutations-cluster.adoc#concept_iq4_bts_zs[Concurrent Document Mutations].
...
For example, operating on a 100KB document named "[.input]`user::j.bloggs`" where a 30 byte fragment is added to a path of length 20 bytes would require sending the following over the network:
...
The cluster node needs to parse the current JSON value for "[.input]`user::j.bloggs`", apply the requested modification (inserting an element into a dictionary in the above example), and then store the result.
