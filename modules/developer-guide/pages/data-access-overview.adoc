...
In a production environment, you access data by using a Couchbase xref:sdks:intro.adoc[SDK].
In addition, xref:cli-overview.adoc[command-line] and xref:webui.adoc[web-based] tools are available to access data during development and testing.
...
The following examples use the xref:cli-overview.adoc[command line] interface to demonstrate the basic CRUD operations.
...
`mnunberg` is the document’s ID.
The command pipes the document's content to the [.cmd]`cbc` command’s standard input.
