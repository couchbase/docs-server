...
If a cluster-level setting has an equivalent node-level setting, then changing the cluster-level setting over-writes the node-level setting for all Query nodes in the cluster.
