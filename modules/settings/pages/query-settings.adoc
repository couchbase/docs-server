...
_Named parameters_ use a <<identifier,variable name>> to define the value of each parameter, while _positional parameters_ use a <<args,list of arguments>> to define the value of each parameter by position.
Requests which use these two types of parameter should contain the appropriate placeholders, as summarized in the table below:
