...
It is also possible to retrieve the certificate using the xref:rest-api:rest-xdcr-data-encrypt.adoc[REST API] or save to a file with the CLI command:
+
----
...
----
+
You can re-generate he SSL certificate by clicking on [.ui]*Regenerate*, trigger a new SSL certificate with  the xref:rest-api:rest-xdcr-data-encrypt.adoc[REST API] or generate a new certificate and save it to a file with the CLI command:
+
Example:
+
----
