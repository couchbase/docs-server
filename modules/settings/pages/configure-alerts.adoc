...
You can define the settings using the Web Console or using the xref:rest-api:rest-cluster-email-notifications.adoc[REST API].

image::admin/settings-alerts.png[,720,align=left]

To setup email alerts using the Web Console:

. From the [.ui]*Couchbase Web Console* > [.ui]*Settings* > select the [.ui]*Email Alerts* tab.
. Select the [.ui]*Enable email alerts* check box to enable the email alerts configuration.
These alerts are raised for errors that you select in the [.ui]*Available Alerts* section.
. Define the following settings and click [.ui]*Save*:
