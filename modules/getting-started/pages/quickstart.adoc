...
1. To select a specific document from a bucket based on the key that it was associated with when it was inserted, append the following Python script to your base `connect.py` file. The following example selects an existing document from the _beer-sample_ bucket that is associated with the key `mybeer1`:

[source,python]
----
rv = bucket.get('myBeer1')
print(rv.value)
----
...
1. To replace an existing document, append the following Python script to your base `connect.py` file. The following example replaces an existing document in the _beer-sample_ bucket associated with the key `mybeer2`. If the document doesn’t exist, Couchbase will not perform a replacement operation.
...
rv = bucket.replace('mybeer2', beerDocument2)
print(rv)
----
...
1. To remove a single existing document,  append the following Python script to your base `connect.py` file. The following example removes an existing document in the beer-sample bucket, associated with the key `mybeer`:

[source,python]
----
rv = bucket.remove(“mybeer”)
print(rv)
----
...
_For more information on managing a Couchbase Server, creating clusters and buckets, and the types of operations you can perform, please see the Getting Started Guide below._
