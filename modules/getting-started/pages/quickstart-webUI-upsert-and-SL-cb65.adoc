...
xref:quickstart-docker-image-manual.adoc[5-minute Couchbase Docker Container Configuration]

//* or run the following command(s) to run an automatically-configured CE docker image
//<commands>


== Step 1: Establish the query working environment
...
First, we will use the query workbench to create 2 indexes to facilitate our exercises: 

* one "primary index" to make initial queries easy to execute
* one "adaptive index" to support faster filtered queries 

If it is not already, open your web browser and navigate to the Couchbase WebUI.  If you are using a local docker image per our instructions above, the Web UI is at http://localhost:8091.  Log in and click "Query" in the left-hand navigation. 

NOTE: If you recieve a "Received error 404 ... " message then just refresh your browser.  The problem should not persist.

The CREATE INDEX statement is used for creating secondary indexes.  Paste both following statements into the Query Editor and click "Execute":
...
In the query workbench run a query that projects a document key and JSON document value.  Paste the following query into the query editor and click "Execute":
...
Let's use N1QL to generate two new records for us.  In order to project multiple records, we express them within an array and pass it in the FROM clause.  Paste and execute the following query to create and project some new data:
