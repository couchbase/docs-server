...
Start by creating a simple "model" class to represent a user. 
...
You can rename the `WeatherForecastController` to `UserController` or delete that file and create your own. In either case, create a constructor that has an `IBucketProvider` parameter. Use the bucketProvider to get an instance of a Couchbase bucket: `_bucket = bucketProvider.GetBucket("default")`.
...
[Note]
====
If you're using Postman, you can copy and paste the above cURL command into Postman. (File->Import->Paste Raw Text)
====
...
Now a single document should appear in your Couchbase bucket. Feel free to add multiple users in this way.

image::bucketOneDocument.png[Template select in Visual Studio]

== Step 4: Query the document

Create a second controller action called `GetData` in UserController.cs. This will be for an API endpoint to retrieve documents, given an email address.
...
If you've written SQL before, this should look familiar. But instead of querying a table, `default` is the name of the bucket.

To execute a N1QL query in .NET, start by creating a N1QL string with a named parameter for email: `var n1ql = "SELECT d.* FROM default d WHERE d.email = $email";`.
...
Compile and run your ASP.NET Core program locally with CTRL+F5. Use cURL or Postman or the HTTP tool of your choice to create a GET with a querystring variable of id the specified route. Example: `\https://localhost:44316/?email=major.tom@acme.com`
