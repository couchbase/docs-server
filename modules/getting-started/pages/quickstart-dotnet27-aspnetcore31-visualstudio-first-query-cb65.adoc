...
Create a second controller action called `GetData`. This will be for an API endpoint to retrieve documents, given a temperature.
...
SELECT w.*
FROM default w
WHERE w.temperatureC = 25
----

If you've written SQL before, this should look familiar. But instead of querying a table, `default` is the name of the bucket.

To execute a N1QL query in .NET, start by creating a N1QL string with a named parameter for the key: `var n1ql = "SELECT w.* FROM default w WHERE w.temperatureC = $temperatureC";`.
