...
On Linux, you can download Couchbase Server directly using the [.cmd]`wget` command.
...
+
If you get a message that says you can't run Couchbase Server due to security restrictions, right-click the icon for Couchbase Server and choose [.uicontrol]*Open* from the pop-up menu.
 ...
 ** When the installer asks if you want to increase the number of ephemeral ports, click [.uicontrol]*Yes*.
...
To set up Couchbase Server in a non production environment, you can accept the default values provided on most of the set-up screens.
...
. On the [.uicontrol]*Configure Server* screen (Step 1 of 5), click [.uicontrol]*Next* to accept the default values for a new cluster.
. On the [.uicontrol]*Sample Buckets* screen (Step 2 of 5), under [.uicontrol]*Available Samples* select the two samples we will use later in this tutorial: [.uicontrol]*beer-sample* and [.uicontrol]*travel-sample* and click [.uicontrol]*Next*.
. On the [.uicontrol]*Create Default Bucket* screen (Step 3 of 5), under Memory Size set the [.uicontrol]*Per Node RAM Quota* to 100 MB and click [.uicontrol]*Next*.
. On the [.uicontrol]*Notifications* screen (Step 4 of 5), enter your registration information, agree to the terms and conditions, and click [.uicontrol]*Next*.
. On the [.uicontrol]*Configure Server* screen (Step 5 of 5), enter and verify a password for the administrator account, and click [.uicontrol]*Next*.
+
The Couchbase Web Console opens and displays the [.uicontrol]*Cluster Overview*.
