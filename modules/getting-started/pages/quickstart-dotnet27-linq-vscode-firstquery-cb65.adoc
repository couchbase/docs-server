...
image::documentsUi.png[Three documents created in the default bucket]

== Step 4: Linq2Couchbase

Now, let's query these documents with Linq2Couchbase. Use the `context.Query<>` method to query documents with the standard Linq extension methods. To query by first name, for instance:
