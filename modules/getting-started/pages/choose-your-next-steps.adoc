...
The xref:guides:intro[Developer How-to Guides] section contains practical how-to guides, which walk you through common Couchbase Server development tasks.
...
Refer to the xref:n1ql:query.adoc[Query] section for details about querying Couchbase Server using the N1QL query language;
the xref:fts:fts-introduction.adoc[Full Text Search] section for information on text search and geospatial queries;
the xref:eventing:eventing-overview.adoc[Eventing] section for server-side programming using Eventing functions;
and the xref:analytics:introduction.adoc[Analytics] section for ad-hoc analytical queries.
