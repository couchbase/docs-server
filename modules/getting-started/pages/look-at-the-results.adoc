...
The .ui]*Scopes and Collections* screen shows, in a succession of page-views, the scopes and the number of collections within each scope, that are contained within the bucket. 

Starting with this release, all documents must be contained in a scope and collection. If you import a dataset that was created in earlier versions of Couchbase Server, the documents are automatically saved within a `_default` scope and a `_default` collection. 

[#travel_sample_collections_screen]
image::travelSampleCollectionsScreen.png[,720,align=left]

The .ui]*Scopes and Collections* screen shows two scopes: `_default` and `inventory`.
...
The [.ui]*Documents* screen shows, in a succession of page-views, the documents that are contained within the selected scope and collection.

The following document retrieval controls are provided:

* *Location*: Three drop-down menus, which respectively allow selection of a _bucket_, a _scope_ within the selected bucket, and a _collection_ within the selected scope.
