...
To use the Query Workbench, log into the Couchbase Server Web Console, and then click [.uicontrol]*Query*:
...
* [.uicontrol]*Query Editor*: Where you will type your N1QL query
* [.uicontrol]*Bucket Insights*: Provides information on the buckets that are currently maintained by your system.
Right now, it shows that just one exists; the bucket `travel-sample`.
* [.uicontrol]*Query Results*: Shows query results and provides a number of options for their display.
...
To execute your query, click [.uicontrol]*Execute*.

The results now appear in the [.uicontrol]*Query Results* panel:
...
* xref:sdk:nodejs/n1ql-queries-with-sdk.adoc#topic_esq_3td_5v[N1QL from the SDK]: Explains how to execute N1QL queries programmatically using the official Couchbase SDKs.
...
* xref:n1ql:n1ql-language-reference/index.adoc#n1ql-lang-ref[N1QL Language Reference]: Describes the N1QL language structure, including syntax and usage.
