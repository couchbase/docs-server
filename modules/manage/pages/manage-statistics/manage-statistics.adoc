...
The Couchbase-Server REST API provides multiple options for gathering statistics.

[#get-cluster-statistics]
=== Get Cluster Statistics

Cluster statistics can be accessed from `/pools/default`, as follows:

----
curl -v -X GET -u Administrator:password localhost:8091/pools/default | jq
----

Note that this example pipes the output to the `jq` tool: this formats the output and makes it easier to read.
...
Included in the output is information on:
