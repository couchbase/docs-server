...
Administrators with either the *Full Admin* or *Cluster Admin* role can perform explicit logging by means of Couchbase Web Console: on the *Logs* page, left-click on the [.ui]*Collect Information* tab, located near the top.
...
Couchbase Customer Support offers the facility to send you an automatic notification on receipt of any log file uploaded for a support case.
This function is offered on an opt-in basis.
Contact your account manager or Couchbase Support for more information.

When a Couchbase Support Engineer requests logs from a customer for which this feature is enabled,
the request will include a unique upload URL and UUID for the individual support case:

image::manage-logging/supportResponse.png[,500,align=left]

You can then use `CURL` to upload the log file using the provided URL and UUID:
...
IMPORTANT: Remember to include the final forward slash (`/`) character at the end of the command.
...
After you have uploaded the log files,
you will receive an acknowledgement attached to your support ticket:
