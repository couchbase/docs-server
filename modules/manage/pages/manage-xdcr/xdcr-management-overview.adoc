...
Advanced settings can be configured to make sure optimal performance.
...
== XDCR In-Flight and Native Encryption-at-Rest 

Couchbase Server provides multi-layered protection for XDCR, securing data both across network boundaries and at rest on physical storage media.

=== In-Flight versus Native Encryption-at-Rest

Understanding the boundary between in-flight transport security and at-rest storage encryption to make sure comprehensive end-to-end protection for replicated workloads:

[cols="1,2,2",options="header"]
|===
| Dimension | In-Flight Encryption (Network) | Encryption at Rest (Storage)

| *Scope*
| Protects replication streams and network packets traversing between the source and target clusters over LAN, WAN, or VPN networks.
| Protects persistent on-disk artifacts residing on cluster nodes (metadata, checkpoints, conflict logs, and diagnostics).

| *Mechanism*
| TLS (Transport Layer Security) configured via Half, Full, or Custom Certificate authentication during Remote Cluster setup.
| Native Encryption-at-Rest utilizing Master Keys and Data Encryption Keys (DEKs) integrated with KMS providers such as KMIP, GCP KMS, Azure KMS, HashiCorp Vault.

| *Configuration Level*
| Configured per Remote Cluster Reference in the XDCR settings.
| Configured at the Cluster Configuration, Bucket, and Log management levels.
|===

=== How Encryption at Rest Protects XDCR On-Disk Data

XDCR does not require dedicated encryption switches or isolated key management.
Instead, enabling native encryption-at-rest across cluster subsystems automatically extends protection to all on-disk XDCR components:

* *Replication Definitions and Remote Credentials:*
Remote cluster hostnames, authentication credentials, and active stream configurations stored in `ns_config` and `metakv` are encrypted via Cluster Configuration Encryption.

* *Replication Checkpoints abd State:*
vBucket replication stream checkpoints and progress markers written to cluster metadata are protected via Cluster Configuration Encryption.

* *Conflict Resolution Documents:*
Conflict logs, revision metadata, and shadow records created during active-active bidirectional replication are encrypted through Bucket Encryption on the respective target buckets.

* *XDCR Service Logs:*
Operational diagnostics, stream state transitions, and error traces written to `goxdcr.log` are encrypted via Cluster Log Encryption.

[NOTE]
====
To make sure you have complete protection for XDCR pipelines:

1. Configure *TLS encryption* on all Remote Cluster references for secure transport across networks.
2. Enable *Cluster Configuration*, *Bucket*, and *Log* encryption to make sure all replication metadata, stored records, and logs are protected on disk.
====
