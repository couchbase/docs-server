...
. If the half-secure connection is being made to a pre-5.5 version of Couchbase Enterprise Server, copy and paste the root certificate for the destination cluster into the interactive panel, below the radio buttons.
...
(If the connection is being made to a 5.5 or a later version, skip this step.)
+
...
For more information on using the REST API to create half-secure connections, see the complete reference, at xref:rest-api:rest-xdcr-create-ref.adoc[Create and Edit a Reference].
