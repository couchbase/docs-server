...
+
However, in cases where only the _external_ network of the target cluster should be used, the name can be specified as one of the following:

** The fully qualified domain name of the target cluster

** A DNS SRV reference for the target cluster

+
If either of these is specified, only the external network of the target cluster is connected to by the source cluster.
For further information, see both xref:xdcr-reference:xdcr-security-and-networking.adoc[XDCR Security and Networking] and xref:learn:clusters-and-availability/connectivity.adoc#alternate-addresses[Alternate Addresses].
