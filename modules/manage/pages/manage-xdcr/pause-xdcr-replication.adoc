...
In addition to manual pause and resume, XDCR can automatically pause a replication pipeline.
XDCR triggers this pause when it detects an excessive number of true conflicts within a configured time window.
This prevents further data drift or unintended overwrites during high-conflict scenarios, giving you time to investigate and resolve issues before resuming data transfer.
...
XDCR automatically pauses replication when the number of true conflicts detected on any source cluster node exceeds the configured threshold within the specified monitoring window.

2 replication settings govern auto-pause behavior:
...
| `0`
| The maximum number of conflicts allowed within the monitoring duration before auto-pause triggers.
When set to 0, the system disables auto-pause, and standard logger hibernation behavior applies.

| `cLogMonitorDuration`
| `0` seconds
| The rolling time interval (in seconds) over which XDCR evaluates conflict rates against cLogPauseReplThreshold.
Valid values range from 0 to 2592000 seconds (up to 30 days).
|===

When you set both settings to non-zero values:
...
When the system auto-pauses a replication:

* In the Couchbase Web Console, under menu:XDCR[Outgoing Replications], the replication status changes from *Replicating* to *Paused*.
* An error banner or status badge indicates that the system automatically initiated the pause due to conflict threshold limits rather than manual operator intervention.
...
=== Recover an Auto-Paused Replication
...
For more information, see xref:learn:clusters-and-availability/xdcr-conflict-logging-feature.adoc#conflict-logger[Conflict Logging].
...
. *Resume replication:*
Once you mitigate the cause, resume the pipeline:
+
* *Web Console:* Navigate to menu:XDCR[Outgoing Replications], locate the paused replication, and click btn:[Resume].
...
NOTE: Resuming the replication resets the conflict counter for the current monitoring window.
If conflict rates continue to exceed `cLogPauseReplThreshold` after resuming, the pipeline auto-pauses again.
