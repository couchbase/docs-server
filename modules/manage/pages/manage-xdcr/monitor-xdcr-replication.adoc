...
xref:manage:manage-xdcr/monitor-xdcr-replication.adoc#access-buckets-statistics-source[Access
...
can be monitored by means of the
xref:cli:cbstats-intro.adoc[cbstats] CLI. See
xref:understanding-couchbase:clusters-and-availability/xdcr-monitor-timestamp-conflict-resolution.adoc[Monitoring
XDCR Timestamp-based Conflict Resolution], for details.
