...
+
NOTE: If the remote cluster is the same as the current cluster (if the source and the target clusters are the same), the replications are not listed in the Incoming Replications section.
