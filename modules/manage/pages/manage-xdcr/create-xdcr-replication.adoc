...
* Create a XDCR replication with `mobile=Active` or update an existing replication. For more information, see _Greenfield deployment_ and _Upgrade an existing setup_ in xref:learn:clusters-and-availability/xdcr-active-active-sgw.adoc[XDCR Active-Active with Sync Gateway]
...
* Create or update a XDCR replication with `mobile=Active` option from UI, starting from Server 7.6.6 version. 


The pre-requisite to use `mobile=Active` is to set the bucket property `enableCrossClusterVersioning`. For more information, see xref:learn:clusters-and-availability/xdcr-enable-crossclusterversioning.adoc[XDCR enableCrossClusterVersioning] and xref:learn:clusters-and-availability/xdcr-conflict-resolution.html#the_conflict_resolution_process[The Conflict Resolution Process].
