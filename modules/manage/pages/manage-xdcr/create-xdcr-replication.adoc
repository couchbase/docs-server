...
== Create an XDCR Replication by enabling Conflict Logging
...
[#prerequisites-create-xdcr-conflict-logging]
=== Prerequisites

Before using the XDCR conflict logging feature, enable it. The prerequisites for enabling XDCR’s conflict logging capability are as follows in the sequence:

* All clusters in the XDCR topology must be on the Server 8.0.0 or a higher version.
* Presence of conflict collection.
* Configure replication settings such as enabling HLV, conflictLogging key, and loggingRules JSON object.

For more information, see Prerequisites for enabling XDCR conflict logging and Enable XDCR conflict logging.

[#xdcr-conflict-logging-process]
