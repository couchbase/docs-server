...
** *Outgoing Replications*: Lists the details of replications that are being created from the current cluster to a remote cluster.
** *Incoming Replications*: Lists the details of remote clusters that are replicating to the current cluster.
For more information, see xref:manage:manage-xdcr/create-xdcr-replication.adoc#identify-target-cluster-in-xdcr[Identify the Target Cluster in a Replication].
+
...
This is recognized as target awareness and is used to display incoming replications on the target cluster.

On the XDCR UI, the target cluster displays incoming replications, showing the source cluster and bucket information for each active replication.

image::manage-xdcr/incoming-replications-on-target.png[,800,align=left]

The information for the above incoming replications is retrieved using the REST API. See xref:rest-api:rest-xdcr-identify-target-cluster.adoc#complete-response[Cluster with replication specifications - a complete example].

To get the target awareness information using the REST API, see xref:rest-api:rest-xdcr-identify-target-cluster.adoc[Identify the Target Cluster in a Replication].
...
Use this calculation to estimate the connection load on your target cluster.

==== Target Awareness Heartbeat and Lag Time

Target awareness relies on heartbeats sent from the source cluster to the target cluster.
These heartbeats provide information about incoming replications.
The incoming replication information is shown on the UI and returned by the `GET /xdcr/sourceClusters` REST API, as documented in xref:rest-api:rest-xdcr-identify-target-cluster.adoc[Identify the Target Cluster in a Replication].

The interval for sending heartbeats is controlled by the target awareness heartbeat interval on the source cluster.
...
Heartbeats are sent as soon as a remote reference is created, even if no replications exist yet.
In this scenario, the incoming replication view on the target cluster shows only the source cluster information.
For the REST API example response, see xref:rest-api:rest-xdcr-identify-target-cluster.adoc#example-source-without-replication-specs[A cluster without replication specifications].
...
For the REST API example response with a replication specification, see xref:rest-api:rest-xdcr-identify-target-cluster.adoc#example-source-with-replication-specs[A cluster with replication specifications].
