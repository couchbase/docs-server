...
The *XDCR Add Replication* page displays the following additional settings, which are specific to logging conflicts during an XDCR replication:
...
* *Default Conflict Log Collection*: Use this setting to specify the default bucket, scope, and collection for conflict logging. If a custom collection is not specified, the default collection is used.
+
...
* *Customise Conflict Logging at scope level*: Use this setting to specify the scope for conflict collection.
+
image::manage-xdcr/customise-conflict-logging-at-scope.png[,400,align=left]

* *Customise Conflict Logging at collection level*: Use this setting to specify the collection to store conflict logs. You can choose a custom conflict collection that overrides the default conflict collection. Choose one of the following collection types:
...
+
image::manage-xdcr/customise-conflict-logging-collection.png[,400,align=left]

For more information, see xref:learn:clusters-and-availability/xdcr-conflict-logging-feature.adoc#use-conflictlogging-and-loggingrules[Use conflictLogging Key and loggingRules Value in XDCR].
...
[#create-xdcr-by-conflict-logging]
== Create an XDCR by Enabling Conflict Logging

XDCR detects and logs the concurrent conflicts occurring in different clusters during XDCR Active-Active replication, so you can manually resolve conflicts and prevent data loss.
