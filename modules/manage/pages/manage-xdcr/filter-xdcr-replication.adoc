...
Note that the replication of deletions is _not_ inhibited by the specifying of a filter, formed with regular and other filtering expressions: to ensure that document-deletions are _not_ replicated, the appropriate deletion-filter checkboxes must be checked.
For more information, see xref:learn:clusters-and-availability/xdcr-filtering.adoc#using-deletion-filters[Using Deletion Filters].
