...
Starting in Couchbase Server 8.5.0 (and 8.0.1, 7.6.10), you can apply granular expression-based filtering to document deletions and expirations using **Key ID filter expressions** (`META().id`).
...
** *Apply Filter Expression to Deletions* (`filterDeletionsWithExpression`): Evaluates the replication's filter expression against deleted documents.
Document deletions replicate to the target *only* if the document ID matches the filter expression.

* *Remove TTL from replicated items* (`filterBypassExpiry`): The filterBypassExpiry flag indicates whether a document’s TTL should be replicated with the document or not. 
...
By default, XDCR filter expressions are not evaluated on deleted documents (tombstones) or expired documents because they lack document bodies. Consequently, deletions and expirations are replicated even when an expression filters out regular mutations. 

When your filter expression evaluates only document keys (`META().id`), you can configure XDCR to apply the expression to deletions and expirations.

To enable Key ID deletion and expiration filtering when creating a replication:
...
. In the *Filter Expression* field, enter an expression that filters exclusively by document key ID (`META().id`).
+
For example:
+
[source,sql]
...
[IMPORTANT]
====
The filter expression must reference *only* the document key (`META().id`). If the expression references document body fields or extended attributes (`xattrs`), key-filter evaluation for deletions and expirations cannot be enabled.
====

. Expand *Advanced replication settings* and configure the deletion and expiration filters:
* To filter deleted documents by key ID:
.. Select *Do not replicate DELETE operations*.
.. Select *Except those matching key-filter expression*.
* To filter expired documents by key ID:
.. Select *Do not replicate document expirations*.
.. Select *Except those matching key-filter expression*.
+
[NOTE]
====
The *Except those matching key-filter expression* checkbox is only active when its parent option (*Do not replicate DELETE operations* or *Do not replicate document expirations*) is selected.
====

. Configure any additional replication settings as needed.
. Click *Save* to create and start the replication.
...
. Locate the replication stream in the *Outgoing Replications* table.
. Click the *Edit* button in the row corresponding to the replication.
. In the Filter Expression panel, unselect the checkbox for *Replicate deletions and expirations only if they match the key only filter expression* or the *Deletion Filters* options.
