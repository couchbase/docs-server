...
NOTE: If you configure cluster alerts under menu:Settings[Alerts], deleting an XDCR replication triggers an alert with the message `XDCR replication has been deleted`.
Couchbase Server sends this alert by email, by UI popup, or both.
The alert fires regardless of the replication's status at the time of deletion.
...
=== Delete a Replication in the Web Console

. Navigate to *XDCR* in the left navigation bar.

. In the *Outgoing Replications* table, locate the replication you want to delete.

. Click the *Delete* button (trash can icon) in the *Row* controls.

. When prompted in the confirmation dialog, click *Delete Replication*.
