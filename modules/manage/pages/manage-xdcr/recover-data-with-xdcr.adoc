...
. If the unavailable nodes and their data absolutely cannot be retrieved, fail each node over, by clicking the *Failover* button.
Then, remove each node from the cluster, by clicking the *Remove* button.
...
. To conclude the data-recovery process, rebalance the nodes on the local cluster, by clicking the *Rebalance* button.
