...
You can migrate a bucket's storage backend if you find the bucket's current backend is not appropriate. You can migrate from Couchstore to Magma, or from Magama to Couchstore. For example, you can migrate a bucket from Couchstore to Magma if the bucket's working set grows beyond its memory quota. 
