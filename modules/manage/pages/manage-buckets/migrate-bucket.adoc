...
See xref:modules:buckets-memory-and-storage/memory.adoc#ejection[Ejection] for more information about ejection policies.

You can change the ejection policy at the same time you change the storage backend.
Both procedures require you to perform a swap rebalance or a graceful failover followed by a full recovery to complete the process.
...
For example, the folowing command changes the storage backend of the `travel-sample` bucket to Magma and sets the ejection policy to Full Ejection without restarting the bucket immediately:
