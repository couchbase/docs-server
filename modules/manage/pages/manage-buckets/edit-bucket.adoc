...
xref:manage-buckets/create-bucket.adoc[Create a Bucket]
...
* *Auto-Compaction*: When established, these settings, which determine the conditions under which data-compaction for the bucket is performed, override the cluster-wide defaults; as discussed in
...
xref:manage-buckets/flush-bucket.adoc[Flush] command for the current bucket.
