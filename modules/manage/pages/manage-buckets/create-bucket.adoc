...
Note that other auto-compaction settings do not apply to Ephemeral buckets, since such settings are for data that reside on disk.
See xref:manage:manage-settings/configure-compact-settings.adoc[Configuring Auto-Compaction] for information on how to use the Metadata Purge Interval; which applies both to Couchbase _and_ Ephemeral buckets.
