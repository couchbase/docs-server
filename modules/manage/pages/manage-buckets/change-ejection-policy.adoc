...
In this case, you can choose whether to have Couchbase Server restart the bucket after you change the ejection policy.
You control this behavior using the `noRestart` parameter in the REST API POST method to `/pools/default/buckets/{BUCKETNAME}`:

* To trigger the automatic bucket restart, set the `noRestart` parameter to `false` or do not supply it in the REST API call.
