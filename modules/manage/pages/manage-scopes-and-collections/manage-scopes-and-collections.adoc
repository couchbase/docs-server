...
This value means that the collection inherits the `maxTTL` value from the bucket.  
...
----
+
The example sets `max-ttl` to `0`. This value means the collection inherits the `maxTTL` setting from the bucket. See xref:learn:data/expiration.adoc[Expiration]) for more information about `maxTTL` and expiration.
+
...
The `maxTTL` for the collection is `0`, which means it inherits its `maxTTL` setting from the bucket.
...
----
+
Setting `maxTTL` parameter to `0` manes that the collection inherits the bucket's `maxTTL` setting.
