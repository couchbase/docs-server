...
Note that the create bucket, scope, and collection commands are asynchronous by default. We recommend that you factor in some time for the cluster to be updated. If you create a collection right after creating a scope, or create an index right after creating a collection, it is possible that you may encounter a failure if the cluster is still being updated.
...
----
+
The `max-ttl` parameter is set at `0`, which indicates that no expiration time is applied to the new collection (this being the default &#8212; for information on expiration, see xref:learn:buckets-memory-and-storage/buckets/expiration.adoc[Expiration]).
+
