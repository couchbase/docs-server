...
----
+
The `maxTTL` parameter is set at `0`, which indicates that no expiration time is applied to the new collection (this being the default &$8212; for information on expiration, see xref:learn:buckets-memory-and-storage/buckets/expiration.adoc[Expiration]).
+
...
(For information on expiration, see xref:learn:buckets-memory-and-storage/buckets/expiration.adoc[Expiration]).
