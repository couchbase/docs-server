...
These can be used to test queries, and determine how indexes can be defined to maximize query-performance.
The buttons are as follows.

==== Execute

*Execute*, when left-clicked on, executes the query that has been typed into the *Query Editor*.
For example, type the following query into the *Query Editor*: `SELECT icao FROM &#96;travel-sample&#96; WHERE name = "SeaPort Airlines";`.
This selects every `icao` key-value pair from the bucket `travel-sample`, where the document contains a `name` value that is `SeaPort Airlines`.
The *Query Editor* therefore appears as follows:
...
The *Explain* button can be left-clicked on to provide an explanation of how query-execution proceeded:
...
This indicates the bucket and primary index that have been used in the query, as well as the filter applied, and the number of terms returned.

==== Advise

The *Advise* button can be left-clicked on to display advice as to what index or indexes might be created, in order to improve the future performance of the query and others similar in structure:
...
In this instance, the advice consists of two options, which are the creation of a _covered_ and a regular index, respectively.
To create an index, left-click on the button provided in the *Query Display* panel.
For example:
...
The following success-message now appears on the *Query* screen:
...
The https://index-advisor.couchbase.com/indexadvisor/#1[External Query Advisor] takes the user to an external web-site, where the *Query Advisor* can be accessed and used.
