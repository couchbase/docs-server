...
* DNS records must be managed, to ensure address-resolution to both IPv4 and IPv6 _during_ the changeover, and to either IPv6 or IPv4 _following_ the changeover (at which point, address-resolution can be disabled for the address family no longer used).
