...
Note that when the CLI is used, the address family for the cluster can be established in either of two ways:

** The specified address family is required, but the other supported address family can also be used.
(This is the default setting, with the IPv4 address family being required.)

** Only the specified address family can be used.
