...
** *Storage Paths* for the Data, Index, and Analytics services, specified during node-initialization.
...
+
The screen provides statistics related to each of the buckets on the cluster.
These include general information on operations; gets, sets, and deletes, read and write failures, and
incoming and outgoing XDCR operations.
Specific details are provided for server resources, vBucket resources, disk and DCP queues, Indexes, and more.
