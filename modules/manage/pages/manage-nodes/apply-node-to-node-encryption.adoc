...
Note that IP address-family and node-to-node encryption-enablement can also be managed when a cluster is being created.
...
. Ensure that the IP family of the cluster is the right one for your needs: either IPV4 or IPV6. To set the family, use the `ip-family` CLI command:
+
----
...
. Confirm the established IP-address family, using the `--get` parameter to `ip-family`:
+
----
/opt/couchbase/bin/couchbase-cli ip-family \
-c http://node1-devcluster.com:8091 \
