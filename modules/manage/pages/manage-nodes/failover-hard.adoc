...
=== Hard Failover with Multiple Unresponsive Nodes
...
image::manage-nodes/hardFailoverDangerDialog.png[,520,align=middle]

This indicates that hard failover of these nodes is dangerous; and should only be attempted if there is no alternative.

For information the _safe_ and _unsafe_ modes of hard failover, see xref:learn:clusters-and-availability/hard-failover.adoc#safe-and-unsafe[Hard-Failover Modes: Safe and Unsafe].
