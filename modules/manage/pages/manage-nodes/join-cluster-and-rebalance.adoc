...
=== The Join Operation and Certificates

Frequently, a node must be joined to a cluster when _x.509 certificates_ have already been administrator-configured, and deployed on the cluster, to ensure its security.
In such cases, prior to proceeding with the join, the administrator should be familiar with the certificate-related information provided at xref:manage:manage-security/configure-server-certificates.adoc#adding-new-nodes[Adding New Nodes]; and should be familiar with Couchbase certificate-management procedures in general -- for an overview, see xref:learn:security/certificates.adoc[Certificates].
...
The examples also assume that only _default_ certificates reside on the cluster -- these require no special procedures to be observed in addition to those listed below.
However, if administrator-configured certificates have already been deployed, and the cluster is in production, the certificate-related information at xref:manage:manage-security/configure-server-certificates.adoc#adding-new-nodes[Adding New Nodes] should be consulted, before proceeding.
