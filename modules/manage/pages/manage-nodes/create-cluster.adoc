...
* *Host Name / IP Address*: The data in this field determines the name that will be used for this node, in order to support programmatic and networked access, including access by other nodes that join the cluster.
...
Enter the hostname or IP address for the machine on which you are configuring Couchbase Server.
