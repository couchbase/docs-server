= Create a Cluster
:page-aliases: install:init-setup,install:hostnames
...
its memory quotas. At this point, it becomes a _cluster_ of one
node.
...
xref:manage:manage-nodes/create-cluster.adoc#provision-a-node-with-the-ui[UI],
the
xref:manage:manage-nodes/create-cluster.adoc#provision-a-node-with-the-cli[CLI],
and the
xref:manage:manage-nodes/create-cluster.adoc#initialize-a-node-with-the-rest-api[REST
API] respectively.
