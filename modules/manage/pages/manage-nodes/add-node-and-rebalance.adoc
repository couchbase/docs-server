...
+
In this example, there is no need to check the *Change default disk paths* checkbox, which allows you to specify that disk paths other than the default be used for the new node.
Note, however, that if custom disk paths have already been specified for the node that is to be added, the custom paths are _retained_, if the checkbox is _not_ checked; and _changed_ to those specified in the corresponding fields, if the checkbox _is_ checked.
+
...
A set of constantly updated figures is provided for each bucket on the cluster.
For a detailed explanation of the _stages_ &#8212; *Move*, *Backfill*, *Takeover*, and *Persistence* &#8212; see xref:learn:clusters-and-availability/rebalance.adoc#data-service-rebalance-stages[Data-Service Rebalance Stages].
Note that the figure for *Persistence* relates only to _Couchbase_ buckets, and therefore remains at zero wherever the display is providing status on an _Ephemeral_ bucket.
Note also that the *Takeover* figure is specific only to _active_ vBuckets: therefore it remains at zero when only _replica_ vBuckets are being moved.
+
The dialog may also include progress information on data-compression.
+
