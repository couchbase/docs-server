...
Note that information can also be found by left-clicking on the *Logs* tab, in the left-hand vertical navigation bar.
This brings up the *Logs* screen, which may contain information such as the following:

image::manage-nodes/rebalanceFailureLog.png[,800,align=middle]

If failure occurs, the cause should be identified.
If an unresponsive node become responsive again, rebalance can simply be reattempted.
