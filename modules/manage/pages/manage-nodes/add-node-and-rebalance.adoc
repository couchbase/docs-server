...
+
[#serviceless-node-addition]
This indicates that the added node will run the *Data Service* only.
(Note that if no *Services* checkbox were checked, the added node would be _serviceless_.
This feature is available in Couchbase Server 7.6 and later, providing two forms of cluster-management support, which are xref:learn:clusters-and-availability/nodes.adoc#fast-failover[Fast Failover] and xref:install:deployment-considerations-lt-3nodes.adoc#quorum-arbitration[Quorum Arbitration].)
+
