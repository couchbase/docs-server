...
The node-to-group assignment can now be checked again, with the following:
...
The command thus specifies the username and password of the new, provisioned node.
Note that if the node is _unprovisioned_, these can be omitted.
Optionally, the `--services` flag is used to specify which services are to be assigned to the new node: here, the Data, Index, and Query Services are specified.
...
This confirms that the group has been deleted.
The node-to-group assignment can now be checked again.
This provides the following output:
