...
To display additional information, clicking on the *Node Resources* tab.

[#dashboard-appearance-with-bucket-addition]
image::manage-ui/ClusterOverview.png[,720,align=left]

To switch between the *Cluster Overview* display and the *All Services* display, use the pull-down menu at the upper left.
...
Five of these six services have an identically named tab corresponding to them, in the left-hand navigation bar.
...
After you have added one or more buckets, the *Buckets* screen displays information about tbe buckets.
...
The tabs are *General* (displayed by default), *Auto-Compaction*, *Alerts*, and *Sample Buckets*.
...
If one or more buckets are installed, the *Workbench* panel enables you to display and edit the internal content of documents.

[#console-documents-screen-with-documents]
image::manage-ui/documentsScreenWithDocuments.png[,700,align=left]

For a full description of this panel and its contents, see xref:manage:manage-documents/manage-documents.adoc[].
...
Content is presently displayed, provided that the cluster is running the Index Service.

[#query-screen-after-interval]
image::manage-ui/queryScreenAfterInterval.png[,700,align=left]

This screen now features the Query Workbench: an interactive tool that lets you compose and execute {sqlpp} queries on the data contained by the bucket.
In its initial display, the tool shows the sequence of {sqlpp} commands that have been executed to create the scopes and collections within `travel-sample`: for information, see xref:learn:data/scopes-and-collections.adoc[Scopes and Collections].
...
To access the *Analytics* screen, click the *Analytics* tab in the navigation bar.
