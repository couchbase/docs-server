...
When first accessed, this is comparatively empty: however, the user is able to add charts incrementally, in order to display continuously updated sets of statistics.
...
Note that system notifications are shown, as required, in the lower left-hand corner of the main panel.
These are green to indicate success, and orange to indicate actual or potential problems.
For example:

[#console-notification]
image::manage-ui/bucketSettingsSaved.png[,220,align=left]
...
After a sample bucket such as `travel-sample` has been installed, the screen appears as follows:
...
A full description of the *Documents* screen and its contents is provided in xref:getting-started:look-at-the-results.adoc[Explore the Server Configuration], which is part of the the _Getting Started_ sequence.
...
For information on N1QL, see xref:n1ql:n1ql-language-reference/index.adoc[N1QL Language Reference].
...
The screen contains and *Analytics Query Editor*, and a panel for *Analytics Query Results*.
...
See xref:n1ql:n1ql-language-reference/index.adoc[N1QL Language Reference] for information.
Index management, including editing, can be performed within Couchbase Web Console.
For example, left-clicking on the row of an index to be edited opens the row, and provides options:

[#console-edit-index-screen]
image::manage-ui/editIndex.png[,700,align=left]

Left-clicking on the *Drop* button removes the index from the bucket.
Left-clicking on the *Open in Workbench* button opens the index in the *Query* workbench:

[#console-query-editor-index-screen]
