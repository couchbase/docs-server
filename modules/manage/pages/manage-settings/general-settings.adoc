...
The default value of 0 has the the Index Service use one thread per core on the node.ß
...
The default cleared setting for *Enable File Transfer Based Rebalance* has an Index Server node rebuild any newly assigned index during a rebalance.
When selected, Couchbase Server copies the index files from one Index Server node to another during a rebalance instead of rebuilding them. 
