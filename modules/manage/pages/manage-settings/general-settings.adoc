...
The timeout period is configurable: the default length is 240 seconds; the minimum permitted is 5; the maximum 3600.
This checkbox can only be checked if *Auto-failover after _x_ seconds for up to _y_ node* has also been checked.
This option is selected by default.

* *Preserve durable writes*: If this checkbox is checked (which is the default), and the node to be automatically failed over is a Data Service node that hosts data for a bucket whose Minimum Durability Level is greater than *none*; auto-failover occurs only if xref:learn:data/durability.adoc#majority[Majority] can be maintained across the nodes on which the bucket's data resides.
