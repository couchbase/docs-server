...
+
NOTE: While queries support unlimited backfill when set to `-1`, GSIs do not perform backfill if the quota is set to `0` or less.
To enable backfill for GSIs, configure the quota to a sufficiently large positive value, such as `5120` or `10240`. 
