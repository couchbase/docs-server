...
* *Preserve durable writes*: If this checkbox is checked, a node is _not_ failed over if this would result in the loss of durably written data -- for example, if data has been durably written to two nodes, a maximum of one of those nodes can be automatically failed over.
The default is that the checkbox is unchecked.
For further information, see xref:learn:data/durability.adoc#preserving-durable-writes[Preserving Durable Writes].
...
Auto-failover is prohibited in cases where this would result in the loss of durably written data.
