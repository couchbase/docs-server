...
+
You can disable this feature from the UI or via REST API. To learn about disabling this feature via REST API, see xref:rest-api:post-settings-indexes.adoc#deactivate-file-transfer-based-rebalance[Curl Command to Deactivate the File Transfer Based Rebalance Setting]. This feature is disabled by default.
+
