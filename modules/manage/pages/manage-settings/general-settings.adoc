...
A high number of reader and writer threads will benefit disk based workloads that require high throughput when using high end disk drives such as NVMe SSDs.
This is likely to be the case when using Magma as the storage engine.
In this case it is best to choose 'Disk i/o optimized' mode for Reader and Writer thread settings.
