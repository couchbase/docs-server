...
The *Retry rebalance* option allows rebalance to be retried, in cases where it has failed.
Check the checkbox, to enable.
The specifiable, maximum number of retries must be in the range of 1 to 3, inclusive.
The specifiable, maximum number of seconds must be in the range of 5 to 3600, inclusive.
...
[source,shell]
----
curl -v -X GET -u $COUCHBASE_USERNAME:$COUCHBASE_PASSWORD \
...
[source,shell]
----
curl -v -X POST -u $COUCHBASE_USERNAME:$COUCHBASE_PASSWORD \
...
You can also override the FBR setting on a per-bucket basis using 
...
==== Limit FBR Backfill Bandwidth Use

By default, Couchbase Server does not limit the network bandwidth FBR uses when transferring vBucket data over the network.
These unlimited transfers could increase latency for other Couchbase Server features, such as key-value operations.
You can use the REST API to set the `snapshot_download_throttle_bytes` option, which limits the FBR's network use.

To determine if there are limits on FBR's network use, make a `GET` request to the `pools/default/settings/memcached/global` REST API endpoint:
...
This sets the maximum transfer rate FBR can use on each node. 
The following example limits FBR file downloads to 600MiB/s per node. 
