...
When cleared, *Optimize Index Placement On Rebalance* (the default) has Couchbase Server only redistribute indexes during a rebalance when the nodes containing them are leaving the cluster.
...
* *Bloom Filter*:
When cleared, *Enable Bloom Filter* (the default) turns off Bloom filters for memory management.
...
When cleared, *Enable File Transfer Based Rebalance* (the default), has an Index Server node rebuild any newly assigned index during a rebalance.
When selected, Couchbase Server copies the index files from one Index Server node to another instead of rebuilding them. 
