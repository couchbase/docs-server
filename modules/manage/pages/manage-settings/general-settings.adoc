...
+
You can deactivate this feature from the UI or via REST API. To learn about deactivating this feature via REST API, see xref:rest-api:post-settings-indexes.adoc#deactivate-file-transfer-based-rebalance[Curl Command to Deactivate the File Transfer Based Rebalance Setting]. This feature is inactive by default.
+
WARNING: Deactivating this feature slows down the Rebalance operation.
