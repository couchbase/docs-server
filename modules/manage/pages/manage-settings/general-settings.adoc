...
However, pending rebalance-retries can be cancelled: see xref:manage:manage-nodes/add-node-and-rebalance.adoc[Add a Node and Rebalance], for information.
...
To determine how many XDCR processes are configured for the current node, use the `GET /settings/replications` http method and URI, as follows.
