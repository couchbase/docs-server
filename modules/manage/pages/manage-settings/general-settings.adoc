...
The *Max moves per node during rebalance* option establishes the maximum number of vBucket moves permitted on every individual node.
...
By means of the REST API, both _rebalance retries_ and _maximum moves per node_ can be configured.
...
==== Maximum vBucket Moves via REST

To inspect the current maximum number of vBuckets permitted for every node, use the `GET /settings/rebalance` HTTP method and URI, with the `rebalanceMovesPerNode` parameter, as follows:
