...
This enables auto-failover, with a timeout of 120 seconds, and an event-maximum of 2. It also enables failover server groups, and specifies, by means of the `c--can-abort-rebalance` flag, that if a node becomes unresponsive during a rebalance, that node can be failed over automatically, and the rebalance thereby aborted.
...
-d '{"all_access":false,"allowed_urls":["https://github.com"],"disallowed_urls":["https://google.com"]}'
----
...
----
{"all_access":false,"allowed_urls":["https://github.com"],"disallowed_urls":["https://google.com"]}
----
