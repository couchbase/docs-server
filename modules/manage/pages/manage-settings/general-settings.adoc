...
image::manage-settings/index-storage-mode.png["The Index Storage Mode panel",448,align=center]
...
Clearing **Optimize Index Placement On Rebalance** (the default) has Couchbase Server only redistribute indexes during a rebalance when the nodes containing them are leaving the cluster.
Selecting it has Couchbase Server redistribute indexes among the Index Service nodes during to optimize performance.
...
* *Bloom Filter*:
Clearing *Enable Bloom Filter* (the default) turns off Bloom filters for memory management.
Selecting *Enable Bloom Filter*, enables Bloom filters. 
...
Clearing *Enable File Transfer Based Rebalance* (the default) during a rebalance the destination node rebuilds any index Couchbase Server assigns to it.
See xref:learn:clusters-and-availability/rebalance.adoc#index-rebalance-methods[Index Rebalance Methods].
When selected, Couchbase Server copies index files between nodes instead of reuilding them.
