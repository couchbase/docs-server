...
In Couchbase Server Enterprise Edition, auto-compaction does not apply to memory-optimized indexes, and there are no settings necessary for configuring the auto-compaction of standard indexes.

However, for installations of Couchbase Server Enterprise Edition running on Linux, note that hole punching is required to enable auto-compaction of standard indexes.
****
