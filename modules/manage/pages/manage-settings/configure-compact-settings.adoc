...
Auto-compaction settings can be managed by means of the REST API.
To return the current auto-compaction settings for the cluster, use the `/settings/autoCompaction` method.

----
curl -i -X GET -u Administrator:password http://127.0.0.1:8091/settings/autoCompaction
----

If successful, this returns a JSON document that contains the current settings.
...
This establishes fragmentation thresholds and sizes for database and view, and specifies the timeperiod during which compaction should occur.
It specifies that compaction be aborted if it should overrun this timeperiod.
Parallel compaction for database and view is switched off.
The metadata pure interval is set to 3; and _circular_ standard compaction is specified for particular days and hours.
