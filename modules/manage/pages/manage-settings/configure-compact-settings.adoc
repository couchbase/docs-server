...
You can also change the default auto-compacton settings which apply to all buckets that do not have overrides.

Auto-compaction settings affect on-disk data, and therefore do not apply to Ephemeral buckets.

Users with the Full and Cluster administrator roles can configure compaction settings using <<configure-auto-compaction-with-the-ui,the Couchbase Server Web Console>>, the <<configure-auto-compaction-with-the-cli,command line tools>>, or the <<configure-auto-compaction-with-the-rest-api,REST API>>.

In Couchbase Server Enterprise Edition, auto-compaction does not apply to memory-optimized index storage.
Also, unlike Couchbase Server it does not have settings for configuring the auto-compaction of Global Secondary Indexes using standard index storage.
...
You can override the global auto-compaction settings for a bucket using the Couchbase Server Web Console either when you create it or when you edit it.
...
Under *Bucket Type*, you must select *Couchbase*.
...
It does not affect Magma buckets.
