...
| A loaded Certificate Revocation List (CRL) is within its warning period before its `nextUpdate`.
The warning period is controlled by the `crlExpirationDays` and `crlWarningValidityFraction` alert limits.
See xref:learn:security/certificate-revocation.adoc[Certificate Revocation].
| `crl_expires_soon`

| Certificate revocation list can no longer be used
| A loaded CRL is expired, not yet valid, no longer trusted, or invalid.
See xref:learn:security/certificate-revocation.adoc[Certificate Revocation].
| `crl_unusable`
