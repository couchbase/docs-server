...
=== Configure Alert Notifications in the Web Console

To enable or disable alerts for deleted XDCR replications:

. In the Couchbase Web Console, navigate to *Settings* in the left navigation bar.

. Select the *Alerts* tab.

. In the *Email Alerts* section:

  * Select **Enable email alerts** (if not already enabled) and configure your SMTP settings and recipient list.
  * Under **Available Alerts**, check the box for **XDCR replication has been deleted**.

. In the *UI Alerts / Pop-ups* section:

  * Check the box for **XDCR replication has been deleted** to display popup notifications in the Web Console when a replication is removed.
...
| Warns that the Index Service is, on a given node, occupying a percentage of available memory that's less than an established threshold, the default for which is `10`%.
...
| XDCR replication has been deleted
| An XDCR replication pipeline was deleted from the cluster. 
This alert is raised whenever any replication is deleted, regardless of the replication's operational status at the time of deletion (active, paused, or encountering errors).
...
To  alerts with the Couchbase REST API, use the `/settings/alerts` method, as follows:
