...
* The *cbimport csv* command-line utility, which imports data in CSV, TSV, and other _separated values_ formats.
...
Before any file has been selected, the default value, *CSV*, is displayed.
...
The selected bucket is the one into which import of the selected file is to occur.
For example:
+
...
The *Value of Field* option specifies that the _value_ that corresponds to a particular _field_ within each document should be used as the document's _id_: this option is only activated _after_ a document has been selected for import.
Selecting this option displays a pulldown menu, which lists those fields that are common to each document.
This is demonstrated below, in xref:manage:import-documents/import-documents.adoc#import-a-json-list[Importing a JSON List].
+
...
This display changes dynamically, to indicate the status of the underlying command that is being prepared for execution; in correspondence with the user's ongoing addition of parameter-values into the UI.
...
[
  ...
  {"name": "jack", "age": 43, "height": 5.11, "weight": 195, "birth": {"dayOfBirth": 3, "monthOfBirth": 12}}
]
----
+
