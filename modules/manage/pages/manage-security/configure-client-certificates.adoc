...
** The client certificate as `travel-sample.pem`.
** The client private key as `travel-sample.key`.

* If the procedures explained in xref:manage:manage-security/configure-server-certificates.adoc#root-intermediate-and-node-certificates[Cluster Proection with Root, Intermediate, and Node Certificates] and xref:manage:manage-security/configure-client-certificates.adoc#client-certificate-authorized-by-an-intermediate-certificate[Client Access: Intermediate-Certificate Authorization] have been followed, specify:
