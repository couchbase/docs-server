...
A sample Java program, which accesses a keystore from a local filesystem, is provided in  xref:3.0@java-sdk:howtos:sdk-authentication.adoc[Authenticating a Java Client by Certificate].
...
This concludes preparation of the Java client's keystore.
Copy the file (in this case, `my.keystore`) to a location on a local filesystem from which the Java client can access it.
A sample Java program, which accesses a keystore from a local filesystem, is provided in  xref:3.0@java-sdk:sdk-authentication-overview.adoc#authenticating-a-java-client-by-certificate[Authenticating a Java Client by Certificate].
