...
TLS versions 1.1 to 1.2 are supported: the highest-supported version of TLS is recommended.
...
The `tls-min-version` flag specifies the appropriate minimum TLS value, which can be either `tlsv1.1` or `tlsv1.2`.
...
The `tlsMinVersion` flag specifies the minimum TLS version to be used; which can be `tlsv1.1` or `tlsv1.2`.
If successful, the command gives a `200 OK` message, and returns and empty array.

For more information, see xref:rest-api:rest-setting-security.adoc[Setting Security].
