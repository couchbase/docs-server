...
* _Authentication_: Couchbase Server can check credentials itself, using _native_ methods &#8212; usernames and passwords, or X.509 client certificates.
It can also delegate the check to a system you already run, using an _external_ method: LDAP, JWT, SAML, or PAM.
External methods give you one place to grant and revoke access across every application, and can add features such as multi-factor authentication and single sign-on.
For an explanation of each method and how to choose between them, see xref:learn:security/authentication-overview.adoc[Understanding Authentication].
For the configuration steps, see xref:manage:manage-security/manage-authentication.adoc[Manage Authentication].
