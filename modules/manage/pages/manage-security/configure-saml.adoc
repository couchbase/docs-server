...
To learn moer about SAML, see xref:learn:security/authentication-domains.adoc#saml-authentication[SAML Authentication]. 
...
SAML authentication requires encryption. You need a key and certificate for your Couchbase cluster to use when encrypt and decrypt this data. You can use a unique certificate specifically for SAML authentication. You can also use the same certificate you use for client authentication. 
 
Most IdPs expect certificates that are signed by a _Certificate Authority_ (CA) to verify authenticity. You can use a certificate signed by an intermediate certificate that was in turn signed by a CA if you also provide the intermediate certificate. For example, suppose your organization creates a certificate for you that they sign using a certificate signed by a CA. You can use your certificate with an IdP that requires CA-signed certificates by including a copy of your organization's CA-signed certificate. The IdP uses the intermediate certificate to verify the authenticity of your cluster's certificate.
  
You can supply Couchbase Server with a certificate from the IdP so it can authenticate the IdP's metadata. Your IdP provides this certificate to you. The metadata includes a certificate that Couchbase Server uses to decrypt and authenticate SAML messages. 

=== Trusted Fingerprints
Instead of using an IdP-provided certificate, you can use trusted fingerprints to verify the authenticity the IdP's metadata or all of the IdP's SAML messages. Using fingerprints reduces the overhead of verification. 
...
The SAML authentication message sent from the IdP to Couchbase Server identify the user using a format called nameID. The nameID format is a colon-delimited string. It can be one of the following values:
...
After you enable SAML authentication, the top of the configuration page lists several URLs you need to configure your IdP: 
...
* *Current SP consume URL*: the URL the IdP sends the user to after they have authenticated.
* *Current SP logout URL*: the URL to which the IdP sends the user when they log out of the Web Console.
...
Add the Couchbase Server's private key and certificate. Either copy and paste their content or upload them by clicking bth:[Select File]. If the certificate was not directly signed by a CA, added the intermediate certificates in *Certificate chain*. 
...
In most cases you do not need to change the settings under *Advanced*. If you're having an issue with the SAML integration, these settings may help resolve it. Most of these settings are self-explanatory. The fllowing need some explanation: 
