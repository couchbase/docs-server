...
Couchbase Server can be configured to work with an LDAP server, so that the user authenticates with the LDAP server in order to gain access to Couchbase Server; and so that _LDAP group_ information for the user can be retrieved from the LDAP server, and mapped to groups and group-based roles defined on Couchbase Server.
For details on using this recommended procedure, see xref:manage:manage-security/configure-ldap.adoc[Configure LDAP].

The `saslauthd` process can still be used to support SASL authentication on LDAP or other servers if required.
...
Note that for Couchbase Server Enterprise Edition 6.5+, the recommended way of setting up LDAP authentication is by means of _Native LDAP Support_.
For an overview, see xref:learn:security/authentication-overview.adoc#introduction-to-ldap-based-authentication[LDAP-based Authenticaton].
For configuration steps, see xref:manage/manage-security/configure-ldap.adoc[Configure LDAP].
