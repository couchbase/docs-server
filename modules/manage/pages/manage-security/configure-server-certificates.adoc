...
. Change ownership of the private key to `couchbase`.
Enter the following:
+
----
sudo chown couchbase ./private/pkey.key /opt/couchbase/var/lib/couchbase/inbox/pkey.key
----
...
. Change the ownership of the private key to `couchbase`.
+
----
sudo chown couchbase /opt/couchbase/var/lib/couchbase/inbox/pkey.key
----
...
When the certificate-management procedures described above are used, as intended, for multi-node clusters, the following modifications should be observed:
...
** A root CA that has already been uploaded into the trust store of the cluster to which the node is being added.

** A node certificate that is signed by that root CA, or by one or more intermediate certificates that confer that root CA's authority.

If certificate-preparation has not been completed, the following error message appears in the *Add Server Node* dialog of Couchbase Web Console: `Attention: Prepare join failed. failed to establish TLS connection to <ip-address>:8091: "unknown ca"`.

Note that although other aspects of the node's prior provisioning will be erased by the addition, the certificates remain in place.
...
This means that the current node and (if they have been uploaded) intermediate certificates are removed; and new, system-generated root and node certificates are made active for the node.
