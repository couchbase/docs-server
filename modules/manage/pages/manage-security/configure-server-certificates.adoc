...
Almost without exception, Couchbase Server-clusters in production each contain _multiple nodes_.
...
Each `chain.pem` should be generated from a new, unique private key (`pkey.key`); must be a concatenation of the node certificate with whatever intermediate certificates have formed its chain; and must have its own node's IP address specified as a `subjectAltName`.
...
An inbox must be created on the node, and the `chain.pem` and `pkey.key` files then moved there.
...
+
Respond by copying the `chain.pem` and `pkey.key` for the node into its inbox, then perform an upload and reload of the cluster certificate, specifying the IP address of the node.
