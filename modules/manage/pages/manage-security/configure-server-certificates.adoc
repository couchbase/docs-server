...
* Concatenation of all intermediate and node certificates into a single _chain.pem_ file.

* Upload of all intermediate certificates into the cluster's _trust store_, and representation of the node certificate alone as the _chain.pem_ file.
...
+
----
openssl req -new -sha256 -newkey rsa:2048 -keyout ../servers/ca-i.key \
...
+
The output from the request consists of the encrypted private key `../servers/ca-i.key` and the signing-request `req/server-signing.csr`.
...
-out issued/server-signing.pem -extfile ca-i.ext
----
+
...
+
The extension file `ca-i.ext` is thus applied to the certificate, so as to limit the certificate's capabilities.
...
+
----
cp issued/server-signing.pem ../servers/ca-i.pem
----
...
+
----
openssl x509 -CA ca-i.pem -CAkey ca-i.key -CAcreateserial \
-CAserial serial.srl -days 365 -req -in reqs/couchbase.node.svc.csr \
-out issued/couchbase.node.svc.pem -extfile temp.ext
----
+
Since this specifies that the certificate should be signed by the encrypted intermediate key, `ca-i.key`, a prompt appears, requesting the appropriate pass phrase.
Enter the phrase against the prompt.
+
The node-certificate file `couchbase.node.svc.pem` is hereby saved in the `issued` folder.
The certificate bears the constraints specified in `temp.ext`, and is granted the authority of the intermediate certificate and key, which are `ca-i.pem` and `ca-i.key` respectively.
...
+
----
openssl verify -trusted ../root/ca.pem -untrusted ca-i.pem \
...
+
----
cat issued/couchbase.node.svc.pem ca-i.pem > chain.pem
...
----
sudo mkdir /opt/couchbase/var/lib/couchbase/inbox/CA/  # if needed
sudo cp ca-i.pem /opt/couchbase/var/lib/couchbase/inbox/CA/.
cd ../root
sudo cp ca.pem /opt/couchbase/var/lib/couchbase/inbox/CA/.
----
