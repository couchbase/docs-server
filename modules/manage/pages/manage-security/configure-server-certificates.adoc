...
+
These are deployed by being moved to the `inbox` directory of the server.
Proceed as follows:
+
----
cd ..
...
+
As this indicates, the signed certificate has now been substituted for the default certificate.
...
For information on enabling client certificates with Couchbase Web Console, see xref:manage:manage-security/manage-security-settings.adoc#client-certificate-security-screen-display[Client Certificate].

To enable client-certificate handling with the REST API, enter the following:
