...
+
Note that in Couchbase Enterprise Server Version 7.2+, the node-name _must_ be correctly identified in the node certificate as a Subject Alternative Name -- as is done here, with `subjectAltName = IP:10.143.192.102`.
...
When a client attempts to access the cluster securely, it must be able to build the certificate chain used to convey the authority of the cluster's root CA (see xref:learn:security/certificates.adoc#intermediate-certificates[Intermediate Certificates]).
In Couchbase Server Version 7.1+, the certificates can be prepared for client inspection in either of two ways, which are:
...
=== Deploying an Intermediate Certificate as Part of the Node's Chain
...
Note that in Couchbase Enterprise Server Version 7.2+, the node-name _must_ be correctly identified in the node certificate as a Subject Alternative Name.
...
[#intermediate-upload]
=== Deploying an Intermediate Certificate via Trust Store
...
=== Using an Encrypted, Private Node-Key

The above examples use an _unencrypted_ private key, for the node.
If an _encrypted_ private node-key is used, a passphrase must be registered for it, so that the key can be securely retrieved and used when required.
See the reference page xref:rest-api/upload-retrieve-node-cert.adoc[Upload and Retrieve a Node Certificate].
