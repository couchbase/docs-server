...
This allows the root certificate to be kept private, when node-certificate or client-certificate signing is required.
...
----
+
The private key `couchbase.node.svc.key` is thus saved in the `private` folder, as the private key for each node in the cluster.
