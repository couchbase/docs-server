...
If a cluster uses _system-generated_ certificates, which Couchbase Server provides by default, no certificate-related work needs to be performed in order to add a new node to the cluster.
However, once a cluster is using _uploaded_ certificates, a node that is to be added must itself be provisioned with conformant certificates, before addition can be successfully performed: this means that the appropriate _chain_ file (containing the node certificate) and _private key_ must have been placed in the node's `inbox`, and the appropriate REST API then called.
From Couchbase Server Version 7.1, the new node is now always added over an _encrypted_ connection.

Note also that when a cluster is using uploaded certificates, and a new node is added to a cluster, the operation is performed with reference to a particular node that is already a member of the cluster.
