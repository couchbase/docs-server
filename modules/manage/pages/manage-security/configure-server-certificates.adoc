...
For steps to prepare a certificate supportive of a Java client, see xref:manage:manage-security/configure-client-certificates.adoc#java-client-certificate-authorized-by-a-root-certificate[Java Client Access: Root-Certificate Authorization].
...
The phrase will be stored in the certificate, and prompted for when administrative attempts to use the key occur.
...
+
The output from the request consists of the encrypted private key `../servers/ca.key` and the signing-request `server-signing.csr`.
...
----
+
Since this specifies that the certificate be signed by the encrypted intermediate key, `ca.key`, a prompt appears, requesting the appropriate pass phrase.
Enter the phrase against the prompt.
+
...
For steps to prepare a certificate supportive of a Java client, see xref:manage:manage-security/configure-client-certificates.adoc#java-client-certificate-authorized-by-an-intermediate-certificate[Java Client Access: Intermediate-Certificate Authorization].
