...
Adding the intermediate certificate to the peer's trust store makes sure the peer can establish a chain of trust from the node's certificate to a CA that it trusts.
...
You can choose to encrypt the private key for nodes when uploading them.
