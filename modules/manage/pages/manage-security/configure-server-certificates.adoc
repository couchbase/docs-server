...
If a new node is to be added to a cluster, and the cluster is already using administrator-uploaded root certificates (rather than the default, system-generated root certificate):

. Load the CA of the "cluster node" to the trust store of the node-to-be-added (load cluster CA basically)
Load CA of this node to the trust store of the node-to-be-added (most likely can be skipped because this CA is usually the same as in 1)
Make sure the cluster trusts this node's CA (it should already be true if CA from 1 and CA from 2 are the same);
