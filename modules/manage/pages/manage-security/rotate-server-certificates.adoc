...
If you use certificate revocation checking, rotating to a new CA has consequences for CRLs:

* A new CA has no CRL of its own until you load or generate one for it.
If revocation checking is enabled for certificates the new CA signs, load a current CRL from the new CA (or, for Couchbase Server's own cluster CA, let Couchbase Server generate one) before or as soon as you rotate to it.
* Removing an old CA from the trust store makes that CA's CRLs untrusted: Couchbase Server can no longer verify their signature, so certificates that depend on them for revocation status become undetermined.
