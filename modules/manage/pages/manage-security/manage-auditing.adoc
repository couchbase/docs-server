...
Events can be _filtered_ for the REST API, the Data Service, the Query and Index Services, the Eventing Service, the Analytics Service, Views, and the Audit facility..
...
Data Service events are represented by an _event name_ (such as *opened DCP connection*), adjacent to a checkbox; with an _event description_ at the right.
All are currently greyed out, since Data Service events have not been enabled.

Note, however, that a subset of the events feature checkboxes that are _already checked_.
This means that these events _have already been enabled_ &#8212; due to the general enablement of events for the node, which was made by switching the *Audit events & write them to a log* toggle to the right.
Inspection of the other event panels, such as *Query and Index Service*, and *Eventing Service*, will likewise show a subset of checked, and therefore enabled events; even though the panel itself has not been enabled.

To elect to log all the events for the Data Service, move the *enable all* toggle for the Data Service panel to the right:
...
To de-select one or more of the individual events whose checkboxes have been checked through enabling the panel, simply uncheck the appropriate checkboxes.
Note, however, that the subset of events that were enabled _prior_ to panel-enablement _cannot_ be disabled: this subset of events will always be audited, whenever auditing for the node has been enabled, by means of the *Audit events & write them to a log* toggle.
...
Auditable events are referred to with individual _ids_, as well as by name, description, and module.
...
If successful, the call returns an array of objects, each of which contains identifying information for an auditable event:
...
A comma-separated list of audit-event _ids_ is provided as the value for the `disabled` parameter; indicating that each corresponding event will be disabled.
Likewise, a list of `disabledUsers` is specified.
See xref:manage:manage-security/manage-auditing.adoc#ignoring-events-by-user[Ignoring Events By User], above, for information.
