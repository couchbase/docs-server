...
By default, the Couchbase SDK will only default to PLAIN when connecting over TLS ports. +
To ensure you cannot through the SDK, external users should use the TLS ports.

More details authentication methods can be found in xref:learn:security/authentication-overview.adoc#authentication-for-applications[Understanding Authentication] 
====
