...
For an overview, see xref:learn:data/scopes-and-collection.adoc[Scopes and Collections].
...
image::manage-security/dataCategoryOpen.png[,240,align=left]

Left-click on *Data Reader*, to reveal the selector for role-assignment options:

image::manage-security/dataReaderRoleAssignmentInitial.png[,240,align=left]

The selector features three fields, each operated by a control at its right-hand side: each control reveals a pull-down menu, from which selections can be made.
...
Left-click on the control for the left-hand field:

image:manage-security/selectBucketForRoleAssignment.png[,240,align=left]

This indicates that three buckets are available for selection, *testBucket*, and *demoBucket*.
Select *testBucket*.
The center field, which has until now been inactive, is activated.
Use its left-hand control to reveal what scopes are available in *testBucket* for assigment:

image:manage-security/selectScopeForRoleAssignment.png[,240,align=left]

This shows that the *MyScope* and *_default* scopes are available.
Select *MyScope*.
Now, left-click on the right-hand control for the field at the right:

image:manage-security/selectCollectionForRoleAssignment.png[,240,align=left]
...
image:manage-security/addRoleAssignmentSelection.png[,240,align=left]

The assigned role now appears immediately below the selector:

image:manage-security/addedRole.png[,240,align=left]

To delete the assigned role, left-click on the *X*, to the right.
...
image:manage-security/addedRoles.png[,240,align=left]
...
image::manage-security/addQueryCurlAccessRole.png[,420,align=left]
...
Roles that permit data-access can be assigned with reference to a bucket, to a scope with a bucket, or to a collection within a scope.
Syntactically, the assignment should be specified in square brackets, immediately after the role; with the bucket-name preceding (if one is specified) the scope-name; and the scope-name preceding (if one is specified) the collection-name.
...
This grants the user `dhiggins` the `data_reader` role on data within the collection `my_new_collection`.
