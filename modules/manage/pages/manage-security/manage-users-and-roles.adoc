...
Note that the group will not be displayed by Couchbase Web Console until at least one user has been created and assigned to the group.
...
==== Create a Local User with Direct Role-Assignment
...
The username and password for the user are defined, and the `query_external_access` and `analytics_reader` roles are assigned.
...
==== Create a Local User with Group-Based Role-Assignment
...
[#create-external-users-with-the-cli]
=== Create External Users with the CLI
...
[#create-an-external-user-direct-role-assignment-using-the-cli]
==== Create an External User Direct Role-Assignment, Using the CLI
...
[#create-an-external-user-with-a-group-mapping-using-the-cli]
==== Create an External User with Group-Based Role-Assignment, Using the CLI

To create an external user _with_ a group-based role-assignment, use the `user-manage` command as follows:
...
==== Delete an External User, Using the CLI
