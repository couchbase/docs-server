...
Note however, that setting `saslauthd` is _not_ required if _Native LDAP Support_ has been configured in order to support external authentication &#8212; as is recommended: see xref:manage:manage-security/configure-ldap.adoc[Configure LDAP] for details.
