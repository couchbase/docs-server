...
[#add-a-user-group-with-the-ui]
=== Add a Group

A _group_ can be defined, and roles assigned to it, by means of Couchbase Web Console.
Defined users can then be assigned to each group: each user is granted the group's roles.

Note that groups do not _need_ to be defined, for users to be added to Couchbase Server: users can be added individually, and each assigned specific roles.
However, in cases where the number of users is high, greater efficiency can be achieved by defining an individual group and its roles first, and then assigning multiple users to that group.
This also allows a group's role-assignments to be modified with an immediate effect on the access-rights of multiple users
...
+
This arrangement of bucket-related groups allows each user to be assigned roles that apply either to all buckets defined on the cluster, or to one or more particular buckets.
...
To create a *Cluster Admins* group, each of whose members is granted the `Cluster Admin` role, enter the following:

[#add-new-group-dialog-no-mapping]
image::manage-security/addNewGroupDialogNoMapping.png[,420,align=left]

As this indicates, no LDAP mapping is required to define a group for the membership only of locally defined users.
For an explanation of such mappings, see xref:manage:manage-security/configure-ldap.adoc#map-ldap-groups-to-couchbase-server-roles[Map LDAP Groups to Couchbase Server Roles].

Save the group by left-clicking on the *Save* button.
Note that the group will not be displayed by Couchbase Web Console until at least one user has been created and assigned to it.
...
The user may now be assigned one or more specific roles, or may be assigned to one or more groups (so as to inherit each group's assigned roles), or both.

[#adding-roles]
==== Assign Roles to a User

Roles can be assigned by checking checkboxes in the [.ui]*Roles* panel, which is displayed by default.
...
A single user-group, called `ClusterAdmins`, is thus shown to have been created.
To add the user to the group, check the corresponding checkbox:

image::manage-security/checkCllusterAdmin.png[,210,align=middle]
...
The single displayed row indicates that user `localUser` has been defined, and have been granted the *Query CURL Access* role (which was specifically assigned) and the *Cluster Admin* role (which has been derived from the user's membership of the *ClusterAdmin* group).
...
Note that the *Security* screen, following the definition of a user, displays two buttons towards the upper right, whereby *Users* and *Groups* panels can be accessed.
To inspect and make changes to currently defined group, left-click on the *Groups* button:

image::manage-security/accessGroupsButton.png[,150,align=left]

The *Groups* panel is now displayed:

image::manage-security/groupSecurityRow.png[,800,align=left]

As with the *User* panel, left-clicking on a group's row displays controls that include *Delete* and *Edit* options.
...
To create a user who is to be _locally authenticated_, enter the following:
