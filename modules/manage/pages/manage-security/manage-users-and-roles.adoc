...
Lef-click in the center field to reveal what scopes are available in *testBucket* for assigment:
...
For each non-global role, the bucket, scope, and collection to which the permissions are restricted are specified, as appropriate.
...
--roles query_external_access,analytics_reader,data_reader[testBucket:MyScope:MyCollection] \
--auth-domain local
----
...
The `query_external_access`, analytics_reader`, and `data_reader` roles are assigned by means of the `--roles` flag: to change direct role assignments, the user must be recreated, with all the new roles specified as the arguments to the `--roles` flag.
Note that the `data_reader` role is specified as applying only to the collection `MyCollection`, within the scope `MyScope`, within `testBucket`.
