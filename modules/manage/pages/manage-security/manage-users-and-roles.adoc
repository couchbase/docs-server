...
Users can be defined as _externally_ authenticated.
...
[#create-an-external-user-without-a-group-mapping]
==== Create an External User without a Group Mapping
...
To add a local user, use the `PUT /settings/rbac/users/local` method and URI.
For example:
