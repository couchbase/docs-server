...
This indicates that the ability to support _external authentication_ by means of `saslauthd` has not yet been enabled.
...
Note however, that setting `saslauthd` is _not_ required if _Native LDAP Support_ has been configured &#8212; as is recommended, in all cases where support of external authentication is required: see xref:manage:manage-security/configure-ldap.adoc[Configure LDAP] for details.
...
The display of roles is divided into sections.
The first section, which appears at the top of the panel, is for *Administration & Global Roles*: these roles are either administrative, or involve access to cluster-wide features.
Below the first group, the *All Buckets (&#42;)* group appears: roles in this section apply to all buckets defined on the cluster.
Below *All Buckets (&#42;)*, additional sections are displayed, one for each individual bucket currently defined on the cluster.
+
This arrangement allows the new group to be assigned roles that apply either to all buckets defined on the cluster, or to one or more particular buckets.
Left-click on the corresponding right-pointing arrowhead to display a section's contents, scrolling down if necessary:
+
