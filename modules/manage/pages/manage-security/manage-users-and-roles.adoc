...
+
Below the first category, the *Bucket*, *Data*, and other categories appear: roles in these categories can be applied to some or all buckets defined on the cluster.
...
[#create-a-group-a-bucket-related-role]
==== Create a Group with a Bucket-Related Role

To create a group with a bucket-related role (for example, *Data Reader*), having entered a *Group Name* into the *Add New Group* dialog, access the *Data* category, in the *Roles* panel.
...
This shows that the *MyCollection* and *_default* collections are available.
Select *MyCollection*.
