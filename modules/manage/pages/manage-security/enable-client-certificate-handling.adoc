...
Enabling client-certificate handling determines whether Couchbase Server requests a client certificate; it does not by itself check whether that certificate has been revoked.
If certificate revocation checking is also enabled for the `clientAuth` scope, a client certificate that appears on a loaded Certificate Revocation List (CRL) is refused, even though client-certificate handling would otherwise accept it.
