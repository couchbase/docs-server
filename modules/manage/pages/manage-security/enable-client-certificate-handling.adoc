...
[abstract]
Couchbase Server can be enabled to support certificate-based authentication of clients.
...
* *mandatory*: All clients _must_ present a certificate, in order to authenticate.
...
The administrator who configures client-certificate handling on Couchbase Server is expected to anticipate what forms of username-specification are likely to occur in the certificates used by client applications, and which are appropriate to handle.
+
...
To check the resulting configuration, use the `Getting /settings/clientCertAuth` http method and URI, as follows.
