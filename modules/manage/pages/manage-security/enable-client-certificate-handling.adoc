...
. Specify whether certificates are enabled, and if so, whether mandatory. The *Require Client Certificate* panel provides the following options.
...
. If *Require Client Certificate* has been set to either *Enable* or *Hybrid* or *Mandatory*, establish how the username within each client certificate is to be determined.
+
