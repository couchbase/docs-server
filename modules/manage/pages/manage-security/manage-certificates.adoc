...
* _Managing certificate revocation_: Couchbase Server can check certificates against Certificate Revocation Lists (CRLs), so that a certificate revoked by its issuer can no longer be used to connect to the cluster.
See xref:manage:manage-security/manage-certificate-revocation.adoc[Manage Certificate Revocation].
