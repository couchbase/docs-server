...
[#get-cluser-wide-settings-with-the-cli]
...
[#establish-other-settings-with-the-cli]
=== Establish Other Settings with the CLI
...
The Couchbase REST API allows cluster-wide on-the-wire security-setting to be retrieved; and allows _global_ and _per service_ settings to be specified.
Use the `GET` and `POST` HTTP methods, with the `/settings/security` URI.

For more information on all calls shown here, see the reference page xref:rest-api:rest-setting-security.adoc[Configure On-the-Wire Security].
