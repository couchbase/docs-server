...
If *Couchbase* is selected, the certificate already installed for the cluster is used.
(See xref:learn:security/certificates.adoc[Certificates] for information on certificates.)
...
+
The text of the appropriate certificate should be copied and pasted into the editable field provided.
...
Note that neither user-authentication with the _query builder_ nor _group authorization_ (both described below) can be performed anonymously.
...
When opened, this provides options for testing the authentication of specific users:
+
...
image::manage-security/addLdapDialogEnableGroupAuth.png[,400,align=left]
+
...
+
The *Test Groups Query* panel, when opened, allows a username to be specified, so that a test search can be run.
To perform the search, left-click on the *Test Groups Query* button.
...
image::manage-security/configureLDAPdialogSaveButton.png[,220,align=left]
