...
[#establish-ldap-settings]
=== Establish LDAP Settings

To establish LDAP settings, left-click on the *LDAP* tab, at the upper right of the *Security* screen:
...
* *Enable LDAP user authentication*.
Enablement means that users who attempt to access Couchbase Server without having been added as _local_ users will be attemptedly authenticated against the *LDAP Host(s)* specified above.
+
Switch on, to enable.
