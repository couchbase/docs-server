...
Note that LDAP _Distinguished Names_ must frequently be specified, as part of the Native LDAP configuration process.
For a complete description, see https://tools.ietf.org/html/rfc4514[RFC 4514 - Lightweight Directory Access Protocol (LDAP): String Representation of Distinguished Names].
...
* *Certificate*. Whether to validate Couchbase Server with the server certificate.
...
+
*Bind DN* requires input of
an LDAP https://ldap.com/the-ldap-search-operation/[search operation], whereby the user's LDAP _Distinguished Name_ is acquired from the LDAP server.
(Note that this user needs to have _read only_ access to the LDAP server, in order to be able to search for users and groups.)
The required format for the search operation is indicated by the placeholder text that appears in the field.
The `uid` will be the username with which the user has logged into Couchbase Server.
+
...
For information on client-certificate configuration, see xref:manage:manage-security/configure-server-certificates.adoc[Configure Client Certificates].
Note that this option is only enabled if either *TLS* or *StartTLSExtension* has been selected from the *Encryption* pull-down menu, described above.
+
...
The corresponding files can be selected by means of the *Select File* buttons, which bring up a file-selection interface.
For information on how the client certificate and private key are used, see xref:learn:security/certificates.adoc#client-certificates[Client Certificates].

Once all appropriate setting have been made to *LDAP Host(s)*, *Encryption*, *Certificate*, and *Contact LDAP host*, optionally, the *Check Network Settings* button can now be left-clicked on.
