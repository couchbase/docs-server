...
Throughout this page, references are made to LDAP Distinguished Names, which must be specified as part of Native-LDAP configuration. For the full definition of Distinguished Names, see https://tools.ietf.org/html/rfc4514[RFC 4514 - Lightweight Directory Access Protocol (LDAP): String Representation of Distinguished Names^].
...
* *Certificate Validation*. Whether to provide Couchbase Server with a copy of the LDAP server's root certificate, so as to allow Couchbase Server to validate the LDAP server when connecting.
...
+
*Paste Cert* should be selected if the LDAP server root certificate is _not_ the same certificate as has been installed for the Couchbase-Server cluster. When *Paste Cert* is selected, the panel expands vertically, to reveal the *Certificate Text* field:
+
...
* *Password*.
The password for the user who has been specified in the *Bind DN* field.
...
For example, if the template is `uid=%u,dc=example,dc=com` and the user logs in as `exampleUser`, the user will be mapped to the DN `uid=exampleUser,dc=example,dc=com`.
...
Couchbase Server performs a search operation to get the user's DN from the LDAP server.
...
image::manage-security/scopePulldown.png[,200,align=left]
