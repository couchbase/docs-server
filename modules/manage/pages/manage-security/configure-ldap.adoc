...
Couchbase Server can be configured to work with an LDAP server.
...
To configure an LDAP server to be used as a source of authentication for Couchbase Server, use the xref:cli:cbcli/couchbase-cli-setting-ldap.adoc[setting-ldap] command.
...
--group-description "Couchbase Server Full Administrators" \
...
-d roles=admin \
-d description=Couchbase+Server+Full+Administrators \
