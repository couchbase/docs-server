...
For example, based on the placeholder, if the user has provided `exampleUser` to Couchbase Server as their username, the user will be mapped to the DN `cn=exampleUser,dc=example,dc=com`.
...
Search scope is always `one`; and `%u` will be replaced with the username that has been presented to Couchbase Server by the user who is attempting to gain access.
