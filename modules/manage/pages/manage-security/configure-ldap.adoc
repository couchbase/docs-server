...
* *Certificate*. Whether to validate Couchbase Server by presenting the root certificate of the LDAP server.
...
+
If *Couchbase* is selected, the root certificate for the Couchbase-Server cluster is used for validation: this assumes that the same root certificate is installed for the LDAP server.
...
+
*Bind DN* requires the LDAP _Distinguished Name_ of the user to perform user-search and groups synchronization.
...
For explanations of *Base*, *Filter*, and *Scope*, see https://ldap.com/the-ldap-search-operation/[The LDAP Search Operation^].
...
--user-dn-query 'cn=admin,dc=example,dc=com' \
--bind-password 'password' \
--authentication-enabled 1 \
--user-dn-template '{"template":"uid=%u,ou=users,dc=example,dc=com"}' \
