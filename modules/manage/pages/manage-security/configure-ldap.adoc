...
[abstract]
Couchbase Server can be configured to authenticate users by means of LDAP; and to map the LDAP _groups_ of which each user is a member to roles defined on Couchbase Server.
...
Each LDAP group can be mapped to Couchbase-Server roles, which are thereby granted to the user.
...
Each example also shows how to map LDAP groups to Couchbase-Server roles, by means of Couchbase-Server user-groups.
...
The fields of the left panel are presented in three principal groups, each corresponding to a key aspect of LDAP configuration:

* _LDAP host configuration_.
Establish which servers run LDAP, and how they are to be contacted.

* _User authentication enablement_.
Establish that users not recognized as _local_ to Couchbase Server will be authenticated by means of the specified _LDAP host configuration_, and specify the procedures to be used.

* _Group authorization enablement_.
Establish that the LDAP group-membership of authenticated LDAP users will be checked, to determine whether mappings to Couchbase-Server user-groups exist, and specify the procedures to be used.

Each of these field-groups is described below.

[#ldap-host-configuration]
=== LDAP Host Configuration

The upper section of the left panel of the *LDAP Configuration* dialog displays the following fields:
...
With data entered in these fields, the dialog might appear as follows:
...
Fields to enable user authentication are made visible by means of the *Enable LDAP user authentication* switch.
...
The `%u` is replaced by the username that has been presented to Couchbase Server by the user who is attempting to gain access.

For example, based on the placeholder, if the user has provided `exampleUser` to Couchbase Server as their username, the user is mapped to the DN `cn=exampleUser,dc=example,dc=com`.
...
Search scope is always `one`; and `%u` is replaced with the username that has been presented to Couchbase Server by the user who is attempting to gain access.
...
If no match is found, Couchbase Server assumes that the DN is the username presented to Couchbase Server by the user who is attempting to gain access (in other words, the user presented a username such as `cn=exampleUser,dc=example,dc=com`).
...
Fields for enabling _LDAP Group Support_ are made visible by means of the *Enable LDAP group authorization & sync* switch.
Enablement means that Couchbase Server retrieves from the *LDAP Hosts(s)* all _LDAP group membership_ information for users that the *LDAP Host(s)* authenticate.
