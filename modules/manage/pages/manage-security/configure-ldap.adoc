...
Turns on and off a network compatibility setting controlling how Couchbase Server securely connects to an LDAP server through an intermediate system such as a proxy or firewall.
By default, this setting is on.
If you have difficulty getting Couchbase Server to connect to an LDAP server through an intermediate system, try turning this setting off. 
