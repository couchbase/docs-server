...
image::manage-security/configureLdapEnableLdapUserAuthField.png[,234,align=left]

This provides three options to map usernames to LDAP _Distinguished Names_ (https://ldap.com/ldap-dns-and-rdns/[DN^]). These next sections describe these options.
...
image::manage-security/ldapQuery.png[,311,align=left]
...
image::manage-security/ldapAdvancedQuery.png[,491,align=left]
...
For example, the following template has two regular expressions. The first attempts to extract the username and organization from an email address. The second captures the entire username and passes that as an LDAP query.
...
If a regular expression matches the username, Couchbase Server applies any captured portions of the regular expression to the back references in the replacement string and attempts to authenticate the user with LDAP. 
If the authentication fails, Couchbase Server returns 
