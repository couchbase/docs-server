...
The the discovery API's list of nodes contains the node's secure port by default. 
...
If you do not want to Prometheus to use the secure ports when retrieving metrics, you can change the port number the discovery API call returns. 
...
With this method, Prometheus defaults to periodically calling the endpoint so it automatically learns of any added or removed nodes. 
