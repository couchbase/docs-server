...
Prometheus connects to all nodes in the Couchbase Server cluster to collect metrics. 
To make these connections, it needs a list of the cluster's nodes. 
Couchbase Server provides a discovery API endpoint named `/prometheus_sd_config` that returns a list of the nodes in the cluster in a format that Prometheus can use.
...
Prometheus needs a username and password to call the discovery API and to gather metrics from the Couchbase Server nodes. 
You should create a Couchbase Server user specifically for Prometheus. 
Assign this user the External Stats Reader role which grants it the ability to call the discovery API. 
This role also allows it read Couchbase Server's metrics. 

The discovery API's list of nodes contains the node's secure port by default. 
Prometheus needs a certificate to connect to this secure port.
You can get the certificate from the Couchbase Server Web Console: 

. Click *Security*.
. Click the *Certificates* tab. 
. Copy the contents of the *Trusted Root Certificates* text field to a file.

Save the certificate file to a location in your Prometheus host. 
...
If you do not want Prometheus to use the secure ports when retrieving metrics, you can change the port number the discovery API call returns. 
See xref:rest-api:rest-discovery-api.adoc[Prometheus Discovery API] for more information. 

== Call the Discovery API

You have two ways to use the discovery API when configuring Prometheus to collect metrics from Couchbase Server: 

* An HTTP service discovery method, where you configure Prometheus to get the list directly from the discovery API endpoint. 
...
* A file-based method where you directly call the discovery API and save the results to a file. 
...
With this method, Prometheus periodically calls the endpoint to automatically learn of any added or removed nodes. 
...
You must provide authentication in this section in addition to the authentication Prometheus uses to retrieve the metrics. 
...
To change the list that the discovery API returns, add query parameters to the URL in the `http_sd_configs` section. 
See xref:rest-api:rest-discovery-api.adoc[Prometheus Discovery API] for a list of these parameters.
...
In this job, add a `file_sd_configs` key entry. 
Set its value to the path to the file containing the list of nodes you created earlier. 
+
