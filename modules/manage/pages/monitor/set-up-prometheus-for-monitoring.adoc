...
Prometheus must connect to all of the nodes in the cluster to collect metrics from your Couchbase Server database. 
Therefore, it needs to have a list of the cluster's nodes. 
To make this list easier to get and help automate updating Prometheus's list of nodes, Couchbase Server provides a discovery API endpoint named `/prometheus_sd_config`. 
