= Monitor with the UI
:description: pass:q[The *Statistics* screen allows statistics for Couchbase, Ephemeral, and Memcached buckets to be displayed; either for a specific server in the cluster, or for the cluster as a whole.]
