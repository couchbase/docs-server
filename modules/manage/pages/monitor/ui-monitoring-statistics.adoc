...
The information displayed in this section always applies to the whole of the current server, whichever bucket has been selected for monitoring.
...
| analytics heap used
| Amount of JVM heap used by Analytics Service on this server.

| analytics system load
| System load for Analytics Service on this server.

| analytics thread count
| Number of threads for Analytics Service on this server.

| analytics gc count/sec
| Number of JVM garbage collections per seconde for Analytics Service on this server.

| analytics gc time (ms.)/sec
| The amount of time in milliseconds per second spent performing JVM gargabe collections for Analytics Service on this server.

| analytics bytes read/sec
| Number of disk bytes read per second for Analytics Service on this server.

| analytics bytes written/sec
| Number of disk bytes written per second for Analytics Service on this server.

| analytics total disk size
| The total disk size used by the Analytics Service on this server.
|===
