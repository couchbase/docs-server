...
[abstract]
From the [.ui]*Buckets* menu, information and statistics about buckets and nodes is displayed for the entire Couchbase Server cluster.
...
* <<dcp_stats,DCP Queues>> - This section provides information about the DCP connections activity for the bucket.
...
* <<cbas_stats,Analytics Statistics>> - This section provides information about the Analytics ingestion activity for the bucket.
...
For Memcached bucket types, the <<memcached_stats,Memcached statistic summary>> is provided.
...
this server including swap usage, free RAM, CPU utilization percentage, connections, port requests, streaming requests, index RAM used, remaining index RAM, FTS RAM used, and Analytics resources used.
...
| analytics heap used
| The amount of JVM heap used by Analytics on this server.
...
| Number of JVM garbage collections for Analytics node per second.

| analytics gc time (ms.)/sec
| The amount of time in milliseconds per second spent performing JVM garbage collections for Analytics node.
...
The statistics information is therefore organized as a table with columns showing the statistics for DCP queues used for replication, XDCR, views/indexes, analytics, and other.
...
The Analytics section provides statistics about Analytics ingestion activity for the bucket.

image::monitor/cbas-stats.png[]
...
| ops/sec
| The number of operations (gets, sets, and deletes) processed by the Analytics service for this bucket per second.

| sync failed records
| The number of records that the Analytics service has failed to parse during bucket synchronization.

| total ops since bucket connect
| The total number of operations (gets, sets, and deletes) processed by the Analytics service for this bucket, since the bucket was last connected.
|===
