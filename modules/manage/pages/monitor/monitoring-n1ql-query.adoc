...
* Ability to access active / completed / prepared requests across all {sqlpp} nodes from {sqlpp}.
...
This is separate from the {sqlpp} clustering view, in that {sqlpp} clustering shows a map of the {sqlpp} cluster, as provided by the cluster manager, while `system:nodes` shows a view of the nodes and services that make up the actual datastore, which may or may not include {sqlpp} nodes.

* The dichotomy is important in that {sqlpp} could be clustered by one entity (e.g. Zookeeper) and be connected to a clustered datastore (e.g. Couchbase) such that each does not have visibility of the other.
...
* {sqlpp} clustering is still reported by the `/admin` endpoints.
...
<.> `identity.bucket`: string (for scoped functions only: the bucket containing the function)
<.> `identity.name`: string (the name of the function)
<.> `identity.namespace`: string (the namespace of the function, for example, default)
<.> `identity.scope`: string (for scoped functions only: the scope containing the function)
...
<.> `scope`: string (for scoped functions only: the scope containing the function)
...
This catalog maintains a list of active Couchbase transactions.
...
<.> [.out]`durabilityLevel`: string (durability level for all mutations within a transaction)
<.> [.out]`durabilityTimeout`: duration (durability timeout per mutation within the transaction)
<.> [.out]`id`: string (the transaction ID)
<.> [.out]`isolationLevel`: string (the isolation level of the transaction)
<.> [.out]`node`: string (the node where the transaction was started)
<.> [.out]`numAtrs`: integer (the total number of active transaction records)
<.> [.out]`scanConsistency`: string (the transactional scan consistency)
<.> [.out]`timeout`: duration (the transaction timeout duration)
