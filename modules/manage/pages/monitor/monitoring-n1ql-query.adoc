...
For the complete output, see <<example-1>>.
...
.Prepare
====
PrimaryScan PREPARE statement in cbq:
...
----
{
  ...
  "results": [
    ...
    "text": "prepare example1 from select * from `travel-sample` limit 1;"
  }
],
"status": "success",
"metrics": {
...
.Prepare
====
SecondaryScan PREPARE statement with WHERE clause, created by cbq:

[source,console]
----
cbq> PREPARE example2 FROM SELECT * FROM `travel-sample` WHERE signature="json" LIMIT 1;
----
...
For full details, refer to xref:settings:query-settings.adoc#_logging_parameters[Logging parameters].
