...
Allows deletion of backups even if other incremental backups depend on them. This can break backup cycles and make dependent backups invalid. Use with caution.

If you try to delete a backup that has dependent incremental backups, a warning dialog will appear:

image::manage-backup-restore/warning-cannot-delete-backup.png[,300,align=left]
