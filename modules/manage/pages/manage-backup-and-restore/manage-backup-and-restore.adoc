...
For more information, see see xref:manage:manage-backup-and-restore/manage-backup-and-restore.adoc#schedule-backup-pruning[Schedule Pruning].
====
...
If you try to delete a backup that has dependent incremental backups, without enabling the *Disable Safe Remove Check* option, the following warning dialog appears.
...
* By running a Prune task on-demand, using the *Prune* button on a repository listed on the *Backup* screen.
...
. Create a *Prune* task, click *Add Task*, and do the following:
...
Now the backups in the repository are pruned automatically according to the specified retention period.

[#prune-on-demand]
=== Prune backups on-demand
...
. Enter *permanently delete* in the *Type 'permanently delete' to confirm* field.
...
Backup retention lets you configure the Backup Service to store or automatically delete old backups.
You can retain backups in a repository for a specified period of time by setting a retention period.
...
=== Set Retention Period after Repository Creation
