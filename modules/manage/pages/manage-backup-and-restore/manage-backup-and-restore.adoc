...
When the fields for the additional task appear, specify the task of type *Merge*, with a frequency of *10* minutes.
For example:
+
...
. When the *Create Plan* dialog appears, enter the *ID* of the plan you are creating, the name of the *Couchbase Bucket* that is being backed up, the appropriate value of *Storage Locations* (here, *Filesystem*), and the location on disk of the archive for the repository.
For example:
+
