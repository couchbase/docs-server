...
[NOTE]
====
* Using the delete backup method deletes _only_ one expired backup in the repository, whether it has dependent incremental backups or not.
...
To set the property Disable the Safe Remove Check or `disable_safe_remove_check` in the API call, see xref:rest-api:backup-delete-backups.adoc[Disable a Backup].
...
* That repository's retention period should be expired.
...
During the creation of a repository or xref:manage:manage-backup-and-restore/manage-backup-and-restore.adoc#schedule-backups[Schedule Backups], you can select one of the following default pruning plans from the *Plan* list on the *Select Plan* dialog:
...
. Continue with the retention period configuration, as explained in xref:manage:manage-backup-and-restore/manage-backup-and-restore.adoc#retain-backups[Retain Backups].
...
You can manually trigger a prune operation for a repository or an individual backup at any time.
...
* xref:manage:manage-backup-and-restore/manage-backup-and-restore.adoc#backup-retention[Retain Backups].

* xref:manage:manage-backup-and-restore/manage-backup-and-restore.adoc#schedule-backup-pruning[Schedule Pruning].
...
. From the *Plan* list on the *Select Plan* dialog, continue with one of the following:
