...
:description: Couchbase Server allows 1 or more buckets, and selected subsets of their data, to be backed up, restored, and archived.
...
== Encryption at Rest for Backup

Encryption at rest for backup protects sensitive information.
It prevents unauthorized access to backup data on disk, even if storage media is compromised.
Use Couchbase Server's centralized Encryption-at-Rest framework to encrypt backup repositories.

When you enable encryption at rest for a backup repository, the Backup Service encrypts Key-value documents, Index definitions, and Metadata.
The Backup Service uses strong cryptographic algorithms to encrypt this data before persisting it to the backup archive.

To restore data from an encrypted repository, you must provide the appropriate decryption material.
This can be either a passphrase or external Key Management Service credentials.

=== Key Concepts

Encryption at rest for backups uses an envelope encryption model:

* Repository Encryption Key — An auto-generated encryption key that encrypts the backup data.
The system creates this key when you configure the encrypted repository.

* Data Encryption Keys (DEKs) — The Backup Service generates keys per bucket and per vBucket to prevent nonce reuse and make sure cryptographic safety for large datasets.

* Key Encryption Keys (KEKs) — The repository encryption key is encrypted using either a passphrase-derived key or an external key from a Key Management Service.

For more information, see xref:learn/security/encryption-at-rest-keys.adoc[Encryption at Rest Keys].

=== Backup Workflows That Support Encryption

Encryption at rest for backups is supported through both of the following backup workflows:

* cbbackupmgr — The command-line backup utility supports creating encrypted repositories, performing encrypted backups, merges, and restores.
Encryption is configured at repository creation time using the `cbbackupmgr` command. 
...
You can specify the KM key URL, region, endpoint, and authentication type directly in the Create Repository dialog.
See <<configure-encrypted-backup-repositories>> for details.
...
Provided that at least 1 node runs the Backup Service, data for the entire cluster can be backed up, restored, and archived.
...
+
The *Backup* screen features 2 tabs, located on the upper, horizontal navigation bar: these are *Repositories* and *Plans*.
By default, the *Repositories* tab is selected: the corresponding *Repositories* view features 3 panels, for *Active*, *Imported*, and *Archived* repositories respectively.
Currently, all panels are blank.

[#schedule-backups]
== Schedule Backups

The Backup Service allows backups (and merges) to be scheduled, as tasks.
This section describes how task-definition and scheduling can be accomplished.
For any given repository, the Backup Service performs 1 task at a time, with each task maintaining a lock on the repository.
...
To schedule 1 or more backups, proceed as follows:

. Choose to add a repository.
When fully defined, the repository combines the definitions of 1 or more backup and related activities, scheduled for 1 or more buckets, targeted at a storage location accessible to all nodes on the cluster.
...
+
Next, specify the services for which data is backed up.
...
This displays the following dropdown menu:
+
...
You can select a bucket from the dropdown menu next to the field.
If you select a specific bucket-name, only data from this bucket is backed up.
If you select *All buckets*, data from all buckets on the cluster (including all Couchbase and all Ephemeral buckets) is backed up.
+
...
+
A location should be used for only 1 repository: when multiple repositories are to be archived, a different location should be used for each.
...
+
NOTE: You must have a Full Admin, a Local User Admin, and an External User Admin role to be able to backup users and user groups.
+
*Retention* is the period for which the backups are to be retained.
When you enable *Retention*, the *Default Retention Period (days)* field appears.
Each repository has a default retention period, measured in days.
Retention period value can be set from 0 to 36,500, where 0 means backups are retained indefinitely.
For more information, see xref:manage:manage-backup-and-restore/manage-backup-and-restore.adoc#backup-retention[Retain Backups].
+
...
** *KM Region* (optional) is the region of the key manager, which is required only for AWS Key Management System (KMS).
...
The upper panel of the *Data* screen provides interactive fields labeled *Key* and *Search Path*.
You can use these fields to search for a specific document within the repository.
Optionally, specify the subset of backups within the repository by using the *Start* and *End* fields.
For example, access the control on the *Start* field to open a list menu of available backups, from which you can select a starting point for the search.
...
image::manage-backup-restore/searchPanelsForKeyAndBucket.png[,820,align=left]
...
CAUTION: Deleting backups with dependent incremental backups can break backup cycles and make dependent backups invalid.
...
* To delete all expired backups in a repository, use the pruning method, which deletes all expired backups in the repository.
For more information, see xref:manage:manage-backup-and-restore/manage-backup-and-restore.adoc#schedule-backup-pruning[Schedule Pruning].
====
...
. Examine the backups in the repository.
You can see a list of backups with expiry status.
+
...
+
image::manage-backup-restore/backups-listed-for-deletion.png[,820,align=left]
...
. Select *Disable the Safe Remove Check* if you want to delete a backup that has incremental backups depending on it.
+
CAUTION: This can break backup cycles and make dependent backups invalid.
...
[#schedule-backup-pruning]
== Schedule Pruning

You can retain backups in a repository for a specified period of time by setting a retention period.
A Prune task allows the Backup Service to delete backups that have reached their retention period.

The backups that meet both the following conditions, are successfully deleted through pruning:

* That backup's retention period is expired.
Means, the number of days specified in its retention period have passed.

* No incremental backups dependent on this backup.

You can Prune backups in a repository in 1 of the following ways:
...
* To delete an individual backup in a repository, whether it has dependent incremental backups or not, see xref:manage:manage-backup-and-restore/manage-backup-and-restore.adoc#delete-backups[Delete Backups].
====

[#using-default-pruning-plan]
=== Schedule a Prune task using default plans

During the creation of a repository or xref:manage:manage-backup-and-restore/manage-backup-and-restore.adoc#schedule-backups[Schedule Backups], you can select 1 of the following default pruning plans from the *Plan* list on the *Select Plan* dialog:

* *&#95;daily_full_backups_with_incrementals*: This plan performs full backups for all services every day and incremental backups every hour, and runs a prune every day.

* *&#95;weekly_full_backups_with_incrementals*: This plan performs full backups for all services every week and incremental backups every 6 hours, and runs a prune every day.

[#creating-new-pruning-task]
=== Create and schedule a Prune task

To manually schedule a Prune task, do the following:

. Start creating a repository, as explained in xref:manage:manage-backup-and-restore/manage-backup-and-restore.adoc#schedule-backups[Schedule Backups].

. On the *Select Plan* dialog, from the *Plan* list, select *+ Create new plan*.

. Create a *Backup* task, as explained in xref:manage:manage-backup-and-restore/manage-backup-and-restore.adoc#schedule-backups[Schedule Backups].
+
[NOTE]
====
...
+
image::manage-backup-restore/add-pruning-task.png[,480,align=left]

. Click *Next*.

. Continue with the retention period configuration, as explained in xref:manage:manage-backup-and-restore/manage-backup-and-restore.adoc#backup-retention[Retain Backups].
...
[#prune-on-demand]
=== Perform an Immediate Pruning

You can manually trigger a prune operation for a repository at any time.
This is useful if you want to free up space instantly by removing old backups without waiting for the scheduled prune tasks.
...
+
NOTE: Only expired backups can be pruned.
...
. Click *Prune* to delete the expired backups.

The screen displays a notification that the pruning task has been successfully triggered on the selected backups.

To run a pruning task on-demand using the REST API, see xref:rest-api:backup-trigger-prune.adoc[Perform an Immediate Prune].

=== View a pruned backup

. Go to *Backup* screen and select the repository.
...
Backup retention lets you configure the Backup Service to automatically delete old backups.
You can retain backups in a repository for a specified period of time by setting a retention period.
When the retention period is reached, the Backup Service automatically deletes the backup through Prune tasks.
The retention period is a value between 0 and 36500 (inclusive), in days, where 0 indicates an indefinite retention period.
...
* By default, the retention period is set to 0, which is an indefinite period.
As a result, Prune tasks do not automatically delete any backups.
You need to configure a value greater than 0 for a retention period to enable automatic deletion.
====

*Prerequisites:*

* To delete backups automatically after the retention period is reached, a Prune task must be created for the repository.
Otherwise, the backups are not deleted/pruned automatically and you need to use manual pruning.
+
For more information about pruning, see xref:manage:manage-backup-and-restore/manage-backup-and-restore.adoc#schedule-backup-pruning[Schedule Pruning].

[#set-retention-period-repo-creation]
=== Set Retention Period when Creating a Repository

To set the retention period for a repository, follow these steps:

. Start creating a repository, as explained in xref:manage:manage-backup-and-restore/manage-backup-and-restore.adoc#schedule-backups[Schedule Backups].

. From the *Plan* list on the *Select Plan* dialog, continue with 1 of the following:

** xref:manage:manage-backup-and-restore/manage-backup-and-restore.adoc#using-default-pruning-plan[Schedule a Prune task using default plans].

** xref:manage:manage-backup-and-restore/manage-backup-and-restore.adoc#creating-new-pruning-task[Create and schedule a Prune task].
+
NOTE: Pruning won't delete backups that have incremental backups depending on them.

. Click *Next*.

. Select *Retention* on the *Create Repository* dialog.
+
image::manage-backup-restore/set-retention-period.png[,480,align=left]

. Use *Default Retention Period (days)* to specify the necessary retention period, in days, for the repository.
+
[NOTE]
====
...
* The retention period can be set from 1 to 36,500 days.
...
. Specify the new default retention period, in days, for the repository in the *New Retention* field.
...
. On the Backup screen, in the *Repositories* list, click the repository.
...
A merge allows multiple backups to be combined as 1; with deduplication occurring.
...
The dialog now disappears, and you will receive a console notification stating that you have triggered a merge.
...
+
image:manage-backup-restore/dataScreenShowingMergedBackup.png[,820,align=left]
+
...
+
image:manage-backup-restore/backupMergeConfirmation.png[,820,align=left]
+
The details in the expanded row confirm that 5 backups were merged by the operation just performed.

== Configure Encrypted Backup Repositories

Before configuring an encrypted backup repository, make sure you have the following prerequisites in place:

* Couchbase Server Enterprise Edition — Encrypted backups are an Enterprise Edition feature.

* Key Management Setup — If using an external KMS, ensure you have:
  * A key created in your chosen KMS provider (AWS KMS, GCP KMS, Azure Key Vault, or HashiCorp Vault Transit Engine).
  * Appropriate credentials or authentication configured for accessing the KMS.
  * Network connectivity from the Couchbase Server node (or the machine running `cbbackupmgr`) to the KMS endpoint.
  
* Permissions — The user must have the Full Admin role (for Backup Service) or the Full Admin or Data Backup & Restore role (for cbbackupmgr).
* HTTPS Endpoints — Use secure connections (https:// or couchbases://) to ensure data is encrypted in transit as well as at rest.

Encrypted backup repositories can be configured in 1 of the following modes:

* KMS Mode (Production)
In KMS mode, an external Key Management Service encrypts the repository encryption key using envelope encryption.
The external key does not directly encrypt the data — it wraps (envelopes) the auto-generated repository key.
This minimizes the number of API calls to the KMS (1 call per operation, except info which requires zero).

* Passphrase Mode (Development/Testing Only)
In passphrase mode, a user-supplied passphrase is used to derive an encryption key.
This derived key encrypts the repository key.

NOTE: Passphrase mode is not recommended for production use due to the inherent insecurity of human-friendly passphrases.

To configure using `cbbackupmgr`, use the `config` command with the `--encrypted` flag.

To configure using the Web Console:

. Navigate to Backup and click btn:[ADD REPOSITORY].
. Select or create a plan, then click *Next*.
. In the Create Repository dialog, under *Encryption Options*, select *Encrypted*.
. Configure the following fields:
* KM Key URL — The URL path to the key in the external key manager, prefixed with the provider scheme.
* KM Region (optional) — The region of the key manager (required for AWS KMS).
* KM Endpoint (optional) — Override the default endpoint for the KMS.
* KMS Authentication Type (optional) — Choose from Environmental Auth, File Auth, or Access Key Auth.
. Click *Add* to create the encrypted repository.

After configuring an encrypted repository, backups are performed using the same commands as unencrypted backups, but you must supply the encryption credentials.
If the KMS credentials are missing, cbbackupmgr will fail with an error requesting the correct credentials.
...
You can restore a backup of data from a bucket using the Couchstore storage engine to 1 using Magma.
...
The restore task only restores a document if 1 of its values matches the regular expression.
+
...
Restoring data from an encrypted backup repository requires access to the same decryption material (KMS credentials or passphrase) that was used when the repository was created.
This section describes how restore works with encrypted repositories, the prerequisites, and behavioral differences from restoring unencrypted backups.
...
. Decrypts the backup data in memory.
. Sends the decrypted data to the target Couchbase cluster.

The decryption process is transparent — the restore command syntax is the same as for unencrypted restores, with the addition of encryption credential flags.

To restore an encrypted backup using the REST API, see xref:rest-api:backup-restore.adoc[Restore a Backup].

To restore an encrypted backup using the Web Console, follow these steps:

. Navigate to Backup in Couchbase Web Console.
. Expand the encrypted repository and click Restore.
. Enter the target cluster URL, authentication credentials, and backup range.
. Click Restore.

The Backup Service handles decryption automatically using the KMS credentials configured when the repository was created.

Cross-Cluster Restore::

When restoring encrypted backups to a different cluster:

* The target cluster does not need to have encryption at rest enabled.
Encryption at rest for backups is independent of the cluster's own encryption-at-rest configuration.
...
+
image:manage-backup-restore/archivedRepositoryExpandedRow.png[,820,align=left]
+
...
image:manage-backup-restore/plansScreen.png[,820,align=left]
...
* The *&#95;daily_full_backups_with_incrementals* plan performs full backups for all services every day and incremental backups every hour, and runs a prune every day.
You must set an appropriate retention period for backups of this plan.

* The *&#95;weekly_full_backups_with_incrementals* plan performs full backups for all services every week and incremental backups every 6 hours, and runs a prune every day.
You must set an appropriate retention period for backups of this plan.
...
A *Weekly Calendar* schedule means that 1 backup happens daily.
...
The dropdown menu contains 3 scheduling options.
...
When the selection is made, press btn:[Next].
...
=====
+
You will require a different set of options depending on which 1 cloud authentication type you choose:
+
'''
+
[%collapsible]
