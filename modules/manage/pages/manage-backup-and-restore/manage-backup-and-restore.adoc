...
The screen provides two possible views, which are *Inspect Backups* and *Task History*: these can be selected by means of the tabs at the upper right.
The *Task History* button is currently selected.
(Left-clicking on the *Inspect Backup* button displays the *Repository* screen, described above in xref:manage:manage-backup-and-restore/manage-backup-and-restore.adoc#inspect-backups[Inspect Backups]).
