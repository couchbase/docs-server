...
To create and archive, proceed as follows;
...
+
This brings up the *Archive Repo* dialog, which appears as follows:
+
...
. Specify a repository to be archived.
Enter the name of the repository to be archived into the *Confirm repository ID to archive* field.
...
The *Restore* button brings up the same dialog as described above, in xref:manage:manage-backup-and-restore/manage-backup-and-restore.png#restore-backups[Restore Backups].
The *Delete* button is described immediately below, in xref:manage:manage-backup-and-restore/manage-backup-and-restore.png#delete-backups[Delete Backups].

[#delete-backups]
== Delete Backups

Backups cannot be individually deleted: they can be deleted only by repository.
No repository can be deleted until it has been archived, as described above, in xref:manage:manage-backup-and-restore/manage-backup-and-restore.png#archive-backups[Archive Backups].

Therefore, to delete a repository, and the backups it contains, proceed as follows:

. Archive the repository, as described in xref:manage:manage-backup-and-restore/manage-backup-and-restore.png#archive-backups[Archive Backups].
...
If the checkbox is unchecked, the deleted archive's contents continue to exist on disk, even though no longer explicitly tracked by the Backup Service, and so can be recovered subsequently; as described below, in xref:manage:manage-backup-and-restore/manage-backup-and-restore.png#import-repositories[Import Repositories].
+
To delete the archive while allowing the contents to continue to exist on disk, enter the name of the archive in the *Confirm Repository ID* field, and left-click on *Delete*.
