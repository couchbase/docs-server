...
It could be done either using property files and `connect-standalone` as in xref:kafka-3.1/quickstart.adoc[Quickstart], using REST interface of `connect-distributed` or using Web UI provided by Control Center.
We will use the last two options.

By default Control Center starts at `http://localhost:9021/`.
Connector configuration is accessible in the "Kafka Connect" section: image:kafka-3.1/images/kafka3-kafka-connect.png[,570]
...
Once the connector class is selected, the UI will render a list of all accessible configuration properties: image:kafka-3.1/images/kafka3-setup-source-connector.png[,570]
