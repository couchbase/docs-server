...
Here is a reference to the http://docs.couchbase.com/sdk-api/couchbase-spark-connector-1.0.1/[Scala docs^].
...
When it comes to Spark, you always need to set up a configuration and initialize the `SparkContext` object.
...
Use the `couchbaseGet` method on the `SparkContext` to fetch documents from Couchbase and create a RDD.
...
Go find your modified document in the Couchbase Server UI!
Look for `"my_airline_10123"` which will just have the name of the airline as its content.

Congratulations!
You've successfully performed your first ETL job (extract-transform-load) using Couchbase and Spark.
...
In addition to creating a `SparkContext`, you'll need a `SQLContext`:
...
Because a `DataFrame` is like a `RDD` but with a schema and Couchbase is a schemaless database at its heart, you need a way to either define or infer a schema.
