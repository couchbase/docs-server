...
NOTE: Currently, the connector issues a backfill, which means that all data stored in the bucket will be streamed first, so you might want to try it on an empty bucket (`from = FromBeginning`).

== DStream Persistence

Since DStreams are RDDs, the same persistence mechanisms apply (see xref:spark-1.2/working-with-rdds.adoc#spark-working-with-rdds[Working with RDDs] for more information).
