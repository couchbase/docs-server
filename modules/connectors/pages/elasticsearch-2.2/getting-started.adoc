...
image::elasticsearch-2.2/couchbaseConsoleInitial.png[,540,align=left]

. Select the [.ui]*XDCR* option, on the upper toolbar:
+
[#XDCRoption]
image::elasticsearch-2.2/selectXDCR.png[,86,align=left]
+
This brings up the [.ui]*XDCR* interface, as follows:
+
[#XDCRui]
image::elasticsearch-2.2/xdcrInitial.png[,540,align=left]

. Left-click on the btn:[Create Cluster Reference] button, at the upper-right of the [.ui]*REMOTE CLUSTERS* panel:
+
[#createClusterRef]
image::elasticsearch-2.2/selectCreateClusterReference.png[,204,align=left]
+
The following dialog now appears:
+
[#createClusterRefDialog]
image::elasticsearch-2.2/createClusterReferenceDialog.png[,320,align=left]
+
Enter appropriate data into the fields of this dialog, and then left-click on the [.ui]*Save* button.
The [.ui]*Cluster Name* can be any name of your choice, to refer to your Elasticsearch cluster.
The [.ui]*IP/hostname* should be the port-number 9091, appended to the word `localhost`.
The [.ui]*Username* and [.ui]*Password* are those you have established for your Elasticsearch cluster, in the `elasticsearch.yml` configuration-file.
You do not need to enable TLS encryption.
+
The Couchbase Web Console now appears as follows:
+
[#createClusterReferenceCreated]
image::elasticsearch-2.2/clusterReferenceCreated.png[,520,align=left]

. Left-click on the btn:[Create Replication] button, to the upper-right of the [.ui]*ONGOING REPLICATIONS* panel.
The following dialog appears:
+
[#createReplicationDialog]
image::elasticsearch-2.2/createReplicationDialog.png[,380,align=left]
+
Enter appropriate data into the fields of this dialog.
The [.ui]*Bucket* should be the bucket whose data you intend to replicate to Elasticsearch (previously designated as _beer-sample_).
The [.ui]*Cluster* should be the Elasticsearch cluster you previously created; which you now select from the drop-down menu:
+
[#pickRemoteCluster]
image::elasticsearch-2.2/pickRemoteCluster.png[,220,align=left]
+
The [.ui]*Bucket* should be the name of the Elasticsearch index you specified to handle data replicated from Couchbase: which was _beer-sample_.
+
Next, left-click on [.ui]*Advanced settings*.
The following dialog appears:
+
[#advancedSettings]
image::elasticsearch-2.2/advancedSettings.png[,380,align=left]
+
Since replication from Couchbase to Elasticsearch is supported only by Version 1 of the XDCR Protocol, you must change the value of the [.ui]*XDCR Protocol* correspondingly, by accessing the pull-down menu, as follows:
+
[#selectVersion1]
image::elasticsearch-2.2/selectVersion1.png[,220,align=left]
+
...
This is represented within the [.ui]*ONGOING REPLICATIONS* panel, as follows:

[#replicationInProgress]
image::elasticsearch-2.2/replicationInProgress.png[,480,align=left]
...
[#esWebUI]
image::elasticsearch-2.2/elasticsearchConsole.png[,480,align=left]
...
+
[#dataBucketsTab]
image::elasticsearch-2.2/dataBucketsTab.png[,120,align=left]
+
This brings up the [.ui]*Couchbase Buckets* screen.

. Left-click on the btn:[Documents] button, towards the right of the [.ui]*beer-sample* row:
+
[#beerSampleLine]
image::elasticsearch-2.2/beerSampleLine.png[,480,align=left]
+
This brings up the [.ui]*Documents* screen for the beer-sample bucket:
+
[#beerSampleDocuments]
image::elasticsearch-2.2/beerSampleDocuments.png[,480,align=left]
...
image::elasticsearch-2.2/beerSampleDocumentsLookUpField.png[,320,align=left]
+
...
image::elasticsearch-2.2/beerSampleDocumentsLookUpResults.png[,480,align=left]
...
[#codeExampleDiagram]
image::elasticsearch-2.2/codeExampleDiagram.png[,520,align=left]
...
[#demoUIinitial]
image::elasticsearch-2.2/demoUIinitial.png[,520,align=left]
...
[#demoUIradioButtons]
image::elasticsearch-2.2/demoUIradioButtons.png[,240,align=left]

Once a beer-style has been selected, querying can be initiated by left-clicking on the btn:[Query Elasticsearch] button.

[#demoUIqueryESbutton]
image::elasticsearch-2.2/demoUIqueryESbutton.png[,180,align=left]
...
image::elasticsearch-2.2/demoUIfullQueryResults.png[,520,align=left]
