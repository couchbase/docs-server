...
If your environment does not have access to the internet you can use the `--jars` argument instead and grab the assembly with all the dependencies from here: xref:spark-2.0/download-links.adoc[Download and API Reference].
...
libraryDependencies ++= Seq(
  "org.apache.spark" %% "spark-core" % "2.0.1" % "provided,test",
  "org.apache.spark" %% "spark-sql" % "2.0.1" % "provided,test",
  "com.couchbase.client" %% "spark-connector" % "2.0.0"
)
----
