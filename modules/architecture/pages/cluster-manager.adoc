...
This is dependent on other criteria being met as detailed on xref:clustersetup:automatic-failover.adoc#topic_fcf_chm_zs[Using Automatic Failover] such as only a single node being unhealthy.
...
* *Data Service* - The health of the Data service is monitored by inspecting the xref:high-availability-replication-architecture.adoc#concept_hj1_njj_vs[DCP traffic] coming out of the node.
