...
. The [.ui]*Root Certificate* link shows that a self-signed certificate has been deployed across the cluster.
This certificate can be regenerated or replaced using xref:rest-api:rest-encryption.adoc[Encryption On-the-Wire API] or xref:cli:cbcli/ssl-manage.adoc[CLI] commands for managing certificates.
