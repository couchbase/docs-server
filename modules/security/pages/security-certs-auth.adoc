...
Use the [.cmd]`couchbase-cli` xref:cli:cbcli/couchbase-cli-ssl-manage.adoc[ssl-manage] command:
...
When upgrading from an earlier version to 5.1, the cluster will be in mixed mode and will return client certificate authentication settings in the older format until the cluster is completely upgraded.
