...
* *Data-key rotation*: This _second level of rotation_ is achieved by changing the data-key, using the xref:cli:cbcli/couchbase-cli-master-password.adoc[CLI command] with the `--rotate-data-key` option, or the REST API xref:rest-api:rest-secret-mgmt.adoc#topic_jdz_bd3_hy[POST /node/controller/rotateDataKey] method.
+
{blank}
...
Note that if the xref:security-auditing.adoc#topic_a5p_npm_lq[auditing] option is enabled, all rotation-requests are audited by Couchbase Server.
