...
RBAC requires configuration with xref:security-ldap-new.adoc[LDAP] so that the administrator's userid in LDAP can be mapped by the Full Administrator to one or more fixed administrative roles.
...
Full administrators in Couchbase can manage user roles using the Couchbase CLI tools (as described in xref:cli:alter-role.adoc[[.cmd]`admin-role-manage`]) or REST API (as described in xref:rest-api:rbac.adoc[Role Based Admin Access (RBAC)]).
