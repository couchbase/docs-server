...
. Now look at the Buckets in the UI the 'metadata' bucket will have 2048 documents realted to the eventing fucntion and three (3) additonal documents realted to the timer, and of course you should see 1 document in the 'source' bucket.
+
...
. Now look at the Buckets in the UI again you will see 1 document in the 'source' bucket and 1 document in the'target bucket'
+
image::docexpiry_05_buckets.png[,100%]

. Wait a few more minutes past the time listed via "DocTimerCallback 2 src expiry:" in the logs above, then check the documents within the source bucket 'source': the original *SourceDocument2* is no longer accessable, having been removed at its defined expiration-time or TTL. (Note if you don't actully try to access the originaly 'source' document the UI will indicate it exists until the expiry pager removes a the tombstone for the original document).
+
