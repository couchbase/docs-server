...
*Create a document with a TTL 10 minutes*

The example requires a document to be created in the 'source' bucket with a key of *SampleDocument2*, a value of *{'a_key': 'a_value'}*, and most importanly that the document's expiration (or TTL) set to 600 seconds or 10 minutes). 

There are several methods to make a test document with an experation set.  The easist is most likey using _cbc_.  However you can use any Couchbase SDK (the command-line KV client is compiled from the C SDK).  FOr example you can use a Python script or a Java program. 
...
C:\Program Files\Couchbase\Server\bin\cbc ^
    ...
    --expiry=600
----
Use the command-line KV client, e.g. the _cbc_ binary, and cut-n-paste one of the the above to create the needed sample document.
...
----
Issue the command _python_ to start the Python interpreter then cut-n-paste the above to create the needed sample document.
