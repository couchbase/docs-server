...
You will need to run a special {sqlpp} statement, a _cbc_ (the command-line KV client), or alternatively an SDK python script or Java program to create or update a document in the 'source' bucket with an expiration time of 600 seconds.
...
+
There are several methods to make a test document with an expiration set. The easiest is most likely using {sqlpp}.  However you can use _cbc_ or any Couchbase SDK (the command-line KV client is compiled from the C SDK).  For example you can use a Python script or a complied Java program. 
+
[{tabs}] 
====
{sqlpp} UPDATE::
...
For information on setting document expiration times via {sqlpp}, refer to xref:n1ql:n1ql-language-reference/insert.adoc#insert-document-with-expiration[Insert a document with expiration]
--
...
. If you select *Scopes & Collections* (on the right) from the bucket *bulk*, then click on *data* (on the left) you should see one (1) document in the `bulk`.`data`.`source` collection (that you poked in via {sqlpp}, cbc, Python or Java).
+
