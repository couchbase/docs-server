...
To run this example a Couchbase SDK is required to manipulate the document expiration for reference both a Java 2.7.7 SDK program and a Python 2.5 SDK script is provided, either can be used create the required test document with a 10 minute expiration. Both the supplied Java and Python code will open a Couchbase cluster on the localhost and authenticate against it, then insert a new document with a 10 minute TTL, named *SampleDocument2*, whose body is *{'a_key': 'a_value'}* into the bucket 'source'.
...
The use of the Python SDK to run the above Python script to create a new document in the source bucket, with an expiration — or Time To Live (TTL) — set to occur ten minutes after the document's creation.
...
----
. Now wait a few minutes and Look at the Log again for *add_timer_before_expiry* at two minutes before the TTL was scheduled the timer will have fired and executed DocTimerCallback (note the logs display by the "*Log*" link are in reverse time order)
+
----
...
+
As a result, a new document — containing data from the original, named *SourceDocument2* — is created in the bucket 'target' with the same Key. 
