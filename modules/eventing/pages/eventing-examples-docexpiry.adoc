...
To run this exmaple a Couchbase SDK is required to manipulate the document expiration for reference both a Java 2.7.7 SDK program and a Python 2.5 SDK script is provided, either can be used create the required test document with a 10 minute expiration.  

Both the Java and Python code will open a Couchbase cluster on the localhost and authenticate against it, then insert a new document with a 10 minute TTL, named *SampleDocument2*, whose body is *{'a_key': 'a_value'}* into the bucket 'source'.

* Java 2.7.7 SDK program
+
----
// Couchbase Java SDK 2.7.7
