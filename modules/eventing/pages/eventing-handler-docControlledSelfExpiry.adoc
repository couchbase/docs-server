...
Using _N1QL(...)_ is much slower than using _couchbase.replace(bucket_binding, meta, doc)_ in the docControlledSelfExpiry_661 variant.

* <<docControlledSelfExpiry,docControlledSelfExpiry (indirect TTL via N1QL)>>
* xref:eventing-handler-advanced-docControlledSelfExpiry.adoc[docControlledSelfExpiry_661 (direct TTL)]
