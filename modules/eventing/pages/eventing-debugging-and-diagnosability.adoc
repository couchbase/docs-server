...
Transpiler accepts the source code provided as input from one programming language and produces an equivalent code in another programming language. During execution, the Function handler code is transpiled. Generally, a transpiled code is unreadable.

Source map is a mapping between the transpiled code and the original source code. For an easy debug operation, the debugger in Couchbase Server detects the source map and presents the Function handler code in its native format.
