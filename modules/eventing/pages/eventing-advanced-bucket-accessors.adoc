...
This operation allows reading a document along with metadata from the bucket this allows subsequent operations to utilize CAS or check/modify the expiry_data.

Contrast this to the _Basic GET operation_ which merely exposes JavaScript:

[source,javascript]
----
// Basic Bucket Accessor GET operation
var adoc = src_bkt[meta.id] 
----

where the return value is only the document without any metadata.
