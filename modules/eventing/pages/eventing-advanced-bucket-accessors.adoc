...
This operation allows reading a document from the bucket with an ability to specify the CAS value to be matched during the read.  Unlike the __Basic GET operation_ "var val = src_bkt[meta.id] in which only the document is returned the _Advanced GET operation_ will among other items return the CAS value.
