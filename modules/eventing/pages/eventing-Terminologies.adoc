...
The Listen To Location as of 7.1.1 can listen to multiple collections via a wildcard of `+++*+++` for the scope and/or the collection. 
...
Prior to 7.1.1 all bucket aliases had to reference a single keyspace. As of 7.1.1 an Eventing Function can listen to multiple collections via a wildcard of "+++*+++" for the scope and/or the collection. For these Functions if the bucket alias (or binding) used by the JavaScript code can also contains a wild card of "+++*+++" for the scope and/or the collection.  If bucket alias contains a wild card of "+++*+++" only the Advanced Keyspace Accessors will be able to read or write the Data Service (or KV). [.status]#Couchbase Server 7.1.1#
