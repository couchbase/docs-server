...
When you are using a series of handlers, ensure that destination buckets to which event handlers perform a write operation, do not have other event handlers configured to listen and track data mutations.
...
* The metadata bucket must not be selected as the source bucket.
