...
Every Function definition needs two distinct buckets: source bucket/destination and metadata bucket.

*Source and Destination Buckets*

Couchbase Functions use a source bucket to listen to data mutations. The Function handler code polls the source bucket for data mutations. The source bucket can be either Couchbase or Ephemeral bucket. However, memcached buckets are not supported. Every Function needs to be associated with a source bucket. A common source bucket may listen to multiple Functions running different code.
...
While using the Metadata bucket:
	...
	• Ensure a separate bucket is destined as a metadata bucket.
