...
Eventing Functions can interact with the Data Service (KV), the Query Service ({sqlpp}), and external REST services via a built-in cURL functoin call.  Eventing Functions can also schedule a callback to an internal JavaScript function sometime in the future by creating a Timer.
