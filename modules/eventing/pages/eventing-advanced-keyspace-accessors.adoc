...
For Eventing Functions that listen to multiple collections via a wildcard of `+++*+++` for the scope and/or the collection, an additional parameter is required if the binding used by the Advanced Keyspace Accessor also contains a wild card of `+++*+++` for the scope and/or the collection.
...
+
The keyspace of the document to be used for the operation with the JavaScript format "keyspace":{"bucket_name":_string_,"scope_name":_string_,"collection_name":_string_}.
