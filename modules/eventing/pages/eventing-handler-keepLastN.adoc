...
+
--
Two varients of this function are given a 6.6 version and a 6.6.1 version which uses CAS
* <<keepLastN,keepLastN>> 
...
function updateNotifyArrayInKV(doc, meta, MAX_RETRY, MAX_ARRAY, DEBUG) {
    ...
    // we utilize a 'random' tag in the documnet itself to emulate a CAS operation.
    for (var tries=0; tries < 16; tries++) {
        ...
        // we use the random values to perfrom a CAS, we give up after 16 attempts
...
function updateNotifyArrayInKV(doc, meta, MAX_ARRAY, DEBUG) {
    ...
    while (res === null) {
        ...
        if (res !== null) {
            // here we had a successfull couchbase.get(...) so both 'user_doc' and 'user_meta' 
            ...
            if (new_doc == null) {
                // duplicate data we already have it, ust ignore/skip
                break;
            }
...
Note, only verson 6.6 will have the property _random_ to the tracking doc used to emulate CAS.
