...
* With an increase in the usage of timers, the metadata bucket's space assignment must also be increased to accomadate all active timers and any potential backlog. 
...
* Bindings can be reused in timers. Bindings, created during the Eventing Function definition, can be accessed by the timer constructs in the handler code.
