 ...
 ** *Delete*: Deletes an undeployed Function.
 ** *Export*: Exports the Function as a JSON document.
 ** *Deploy*: Deploys the Function, making it active across the cluster.
 ** *Undeploy*: Undeploys a deployed Function, making it inactive across the cluster.
...
NOTE: When a Function lifecycle operation is in progress, Couchbase Server doesn't allow the Eventing rebalance +
operation. Deploy, undeploy and delete are the usual Function lifecycle operations. Similarly, when the Eventing +
rebalance operation is in progress, you are not allowed to perform a Function lifecycle operation.
--
