...
* It is expected that the TBD Application that processes the "notify" bucket will purge the notification documents.
* The notification history is stored in the shipping document and archive for all time.
...
Note, for key "ship:dea0fca2-e7b7-11ea-adc1-0242ac120002" you may want to adjust the timestamps, since the times are in seconds since Unix epoch use a tool like https://www.dcode.fr/timestamp-converter or https://www.epochconverter.com/
...
To fully exercise the logic do the following steps(to re-run flush the 'active', 'archive' and 'notify' buckets and redo the UPSERT the data):
