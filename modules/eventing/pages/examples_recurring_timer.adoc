...
** *Test 1*: The control document is created or mutated in such a way a Timer is created and fires approximently 30 seconds in the future at which point a document representing user work is written to the source bucket with the scheduled time of execution (this represents user work). The original control document, in the source bucket, is not changed.  The Timer is re-armed and will execute again until canceled.
