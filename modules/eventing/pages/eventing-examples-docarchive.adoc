...
* The edge case ignored for brevity in *Document Expiry* for a misisng source document when we '
  trigger our callback is now caught and it is logged as an error.
* No logging except for an error case of a misisng document when we are archiving.
* We will not rely on a Couchbase SDK, but rather on a expiry set on the bucket where all documents 
  in a bucket will have an expiraton of 10 minutes form the time of their first mutaton or creation.
...
The original document in the source bucket is not changed (and will be deleted automatically acording to the bucket's exiration time).
...
In addtion we will use soem data from *travel-sample* if you don't have the bucket 'travel-sample' 
...
The 'source' bucket will have an Bucket Max Time-To-Live or TTL of 600 seconds.
...
** Check the 'travel-sample' chackbox.
** Click *Load Sample Data*.
. Update the advanced sttings for the "source" bucket from the *Couchbase Web Console* > *Buckets* page
...
** After configuring your setings for the bucket 'source' settings your screen should look like:
+
