...
* Does rebalancing have any effect on the firing of events?
+
No. Functions do not lose any mutations during a rebalance operation.

* I have deployed Functions on my cluster, when can I perform rebalancing?
+
Couchbase Server prevents rebalancing during Function lifecycle operations. Before you start cluster rebalancing ensure that no Function lifecycle operations are currently in progress. Deploy,
undeploy and delete are the usual Function lifecycle operations.
