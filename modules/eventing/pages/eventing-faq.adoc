...
+
The Functions offering is about server-side processing or compute of busniss logic; it does not require any additional infrastructure layer or middleware to be deployed or managed.
...
* Is it possible to get additional state during a Function execution? For example, can you read from the data service in a Function to fetch related data -or- can your enrich the updated document with data from another document (using a document id)?
+
Yes.
