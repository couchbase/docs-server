...
Memcached buckets (and thus undelying collections) are not supported.
...
+
It should be noted, “document metadata” is different from the "Evenitng Storage" keyspace (metadata collection), described in the next section, used by the Eventing Service to maintain state and checkpoints.
...
* When I undeploy all handlers the metadata collection is not fully cleared (this ia a regression in 6.6.1 only).
+
...
* Why do I see a burst of activity in bucket OPs (in the bucket tha holds the metadata collection) after a timer is paused for an extended period of time?
+
