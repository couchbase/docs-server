...
** *Test 3*: The control document is mutated in such a way that any existing Timer with the reference of the control document's id (meta.id) is overwritten. This is equivalent to canceling a timer as it already exists, and then creating a new Timer that will fire approximately 60 seconds in the future.
