...
** *Test 3*: The control document mutated in such a way that any existing Timer with the refrence of the control documents id (meta.id) is overwritten, this is equivlent to canceling a timer is it already exists and then creating a new Timer that will fire approximently 60 seconds in the future.
...
. The *OnUpdate* routine specifies that when a change occurs to data within the bucket, actions will be processed acordin to thefield within the document.  First we ignore all documents that do not have a doc.type of "type_of_interest" this is the control document.  Next we use the field as "needed_condition", "cancel_timer", and "overwrite_timer" to detemine which action we take.  

* If "needed_condition" is true and both "cancel_timer", and "overwrite_timer" are false we will create a Timer that will fire approximently 60 seconds in the future.
* If "needed_condition" is true and "cancel_timer" is true we will cancel the existing Timer.
* If "needed_condition" is true and "overwrite_timer" is true we will overwite the existing Timer with a new one (assuming that our Timer is still active) which will fire approximently 60 seconds in the future.
...
----
+
Now change "needed_condition" to true to create a mutation, then Click *Save*.  This will create a mutation and then the Function will generate a Timer.  The control docuemnt is now:
+
----
{
  "type": "type_of_interest",
...
. Access the *Couchbase Web Console* > *Eventing* page and click on the Function *cancel_overwrite_timer* then Click the "Log" link for Deployed Function *cancel_overwrite_timer* to view the activity.  Here we see from the Application log that we created a timer.
+
----
2020-08-03T11:08:06.690-07:00 [INFO] "From OnUpdate: creating timer" "type_of_interest::1"
----
+
image::cancel_overwrite_timer_04_log_active1.png[,500,align=left]

. Wait about 80 seconds and click the "Log" link for Deployed Function *cancel_overwrite_timer* to view the activity.  Here we see the timer fired and executed the callback *DocTimerCallback* near our scheduled time.
+
----
2020-08-03T11:09:10.624-07:00 [INFO] "From DocTimerCallback: timer fired" {"docId":"type_of_interest::1","random_text":"arbitrary text","tmr_time_to_fire":"2020-08-03T18:09:06.690Z"}
----
+
image::cancel_overwrite_timer_04_log_fired1.png[,500,align=left]
...
----
{
  ...
  "tmr_time_to_fire": "2020-08-03T18:09:06.690Z"
}
----
...
----
+
Now change "cancel_timer" to true to create a mutation, then Click *Save*.  The control docuemnt is now:
+
----
{
  "type": "type_of_interest",
...
. Access the *Couchbase Web Console* > *Eventing* page and click on the Function *cancel_overwrite_timer* then Click the "Log" link for Deployed Function *cancel_overwrite_timer* to view the activity.  Here we see from the Application log the timer was cancled and will never fire.
+
----
2020-08-03T11:22:34.322-07:00 [INFO] "From OnUpdate: cancel request, timer was canceled" "type_of_interest::1"
----
...
----
+
Now change "cancel_timer" to false to create a mutation, then Click *Save*.  This will create a mutation and then the Function will generate a Timer.  The control docuemnt is now:
+
----
{
  "type": "type_of_interest",
...
----
+
Now change "overwrite_timer" to true to create a mutation, then Click *Save*.  The control docuemnt is now:
+
----
{
  "type": "type_of_interest",
...
. Access the *Couchbase Web Console* > *Eventing* page and click on the Function *cancel_overwrite_timer* then Click the "Log" link for Deployed Function *cancel_overwrite_timer* to view the activity.  Here we see from the Application log the timer was overwritten and will fire at a later time.
+
----
2020-08-03T11:18:31.521-07:00 [INFO] "From OnUpdate: overwriting timer with same reference" "type_of_interest::1"
----
