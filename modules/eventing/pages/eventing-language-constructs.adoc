...
_result = couchbase.get(binding, meta)_
+
...
function OnUpdate(doc, meta) {
    ...
    if (result.success) {
        log('success read meta',result.meta);
        log('success read doc ',result.doc);
    } else {
        log('id',new_meta.id,'failed',result.error)
    }
}
----
...
** result – _the return value (type: Object)_
+
