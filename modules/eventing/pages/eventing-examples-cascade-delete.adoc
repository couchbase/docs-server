...
This Example uses {sqlpp} unlike the similar scriptlet xref:eventing-handler-cascadeKvDeleteWithDoc.adoc[cascadeKvDeleteWithDoc] which uses just KV or the Data Service.
 ...
 *** For the binding, select "bucket alias", specify *src_user* as the "alias name" of the collection, 
 select *bulk*, *data*, *users* as the associated keyspace, and select "read only" for the access mode. *Note we will not be using this binding as we have no need of it because we will use {sqlpp}.*
...
----
+
Note, the {sqlpp} keyspace of *bulk.data.transactions* in earlier releases would be simply the bucket *transactions*.
+
...
. The *OnDelete* handler above is triggered for user delete mutation. The handler checks if the *user_id* is greater than or equal to 100 (_this contrived filter never deletes the orphaned transactions from any user with an id of < 100_). When this condition is fulfilled, then an {sqlpp} query is triggered to delete all user related information. The handler also logs if the orphaned transactions were removed or kept to Function specific application log file (refer to the log(...) statements in the above JavaScript).
...
----
** Next create some indices so that we can perform some {sqlpp} queries.
+
[source,sqlpp]
----
...
----
+
After executing the above {sqlpp} statement you should see a table of 'user' data as follows:
+
...
----
+
After executing the above {sqlpp} statement you should see a table of 'transaction' data as follows:
+
...
----
+
After executing the above {sqlpp} statement you should see a table showing counts as follows:
+
...
. Access the *Couchbase Web Console* > *Query* and run the following {sqlpp} statements
...
----
+
After executing the above {sqlpp} statement you should see only one 'transaction' item follows:
+
