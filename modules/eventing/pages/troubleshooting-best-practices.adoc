...
Always make sure that the memory quota on your metadata bucket is sufficiently large to ensure a residency ratio of 100%. Additionally avoid using an Ephemeral bucket for your metadata bucket (refer to next question for details). 

== Can I use Ephemeral Buckets with Eventing?

Yes, but not for the metadata bucket. 

The source bucket and any bucket bindings of your Eventing Function can be Ephemeral.  

However, the Eventing metadata bucket should always be persistent. If the Eventing Storage keyspace is not persistent, the Data Service, or KV, will evict timer and checkpoint documents on hitting quota and Eventing can lose track of both timers and mutations processed.
