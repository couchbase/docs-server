...
For example trying to run xref:eventing:troubleshooting-best-practices.adoc[ConvertBucketToCollections] in a single bucket (as both source and target) will emit an 'ERR_INTER_FUNCTION_RECURSION' error.
