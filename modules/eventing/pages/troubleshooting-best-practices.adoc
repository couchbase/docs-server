...
Always make sure that you Memory Quota on your metadata bucket is sufficiently large to ensure a residency ratio of 100%.

== Eventing worked fine when application was first deployed but now I am getting ETMPFAIL failures

A low residency ratio for either the source or the destination bucket (sometimes these two can be the same) can result in the in an inability of the system to keep up with rate of mutations and internal the logic's required reads and writes to the data service.
...
. The second solution is adding more Data nodes, faster disk IO, more meemory to eliminate the resource bottleneck.
