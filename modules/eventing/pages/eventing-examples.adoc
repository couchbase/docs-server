...
Proceed as follows:
 ...
 .. When the `source` bucket appears on the *Buckets* screen, left-click on the [.ui]*Documents* tab, towards the right-hand side of the `source` bucket's row.
...
. Access the *Eventing* screen, by left-clicking on the [.ui]*Eventing* tab, in the navigation panel at the left-hand side of the Couchbase Web Console:
+
[#eventing_tab]
...
. Return to the *Eventing* screen, by left-clicking on the [.ui]*Eventing* tab, in the navigation bar at the left.
...
Information on installing the Couchbase Python SDK can be found in xref:python-sdk::start-using-sdk.adoc[Start Using the SDK].
Information on using the Couchbase Python SDK to establish bucket-expiration can be found in xref:python-sdk::document-operations.adoc[Document Operations].
...
. Access the *Eventing* screen, by left-clicking on the [.ui]*Eventing* tab, in the left-hand navigation panel.
Then, when the *Eventing* screen is displayed, click on the [.ui]*Add Function* tab, at the upper right.
+
...
When the *Confirm Deploy Function* dialog appears, select `Everything` from the *Feed boundary* menu, and leave [.ui]*Cleanup artifacts?* unchecked.
