...
[#examples-scriptlets-advanced-accessors-cache]
[.status]#Couchbase Server 7.0.2#
*advancedGetOpWithCache*: The following Scriptlet demonstrates using advancedGetOp with an optional third parameter to enable caching of documents for up to 1 second. This is 18X-25X faster than reading near static data directly from the Data Service (or KV).  An example might be a document containing a sales tax table by zipcode
