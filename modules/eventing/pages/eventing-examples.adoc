...
*advancedGetOpWithCache*: The following Scriptlet demonstrates using advancedGetOp with an optional third parameter to enable caching of documents for up to 1 second. This is 18X-25X faster than reading near static (such as lookup tables) data data directly from the Data Service (or KV). 
