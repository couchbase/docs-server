...
| DELETE
| [.path]_/api/v1/functions/[function_name]_
a| Deletes a specific Functions from the cluster (WARNING this a function use with caution).
...
| DELETE
| [.path]_/api/v1/functions_
a| Deletes multiple Functions from the cluster (WARNING this deletes all functions use with caution).
...
a|
Export or return the full definition for one Eventing Function in the cluster.  The definition can be subseqently imported.  
