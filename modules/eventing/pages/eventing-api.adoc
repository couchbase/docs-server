...
As of version 7.1 Evenitng now supports full xref:eventing-rbac.adoc[Eventing Role-Based Access Control] (RBAC). 

In most of the below examples the first format is for privileged functions with a "Function Scope" of *+*+.+*+*, by "Full Admin", "Eventing Full Admin" or via an upgrade form a prior version.

If you are using RBAC you will have to use the second format where pass the parameters *bucket=[fs_bucket]&scope=[fs_scope]* to specify the *bucket* and the *scope* of the "Function Scope" to the REST call.  In addition due to the use or *&* and *?* characters you will need to qoute your REST call on the command line.

Example with only a single format do not require the parameters *bucket=[fs_bucket]&scope=[fs_scope]* and are limited via the credtials passed to the REST call.
