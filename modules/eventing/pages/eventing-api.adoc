...
* The first format is for privileged Eventing functions with a "Function Scope" of *+*+.+*+*, these functions can only be made or managed by users with the "Full Admin", "Eventing Full Admin" role.  In addition this is the default "Function Scope" for all functions after an upgrade from a prior version.
...
If the sample REST API shows just a single format the REST API call do not require the parameters *bucket=[fs_bucket]&scope=[fs_scope]* and the action is fully determined by the *username* and *password* credentials passed to the REST call.
