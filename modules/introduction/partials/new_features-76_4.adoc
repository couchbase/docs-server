...
* The Search Service now supports pre-filtering on Vector Search queries. 
  Use pre-filtering to execute a vector search over a subset of your Vector Search index, 
  through a defined filter request. +
For more information, see xref:vector-search:pre-filtering-vector-search.adoc[]
