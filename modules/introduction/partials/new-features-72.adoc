...
For instructions on reading the change history, see the xref:4.1@kafka-connector:release-notes.html#v4.1.11[Release Notes for Kafka 4.1.11].
...
* The tools `cbbackupmgr`, `cbimport`, and `cbexport` can now specify that an alternate address be used, when connecting from an external network. See xref:cli:backup-restore.adoc[cbbackupmgr], xref:tools:cbimport.adoc[cbimport], and xref:tools:cbexport.adoc[cbexport].
