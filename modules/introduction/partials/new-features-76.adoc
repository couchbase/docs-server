...
* Couchbase Server 7.6 introduces KV Range Scan.
A method for accessing documents without necessarily knowing the IDs (keys).
The most common use case is to get all documents whose IDs start with the same prefix, a `prefixScan`,
but other ranges and even random documents may be retrieved.
...
* Couchbase Server 7.6 introduces Query without Index, using Sequential Scans.
If a keyspace does not have any suitable primary or secondary indexes for a query,
the Query service can fall back on a sequential scan of the data to retrieve the document keys.
Sequential scans are intended for simple, ready access to data, and are not intended as a high performance solution.
See xref:learn:services-and-indexes/indexes/query-without-index.adoc[] for more information.
