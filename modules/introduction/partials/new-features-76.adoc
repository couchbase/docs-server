...
* You can no longer set the `sendStats` to `false` in Couchbase Server Community Edition clusters. These clusters will always send statistics about their use back to Couchbase for analysis. You can still set `sendStats` to `false`` on Couchbase Server Enterprise Edition clusters.