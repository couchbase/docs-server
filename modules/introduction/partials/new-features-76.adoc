...
* Couchbase Server's LDAP support now has a setting that turns on and off middlebox compatibility.
This setting controls low-level network communication options when Couchbase Server connects to an LDAP server through intermediate systems such as proxies and firewalls.
