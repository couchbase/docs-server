...
The new `noRsestart` parameter for the `/pools/default/buckets/{bucket-name}` REST API lets you change the ejection policy of a Couchbase bucket without automatically restarting it.
If you prevent the restart, perform a swap rebalance or a graceful failover followed by a full recovery and rebalance of each data node to apply the new ejection policy.
Either of these procedures let you avoid the downtime associated with restarting the bucket.
