...
Users can now use optimizer hints with `DELETE`, `UPDATE`, and `MERGE` statements.
+
For more information, see xref:n1ql:n1ql-language-reference/optimizer-hints.adoc[].

https://jira.issues.couchbase.com/browse/MB-51737[MB-51737]::
Couchbase Server 8.0 now supports negative keyspace hints.
Negative hint is a type of optimizer hint that instructs the optimizer not to use certain indexes or join methods.
The hints are `NO_INDEX`, `NO_INDEX_FTS`, `NO_USE_NL`, and `NO_USE_HASH`.
+
For more information, see xref:n1ql:n1ql-language-reference/negative-keyspace-hints.adoc[].
...
--
+
For more information, see xref:n1ql:n1ql-manage/monitoring-n1ql-query.adoc#sys-completed-req[].

https://jira.issues.couchbase.com/browse/MB-55470[MB-55470]::
Added a new `INDEX_ALL` hint that directs the optimizer to consider all specified secondary indexes.

https://jira.issues.couchbase.com/browse/MB-59400[MB-59400]::
Introduces nine new SQL++ statements to improve user, group, and bucket management.
+
--
* `CREATE USER`
* `ALTER USER`
...
--
+
For more information, see xref:n1ql:n1ql-language-reference/createuser.adoc[].
+
Also, `GRANT` and `REVOKE` now accept the keywords `USER/USERS` and `GROUP/GROUPS`.
+
For more information, see xref:n1ql:n1ql-language-reference/grant.adoc[] and
xref:n1ql:n1ql-language-reference/revoke.adoc[].
+
We have also introduced two new system keyspaces for group and bucket management:
+
--
[horizontal]
`system:group_info`:::
Provides group information.
For more details, see xref:n1ql:n1ql-intro/sysinfo.adoc#querying-groups[]
...
For more information, see xref:n1ql:n1ql-intro/sysinfo.adoc#querying-bucket-info[].
--


https://jira.issues.couchbase.com/browse/MB-62471[MB-62471]::
A new feature that keeps the optimizer statistics up to date by automatically identifying and refreshing outdated statistics.
These statistics help the Cost-Based Optimizer generate accurate and cost-effective query plans.
+
For more information, see xref:n1ql:n1ql-language-reference/auto-update-statistics.adoc[].

https://jira.issues.couchbase.com/browse/MB-62653[MB-62653]::
A new feature that captures and maintains performance statistics for queries executed on a Couchbase cluster.
It acts as a centralized repository for query performance data, enabling users to track query activity, analyze workload trends,
and identify performance bottlenecks.
+
For more information, see xref:n1ql:n1ql-manage/query-awr.adoc[]

https://jira.issues.couchbase.com/browse/MB-63204[MB-63204]::
When the auto-reprepare feature is active,
a prepared statement automatically updates (reprepares) its plan whenever the GSI metadata version changes.
...
+
For more information, see xref:n1ql:n1ql-language-reference/prepare.adoc[].
...
Added a new SQL++ statement called USING AI  that enables users to leverage AI capabilities to generate SQL++ queries from natural language prompts.
+
For more information, see xref:n1ql:n1ql-language-reference/using-ai.adoc[].

https://jira.issues.couchbase.com/browse/DOC-13215[DOC-13125]::
Introduces two new SQL++ string functions:
+
--
[horizontal]
`COMPRESS()`:::
Compresses a string using zlib compression and encodes the compressed data into base64 format.
...
`UNCOMPRESS()`:::
Takes a base64 encoded, compressed string as input and returns the original uncompressed string.
+
For more information, see xref:n1ql:n1ql-language-reference/stringfun.adocl#fn-str-uncompress[UNCOMRESS].
--

https://jira.issues.couchbase.com/browse/MB-67450[MB-67450]::
The `INFER` statement now supports the following options: `array_sample_size`, `max_nesting_depth`, and `flags`.
+
Additionally, this statement now automatically returns the `meta.id()` attribute, which provides the document keys in the output.
Users can control this behavior using the `flags` option.
+
For more information, see xref:n1ql:n1ql-language-reference/infer.adoc[].
