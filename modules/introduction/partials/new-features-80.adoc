...
* This feature is useful to prevent users from connecting to the database during maintenance, as Time-To-Live (TTL) locking can block user authentication during planned maintenance or system upgrades and automatically restore access when the lock expires.
* This feature is applicable to local domain user accounts only.
* This is an Enterprise only feature, and only available after the entire cluster is upgraded to 8.0 or a later version.
--
+
...
As an administrator, you can force a local domain user account to change its password when it next authenticates with Couchbase Server.
This feature is useful to assign a temporary password to new user accounts by selecting the option to force the user to change their password when creating a new user.
+
...
You can now audit user activity.
This auditing tracks the last time the users made a request to Couchbase Server.
It's useful to locate users whose accounts are no longer active. 
