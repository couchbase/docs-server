...
[#anchor-61457]
https://jira.issues.couchbase.com/browse/MB-61457[MB-61457] Added new Data Services settings through Bucket REST API::
...
https://jira.issues.couchbase.com/browse/MB-66615[MB-66615]: Modified the REST API endpoint for bucket statistics::
...
https://jira.issues.couchbase.com/browse/MB-65779[MB-65779]: Support for DELETE function through the REST API for memcached global settings::
...
https://jira.issues.couchbase.com/browse/MB-60349[MB-60349]: Dynamically add or remove non-Data Services on existing nodes::
...
=== Backup and Restore
https://jira.issues.couchbase.com/browse/MB-44863[MB-44863]: Support for retention periods for repositories and pruning of old backups::
...
https://jira.issues.couchbase.com/browse/MB-60337[MB-60337]: Customize the deletion of backups::
...
https://jira.issues.couchbase.com/browse/MB-58989[MB-58989]: XDCR Conflict Logging Feature::
...
https://jira.issues.couchbase.com/browse/MB-11575[MB-11575]: Identifying incoming replications in an XDCR topology::
XDCR now supports the identification of Incoming Replications on a cluster.
The Incoming Replications can be:

* Viewed on the XDCR tab of the UI.
See xref:manage:manage-xdcr/incoming-xdcr-replications.adoc#view-incoming-replications[View Incoming Replications] in xref:manage:manage-xdcr/incoming-xdcr-replications.adoc[Incoming Replications].
* Retrieved by using the REST API command `GET /xdcr/sourceClusters`.
See xref:rest-api:rest-xdcr-list-incoming-replications.adoc[Listing Incoming Replications].

https://jira.issues.couchbase.com/browse/MB-62564[MB-62564]: xdcrDiffer diagnostic utility::
...
https://jira.issues.couchbase.com/browse/MB-66615[MB-66615]: Modified the REST API endpoint for XDCR statistics::
...
https://jira.issues.couchbase.com/browse/MB-50939[MB-50939] and https://jira.issues.couchbase.com/browse/MB-64921[MB-64921]: Hybrid authentication mode::
...
https://jira.issues.couchbase.com/browse/MB-47905[MB-47905]: Simultaneous support for certificate-based client authentication and node-to-node encryption::
Couchbase Server now supports both certificate-based client authentication and node-to-node encryption simultaneously, when the Mandatory option is selected.
+
For more information, see xref:manage:manage-security/enable-client-certificate-handling.adoc[Enable Client Certificate Handling].

https://jira.issues.couchbase.com/browse/MB-30260[MB-30260]: Locking and unlocking local user accounts::
...
https://jira.issues.couchbase.com/browse/MB-31823[MB-31823]: Forcing password change::
...
https://jira.issues.couchbase.com/browse/MB-63755[MB-63755]: Auditing user activity::
...
[#section-new-feature-800-eventing-service]
=== Eventing Service

https://jira.issues.couchbase.com/browse/MB-56835[MB-56835]::

*Scope-Level Configuration*:::
Configurations can now be defined at the bucket, or scope level using the endpoint:
+
`api/v1/config?bucket=[bucketName]&scope=[scopeName]`.
+
Customers can set options like `enable_curl` or `enable_debugger` at the function-scope or bucket level.

*Node Configuration for Eventing Functions*:::
Customers can configure the number of nodes on which an eventing function within a function scope executes using the `+num_nodes_running+` setting. This can be set at function_scope config level
* By default, all functions run on all nodes.
* If `+num_nodes_running+` is set, the system will attempt to run the function on the specified number of nodes. If fewer nodes are available, the function will run on all available nodes.
