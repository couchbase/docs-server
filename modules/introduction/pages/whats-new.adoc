...
Couchbase Common Table Expression is yet another ANSI CTE equivalent to enhance the manageability of complex queries where the developer can isolate the SQL statement into a temporarily named result set which can be referenced as a source table in the context of a larger query.
