...
Setting it to [.input]`false` disables pretty formatting of the query results and removes all occurrences of white space characters from the query results.
...
For more information, see xref:n1ql:n1ql-language-reference/constructionops.adoc#topic_7_4[Construction Operators].
...
For more information, see xref:sdk:subdocument-operations.adoc#subdoc-operations[Sub-Document Operations].
...
+
You can enable the new memory-optimized global secondary indexes through the index settings under the [.uicontrol]*Settings* tab in the Couchbase Server Web Console.
...
To use the new integrated Query Workbench, switch to the new [.uicontrol]*Query* tab in the Couchbase Server Web Console.
...
The xref:fts:fts-whats-new.adoc#topic_jzy_p5v_vv[What's New?] page lists the FTS features added in this release.
...
See xref:install:getting-started-docker.adoc#topic_mln_twc_3w[Getting Started with Server on Docker Containers] and xref:install:plan-for-production.adoc#topic_tqm_rcx_fw[Planning for Production] for more information.
...
See xref:security:concepts-rba.adoc#concept_ntl_jph_hr[Role-Based Access Control (RBAC) for Administrators] for details.
...
See xref:backup-restore:enterprise-backup-restore.adoc#topic_rr5_sv3_t5[[.cmd]`cbbackupmgr` Tool] for detailed information.
