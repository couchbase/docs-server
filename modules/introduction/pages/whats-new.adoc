...
We extend xref:security:security-certs-auth.adoc[X.509 certificate authentication] beyond key-value data access to query and search services for xref:2.7@dotnet-sdk::sdk-authentication-overview.adoc[all SDK].

We also support xref:2.7@java-sdk::encryption.adoc[field-level encryption] in our SDKs to protect sensitive user data.
...
See xref:2.7@dotnet-sdk::threshold-logging.adoc[Threshold Logging Tracing through the SDK] for details.
...
See the xref:2.7@java-sdk::start-using-sdk.adoc[SDK documentation] for details.
