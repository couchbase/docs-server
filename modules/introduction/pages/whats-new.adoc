...
For more information, see xref:security:security-authorization.adoc#authorization[RBAC].
...
* Integrated web UI with a new [.uicontrol]*Search* tab for Full Text Search on the Web Console.
* Up to 30x performance improvement due to many enhancements, small and large.
See <<performance-scale-ha>> for details.

For more information, see xref:fts:full-text-intro.adoc#topic_kp4_qth_t5[Full Text Search: Fundamentals].
...
For more details and examples, refer to the xref:n1ql:n1ql-language-reference/adaptive-indexing.adoc#untitled1[Adaptive Index] section.
...
For more information, see xref:n1ql:n1ql-language-reference/curl.adoc#topic_8_8[CURL Function].
...
See xref:monitoring:monitoring-n1ql-query.adoc#topic_nvs_ghr_dz[Monitoring N1QL Queries] for details.
...
For more details, see xref:performance:index_pushdowns.adoc#index_pushdowns[Index Pushdown Optimizations].
[#ul_php_pzk_wz]
...
Plasma is the underlying storage engine used for [.input]`Standard Global Secondary Indexes`, which you can choose as part of Index Settings while setting up the cluster.
...
For more information, see xref:clustersetup:automatic-failover.adoc#topic_fcf_chm_zs[Automatic Failover].
...
For more information, see xref:architecture:core-data-access-buckets.adoc#concept_qqk_4r2_xs[Ephemeral buckets].
...
For details, see xref:admin:ui-intro.adoc#topic1980[Couchbase Web Console].
...
For more information, see xref:tools:cbimport.adoc#topic_fsg_5d3_vx[cbimport] and xref:tools:cbexport.adoc#topic_lt4_wd3_vx[cbexport].
...
For more information, see xref:tools:query-workbench.adoc#topic_prr_nyh_t5[Query Workbench].
...
*Spark Connector 2.1*:: First stable release of 2.1.x series brings various improvements in the following areas.
+
*Spark Core*
[#ul_p4t_k2d_wz]
...
For more information, see xref:connectors:spark-2.1/spark-intro.adoc#concept_l11_ppm_pp[Spark Connector 2.1].
...
Refer to the xref:connectors:elasticsearch/overview.adoc#topic1393[documentation] or follow installation and https://github.com/couchbaselabs/elasticsearch-transport-couchbase/blob/master/README.md[configuration walkthrough].
