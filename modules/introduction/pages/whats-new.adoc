...
When replicas exist, and an Analytics node fails over, the Analytics Service continues to work: one of the replicas is promoted to serve the shadow data that was stored on the failed over node.
...
The data definition language has been extended to support creating and dropping of array indexes.
Refer to xref:analytics:5_ddl.adoc[].

=== Analytics Service: Views

Analytics now supports _views_, which are virtual Analytics collections derived from actual stored Analytics collections.
...
Analytics also supports _tabular views_, a type of view that presents data in the form of a table with uniform rows and columns and with a well-defined schema.
Tabular views enables Analytics to interact with software tools that are designed for relational databases, such as Tableau.

For further details on non-tabular and tabular Analytics views, refer to xref:analytics:5a_views.adoc[].
