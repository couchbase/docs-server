...
====== {empty}
[.content]
Welcome to Couchbase Server Documentation! 

Couchbase Server is an open source, distributed, NoSQL document-oriented engagement database. It exposes a fast key-value store with managed cache for sub-millisecond data operations, purpose-built indexers for fast queries and a powerful query engine for executing SQL-like queries. Couchbase Server is specialized to provide low-latency data management for large-scale interactive web, mobile, and IoT applications.

Take a look at xref:server:introduction:whats-new.adoc[What's New?] in this release.
