...
In addition to bug fixes in multiple components, this release adds support for additional bucket operations from Eventing functions.  
...
| *Summary*: Improved failover handling in Eventing service.
...
| *Summary*: Eventing service was not retrying bucket ops failures like ETMPFAIL that could be retried. This has been fixed.

| https://issues.couchbase.com/browse/MB-40357[MB-40357^]
| *Summary*: Fixed an issue so that a function action does not execute after a REST API validation error.
|===
