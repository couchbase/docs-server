...
Couchbase Server 6.5.1, released in April 2020, is the first maintenance release in the 6.5.x series for Couchbase Server. This release adds support for bounded polygons in geospatial search queries in addition to improvements in Query service and tools components. It also includes important and bug fixes.

_Release Month/Year_ : April 2020
...
The areas within which geospatial search queries return matches can now be specified as polygons, in addition to circles and rectangles. Each polygon is expressed as a series of longitude-latitude coordinates, each determining the location of one corner of the polygon. For details, see xref:fts:fts-geospatial-queries.adoc[Geospatial Queries].

[#improvements-651]
=== Other Improvements

* https://issues.couchbase.com/browse/MB-33240[MB-33240^]: Configure filtered events in audit logs using `couchbase-cli`. See xref:cli:cbcli/couchbase-cli-setting-audit.adoc[setting-audit] for details.
* https://issues.couchbase.com/browse/MB-35633[MB-35633^]: Use N1QL to set document expiration. 
*  https://issues.couchbase.com/browse/MB-37321[MB-37321^]: New _file append mode_ in `cbq` to append redirected output to the end of an existing file, rather than overwriting the existing file. See xref:tools:cbq-shell.adoc[cbq: The Command Line Shell for N1QL] for details.
...
| https://issues.couchbase.com/browse/MB-32401[MB-32401^]
| *Summary*: Couchbase Server responded with a WWW-Authenticate header to unauthenticated requests which allowed a user to authenticate via a user / password dialog in a web browser. The problem is that these credentials are cached by the browser which allows a hacker to use CSRF to attack a cluster in the event that an administrator has used their browser to check the results of a REST API request.

This has been fixed by providing an option to disable this behavior using `couchbase-cli` (`couchbase-cli setting-security -set -disable-www-authenticate 1 -c localhost:8091 -u <user> -p <password>`). Note that this is not disabled by default. See `couchbase-cli` xref:cli:cbcli/couchbase-cli-setting-security.adoc[setting-security] for details.
|===
...
| https://issues.couchbase.com/browse/MB-38029[MB-38029^]
| *Summary*: Fixed an issue where an index that was both partitioned and partial was not updated correctly.
