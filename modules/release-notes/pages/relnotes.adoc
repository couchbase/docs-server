...
Upgrading from version 6.0.4 to version 6.5.0 does not work as version 6.0.4 included a per-node configuration migration (https://issues.couchbase.com/browse/MB-37269[MB-37269^]), and it was released after 6.5.0. Note that this issue is observed when using offline upgrade or rolling online upgrade using graceful failover and delta node recovery.

To upgrade successfully from version 6.0.4 to 6.5.0, you must use swap rebalance until the next maintenance release on the 6.5.x series is available.
