...
| *Summary*: By default, XDCR will prevent replications from being created on a source ephemeral bucket with NRU policy. This release of Couchbase Server introduces a special XDCR internal settings flag, `AllowSourceNRUCreation`, that allows replications to be created on a source ephemeral bucket with NRU policy. 
...
| *Summary*: After upgrading to 6.0.x, it was observed that the compression type not set automatically. This has been fixed.
...
| *Summary*: Fixed an issue observed when attempting to modify via subdoc by reseting the datatype to XATTRs if the pre-expiry callback returns a non-empty response.
...
| *Summary*: Fixed the cursor dropping trigger issue which caused Couchbase Server to get into a livelock state due to high checkpoint memory usage.

| https://issues.couchbase.com/browse/MB-33854[MB-33854^]
| *Summary*: Memcached returned ENGINE NOT_STORED to client after trying to perform arithmetic operations and caused a race condition.
...
| *Summary*: The failure to query view on the UI has been fixed.
