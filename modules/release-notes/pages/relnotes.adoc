...
| The topology change detector, upon detecting that a target bucket no longer exists, would delete the replication spec right away. However, in case the target node was being failed over and had stopped serving the bucket, this could cause the replication spec to be incorrectly deleted. This has been fixed and the topology change detector does not delete replication spec when the target bucket cannot be found.
|===
