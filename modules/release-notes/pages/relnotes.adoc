...
| Leaking credentials in REST api crash dump
...
| Index UI stats not working when "view by server node"
...
| index attempting to store array element is not storing any items
...
| BP Snapshot latency histograms to 6.6.6

| https://issues.couchbase.com/browse/MB-53997[MB-53997^]
| Update GoLang to 1.18 or 1.19
...
| Index rollback to zero after KV node failover
...
| 	Error messages like "DDLServiceMgr:rebalanceDone(): Failed to initialize metadata provider" seen in indexer log during upgrade
...
| Online upgrade using swap rebalance from 6.6.5-MP8 -> 6.6.5-MP9 with n2n enabled + encryption level strict fails while rebalancing cbas
...
| Online upgrade using graceful failover/delta recovery/rebalance fails during rebalance of eventing service
