...
////
Start of Server 7.2.1 Release Notes.
ALL Server 7.2.1 Release Notes commented out until GA.
...
* Improvements in Rebalance times 
...
|https://issues.couchbase.com/browse/MB-57059[MB-57059^]
|Creation of Bloom filter for plasma back indexes was not enabled by default. Enabling it assists partial indexes and insert workloads by optimizing the back index lookups.
|Bloom filter creation for plasma back indexes is now enabled by default. When upgrading, nodes with support for bloom filters start creating it. You can disable it and opt out.

|https://issues.couchbase.com/browse/MB-57629[MB-57629^]
|For Indexes created on a default scope and collection with dedicated plasma disk files, when there were many indexes, the overhead of maintaining dedicated files was an issue. The issue was aggravated on low end systems.
|Shared plasma disk files similar to indexes created on collections are now used, so that the per index overhead is smaller.
...
End of Server 7.2.1 Release Notes.
////
