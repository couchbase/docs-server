...
| https://issues.couchbase.com/browse/MB-55154[MB-55154^]
| Improve server certificate validation (validate SAN).
...
| https://issues.couchbase.com/browse/MB-53582[MB-53582^]
| cbq tool does not handle special characters in username and password.

| https://issues.couchbase.com/browse/MB-54034[MB-54034^]
| Display the number of uses for prepared statements accurately.

| https://issues.couchbase.com/browse/MB-54039[MB-54039^]
| Query returning incorrect number of documents.

| https://issues.couchbase.com/browse/MB-54040[MB-54040^]
| [SQL++] insert does not trigger a memory quota exceeded.

| https://issues.couchbase.com/browse/MB-54041[MB-54041^]
| Hash join should not be allowed under the inner of a nested-loop join.
...
| https://issues.couchbase.com/browse/MB-54045[MB-54045^]
| CTE with sub select and hash join returns wrong result.

| https://issues.couchbase.com/browse/MB-54046[MB-54046^]
| LEFT JOIN on array expression is a different result using HASH(PROBE).

| https://issues.couchbase.com/browse/MB-54047[MB-54047^]
| SQL++  query with an empty span is generating the wrong explain output.
...
| https://issues.couchbase.com/browse/MB-54971[MB-54971^]
| Planner error and other indexes are not considered.
...
| https://issues.couchbase.com/browse/MB-55120[MB-55120^]
| Intermittent high reclaim pending up to 1.4GB for a 5M scan.
...
| https://issues.couchbase.com/browse/MB-53269[MB-53269 ^]
| Eventing Bucket Backed Cache in the Advanced Accessor couchbase.get() not efficient for very large documents. The final implementation returns small documents with a speed up is 20-25X. Large documents are returned with a speed up of 400-500X when using {"cache": true}.
...
| https://issues.couchbase.com/browse/MB-56379[MB-56379^]
| Eventing writes to the wrong keyspace (_default._default) when the collection name is over 30 characters.
...
[#table-known-issues-720-clustermanager, cols="25,66"]
|===
|Issue | Description

| https://issues.couchbase.com/browse/MB-56321[MB-56321^]
| *Summary:* Databases are authenticated using client certificate authentication (mTLS). 
However, the Rest Client initializer does not handle situations where authentication for a username and password is optional.

*Workaround:* Before a database is initialized, use placeholders for the username and password. 
After a database is initialized, a username and password must be supplied.
