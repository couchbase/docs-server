...
| https://issues.couchbase.com/browse/MB-39838[MB-39838^]
...
| *Summary*: To ensure consistent results with covering and non-covering flex index queries, we mandate a keyword analyzer for queries that don't use an analyzer. You must specify the index name, or use a match query and explicitly specify the analyzer to be used.
