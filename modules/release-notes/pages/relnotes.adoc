...
* Automatic failover can now fail over more than three nodes concurrently; additionally, automatic failover of index services is supported.
...
_Automatic failover_ can now fail over more than three nodes concurrently

Note that this improvement has permitted the deprecation of pre-7.1 interfaces that were specific to triggering _auto-failover for server groups_.
