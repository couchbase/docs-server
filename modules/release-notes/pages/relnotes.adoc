...
** Supports binary documents in addition to JSON documents.
...
// | https://issues.couchbase.com/browse/MB-43847[MB-43847^]
// | *Summary*: 

| https://issues.couchbase.com/browse/MB-43744[MB-43744^]
| *Summary*: SetVBucketState items are not persisted after a warmup if no other items are. If the SetVBucketState item was generated due to a crash (i.e. a new failover table entry), no new items were created, and memached was shutdown and restarted cleanly then the net result would be that no new failover table entry would be persisted for a crash/unclean shutdown. This could cause replica vBuckets that are ahead (due to the crash/unclean shutdown not persisting items) not to rollback when connecting to the active vBucket if it now has a higher seqno than the replica. This can result in data loss/corruption if the active vBucket is then failed over.
...
| https://issues.couchbase.com/browse/MB-43205[MB-43205^]
| *Summary*: Rebalance on 6.6.0 and 6.6.1 could fail after an offline upgrade from pre-6.6 in the case where pre-6.6 data nodes stored invalid tombstones. This has been fixed in 6.6.2 by sanitizing invalid tombstones as soon as they are received by DCP consumers.
...
| https://issues.couchbase.com/browse/MB-41942[MB-41942^]
| *Summary*: Shutting down or crashing during warmup may cause a failover table entry to be lost. If an active vBucket crashes then a replica vBucket may be ahead if the active node has not persisted all mutations. If the active were then to process expirations or complete durable writes during warmup it may process more total mutations than the replica is ahead by. If the active were then to shut down or crash during the warmup, the replica could reconnect once the active comes back online and fail to roll back to the last consistent state as the active would not persist a failover table entry. This would cause the replica to become out of sync with the active which could cause crashes if durable writes are used. If the replica were then promoted, non-durable writes could be lost.
...
// | *Summary*: [Need help summarizing this issue]

| https://issues.couchbase.com/browse/MB-43762[MB-43762^]
| *Summary*: The Admin UI now provides an option to configure the "dcp_stream_boundary" to "Everything" or "From now" per Function.
...
| https://issues.couchbase.com/browse/MB-44927[MB-44927^]
| *Summary*: Updated to Python Requests 2.23.0
