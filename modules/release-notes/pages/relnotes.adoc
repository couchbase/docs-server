...
| *Summary*: Fixed an issue where the replication of deletes with XATTRS failed if a EWOULD_BLOCK occurred.

| https://issues.couchbase.com/browse/MB-40531[MB-40531^]
| *Summary*: Fixed an issue where the HiFi-MRU Item Pager evicted items from active vBuckets even when there were ample replica vBucket items which could be evicted first.
...
| *Summary*:  Fixed an issue where StateLock was not held in all cases.
|===
