...
There are a few significant changes to the production ready version of the Analytics service. You'll find this information handy if you've played with one of the Developer Preview versions of the Analytics service.
...
``
+
The Analytics service buckets now automatically mirror Couchbase Server buckets, e.g. in the previous example other Analytics DDL commands can refer to a bucket health without any extra configuration.
...
+
Starting this release, you no longer require the SHADOW keyword and doing do returns an error. You can use the following syntax to create a dataset:
+
----
...
+
The CONNECT and DISCONNECT commands would activate or deactivate dataset processing of data from Couchbase Server buckets.
+
...
The notable fixes in this release are listed below. See https://issues.couchbase.com/issues/?filter=16883[JIRA^] for the complete list of enhancements and fixes in this release.
