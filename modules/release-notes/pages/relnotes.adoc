...
[#table-known-issues-720-indexservice, cols="25,66a"]
|===
|Issue | Description

| https://issues.couchbase.com/browse/MB-56429[MB-56429^]
| *Summary:* In Couchbase Server 7.2.0, the Indexing service specifies that it can consume data from the Data service in out-of-sequence order (OSO) when building indexes on non-default scopes and collections only.
These OSO-enabled index builds may experience slowness compared to OSO-disabled index builds.
As index rebalance uses index builds, overall index rebalance may also experience slowness for OSO-enabled index builds.
