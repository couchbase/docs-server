...
| https://issues.couchbase.com/browse/MB-47667[MB-47667^]
| *Summary*: Views are deprecated in this release. The Query Workbench currently includes the [.ui]*Views* link and will be removed in an upcoming release.

*Workaround*: Do not use the [.ui]*Views* link in the Query Workbench. Support for Views is only available through the xref:php-sdk:howtos:view-queries-with-sdk.adoc[SDKs].
|===
