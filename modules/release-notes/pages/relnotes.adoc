...
| *Summary*: In Couchbase Server version 6.0.3, the flag `LCB_CMDN1QL_F_PREPCACHE` was set to run all N1QL queries in Eventing as prepared queries. However, this causes a SEGFAULT error when a function involving a N1QL query is deployed and undeployed in a loop. 
To avoid this error, the flag `LCB_CMDN1QL_F_PREPCACHE` has been disabled.
|===
...
| *Summary*: Fixed an issue where the number of scorch index files on disk grew due to 
|===
