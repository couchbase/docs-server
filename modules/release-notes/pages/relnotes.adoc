...
[#release-712]
== Release 7.1.2 (August 2022)

Couchbase Server 7.1.2 was released in August 2022.
...
* The tools `cbbackupmgr`, `cbexport` and `cbimport` are now provided in a new, separate package, supporting Couchbase Capella.
...
* The `cbbackupmgr` tool now provides _read-only_ repositories, which allow backups made on Couchbase Capella to be downloaded.

* The Search Service now supports Hebrew.

* Support is now provided for ARM on Ubuntu 20.04.
...
* A user's password can now be changed by means of the REST API without roles needing to be specified. See xref:rest-api:rbac.adoc#create-a-local-user-and-assign-roles[Create a Local User and Assign Roles].
