...
Couchbase Server 6.6.4 was released in December 2021. This maintenance release includes bug fixes in multiple components.
...
| *Summary*: ns_server should generate DCP connection names that are no longer than allowed by memcached (relates to xref:release-notes:relnotes.adoc#data-service[MB-34280])
|===
...
| *Summary*: Investigate eventing-producer RSS bloat when eventing-consumers are re-spawned continuously 
