...
| Support FTS's docid_regexp mode for {sqlpp}
...
| SEARCH_META().score behaves different from SEARCH_SCORE() in some {sqlpp} queries
...
Any attempt to execute a function with {sqlpp} udfs replicated from a 7.1 node will fail with "no library found in worker" on a 7.0.x node.
