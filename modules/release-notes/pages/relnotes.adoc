...
| Fixed an issue where replication from version 4.5.1 to version 5.5.x caused corruption on deleted documents leading to inability to rebalance.
|===
...
| If the memory usage of the index nodes is close to quota, during rebalance, some of the index replicas may not get repaired when an index node is rebalanced out of the cluster. In release 5.5.2, the replicas will always be repaired. This issue is applicable only for the standard secondary storage mode.
