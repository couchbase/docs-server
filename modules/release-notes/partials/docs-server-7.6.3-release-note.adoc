...
| An issue with the HTTP client lifecycle resulted in leaked TCP connections between the Analytics service and the Java driver, eventually leading to ephemeral port exhaustion.
| Issue resolved.
