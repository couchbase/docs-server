...
If you expand a keyspace in the Data Insights, the tool learns and includes the field names from the schema of the expanded keyspace.
...
Since typing a semicolon and pressing kbd:[Enter] executes the query, you must compose the sequence of statements in a separate text editor, and then copy the whole sequence into the Query Editor.
You can then click btn:[Execute] to run the sequence of statements.
...
To execute a single statement as a transaction, enter the statement in the Query Editor and click btn:[Run As Transaction].

The btn:[Run As Transaction] button is hidden by default.
To display it, click the "show more actions" icon:chevron-right[] icon to the right of the btn:[Execute] button.
To hide it again, click the "hide this" icon:times-circle[] icon.

You can also use the Query Editor to execute a transaction containing multiple statements.
The simplest way of doing this is to compose the sequence of statements in a separate text editor, and then copy the whole sequence into the Query Editor.
You can then click btn:[Run As Transaction] to execute the transaction.

To compose a transaction one statement at a time, you must begin a transaction with the {begin-transaction}[BEGIN TRANSACTION] statement, then set the <<query-preferences>> to specify the `txid` parameter for the rest of the statements in the transaction.
In this case, be sure to unset the `txid` parameter after you have committed the transaction.
...
image::query-workbench-context.png[]
...
To unset the query context, open the scope drop-down menu and select `bucket.scope`.
...
Use the arrowhead icons icon:angle-left[] icon:angle-right[] at the top of the editor to navigate through the history.
...
Use the forward or back buttons icon:angle-left[] icon:angle-right[] to move to the next or previous query in the history.
The forward button icon:angle-right[] can also create a new blank query when you are already at the end of the query history.
...
+
image::query-workbench-import.png[]
...
+
image::query-workbench-export.png[]
...
. Choose btn:[Next] to export the data, or btn:[Cancel] to cancel.

NOTE: When using Safari, clicking btn:[Next] loads the data into a new window.
You have to save the file manually using the menu:File[Save As] menu.

[#query-preferences]
== Query Preferences

You can specify the query settings by clicking the btn:[Preferences] button icon:cog[].
...
Only applies to {begin-transaction}[BEGIN TRANSACTION] statements, or statements executed with the btn:[Run As Transaction] button.
For more information, refer to the {txtimeout_req}[txtimeout] parameter.

[#bucket-analyzer]
== Viewing the Data Insights

The *Data Insights* area displays all installed keyspaces in the cluster.
By default, when the Query Workbench is first loaded, it retrieves a list of available keyspaces from the cluster.
The [.ui]*Data Insights* area is automatically refreshed when keyspaces or indexes are added or removed.

Click the btn:[Resize] button icon:expand-alt[] to enlarge the [.ui]*Data Insights* area -- the [.ui]*Query Editor* and [.ui]*Query Results* areas are resized accordingly.

The keyspaces are grouped into the following categories based on the indexes created for the keyspace:

* *Fully Queryable Keyspaces*: Contain a primary index or a primary index and secondary indexes.
* *Queryable on Indexed Fields*: Do not contain a primary index, but have one or more secondary indexes.
* *Non-Indexed Keyspaces*: Do not contain any indexes.
These keyspaces do not support queries.
You must first define an index before querying these keyspaces.

You can expand any keyspace to view the schema for that keyspace: field names, types, and if you hover the mouse pointer over a field name, you can see example values for that field.
Keyspace analysis is based on the N1QL {infer}[INFER] statement, which you can run manually to get more detailed results.
This command infers a schema for a keyspace by examining a random sample of documents.
...
----
INFER keyspace-name [ WITH options ];
----
...
.Sample INFER statement: `+++INFER `travel-sample`.inventory.landmark with {"sample_size": 3000};+++`
...
The following figures display the result of the query `pass:c[SELECT * FROM `travel-sample`.inventory.airline LIMIT 1;]` in different formats.
...
The btn:[Explain] button is hidden by default.
To display it, click the "show more actions" icon:chevron-right[] icon to the right of the btn:[Execute] button.
To hide it again, click the "hide this" icon:times-circle[] icon.
...
➀ At the top, it shows a summary which also shows lists of the keyspaces, indexes, and fields used by the query.
...
The btn:[Advise] button is hidden by default.
To display it, click the "show more actions" icon:chevron-right[] icon to the right of the btn:[Execute] button.
To hide it again, click the "hide this" icon:times-circle[] icon.
