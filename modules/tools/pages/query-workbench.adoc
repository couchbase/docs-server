...
:escape-hatch: cloud:clusters:query-service/query-workbench.adoc
...
:max_parallelism: xref:n1ql:n1ql-manage/query-settings.adoc#max_parallelism_req
:timeout_req: xref:n1ql:n1ql-manage/query-settings.adoc#timeout_req
:txtimeout_req: xref:n1ql:n1ql-manage/query-settings.adoc#txtimeout_req
:query_context: xref:n1ql:n1ql-manage/query-settings.adoc#query_context
...
Using the [.ui]*Query Workbench*, you can conveniently explore data, create, edit, run, and save {sqlpp} queries, view and save query results, and explore the document structures in a keyspace -- all in a single window.

Features of the Query Workbench include:

* A single, integrated visual interface to perform query development and testing.
* Easy viewing and editing of complex queries by providing features such as multi-line formatting, copy-and-paste, syntax coloring, auto completion of {sqlpp} keywords and keyspace and field names, and easy cursor movement.
* View the structure of the documents in a keyspace by using the {sqlpp} {infer}[INFER] command.
...
* *Syntax coloring* -- For easy viewing, {sqlpp} keywords, numbers and string literals are differently colored.
* *Auto-completion* -- When entering a keyword in the Query editor, if you press the kbd:[Tab] key or kbd:[Ctrl+Space], the tool offers a list of matching {sqlpp} keywords and keyspace names that are close to what you have typed so far.
...
=== Index Advice in Community Edition

[.labels]
[.edition]#{community}#

Note that in Couchbase Server Community Edition, index advice is provided in a different way.
The area immediately below the *Query Editor* appears as follows:

image::manage:manage-ui/ceIndexAdvisorLink.png["Execute button, Explain button, and External Query Advisor link",320]

To get index advice, click the https://index-advisor.couchbase.com/indexadvisor/#1[External Query Advisor^] link to access the Couchbase *Index Advisor* web site.
...
Keyspace analysis is based on the {sqlpp} {infer}[INFER] statement, which you can run manually to get more detailed results.
