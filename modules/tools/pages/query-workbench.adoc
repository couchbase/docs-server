...
After entering a query, you can execute the query by either typing ‘;’ and pressing [.uicontrol]*Enter*, or by clicking the btn:[Execute] button.
When the query is running, the btn:[Execute] button changes to [.uicontrol]*Cancel* that allows you to cancel the running query.
When you cancel a running query, it stops the activity on the server side as well.
After running the query, you can use the [.uicontrol]*Explain* link to view the execution plan for the query that is executed in the [.uicontrol]*Query Results* pane.
...
Click [.uicontrol]*Export* to display [.uicontrol]*Export Query / Data* window.
...
The [.uicontrol]*Run-Time Preferences *window is displayed.

[#image_fl4_frr_dz]
image::query-workbench-preferences.png[,570]

Define the following options and click [.uicontrol]*Save Preferences*.
