...
*course*:: the courses that the student can take. You will need to store the course name, faculty and the number of credit points associated with the course. Against each student you will somehow need to record which of the course they are taking.
+
NOTE: Bear in mind that a student can take more than one course at a time.
...
----
{
  ...
  "enrollments": [   // <1>
    {
      "course-id": "ART-HISTORY-00003",    // <2>
      "date-enrolled": "07-9-2021"
    },
    {
      "course-id": "GRAPHIC-DESIGN-00001",    // <2>
...
<1> The document is stored in JSON format, which allows for the storage of complex types such as arrays without decomposing them to a second table. In this case, the list of `enrollment` records is stored with the student record.

<2> Each `enrollment` record holds a reference to the course it relates to.
+
[NOTE]
====
...
. It would make it very difficult to maintain the data. If the `credit-points` for a course needed to be changed, then you would need to access every student record to make the change.
====
...
:previous-page: {empty}
:next-page: xref:install-couchbase-server.adoc[]
