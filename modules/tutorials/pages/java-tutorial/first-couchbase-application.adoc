...
====
{empty}
<.> Couchbase is completely freeform in that it will allow you to store different document types in the same collection.
You can, for example, store the student records in the same collection as the course records.
This is why it's a good idea to include a `type` field in your document; it'll make it easier to filter them later on.

<.> The oddly-named `upsert` function is used to insert or update documents in a collection.
