...
:description: pass:q[In this section, you're going to add enrollment details to the student records using the Couchbase SDK]
...
<8> Now `Json` elements are added to the `enrollments` array. Looking back at the <<enrollments-structure,data structure>> for enrollments, we need store just two items: the course that the enrollment relates to, and the date the student enrolled.
+
[source, java]
----
enrollments.add(JsonObject.create().put("course-id", graphic_design.getString("id"))
...
:previous-page: xref:java-tutorial/retrieving-documents.adoc[]
:next-page: {empty}
