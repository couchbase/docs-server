...
image::add-student-bucket.png[alt="Adding student bucket to Couchbase", width="{image-width}"]
...
There's nothing stopping you from dumping every document into the same bucket, but in all but the simplest cases, it's better to provide some kind of separation between documents of different types.
...
image::create-scope.png[alt="Dialog to create a new scope", width={image-width}]
