...
There's nothing stopping you from dumping every document into the default bucket. But in all but the simplest cases, it's better to provide some kind of separation between documents of different types. Couchbase has a simple hierarchy model which allows for such separation:
