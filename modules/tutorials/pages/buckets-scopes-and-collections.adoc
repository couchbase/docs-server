...
:description: pass:q[In this section, you'll learn how to logically partition your data in Couchbase using buckets, scopes and collections.]
...
There's nothing stopping you from dumping every document into the bucket. But in all but the simplest cases, it's better to provide some kind of separation. between documents of different types. Couchbase has a simple hierarchy model which allows for such separation:
...
their enrollments. Again, this moves away from the standard relational decomposition since we're actually storing the
enrollments as part of the student's record, instead of implementing it as a link table between the students and the
courses.
...
student's enrollments and changing the credit details on each one. This is why the document model and relational model
...
So you have your cluster, bucket, scope and collections set up and ready to be populated. The following sections will build an application to insert and extract documents for your collection.

Couchbase supports a number of languages and frameworks; select the language/framework tutorial you require from the list below


|===
3+^|Tutorials

|xref:java-tutorial/install-couchbase-java-sdk.adoc[Start the Java tutorial]
| Start the Python tutorial
| Start the NodeJS tutorial


|===


:previous-page: xref:install-couchbase-server.adoc[]
:next-page: {empty}
