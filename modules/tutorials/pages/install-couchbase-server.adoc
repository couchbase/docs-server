...
:description: pass:q[A short tutorial that will guide the developer in downloading and installing Couchbase, then creating a database to store our student records.]
...
+
--
Instructions for provisioning a Couchbase server as a cloud instance can be found on xref:cloud:ROOT:index.adoc[] site.
--
====
...
image::launch-couchbase-server-page.png[width="60%", align="center"]

Press btn:[Setup New Cluster] to create your new cluster. The next page will allow you to fill in details for the cluster:

image::enter-cluster-details.png[width="60%"]

Since we're going to be housing student records, let's call the cluster `student-cluster`. You'll also need an admin username and a strong yet memorable password. Since this is just a demonstration, leave the administrator username as `Administrator`. After you've filled in the details, press btn:[Next: Accept Terms].

TIP: Since this is a tutorial, then feel free to use something like `password` for your password. In a real system,
of course, you'll use something much stronger.
...
== What's next …?
So you have a database. Next thing to do is add stuff to it and retrieve it again. To do that, you're going to xref:install-couchbase-java-sdk.adoc[install the Couchbase SDK].
