...
In cases where rebalance is interrupted (for example, by the user's left-clicking on the *Stop Rebalance* button, in Couchbase Web Console, or due to auto-failover), a report may be generated to describe a partially unsuccessful rebalance; indicating, in certain fields, the degrees to which individual rebalance-processes were left incomplete.
...
The `afterChain` indicates that following rebalance, the active vBucket resides on node `10.143.194.102`, and the two replicas on nodes `10.143.194.101` and `10.143.194.104` respectively.

Respective strings for `startTime` and `completedTime` and an integer for `timeTaken` are provided for each of the move stages for this vBucket.
