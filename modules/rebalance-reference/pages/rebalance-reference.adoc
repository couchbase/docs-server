...
(An example is _delta_recovery_, which is a substage reflected in the `data` output, when Delta Recovery has indeed been performed on one or more nodes running the Data Service.)
...
** `failed_nodes`.
The nodes for which rebalance failed.

* `master_node`.
The node on which rebalance was initiated.
