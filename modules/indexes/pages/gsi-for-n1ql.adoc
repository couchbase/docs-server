...
// API.
At_plus scan consistency flag automatically degrades to the same characteristics as `stale=false` in the view API.
...
. Click [.ui]*Remove* on each of the nodes that has the index service enabled and push rebalance to remove the nodes from the cluster.
+
