...
image:xdcr-reference:DataTypeConversionTableWithAnnotations.png[,700,align=left]

These conversion-support options are further described in the following table, each row of which starts with an integer that corresponds to an annotation in the diagram above.
...
| `Object`
| image:xdcr-reference:cross-mark-red-sm-wide.png[] Invalid.
| Comparison is performed along the following lines: if the length of `<Any>` is greater than the length of 'Object`, then `<Any> > Object` returns `true`; otherwise, corresponding bytes are compared in sequence, until one is found to be the greater, and `true` is returned &#8212; _i.e._ `<Any>[byte_1] > Object[byte_1]`, `<Any>[byte_2] > Object[byte_2]`, and so forth.
...
| `Array`
| image:xdcr-reference:cross-mark-red-sm-wide.png[] Invalid.
| Comparison is performed along the following lines: if the length of `<Any>` is greater than the length of 'Array`, then `<Any> > Array` returns `true`; otherwise, corresponding objects are compared in sequence, until one is found to be the greater, and `true` is returned &#8212; _i.e._ `<Any>[object_1] > Array[object_1]`, `<Any>[object_2] > Array[object_2]`, and so forth.
