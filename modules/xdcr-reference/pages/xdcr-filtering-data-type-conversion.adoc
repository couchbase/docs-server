...
[cols="3,3,7,7"]
|===
| *Type 1* | *Type 1* | *Comparison* | *Operation*
...
If the result is _not_ a valid `Uint` (due to the result's being negative), the result is converted to `Int`, and is returned as such; _unless_ the conversion to `Int` would result in overflow, in which case, `InvalidValue` is returned.
