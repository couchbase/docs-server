...
| `Object`
| image:xdcr-reference:dash-black-inline.png[] No conversion need be performed.
| Comparion is performed along the following lines: the specified expression is first applied to the objects in terms of their respective lengths.
For example, if the expression is `arg_1 > arg_2`, this gets applied as `Object_1 > Object_2`; and if the length of `Object_1` is indeed greater than the length of `Object_2`, the requirement is considered to have been met; and the comparison-process ends.

In cases where the requirement is _not_ met based on object-lengths, the comparison-process continues as follows: pairs of correspondingly positioned data-byes from the objects are compared, until the specified requirement is met, or is finally determined _not_ to have been met.
For example, if the expression is `arg_1 > arg_2`, this gets applied as `Object_1[byte_1] > Object_2[byte_1]`, `Object_1[byte_2] > Object_2[byte_2]`, and so forth.
|===
