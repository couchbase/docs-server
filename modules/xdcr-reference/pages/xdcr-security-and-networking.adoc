...
Therefore, if DAC is being used, the Root Certificate referred to on the client when setting up a fully secure replication must be the one whose authority, on the server, is relied on for these DAC operations.
