...
XDCR optimistic replication operation only applies when the `enableCrossClusterVersioning` property is not being used. When the `enableCrossClusterVersioning` property is set to `true` (enabled), the XDCR processing always retrieves metadata from the target cluster for every document of any size that undergoes replication.
