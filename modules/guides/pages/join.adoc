...
You can use a _join_ to read objects from one data source (the _left-hand side_ of the join), combine them with corresponding objects from another data source (the _right-hand side_ of the join), and return the joined objects.
...
It enables you to join one keyspace to another using arbitrary fields.
...
TIP: To use a xref:learn:data/data.adoc#keys[document key] in the join predicate, use the META() function to return the `id` field from the document metadata.
...
N1QL also provides two legacy syntaxes for joins.

* A _lookup join_ uses a foreign key field on the left-hand side of the join with the primary document key on the right-hand side of the join.
For more information and examples, refer to xref:n1ql:n1ql-language-reference/join.adoc#lookup-join-clause[Lookup JOIN Clause].

* An _index join_ uses the primary document key on the left-hand side of the join with a foreign key field on the right-hand side of the join.
For more information and examples, refer to xref:n1ql:n1ql-language-reference/join.adoc#index-join-clause[Index JOIN Clause].

Lookup joins and index joins only support inner and left outer joins.

include::partial$recursive-joins.adoc[]

== Next Steps

* xref:nest-unnest.adoc[Deep Traversal for Nested Documents]
