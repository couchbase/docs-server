...
:description: Creating an {sqlpp} User-Defined Function to call your JavaScript function.
...
Before you can run the JavaScript function you have created (see xref:create-javascript-library.adoc[]), you will need to create an {sqlpp} User-Defined Function to reference it.
You can create an {sqlpp} User-Defined Function by using:
...
+
--
Run a `curl` command from the shell to add an {sqlpp} User-Defined Function that calls your Javascript function.
...
Having created your {sqlpp} User-Defined Function, the xref:call-user-defined-function.adoc[next step] is to use an  {sqlpp} statement to call the function.
