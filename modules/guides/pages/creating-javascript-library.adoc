...
*Create library*:: This creates the logical storage area where a set of functions are kept. The library can be `global` or `scoped`.

*Global*::: The library is accessible is not tied to any bucket or scope, so is accessible across the cluster. As long as the user can access the cluster then they can access the function. 
*Scoped*::: The library is bound to a scope within a bucket, so access rights to that scope are needed to access it. 

*Add function*:: The developer adds a JavaScript function to the library before saving it.

*Create N1QL User-Defined Function*:: The N1QL User-Defined Function registers the JavaScript function with the cluster so that it can be called as part of N1QL statements (such as `SELECT` and `EXECUTE FUNCTION`). 
...
NOTE: As shown in xref:create-library-udf-sequence[xrefstyle=short], adding the JavaScript function is done whilst the library is being created.
+
