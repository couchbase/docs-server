...
When you start the transaction, the `BEGIN TRANSACTION` statement returns an ID for the transaction.

For each subsequent statement in the transaction, you must set the `txid` parameter to specify the ID of the transaction to which the statement belongs.
