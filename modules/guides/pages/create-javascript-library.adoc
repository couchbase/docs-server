...
////
Before describing how to create a library, here is a brief description of what we're actually doing
////
...
*(3) Create N1QL User-Defined Function*:: The N1QL User Defined Function is needed so that it can be called as part of N1QL statements (such as `SELECT` and `EXECUTE FUNCTION`). 
Creating the N1QL User-Defined Function is covered in xref:create-user-defined-function.adoc[].
...
In the xref:create-user-defined-function.adoc[next step], you'll create a N1QL User-Defined function that references your library function, so it can be called as part of any N1QL statement.
