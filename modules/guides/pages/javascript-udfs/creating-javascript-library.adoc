...
To call a JavaScript function, you first have to add a N1QL function to reference it.
This can be done through the UI, the REST API, or a N1QL query.
...
+
*Function Name*:: This is the name of the function that will be used in N1QL statements.
...
The `global` namespace is set at the level of the bucket, so any N1QL statement run by a user with access to the bucket will be able to run the function. 
...
*Javascript Library*:: The name of the JavaScript library where your function is stored.

*Library Function Name*:: The name of the JavaScript function in the specified library that this definition will call.

. Press the btn:[Save Function] button to save the definition.

NOTE: In the background, The dialog will use the gathered information to generate and execute a N1QL xref:n1ql:n1ql-language-reference/createfunction.adoc[`CREATE FUNCTION`] call to generate the function definition.
...
. Enter the N1QL statement to create the reference function:
+
[source, n1ql]
----
