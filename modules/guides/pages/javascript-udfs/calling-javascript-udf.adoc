...
In this example, we're going to label this function as `GetBusinessDays`.
...
Since the `GetBusinessDays` function is very generic, then set `Namespace` to `global`. 
...
include::partial$javascript-udfs/diagrams-plantuml.adoc[tags="multiple-references"]
