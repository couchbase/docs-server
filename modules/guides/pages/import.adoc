...
If you are using the xref:java:howtos:concurrent-async-apis.adoc#reactive-programming-with-reactor[Reactive API], then use the reactive collection instead:
...
+
--
Hook the data prepared above into an `upsert` routine. For the blocking API, uses the method below:
