...
=== Parse the import into an array or stream of records
...
+
--
For *CSV* and *TSV*, use the https://joshclose.github.io/CsvHelper/[CsvHelper^] library: 
...
For *JSON* or *JSONL* format, use https://www.newtonsoft.com/json[Newtonsoft^]: 
...
+
--
For *CSV* and *TSV*, use the http://opencsv.sourceforge.net/[opencsv^] library: 
...
+
--
For *CSV* and *TSV*, use the https://csv.js.org/parse/[csv-parse^] library:
...
For *JSON* or *JSONL* format, use the https://github.com/uhop/stream-json[stream-json^] library.
...
+
--
For *CSV* and *TSV*, use the https://docs.python.org/3/library/csv.html[csv^] library:
...
For *JSON* and *JSONL*, use the https://docs.python.org/3/library/json.html[json^] library:
...
+
--
Hook the prepared data into an `upsertDocument` routine:
...
NOTE: Can you add links to the kv-operations page for every SDK? Copy from @maria-robobug 's Creating Data how-to guide.
////
