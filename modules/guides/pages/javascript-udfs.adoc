...
JavaScript that is supported in Couchbase shares the same constructs of the https://en.wikipedia.org/wiki/ECMAScript[ECMAScript], though there are a number of restrictions and also extensions with the Couchbase JavaScript implementation. (These are covered in xref:javascript-udfs:javascript-functions-with-couchbase.adoc[])
