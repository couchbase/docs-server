...
JavaScript functions are stored inside a `library` which is simply a file stored within a bucket on the Couchbase server, alongside collections and documents. 
...
include::partial$javascript-udfs/diagrams-plantuml.adoc[tags=javascript-scopes]

NOTE: You can find an introduction to scopes in our xref:tutorials:buckets-scopes-and-collections.adoc#_scopes_and_collections[Couchbase Tutorial].

Furthermore, access restrictions can be applied to scopes, so that only certain groups of users will be able to access buckets and libraries within that scope.
...
If you wish to look into the constructs and available in the language itself, then you can have a look through the following pagess:

* xref:javascript-udfs:javascript-udf-language-constructs.adoc[]
