...
:tabs:
:page-pagination:
:description: How to create indexes and primary indexes.
...
[source,N1QL]
----
include::learn:example$services-and-indexes/indexes/create-pri-nameless.n1ql[]
----
====
...
[source,N1QL]
----
include::learn:example$services-and-indexes/indexes/create-pri-name.n1ql[]
----
====
...
====
The following query creates a secondary index on the `name`, `id`, `icao`, and `iata` fields in the `airline` keyspace.
...
An _adaptive index_ enables you to index all or specified fields in a keyspace.
This enables you to create ad-hoc queries on those fields, without having to tailor a specific index for each query.

NOTE: An adaptive index cannot be used as a covering index.

To create an adaptive index, specify the index key as follows:

1. Use the `ALL` keyword to index all values in the specified fields, or `DISTINCT` to index only distinct values.

2. Use the xref:n1ql-language-reference/metafun.adoc#pairs[PAIRS()] function to specify the fields to index.

====
The following example creates an index on distinct values of the `airportname`, `city`, and `faa` fields, which can be used for any ad-hoc queries using those fields.

[source,n1ql]
----
include::n1ql:example$n1ql-language-reference/adaptive-idx-c4.n1ql[]
----
====

For further details and examples, refer to xref:n1ql:n1ql-language-reference/adaptive-indexing.adoc[Adaptive Index].
