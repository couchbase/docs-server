...
[#indexes-for-missing-leading-key]
=== Indexes for Missing Leading Key
[.status]#Couchbase Server 7.1.2#

==== Prior to version 7.1.2
if a document doesn't contain a field that was defined when the index was created,
then the document will not be indexed.

For example, consider this index creation statement:

[source, n1ql]
----
include::learn:example$services-and-indexes/indexes/create-idx-missing-field.n1ql[]
----

Documents that don't have a `district` field will not be indexed.

This behaviour has changed for Couchbase Server 7.1.2{plus}

==== Couchbase Server 7.1.2 and above

A change to the `CREATE INDEX` syntax supports the indexing of documents where the indexed field is missing:
...
In this case, the document will be indexed, even if the `district` field is missing.
