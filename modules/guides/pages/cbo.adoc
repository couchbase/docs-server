...
xref:n1ql-language-reference/merge.adoc[MERGE], and
xref:n1ql-language-reference/insert.adoc[INSERT INTO with SELECT] queries.
...
This section covers shows how to activate or deactivate the Cost-Based Optimizer for a request.
You can also activate or deactivate the Cost-Based Optimizer for a query node, or for all the query nodes in the cluster.

NOTE: If the Cost-Based Optimizer is unavailable or inactive, the Query service falls back on a legacy _rules-based optimizer_ to generate the query execution plan.
...
[source,n1ql]
----
include::n1ql:example$utility/statistics-expr-predicate.n1ql[tag=indexes]
----
