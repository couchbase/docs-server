...
The `ADVISOR()` function returns a JSON object containing the details of any current indexes used by each query, and any indexes that the Index Advisor recommends, along with the reasons for recommendation.
For each index, a N1QL statement is provided: you can copy and run this index statement to create the recommended index.
...
. Use the `ADVISOR()` function with an object argument to start the session.
...
. If you want to stop the session early, use the `ADVISOR()` function with an object argument to stop the session and save the results.
+
...
. When the session is complete, use the `ADVISOR()` function with an object argument to get the index recommendations.
+
