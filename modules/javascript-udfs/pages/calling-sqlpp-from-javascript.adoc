...
In addition to the inline call, you can also execute a N1QL statement by calling it from the `N1QL(…)` function.
[source, javascript]
----
include::example$add-airline-n1sql-call.js[]
----
...
----
+
Note that the names of the parameters passed into the Javascript function are used in the N1QL statement without any need to assign the parameters in a separate step.
...
If a transaction is started during the iteration process, then the transaction cannot be rolled entirely.
...
It is often the case that Javascript UDF will call a N1QL statement that may itself call another Javascript UDF. 
...
----
include::example$do-recursion-response.json5[]
----
