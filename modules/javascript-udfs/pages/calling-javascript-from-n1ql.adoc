...
== Scopes and Reference Functions
...
The important thing to note is that the two reference functions the same name and parameter signature, but have been created to point to two different functions in different libraries/scopes:
...
Now when the reference function is called:
...
the context of the user will define which of the  reference functions is executed (even though they have the same name), and so which underlying JavaScript function is called.
