...
xref:scopes-for-udf[xrefstyle=short] demonstrates that the library `my-library` exists under the `inventory` scope. 
...
The user does not call the JavaScript functions (shown here as `getBusinessDays(startDate, endDate)`) directly; a *N1QL User-Defined Function* is defined to act as a proxy caller to the JavaScript UDF. 
