...
Delta recovery permits nodes to be re-added to the cluster and incrementally caught up with the data changes.

Delta recovery permits a failed over node to be recovered by re-using the data on its disk and then resynchronizing the data based on the delta change.
...
The data files are used either for Couchbase support, data recovery, or delta recovery.
...
There are conditions where delta recovery either defaults to full recovery or is not available.

The following are conditions for delta recovery failures:

* If topology changes occur while a node is pending delta recovery, delta recovery is impacted.
...
* If certain bucket operations are performed while a node is pending delta recovery, delta recovery is impacted.
For example, a new bucket is added, a bucket's replica configuration is changed, or a bucket is flushed.

The following describes scenarios where delta recovery either defaults to full recovery or is not available.
...
+
image::admin/delta-node-01.png[,570,align=left]
...
. Rebalance is performed and fails.
+
image::admin/delta-node-02.png[,570,align=left]
