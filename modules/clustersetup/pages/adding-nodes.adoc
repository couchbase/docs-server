...
| Services
| By checking all appropriate check boxes (Data, Index, Query, and Full Text), you can define what kind of xref:architecture:services-archi-multi-dimensional-scaling.adoc#concept_v4w_zmj_vs[services] will be provided on the server node that's been added.
...
To add a new node to an existing cluster after installation, click [.uicontrol]*Add Server* within the [.uicontrol]*Manage Server Nodes* area of the Couchbase Web Console.
