...
Couchbase Server specifies limits that may affect server usage and implementation.
...
| 1024 bytes
| The maximum length of a sub-document path that may be used in one of the xref:2.7@java-sdk::subdocument-operations.adoc[sub-document] operations.

| Sub-document path nesting level
| 32
| The maximum number of _levels_ a path may reference in xref:2.7@java-sdk::subdocument-operations.adoc[sub-document] operations.
|===
