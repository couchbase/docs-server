...
As explained in xref:concepts:distributed-data-management.adoc#concept_cz5_szf_vs[Distributed Data Management],  data is logically partitioned between all of the nodes running the Data service in the cluster.
...
Usually Couchbase Server deployments have a single service deployed on each node, see xref:concepts:multidimensional-scaling.adoc#concept_ckt_svy_zs[Multidimensional scaling].
