...
When the failed node has been repaired, it can be added back into the cluster via xref:rejoin-cluster.adoc[Delta Node Recovery] or xref:rejoin-cluster.adoc[Full Recovery].
Alternatively, it can be replaced, with an entirely new node.
