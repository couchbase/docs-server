...
If you are not familiar, click xref:concepts:buckets-vbuckets.adoc#concept_v22_mvy_zs[here].
...
For that purpose see the explanations provided in xref:setup-failover-graceful.adoc#topic_ysk_ycm_zs[Using Graceful Failover] or xref:remove-nodes.adoc#topic_byh_lbg_q4[Removing a server].
...
Hard failover affects each of the services slightly different, depending whether they are on the same server or instance or whether they are using xref:concepts:multidimensional-scaling.adoc#concept_ckt_svy_zs[Multidimensional scaling]and are on different servers or instances.
...
For more information, see the documentation on xref:indexes:indexing-overview.adoc#concept_ssb_qhb_ys[Indexing in Couchbase].
...
For more information see xref:architecture:smart-data-placement-rack-zone-aware.adoc#concept_bln_cnv_vs[Smart data placement with rack and zone awareness].
