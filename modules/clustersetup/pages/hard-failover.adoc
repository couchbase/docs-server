...
If you are not familiar, click xref:concepts:buckets-vbuckets.adoc[here].
...
For that purpose see the explanations provided in xref:setup-failover-graceful.adoc[Using Graceful Failover] or xref:remove-nodes.adoc[Removing a server].
...
Hard failover affects each of the services slightly different, depending whether they are on the same server or instance or whether they are using xref:concepts:multidimensional-scaling.adoc[Multidimensional scaling]and are on different servers or instances.
...
For example, f you have four nodes running the Data service, and one node has failed, there are 256 active and 256 replica vBuckets missing from the cluster.
...
For more information, see the documentation on xref:indexes:indexing-overview.adoc[Indexing in Couchbase].
...
If or when the failed node is repaired and ready, it can be added back to the cluster via xref:delta-node-recovery.adoc[Delta Node Recovery], xref:full-recovery.adoc[full recovery] or an entirely new node could be added.
...
For more information see xref:architecture:smart-data-placement-rack-zone-aware.adoc[Smart data placement with rack and zone awareness].
