...
There are pros and cons to graceful failover compared to xref:hard-failover.adoc#topic_a4s_d24_vs[hard failover] and regular node removal and rebalance.

NOTE: Graceful failover only applies to the Data service

NOTE: To understand how graceful failover works, you must understand the xref:concepts:buckets-vbuckets.adoc#concept_v22_mvy_zs[data distribution] and what Couchbase Server does to partition data across a cluster.
...
xref:hard-failover.adoc#topic_a4s_d24_vs[Hard failover] is the corrective action for an unplanned outage of a node that is unhealthy or unstable, whereas graceful failover is meant for planned maintenance.
