...
* Designed to failover a node only when all active vBuckets on the node have replicas.
* Designed to failover after 120 seconds by default, with a minimum of 5 seconds.
...
* Intended to perform only intra-cluster, so it does not operate with Cross Datacenter Replication xref:xdcr:xdcr-intro.adoc[(XDCR]).
...
You can reset this counter with the Couchbase Web Console by clicking on the btn:[Reset Quota] button.
