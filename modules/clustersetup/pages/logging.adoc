...
Explicit logging can also be performed by means of Couchbase Web Console: on the *Logs* page, left-click on the [.uicontrol]*Collect Information* tab, located near the top.
...
To start the collection-process, left-click on the [.uicontrol]*Start Collection* button.
...
To upload by means of Couchbase Web Console, before starting the collection-process, check the [.uicontrol]*Upload to Couchbase* checkbox:
...
In each modified log file, encrypted text is substituted for redacted text.
...
Redaction can be applied as required, when performing _explicit logging_, by means of either `cbcollect_info` or the *Logs* facility of Couchbase Web Console.
...
Make sure the [.uicontrol]*Partial Redaction* radio-button is selected.
