...
Current [.uicontrol]*Memory* and [.uicontrol]*Disk* status is shown further to the right.
...
The full range of settings applies to and can be changed for Couchbase buckets; while only the [.uicontrol]*Metadata Purge Interval* applies to and can be changed for Ephemeral buckets.
+
{blank}
...
Note that when flushing is enabled, left-clicking on the bucket's display-row on the [.uicontrol]*Buckets* screen displays the btn:[Flush] button:
+
[#flush_bucket_button]
