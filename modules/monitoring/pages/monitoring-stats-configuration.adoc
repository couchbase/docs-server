...
See the documentation for xref:cli:cbcli/couchbase-cli.adoc[couchbase-cli], xref:cli:cbstats-intro.adoc#cbstats-intro[cbstats], and xref:rest-api:rest-intro.adoc[REST API] for further details about using these interfaces into Couchbase Server’s operational information.
