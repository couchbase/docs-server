...
It can be disabled using the [.uicontrol]*Preferences* option.
For more information refer to the xref:tools:query-workbench.adoc#topic_prr_nyh_t5[Query Workbench] section.
...
For more details and examples, refer to the xref:settings:query-settings.adoc#topic_wk1_5v3_t5[Query Settings] section.
...
2+| `phaseTimes` - Cumulative execution times for various phases involved in the query execution, such as authorise, indexscan, fetch, parse, plan, run etc.,

A new query on `system:active requests` will return different values.
| "phaseTimes": { "authorize": "823.631µs", "fetch": "656.873µs", "indexScan": "29.146543ms", "instantiate": "236.221µs", "parse": "826.382µs", "plan": "11.831101ms", "run": "16.892181ms" }


This value will be dynamic, depending on the documents processed by various phases up to this moment in time.
2+| `phaseCounts` - Count of documents processed at selective phases involved in the query execution, such as authorise, indexscan, fetch, parse, plan, run etc.,

A new query on `system:active requests` will return different values.
| "phaseCounts": { "fetch": 16, "indexScan": 187 }
...
| "phaseOperators": { "authorize": 1, "fetch": 1, "indexScan": 2 }
...
Attribute Meta in System Keyspaces:: This is a new virtual attribute in document metadata introduced in Couchbase Server 5.0.
...
Create and Execute Prepared Statements:: Create a prepared statement with the syntax `PREPARE [ name ( FROM | AS ) ] statement`.
+
...
Specify [.input]`0` to log all requests and [.input]`-1` to not log any requests to the keyspace.
+
