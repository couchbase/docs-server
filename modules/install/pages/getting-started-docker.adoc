...
This opens the [.ui]*Add Server Node* dialogue.
+
...
+
image::cluster-setup-add-server-db1.png["The 'Add Server Node' dialogue showing an IP address having been entered.",450]
...
Note that each physical host must be able to discover one another over the network and be able to communicate over the xref:install-ports.adoc[required ports].
