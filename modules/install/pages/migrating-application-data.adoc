...
// create and index on mycollection using cbq
./cbq --engine=localhost:8093 -u Administrator -p password --script="create index myidx1 on testBucket.myscope.mycollection(field1,field2);" <1>
----
...
Using replication (XDCR) to consolidate from multiple buckets to a single bucket:: This is the consolidation scenario.
+
