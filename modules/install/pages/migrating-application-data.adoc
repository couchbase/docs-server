...
Set up XDCR to replicate data from a source cluster to a target cluster. You have two options:

. Self-XDCR: Replicate between buckets on the same cluster (requires sufficient disk space and compute resources)
. Regular XDCR: Replicate to a separate cluster
