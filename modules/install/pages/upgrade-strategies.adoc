...
see xref:manage:manage-failover/graceful-failover.adoc[Graceful Failover] or
...
xref:manage:manage-nodes/graceful-failover.adoc[Graceful Failover] followed by
...
For this option, another Couchbase Server cluster is created (or already exists) and connected via Cross Datacenter Replication (xref:xdcr:xdcr-intro.adoc[XDCR]).
