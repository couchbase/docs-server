= Upgrade Options
:description: A Couchbase-Server cluster can be upgraded by means of difference procedures.
...
Couchbase Server can be upgraded by means of different procedures; each having its own benefits and requirements.
This page presents an overview of the available options: all should be fully evaluated before any is selected.
...
From this point, the diminished cluster continues to serve data at acceptable performance-levels; while the removed nodes are used as spares throughout the upgrade.
