= Upgrade Options
:description: Different procedures are available for the _upgrade_ of Couchbase Server
...
When all nodes have been so upgraded, the cluster can be brought back online, so that the serving of data can resume.
