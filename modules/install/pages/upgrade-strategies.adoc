...
Note that upgrade always requires that the individual node be unavailable for the serving of data, during its upgrade.
...
When all node have been individually upgraded, the cluster is brought back online, and the serving of data is resumed.
Noe that no rebalance is required, since data has remained unmodified during the upgrade.
+
...
The step-by-step procedure is provided in xref:install:upgrade-cluster-online-full-capacity.adoc[Upgrade an Online, Full-Capacity, Multi-Node Cluster].
...
The step-by-step procedure is provided in xref:install:upgrade-cluster-online-reduced-capacity.adoc[Upgrade an Online, Reduced-Capacity, Multi-Node Cluster].
...
User and configuration data should be backed up, before proceeding.
Step-by-step instructions are provided at xref:install:upgrade-a-single-node-cluster.adoc[Upgrade an Offline, Single-Node Cluster].
