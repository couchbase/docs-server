...
Symptoms of the TCP/IP memory using being under pressure include increased latency and limited throughput.
...
To set `tcp_mem` permanently, you add or modify a settings in the Linux `sysctl` configuration. 
...
Once you have determine the file to edit, add or modify the following line. 
...
For example, if both `/etc/sysctl.conf` and a file in `/etc/sysctl.d/` set `net.ipv4.tcp_mem`, the kernel may use the value from `/etc/sysctrl.conf`.
