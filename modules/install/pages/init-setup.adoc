...
Click [.btn]*+* to specify a new location, or click [.btn]*-* to remove a location.
...
For example, _uncheck_ the checkboxes for [.ui]*Search*, [.ui]*Query*, [.ui]*Eventing*, and [.ui]*Analytics*.
The checkbox-display now appears as follows:

[#join_cluster_service_checkboxes]
image::admin/joinClusterServiceCheckboxes.png[,140,align=left]
...
This dialog acknowledges that a new service has been added to the cluster for the first time, and provides the opportunity to edit both of the services now featured (Data and Index), and also to make a selection for the [.ui]*Index Storage Setting*; as is now required due to your addition of the Index service.
