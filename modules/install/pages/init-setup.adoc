...
* [.ui]*Couchase Memory Quotas*: A series of fields that allows you to specify how much memory should be allocated to each service you select for both the current node and for each node you may subsequently add to the cluster.
...
An architectural description of buckets can be found in the section xref:architecture:core-data-access-buckets.adoc[Buckets].
...
[#join_cluster_expanded]
image::admin/joinWithCustomConfig.png[,390,align=left]

Newly displayed fields show default values for the four Couchbase services (with each box checked, indicating that each will be added to this node by default), the [.ui]*Host Name/IP Address* for the current node, and default paths for [.ui]*Data* and [.ui]*Indexes*.
...
For example, _uncheck_ the checkboxes for [.ui]*Query Service* and [.ui]*Search Service*.
The checkbox-display now appears as follows:

[#join_cluster_service_checkboxes]
image::admin/joinClusterServiceCheckboxes.png[,220,align=left]
