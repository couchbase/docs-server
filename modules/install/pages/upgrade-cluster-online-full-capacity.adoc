...
It also explains how, if additional nodes are available, the cluster is maintained at full capacity during the upgrade.
...
NOTE: In Couchbase Server versions 8.0 or earlier, running Eventing functions while rolling an upgrade increases rebalance duration.
Pause all your Eventing functions before starting the upgrade to reduce rebalance time, and resume them after the upgrade completes.
...
This is performed either with xref:backup-restore:enterprise-backup-restore.adoc[cbbackupmgr] or with the xref:learn:services-and-indexes/services/backup-service.adoc[Backup Service]; and must be a _full_ (rather than an incremental) backup.
