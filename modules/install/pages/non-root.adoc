...
To start and stop the non-root-installed Couchbase Server, see the per platform instructions provided on the following pages: xref:install:rhel-suse-install-intro.adoc#start-and-stop[Red Hat & CentOS], xref:install:ubuntu-debian-install.adoc#start-and-stop[Ubuntu and Debian], xref:install:install_suse.adoc#start-and-stop[SUSE Linux Enterprise Server], xref:install:install-oracle.adoc#start-and-stop[Oracle Linux], xref:install:amazon-linux2-install.adoc#start-and-stop[Amazon Linux 2].
