...
The maximum permitted number of _concurrent user processes_, and the maximum permitted number of _currently open file descriptors_, are both established on Linux systems with default values that are too low for Couchbase Server.
The maximum number of user processes for Couchbase Server must therefore be reset to at least 10,000; and the maximum number of file descriptors to at least 70,000.
Proceed as follows:

. Check the current setting for user processes:
+
----
ulimit -u
----
+
The response indicates the maximum number of user processes currently permitted by default for all users.
+
----
3829
----
...
----
+
The response indicates the maximum number of file descriptors currently permitted by default for all users:
+
----
1024
----
...
. Raise the current limits on user processes and file descriptors, for Couchbase Server.
To do this, access `/etc/security/limits.conf`, and add `hard` and `soft` lines for the `nproc` and `nofile` types, for the `couchbase` domain, specifying appropriate values.
+
...
. Following reboot, check all current limits as follows:
+
----
ulimit -a
----
+
This displays a list of limits, among which appear the modified values for user processes and file descriptors:
+
----
...
This concludes the procedure for establishing new limits for user processes and file descriptors.
...
For example, access the command's directory as follows:
