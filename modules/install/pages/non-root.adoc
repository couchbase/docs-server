...
To do this, access `/etc/security/limits.conf`, and edit (or add) `hard` and `soft` lines for the `nproc` and `nofile` types.
Note that root or sudo permissions _will_ be required, for this.
+
