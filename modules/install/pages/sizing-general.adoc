...
=== Accommodating the Access Scanner

The _access scanner_ is used to ensure high-performance data-loading, when buckets are restarted.
The access scanner is highly CPU-intensive: if it is running at the same time that indexes are being updated by the Index Service (either on this node, or on one or more other nodes), the performance of the index updates may be adversely affected.
Since the access scanner's schedule is configurable, it should be configured to minimize the likelihood of this problem.
For information, see xref:learn:buckets-memory-and-storage/memory.adoc#initialization-and-warmup[Initialization and Warmup] and xref:cli:cbepctl/set-flush_param.adoc[set flush_param].
