...
+
Couchbase Server uses the https://en.wikipedia.org/wiki/Snappy_(compression)[Snappy^] compression algorithm, which is optimized for speed and reasonable compression.
You can estimate the compression ratio Couchbase Server can achieve by compressing a sample set of documents using a snappy-based command line tool such as `snzip`.
...
The storage engine affects the append-only multiplier that you should use when sizing disk space.
