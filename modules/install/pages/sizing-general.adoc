...
\begin{equation}
\begin{split}
\text{Per Doc Index Bytes} = ( ( W \cdot 1024 \cdot \text{f_text} \cdot \text{m_text} ) + ( W \cdot 1024 \cdot 0.02_{\mathrm{f\_numeric}} \cdot 2.0_{\mathrm{m\_numeric}} ) \\
+ ( W \cdot 1024 \cdot 0.002_{\mathrm{f\_geo}} \cdot 2.0_{\mathrm{m\_geo}} ) + \\
( W \cdot 1024 \cdot \text{f_kw} \cdot \text{m_kw} ) + B ) \times (1 + D)
\end{split}
\end{equation}
++++

The values for stem:[{\text{f_numeric}}], stem:[{\text{m_numeric}}], stem:[{\text{f_geo}}] and stem:[{\text{m_geo}}] are reasonable defaults for most numeric and geospatial search workloads.
