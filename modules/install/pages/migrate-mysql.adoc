...
With Couchbase Server, you also get an expressive SQL-like query language and query engine called
xref:n1ql:n1ql-language-reference/index.adoc[N1QL], which is
combined with a new powerful indexing mechanism –
xref:learn:services-and-indexes/indexes/global-secondary-indexes.adoc[Global
Secondary Indexes].
...
In a case like that, you can use an extract-transform-load (ETL) tool such as xref:talend-connector::index.adoc[Talend].
