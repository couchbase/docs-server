...
On Mac OSX Sierra, you need to right-click on `Couchbase Server.app` and select [.uicontrol]*Open*.
If prompted with a dialog such as "[.output]`Do you want the application “beam.smp” to accept incoming network connections?` ", select [.uicontrol]*allow* and proceed.
