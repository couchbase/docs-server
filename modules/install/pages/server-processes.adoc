...
Otherwise, Couchbase Server cannot not function properly.
...
When possible, add the parent directories of these components to any allowlists in security systems installed on your nodes. 
For example, on Linux-based nodes, add the pattern `/opt/couchbase/lib/erlang/**` to allow lists.
Adding these patterns means you do not need to update your security policies for every Couchbase Server maintenance release.
...
| Data
| `/opt/couchbase/lib/erlang/{erts-version}/bin/`
...
| Data
| `/opt/couchbase/lib/erlang/lib/{osmon-version}/priv/bin/`
