...
See xref:ipv6-setup.adoc[Using IPv6 With Couchbase] for instructions to enable IPv6.
Note that this cluster needs to be sized appropriately for the workload,
but does not need to be identical to the source cluster.
Create a cluster reference and replication stream from the source to the destination cluster.
(See
xref:learn:clusters-and-availability/xdcr-overview.adoc[Cross Data Center Replication
(XDCR)]).
...
. xref:manage:monitor/ui-monitoring-statistics.adoc#outgoing_xdcr_stats[Monitor the XDCR queue] from the source until all mutations are replicated to the destination cluster.
