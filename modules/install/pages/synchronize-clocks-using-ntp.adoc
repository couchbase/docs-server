...
Correct time synchronisation is important in Couchbase Server for Expiry behavior and to maintain accurate
xref:understanding-couchbase:clusters-and-availability/xdcr-conflict-resolution.adoc#time-synchronization[Time Synchronization] 
in timestamp-based conflict resolution.
...
You should have at least two NTP sources on the same local network as each Couchbase xref:clustersetup:manage-groups.adoc[server group].
