...
| xref:learn:security/certificate-revocation.adoc[Certificate Revocation]
| Not usable in mixed mode.
Until every node has been upgraded, the certificate-revocation REST endpoints return `404` with `CRL feature not yet enabled in this cluster`.
