...
:fn-moxi: footnote:fn-moxi[Support for server-side Moxi was xref:5.0@release-notes:relnotes.adoc#release-500[deprecated in version 5.0], and the ability to create a bucket with a Moxi port was xref:5.5@release-notes:relnotes.adoc#deprecation-550[removed in version 5.5]. Starting in version 6.5, support for port-based buckets is xref:6.5@release-notes:relnotes.adoc#changes-in-behavior-650[completely removed]. Attempts to upgrade to version 6.5 or later will fail during rebalance if a port-based bucket is present in the cluster. Use xref:cli:cbcli/couchbase-cli-bucket-edit.adoc[`bucket-edit --remove-bucket-port`] in the CLI to remove the port from each bucket, and consider using xref:install-client-server.adoc[client-side Moxi].]
:fn-moxi-ref: footnote:fn-moxi[]
:fn-fts-encryption: footnote:[In Couchbase Server 6.0 Enterprise Edition, the Search Service explicitly requires encrypted port 18094 (`fts_ssl_port`) for scatter-gather operations between nodes that are running the Search Service (even if you're still using unencrypted port 8094 for client-to-node traffic). The node-to-node traffic uses the HTTP/2 protocol and is encrypted with either the auto-generated certificate from installation, or with a xref:learn:security/certificates.adoc[custom node certificate] if one has been specified.]
...
| _Client-to-node_
| *Unencrypted*: 8091-8096, 9140 footnote:eventing-debug-port[The Eventing Service Debugger port (`debugPort`) is an internal port and is not supported for external access outside of the cluster. You should only use this port in your development environments.], 11210, 11211

*Encrypted*: 11207, 18091-18096 footnote:analytics-encrypted-port[The Analytics Service encrypted port (18095) is not currently used, but is reserved for future use.]
...
| `cbas_http_port` / `cbas_ssl_port` footnote:analytics-encrypted-port[]
| 8095 / 18095
...
| `cbas_cluster_port` footnote:[In version 5.5 Beta, this port was named `cbas_hyracks_console_port`.]
| 9115
| Analytics service
| Yes
| No
...
| `debugPort` footnote:eventing-debug-port[]
| 9140
...
| `moxi_port_internal` footnote:fn-cannot-remap[] {fn-moxi-ref}
| 11213
| Moxi port (node-local only)
| No
| No
| No
