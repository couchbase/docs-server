...
The following services fail to start if they cannot listen on all required ports:
...
If a service can't bind to the required ports using the configured IP address family, that service fails to start.
