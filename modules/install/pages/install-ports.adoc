...
An _ephemeral_ port is one temporarily allocated by a server's operating system, to handle client-communications initiated on a _well-known_ port that is currently busy.
For Couchbase Server, an example of a well-known port would be _8091_.

Each operating system provides a default range of port numbers that can be assigned to ephemeral ports, when necessary.
Couchbase Server relies on the _full_ default range provided by each operating system: therefore, the default range should _not_ be reduced by the administrator; since reduction may cause Couchbase Server processes to fail.
