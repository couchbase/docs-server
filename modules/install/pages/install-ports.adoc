...
An _ephemeral_ port is one temporarily allocated by a server's operating system, as the source port for an outgoing communication.
Each operating system provides a default range of port numbers that can be assigned to ephemeral ports.
For Linux distributions, the typical range is `32768` to `61000`.

To ensure that no numbering-conflicts can occur with Couchbase Server's _well-known_ ports (as listed xref:install:install-ports.adoc#detailed-port-description[below]), it is strongly recommended that the range _not_ be extended below its default, lowest number (which is, typically, on Linux, the number `32768`).
...
*Encrypted*: 9999, 11207, 18091-18094, 19102, 19130, 21150
