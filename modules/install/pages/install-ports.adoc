...
To ensure that no numbering-conflicts can occur with Couchbase Server’s well-known ports (as listed xref:install:install-ports.adoc#detailed-port-description[below]), it is strongly recommended that the range not be extended below its default, lowest number (which is, typically, on Linux, the number 32768).
Note that if such conflicts were to occur, essential Couchbase-Server processes might either fail to start, or cease to function properly.
...
Refer to xref:learn:clusters-and-availability/xdcr-advanced-settings.adoc[XDCR Advanced Settings] for more information about XDCR versions.
...
*Encrypted*: 9999, 11206, 11207, 18091-18094 {fn-fts-encryption}, 19102
