...
* Your cluster is in mixed mode networking where some nodes use IPv4 addressing and others use IPv6. See xref:manage:manage-nodes/manage-address-families.adoc#changing-address-family-to-IPv6[Changing Address Family] for steps to determine if your cluster is running in this mode.
...
You cannot upgrade directly from a version of Couchbase Server earlier than 7.1 to version 7.6 or later. 
This limitation is caused by a compatibility issue with the Erlang/OPT libraries used in these earlier server versions. 
To upgrade from server versions 6.5, 6.6, or 7.0 to version 7.6 or later, first upgrade to version 7.1 or 7.2. 
