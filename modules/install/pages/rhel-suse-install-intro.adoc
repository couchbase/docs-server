...
The procedures for non-root installation, upgrade, and uninstallation &#8212; which are the same for all supported Linux distributions &#8212; are provided at xref:install:non-root.adoc[Non-Root Install and Upgrade].

[#start-and-stop]
=== Starting and Stopping a Non-Root-Installed Couchbase Server

To start, get status on, and stop Couchbase Server following non-root installation:
