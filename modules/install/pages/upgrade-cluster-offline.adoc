...
Disablement is performed by means of Couchbase-Server xref:manage:manage-settings/general-settings.adoc[General Settings]; and can be performed by means of the UI, the CLI, or the REST API.
Disablement nees to be performed only once, on one node.
+
...
By editing the file `/etc/yum/yum.conf`, specify that the package `couchbase-server` (or the package `couchbase-server-community`, if using Community Edition) is _excluded_.
The package-name must appear on a list of one or more package-names, following the `exclude=` statement.
For example:
