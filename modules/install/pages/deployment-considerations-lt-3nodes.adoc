...
+
When a deployment of Couchbase Server has less than three nodes, auto-failover is disabled.
This is because with less than three nodes in the deployment, it's not easy to determine which node is having an issue and thus avoid a split-brain configuration.
