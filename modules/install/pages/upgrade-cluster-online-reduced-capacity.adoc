...
. Manually remove the files in `/opt/couchbase`.
...
. Put automated Couchbase-Server updates on _hold_.
...
. _Add_ the upgraded swap node to the cluster, and _remove_ a node that is currently part of the cluster.
+
...
. Upgrade the newly removed node in the same way as described above.
Then, repeat the process of addition and removal; and upgrade the next removed node; and so continue.
