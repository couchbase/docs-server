...
[abstract]
Full Text Alias is the name that can be used in place of actual full text index name. You can perform the searches across multiple buckets, by means of index aliases.
...
Now, click *+ Add Alias* to add a new alias. The Add Alias page opens.

[#fts_add_alias_screen]
image::fts-add-alias-screen.png[,620,align=left]

In the Add Alias page, add the alian name in the *Index Name* field. After that select one or more indiexes from the *Target Indexes* list for which you want to add alias. The selected index is highlighted in a separate color.

Fianlly, click *Create Index Alias*. The new index alias is added to the list in the Full Text Aliases panel.
