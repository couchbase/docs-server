...
It is highly recommended to specify the `type` of the sort fields like `string` for text fields, `date` for datetime fields or `number` for numeric fields while mentioning the sort fields in the sort section of the search request. This would help in fetching more accurate sort results. 
