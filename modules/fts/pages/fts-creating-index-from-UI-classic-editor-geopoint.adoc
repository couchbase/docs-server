...
This section describes creating and Index on a bucket's collection across all field created on a default scope/collection.  In additoin a specific field is overridden as a geospatial type of geopoint.

This is equivelent to the xref:fts-creating-index-from-UI-classic-editor-legacy.adoc#main[Creating a Legacy Index] example with two exceptions:
 
 * The index is defined using the newer collections paridigm (instead of the "legacy" default mapping).
 
 * The field *geo* is overriden from the dynmic indexing to support geospatial queries as a type geopoint.
