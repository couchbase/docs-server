= Geospatial Queries
:description: Geospatial queries return documents that contain location-data.
...
Note that the `geo` field needs to contain the longitude-latitude information in the form of a string (comma separated numeric content or a hash), array or an object.
