...
* An array of longitude-latitude coordinate pairs.
...
The following query body uses an array, containing multiple longitude-latitude pairs, to specify each of the corners of a _polygonal_ bounding box.
Note that in the array, the `lon` value must precede the `lat`.

The final pair in the array is identical to the initial pair, thus explicitly closing the box.
...
Note that no _holes_ can be specified within the polygon: therefore, all matches within the perimeter of the polygon are returned.
However, a `must_not` clause can be used to exclude matches in certain inner areas.
