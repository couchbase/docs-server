...
*Q: Can the user store specific document fields within FTS and retrieve those as a part of the search results?*

Yes, It involves a 2 step procedure.

Indexing - the user needs to specify the desired fields of the matching documents to be retrieved as a part of the index definition. The user needs to enable the "store" option checkbox in the field mapping definition for the desired fields. The FTS index will store the original field contents intact (without applying any text analysis) as a part of it's internal storage.

For example, if the user wants to retrieve the field "description" in the document, then enable the "store" option like below.

[#fts-type-mappings-child-field]
image::fts-type-mappings-child-field-dialog-complete.png[,460,align=left]

Searching - the user needs to specify the fields to be retrieved as a part of the "Fields" option in the search request. This field can take an array of field names which must be loaded and returned with the search response. The field names are specified as string values. While there is no field name pattern matching available, you can use an asterisk "*" to specify that all the fields must be loaded and returned with the response.

For retrieving the contents of the aforementioned "description" field, the user may use the following search request.
