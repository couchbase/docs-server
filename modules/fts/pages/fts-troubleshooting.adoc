...
Alternatively, the users can also add more FTS nodes into the cluster so that there is enough nodes to support the number of replica count specified in the index definition.