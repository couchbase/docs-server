...
Search service allows you to index as many collections as you want in a scope by using Type Mappings. For further details to create an index on non-default collection(s), see xref::fts-type-mappings.adoc#Specifying-Type-Mapping-for-Collection[Specifying Type Mapping for Collection]
