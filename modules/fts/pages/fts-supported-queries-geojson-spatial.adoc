...
A dataset modification can be eaisly accomplished via either

* Adding a new GeoJSON object(s) to your documents.  

* Converting Geopoints to GeoJSON Point types in your documents.

To run the examples in this doucmentation the first update method *"Adding a new GeoJSON object(s) to your documents"* is needed.
...
The Sample Query Structures above just introduces some of the basic QueryShapes, the full list below coveres the nine (9) unique QueryShapes utilizes each of them to query 1) a set of GeoJSON points and 2) a set of GeoJSON area shapes in this case Circles (but the "area shapes" could be anything):
...
This example quickly creates the same index as xref:fts-creating-index-from-REST-geojson.adoc[Creating a GeoJSON Index via the REST API].  Note it hase the two (2) additional child field definitions to allow keyword sorting.
...
image::fts-geojson-mod-index-full.png[,,align=left]
