...
Multiple unit-types can be used to express the radius (or distance).
...
To be successful, a geospatial GeoJSON query must reference an index that applies the _geojson_ type mapping to the field containing any of the standard *Point, LineString, Polygon, MultiPoint, MultiLineString, MultiPolygon, and GeometryCollection* constructs or the extended  *Circle, and Envelope* construct.
