...
The *indexing progress* is determined as index_doc_count / source_doc_count. While index_doc_count is retrieved from the search endpoint, source_doc_count is retrieved from a KV endpoint. In the event where one or more of the nodes in the cluster running data service go down and/or are failed over, *indexing progress* may show a value > 100% as the source_doc_count for the bucket would be missing some active partitions.
...
Note, however, that the addition of credentials for authentication are required for the execution of the statement outside the context of the current session within Couchbase Web Console.
