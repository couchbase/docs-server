...
* Ensure that Couchbase Server has the _Search_ service enabled for at least one node in the cluster.
The service must be enabled for a node's configuration.
Refer to xref:manage:manage-nodes/create-cluster.adoc[Create a Cluster] for more information.
...
Their functionality and inner components are described in detail in xref:fts-index-analyzers.adoc[Understanding Analyzers].
