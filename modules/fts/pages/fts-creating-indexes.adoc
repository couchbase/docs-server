...
This section provides detailed explanations of how indexes can be created by means of the Couchbase Web Console. Additionally, it explains how index creation can be achieved with the Couchbase REST API: see xref:fts-creating-indexes.adoc#index-creation-with-the-rest-api[Index-Creation with the REST API], below.
...
In these use cases, users might have to create large number of indexes to enable the search on data across numerous collections.
...
To create Full Text Index for documents in buckets, specify the following:

- Index name
- Bucket
-	xref:fts-creating-indexes.adoc#specifying-type-identifiers[Type Identifier]
-	xref:fts-creating-indexes.adoc#specifying-type-mappings[Type Mappings]
-	xref:fts-creating-indexes.adoc#specifying-fields[Fields]
-	xref:fts-creating-indexes.adoc#creating-analyzers[Analyzers]
-	xref:fts-creating-indexes.adoc#adding-custom-filters[Custom Filters]
-	xref:fts-creating-indexes.adoc#date-time-parsers[Date/Time Parsers]
-	xref:fts-creating-indexes.adoc#specifying-advanced-settings[Advanced Parameters]

To create Full Text Index for documents in scope and collections, specify the following:

-	Index name
-	Bucket
-	xref:fts-creating-indexes.adoc#enabling-scope-collections[Enable Scope/Collection(s)]
-	xref:fts-creating-indexes.adoc#selecting-scope[Select Scope]
-	xref:fts-creating-indexes.adoc#specifying-type-identifiers[Type Identifier]
-	xref:fts-creating-indexes.adoc#specifying-type-mappings[Type Mappings]
-	xref:fts-creating-indexes.adoc#specifying-fields[Fields]
-	xref:fts-creating-indexes.adoc#creating-analyzers[Analyzers]
...
[#enabling-scope-collections]
== Enabling scope/collection(s)
The Full Text Search indexing is enhanced to include scopes and collections in the index definition.

Some of the important benefits that you can get in indexing with scopes and collections are:

* Increased indexing efficiency as the Data Service can provide documents from specific collections to the Full Text Search indexes.
*	Simplified query as you can easily specify a particular subset of documents in the query statements.
*	Easy migration from relational databases to Couchbase Server because you can design collections corresponding to the existing relational tables.
*	Secured isolation of different document types within a bucket, which allows authorizing the applications to use only their appropriate subsets of data.

If the user wants the index to stream data from a non-default scope and collection(s) on the source bucket, then they must select the *Use non-default scope/collection(s)* check box.
For more information, see xref:introduction:whats-new.adoc[Scopes and Collections].

[#fts_select_scope_collections]
image::fts-select-scope-collections.png[,460,align=left]

Selecting this check box lets users enter the source scope and collection details.

Note: Couchbase supports creating indexes separately for each scope or bucket. The single index cannot span across multiple scopes or buckets.

[#selecting-scope]
== Selecting a Scope
After enabling the non-default scope/collection(s), the Scope field becomes visible. Click the drop-down list and select the required scope for the index.

The *Scope* dropdown lists all the available scopes from the selected bucket. For example, travel-sample.

[#fts_select_scope_collections_2]
image::fts-select-scope-collections-2.png[,460,align=left]

For selecting multiple collections, see xref:fts-creating-indexes.adoc#type-mapping-multiple-collections[Type Mappings with Multiple Collections]
...
[#specifying-type-mapping-collection]
...
[#fts_type_mappings_select_collection]
image::fts-type-mappings-select-collection.png[,540,align=left]
...
Note: In Type Mappings, you can add multiple collections to the index. However, you can either select only one collection to create single collection index or select multiple collections to create index with multiple collections.
The Type Mappings panel appears as follows:

[#type-mapping-single-collection]
...
[#fts_type_mappings_single_collection_added]
image::fts-type-mappings-single-collection-added.png[,540,align=left]

[#type-mapping-multiple-collections]
...
[#fts_type_mappings_multiple_collections_added]
image::fts-type-mappings-multiple-collections-added.png[,540,align=left]

[#type-mapping-specific-document-type]
=== Type Mapping with Specific Document Type
With specific document type, you can search documents of specific type from single collection or multiple collections.
...
[#fts_type_mappings_specific_document_type1]
image::fts-type-mappings-specific-document-type1.png[,540,align=left]
...
[#fts_type_mappings_specific_document_type2]
image::fts-type-mappings-specific-document-type2.png[,540,align=left]

==== Document Type with multiple collection

[#fts_type_mappings_specific_document_type3]
image::fts-type-mappings-specific-document-type3.png[,540,align=left]

Now, when you search for the airport document type, the index will display all documents from a single collection or multiple collections where the *type* field is *airport*.

[#fts_type_mappings_specific_document_type4]
image::fts-type-mappings-specific-document-type4.png[,540,align=left]
...
Index Replicas support availability: if an Index Service-node is lost from the cluster, its indexes may exist as replicas on another cluster-node that runs the Full Text Search indexes.
