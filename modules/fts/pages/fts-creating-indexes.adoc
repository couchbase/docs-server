...
[#selecting-scope-collections]
== Selecting non-default scope/collection(s)
...
If the user is indexing the default scope and collection, then the index creation steps look exactly similar to that of the pre-collection days. If the user wants to index a non-default scope and collection(s), then they must select the *Use non-default scope/collection(s)* check box.
...
Selecting this check box lets users enter the source scope and collection details. The *Scope* dropdown lists all the available scopes from the selected bucket. For example, travel-sample.

[#fts_select_scope_collections_2]
image::fts-select-scope-collections-2.png[,460,align=left]

For selecting multiple collections, see xref:fts-creating-indexes.adoc#type-mappings-with-multiple-collections[Type mappings with multiple collections]
...
[#type-mappings-with-multiple-collections]
== Type mappings with multiple collections
When creating a new index, you can create type mappings for multiple collections. The *Collection* drop-down lists all available collections from the selected scope.
Select the required collection and click *ok* to add the collection to the index.

[#fts_select_multiple_collections]
image::fts-select-multiple-collections.png[,700,align=left]
