...
This optimises (but it not essential for) xref:fts:fts-response-object-schema.adoc[Handling Response Objects] and the use of xref:fts:fts-response-object-schema.adoc#search-facets[Search Facets].
When this checkbox is checked, the resulting index will be correspondingly larger.
