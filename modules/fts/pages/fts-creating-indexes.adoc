...
Full Text Indexes are accessed from the *Search* tab: click on this to display the *Full Text Search* panel, which contains a tabular presentation of currently existing indexes, with a row for each index.
(See xref:fts-searching-from-the-ui.adoc[Searching from the UI] for a full illustration.) To manage an index, click on its row.
...
Click on the btn:[+ Add Type Mapping] button.
...
Clicking on the btn:[edit] button displays the following interface:
...
Clicking on the btn:[+] button displays a pop-up that features two options:
...
Click on this, and a child-field, such as `content`, can be specified:
...
To create a new analyzer, click on the btn:[+ Add Analyzer] button.
...
+
Following addition of one character filter, to add another, click on the btn:[+ Add] button, to the right of the field.
+
...
+
Following addition of one token filter, to add another, click on the btn:[+ Add] button, to the right of the field.
+
...
When these fields have been appropriately completed, save; by clicking on the btn:[Save] button.
...
Clicking on the btn:[+ Add Character Filter] button displays the *Custom Character Filter* dialog:
+
...
Clicking on the btn:[+ Add Tokenizer] button displays the *Custom Tokenizer* dialog:
+
...
Clicking on the [.ui]*+ Add Token Filter* displays the *Custom Token Filter* dialog:
+
...
Clicking on the btn:[+ Add Word List] button displays the *Custom Word List* dialog:
+
...
After each word has been added, click on the [.ui]*+ Add* button, at the lower-right.
...
+
To remove a word, select the word within the *Words* panel, and click on the btn:[Remove] button.
To save, click on [.ui]*Save*.
...
When the *+ Add Date/Time Parser* button is clicked on, the *Customer Date/Time Parser* dialog appears:
...
Then, successively add the _layouts_ for the parser in the interactive field below the *Layouts* field, clicking on the *+ Add* button after each one: this adds the layout to a list of layouts displayed in the *Layouts* field.
To remove any of these, select its name in the *Layouts* field, and click on the *Remove* button.
When the list is complete, click on the *Save* button, to save.
...
To create an Index Alias, access the *Full Text Search* screen, by clicking on the [.ui]*Search* tab, in the navigation bar at the left of the console.
...
Clicking on the btn:[Add Alias] button displays the *Add Alias* screen:
...
To select indexes within this pane, click on each: when the index-name is highlighted, the index has been selected.

To create the alias, click on the btn:[Create Index Alias] button.
...
Note that when the *Add Alias* screen is again accessed, by clicking the btn:[Add Alias] button, the `travelSampleIndexesAlias` appears in the *Target Indexes* panel, along with the two existing indexes.
...
By clicking on the tab [.ui]*copy to clipboard*, the definition can be saved.
