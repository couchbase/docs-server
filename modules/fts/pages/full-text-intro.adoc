...
_Full Text Search_ provides Google like search capability on the JSON documents.  Couchbase's Global Secondary Indexes (GSI) power range scans and regular patten search, whereas FTS provides extensive capabilities for natural-language querying.  The query below looks for documents with all of the strings ("paris", "notre", "dame").
...
This query will return the following result (shown partially) from the FTS index scan on travel-sample sample bucket. The hits field show the document id of the matched documents, score, fields and the position of the matched string.
[source,json]
----
"hits": [
    {
