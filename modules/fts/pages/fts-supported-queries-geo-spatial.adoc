...
Once pasted hit the "Search" button

image::fts-geopoint-search_03.png[,620,align=left]
Then check "show advanced query settings" and check "show command-line query example"

Running the above cURL request will result in 847 total_hits but only return the first 10 hits.  A subset of formatted console output might appear as follows:
