...
You can use the UI to create a cURL command to do this first check "show advanced query settings" and then check "show command-line query example"

Running the above cURL command via the console request will report that there are 847 total_hits but only return the first 10 hits.  A subset of formatted console output might appear as follows:
