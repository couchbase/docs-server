...
You can do this using the xref:fts:fts-creating-index-using-the-index-definition-preview.adoc[Index Definition Preview] in the Query Workbench, or the xref:rest-api:rest-fts-indexing.adoc[Index Definition] endpoints provided by the Full Text Search REST API.
