...
** Change the pull-down *type* to *dynamic*.
+
By configuring the child field information form, specifically identify the object *geo* as type *dynamic* this will tell the Search indexer to recongnize top level sub-objects like:
+
[source, json]
----
  "geo": {
...
+ 
image::fts-index-menu2-dynamic-filled.png[,600,align=left]
...
* Click on the index you just created (named "test_dynamic") to expand the Index's controls.
