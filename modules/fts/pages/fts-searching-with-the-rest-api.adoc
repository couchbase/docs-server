...
[#searching-with-query-timeouts]
== Searching with timeouts

Once the the index becomes really large, it is possible that certain search requests may take
longer time to finish the execution.Such slow responses would affect the query latencies as well as the resource consumption on the server side.

For avoiding such unplesant situation, users can pass a search "timeout" value in the request.
By specificing timeout parameters in the seach request, users are bound to get a response within the stipulated time.
Here the timeout value is specified in unit of milliseconds.

If the search request times out before gathering all results, then the partial results will be
returned to the user with detailed partition level search result status.
In case of timeouts, the response's http status code would be 412.
