...
By specificing a timeout parameters in the seach request, users are bound to get a response within the stipulated time.
Here the timeout value is specified in milliseconds.
...
Please note that the default value of timeout is `10` seconds and users may override this
with suitable values for their use case. A value `0` for timeout indicates a search request 
with no timeout or expiry period.
