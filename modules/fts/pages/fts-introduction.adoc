...
Full Text Search includes pre-built _text analyzers_ including but not limited to the following languages: Arabic, CJK characters (Chinese, Japanese, and Korean), English, French, Hebrew, Hindi, Italian, Kurdish, Persian, and Portuguese.  For the current list of all supported languages in the Couchbase Server refer to: xref:fts-index-analyzers.adoc#Supported-Languages[Supported Languages]
