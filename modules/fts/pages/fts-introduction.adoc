...
* xref:fts-supported-queries-numeric-range.adoc[Numeric Range] and xref::fts-query-string-syntax-date-ranges.adoc[Date Range] 
* xref:fts-supported-queries-geo-spatial.adoc[Geospatial] queries
* xref:fts-supported-queries-query-string-query.adoc[Query String Query] which employ a special syntax to express the details of each query (see xref:fts-query-types.adoc#query-string-query-syntax[Query String Query] for more information)
