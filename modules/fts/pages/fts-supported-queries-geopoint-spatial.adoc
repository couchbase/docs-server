...
NOTE: When we sort on a string that uses the default analyzer that string is tokenized and you may get unexpected results as you are sorting on the tokenized field.  If you want to sort on the actual text in the field should use the *analyzer: "keyword"* and either check *[X] store* or check *[X] docvalues* to sort by the original text in the field.
...
* for type="airport"
+
image::fts-geopoint-update1.png[,620,align=left]

* for type="landmark"
+
image::fts-geopoint-update2.png[,620,align=left]
...
image::fts-geopoint-updated-index.png[,620,align=left]

Now repeating the above "Polygon-Based" Query we see that the data is sorted based on the orginal field names.
...
image::fts-geopoint-updated-index-seach-stored.png[,620,align=left]
