...
NOTE: When we sort on a string that uses the default analyzer that string is tokenized and you may get unexpected results as you are sorting on the tokenized field.  If you want to sort on the actual text in the field should use the *analyzer: "keyword"* to sort by the original text in the field.  In additon if you want to include the keyword in the index itself you will need to check *[X] store* or check *[X] docvalues*.
