...
If indexer RAM usage goes above 75% of the Index Service memory-quota, an xref:settings:configure-alerts.adoc[error-notification] is provided.
...
Other storage-settings are described in detail in xref:settings:configure-compact-settings.adoc[Configuring Auto-Compaction].
