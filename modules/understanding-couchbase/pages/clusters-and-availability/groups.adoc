...
For information on vBuckets, see xref:buckets-memory-and-storage/vbuckets.adoc[vBuckets].
...
As described in xref:understanding-couchbase:services-and-indexes/indexes/index-replication.adoc[Index Availability and Performance], the Index Service allows index replicas to be defined in either of two ways:
...
Examples of these different forms of replica-definition are provided in xref:understanding-couchbase:services-and-indexes/indexes/index-replication.adoc[Index Availability and Performance].
...
(Indexes and index replicas always exist each on their own Index Server node, with index-creation failing if there is an insufficiency of such nodes to accommodate the specified number of index replicas &#8212; see xref:understanding-couchbase:services-and-indexes/indexes/index-replication.adoc[Index Availability and Performance].)
...
* With Couchbase Web Console, see xref:clustersetup:manage-groups.adoc[Managing Server Groups].
