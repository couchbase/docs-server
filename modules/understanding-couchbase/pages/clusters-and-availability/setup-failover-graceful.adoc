...
[abstract]
Graceful failover is used to eject a node containing the Data service from the
cluster after all in-flight operations, and data synchronizations are complete.

== Graceful Failover Overview

Graceful failover is a method of safely ejecting a node containing the
Data Service from a Couchbase cluster: the Cluster Manager initiates data
synchronization between nodes of the cluster before removal.
This is the failover-type most commonly used for planned maintenance on the
cluster, to speed up the node removal.

Note that graceful failover only applies to the Data service.
...
== When to Use Hard Failover

Hard failover is the corrective action for an unplanned outage of a node that is unhealthy or unstable, whereas graceful failover is meant for planned maintenance.
