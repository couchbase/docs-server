...
batch_size;;
[Optional] An integer specifying the maximum number of index expressions in each batch.
Only applies when processing a large number of index expressions.
If omitted, the default value is `10`.
You can specify a different value based on the memory availability of the system.
Note that when index expressions are processed in batches, the update statistics timeout value (above) applies to each batch.
