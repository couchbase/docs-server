...
:window-specification: {window}#window-specification
...
In addition, aggregate functions can be used as {windowfun}[window functions] when they are used with a window specification, which is introduced by the `OVER` keyword.

This page describes the syntax of aggregate functions.
Refer to {window}[WINDOW Clause] for the syntax of the window definition.
...
[subs="normal"]
----
aggregate-function ::= aggregate-function-name '(' [ <<aggregate-quantifier>> ] {expression}[expr] ')'
----
...
[subs="normal"]
----
aggregate-function-window ::= aggregate-function-name '(' [ <<aggregate-quantifier>> ] {expression}[expr] ')'
                              <<over-clause>> )
----

image::n1ql-language-reference/aggregate-function-window.png["aggregate-function-name '(' aggregate-quantifier? expr ')' 'OVER' ( window-ref | '(' window-specification ')' )"]
