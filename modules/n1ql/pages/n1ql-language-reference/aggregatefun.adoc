...
+
For this example, set the query context to `travel-sample` and `inventory`.
...
=== Examples
====
Find the number of documents with an airline route stop in `travel-sample` regardless of its value:
...
====
Find the number of unique values of airline route stops in `travel-sample`:
...
====
Find the median of distinct altitudes of airports in the `travel-sample` keyspace:
