...
image::n1ql-language-reference/aggregate-function.png["function-name '(' aggregate-quantifier? expr ')'"]
...
[subs="normal"]
----
aggregate-function-window ::= function-name '(' [ <<aggregate-quantifier>> ] {expression}[expr] ')' <<over-clause>> )
----

image::n1ql-language-reference/aggregate-function-window.png["function-name '(' aggregate-quantifier? expr ')' over-clause"]
