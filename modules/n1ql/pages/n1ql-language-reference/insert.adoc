...
.Insert with SELECT
====
Query the `travel-sample` bucket for documents of `type` "airport" and `airportname` "Heathrow", and then insert the projection (2 documents) into the `travel-sample` bucket using unique keys generated using `UUID()`.
...
====
Query the `travel-sample` bucket for the field `_country` where the `airportname` is "Heathrow" and `type` is "airport".
