...
VALUES ("key1", { "type" : "hotel2", "name" : "new hotel scopes" });
----
...
* To execute the following statement, user must have the _Query Insert_ privilege on `pass:c[`travel-sample`.invetory.hotel]` and _Query Select_ privilege on `pass:c[`travel-sample`]`.
+
[source,n1ql]
----
INSERT INTO `travel-sample`.inventory.hotel (KEY foo, VALUE bar)
SELECT "foo" || meta().id, bar FROM `travel-sample`.inventory.hotel;
----
...
====
Insert a document into the `travel-sample` keyspace using an expiration of 5 days.
...
.Insert with SELECT
====
Query the `travel-sample` keyspace for documents of `type` "airport" and `airportname` "Heathrow", and then insert the projection (1 document) into the `travel-sample` keyspace using a unique key generated using `UUID()`.
...
====
Query the `travel-sample` keyspace for documents of `type` "airport" and `airportname` "Heathrow", and then insert the projection into the `travel-sample` keyspace using a unique key and an expiration of 2 hours.
...
.Overview
====
The following statement inserts a single JSON document into the `travel-sample` keyspace with key "k001".
...
====
Insert a new document with `key` "1025" and `type` "airline" into the `travel-sample` keyspace.
...
====
Insert two documents with `key` "airline_4444" and "airline_4445" of `type` "airline" into the `travel-sample` keyspace:
...
====
Query the `travel-sample` keyspace for documents where the `airportname` is "Heathrow" and `type` is "airport".
Then insert the result of the select statement (a well-formed JSON document) into the `travel-sample` keyspace with a key generated using the `UUID()` function.
...
EXPLAIN INSERT INTO `travel-sample`.inventory.airport (key UUID(), value airport)
    SELECT airport FROM `travel-sample`
            ...
            },
            {
              ...
              "~child": {
                ...
                "~children": [
                  {
                    "#operator": "Filter",
                    "condition": "(((`travel-sample`.`type`) = \"airport\") and ((`travel-sample`.`airportname`) = \"Heathrow\"))"
                  },
                  {
                    "#operator": "InitialProject",
                    "result_terms": [
                      {
                        "expr": "(`travel-sample`.`airport`)"
                      }
                    ]
                  }
                ]
...
The Query Engine first executes the `SELECT` statement and then uses the projection to insert into the `travel-sample` keyspace, performing the operations in the order listed:
...
<5> An `Insert` of the value along with the auto-generated key into the `travel-sample` keyspace.
====
