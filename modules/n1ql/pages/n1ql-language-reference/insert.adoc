...
TIP: Use the xref:n1ql-language-reference/upsert.adoc#topic_11_9[UPSERT] statement if you want to overwrite a document with the same key, in case it already exists.

*Example 1*: The following statement inserts a single JSON document into the `travel-sample` bucket with key "k001".
...
See xref:settings:install-sample-buckets.adoc#topic_jqr_1rn_vs[Install Sample Buckets] for information on how to install the sample buckets and xref:sdk:sample-application.adoc#story-h2-3[Sample Application] for details about the travel-sample data model.
...
See xref:security:concepts-rba.adoc#concept_ntl_jph_hr[RBAC for Administrators] for details about access privileges for various administrators.
...
To execute the following statement, user must have the _Query Insert_ privilege on ``travel-sample``.
...
To execute the following statement, user must have the _Query Insert_ privilege on ``travel-sample`` and the _Query Select_ privilege on ``travel-sample``.
...
To execute the following statement, user must have the _Query Insert_ privilege on ``travel-sample`` and _Query Select_ privilege on ``beer-sample``.
...
To execute the following statement, user must have the _Query Insert _privilege on ``travel-sample`` and _Query Select_ privilege on ``travel-sample``.
...
For more information on keyspaces, see xref:n1ql-intro/sysinfo.adoc#topic_2_4[Keyspaces].
...
*Example 3*: If you don’t require the document key to be in a specific format, you can use the function xref:n1ql-language-reference/metafun.adoc#topic_8_8[UUID()] to generate a unique key as shown here:
...
insert-select:: Use the projection of a SELECT statement which generates a well-formed JSON to insert.
...
See <<Example_15_copy_bucket,Example 15>> to use the INSERT statement to copy one bucket's data to another bucket.
select;;
SELECT statements let you retrieve data from specified keyspaces.
For details, see xref:n1ql-language-reference/select-syntax.adoc#topic_vr1_zzk_dx[SELECT Syntax].
...
result-expr;; Specifies an expression on the inserted documents, that will returned as output.
...
[#ul_wvk_sq5_zw]
* [.output]`requestID`: Request ID of the statement generated by the server.
* [.output]`signature`: Signature of the fields specified in the returning clause.
* [.output]`results`: If the query specified the returning clause, then results contains one or more fields as specified in the returning clause.
If not, returns an empty results array.
* [.output]`errors`: Returns the error codes and messages if the statement fails with errors.
Returned only when the statement fails with errors.
Errors can also include timeouts.
* [.output]`status`: Status of the statement - "[.output]`successful`" or "[.output]`errors`".
* [.output]`metrics`: Provides metrics for the statement such as [.output]`elapsedTime`, [.output]`executionTime`, [.output]`resultCount`, [.output]`resultSize`, and [.output]`mutationCount`.
...
For more information, see xref:n1ql-language-reference/identifiers.adoc#topic_6[Identifiers].

[#insert-examples]
== Examples

Inserting a Single Document:: The simplest use case of an INSERT statement is to insert a single document into the keyspace.
...
Performing Bulk Inserts:: Use the INSERT statement to batch insert multiple documents using the following syntax:
+
----
...
Inserting Values using SELECT:: Instead of providing actual values, you can specify the data to be inserted using the SELECT statement which selects the data from an existing bucket.
+
*Example 13*: Query the `travel-sample` bucket for the field `_country` where the `airportname` is "Heathrow" and `type` is "airport".
...
*Example 14*: Generate a document key as a combination of the projection and some function, such as `<countryname>::<system-clock>`.
...
[[Example_15_copy_bucket]]Using Insert to Copy Bucket Data to Another Bucket:: *Example 15*: Use the INSERT statement to create a copy of `bucket_1` under the new name `bucket_2`:
+
----
...
. Finally, the `INSERT` statement inserts the result of Step 2 with `UUID()` generated keys.
...
Inserting Values Using Functions:: *Example 17*: Use multiple functions during the INSERT:
[#ul_qrt_k25_zw]
...
Inserting Values Using Prepared Statements:: *Example 18*: Prepare an `INSERT` statement and show how to execute it by passing parameters with cbq and REST.
...
----
+
NOTE: The REST parameters should not have any spaces around ‘&’ when passing multiple parameters.
For example, the following REST API throws an error because of spaces before the `$faa_code` parameter.
...
----
+
See xref:n1ql-rest-api/extimeout.adoc#concept_qm1_2tx_nq[Request Timeout] for more information on timeouts.
...
. An `IntersectScan` to obtain the documents that satisfy both conditions of Step 1 and Step 2.
. A `Fetch` for the value on the field `_country`.
. An `Insert` of the value along with the auto-generated key into the `travel-sample` bucket.
...
* [.output]`elapsedTime`: Total elapsed time for the statement.
* [.output]`executionTime`: Time taken by Couchbase Server to execute the statement.
This value is independent of network latency, platform code execution time, and so on.
* [.output]`resultCount`: Total number of results returned by the statement.
In case of `INSERT` without a `RETURNING` clause, the value is `0`.
* [.output]`resultSize`: Total number of results that satisfy the query.
* [.output]`mutationCount`: Specifies the number of documents that were inserted by the `INSERT` statement.
...
The REST API for bucket statistics is available at .
CLI tools like `cbstats` provide detailed stats about the cluster.
The same information can also be obtained using cbq shell or REST API.
For more information, see xref:tools:query-monitoring.adoc#topic_yg4_5n1_q1b[Query Monitoring].
