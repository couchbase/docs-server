...
See xref:settings:install-sample-buckets.adoc#topic_jqr_1rn_vs[Sample Buckets] for information on how to install the sample buckets and xref:travel-app:travel-app-data-model.adoc#concept_hyy_k1z_pr[Travel App Data Model] for details about the travel-sample data model.
...
See xref:security:concepts-rba.adoc#concept_ntl_jph_hr[Role-Based Access Control (RBAC) for Administrators] for details about access privileges for various administrators.
...
If you don’t require the document key to be in a specific format, you can use the function xref:n1ql-language-reference/metafun.adoc#topic_8_8[UUID()] to generate a unique key as shown in the following example:
...
Using Insert to Copy Bucket Data to Another Bucket:: Use the INSERT statement to create a copy of bucket_1 under a new name bucket_2 using:
+
----
...
Inserting Values Using Prepared Statements:: The following example prepares an INSERT statement and shows how to execute it by passing parameters with cbq and REST.
...
For more information, see xref:tools:query-monitoring.adoc#topic_wk1_5v3_t5[Query Monitoring]
