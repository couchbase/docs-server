...
* To execute the following statement, you must have the _Query Insert_ privilege on `pass:c[`travel-sample`.inventory.hotel]` and _Query Select_ privilege on `pass:c[`travel-sample`]`.
+
[source,n1ql]
----
...
<2> A `Fetch` for the document in the `travel-sample`.inventory.airport keyspace.
<3> A `Filter` for documents with `airportname="Heathrow"`.
<4> An `Insert` of the value along with the auto-generated key into the `travel-sample`.inventory.airport keyspace.
====
