...
You can gather distribution statistics and create histograms for multiple index expressions at once.
If there is a large number of index expressions to process, the cost-based optimizer deals with them in batches.
The _batch size_ specifies the maximum number of index expressions in each batch.
