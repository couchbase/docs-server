...
The execution of a query involves {query-execution}[several possible operations]: scan, fetch, join, filter, and so on.
When the query processor is planning the query execution, there may be several possible choices for each operation: for example, there may be several different possible indexes, or a choice of join types.
...
The legacy {query-service-architecture}[rules-based optimizer] (RBO), as its name suggests, takes a rules-based approach when attempting to choose the most efficient option for each operation.
...
With the cost-based optimizer, both join methods are considered, and the optimzer chooses a join method based on cost information.
...
If you do not specify a sample size, or if the specified sample size is smaller than the default sample size, the default sample size is used instead.
