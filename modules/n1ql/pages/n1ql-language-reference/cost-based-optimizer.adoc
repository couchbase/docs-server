...
The _cost-based optimizer_ (CBO) is a built-in tool that enables the Query service to find the most efficient way of fetching data and performing a query.
...
Before you can use the cost-based optimizer with a query, you must first gather the statistics that it needs using the {updatestatistics}[UPDATE STATISTICS] statement.
