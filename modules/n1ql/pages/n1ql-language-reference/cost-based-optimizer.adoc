...
You may gather distribution statistics on several predicate expressions at once.
If there is a large number of predicate expressions to process, the cost-based optimizer constructs histograms in multiple passes.
The _batch size_ specifies the maximum number of predicate expressions to be processed in each pass.

The batch size can be specified when you use the {updatestatistics}[UPDATE STATISTICS] statement.
