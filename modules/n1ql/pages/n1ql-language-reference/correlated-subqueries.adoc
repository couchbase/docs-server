...
For example:
[#ul_oxc_jqx_mz]
* FROM clause of the subquery in xref:n1ql-language-reference/subqueries.adoc#Q7[Q7] is an independent keyspace identifier ``travel-sample`` and hence the correlation with parent query is established explicitly using the USE KEYS clause through the referential attribute `t1.airlineid`.
[#ul_pxc_jqx_mz]
* Similarly, the subquery in xref:n1ql-language-reference/subqueries.adoc#Q6A[Q6A] has an independent keyspace identifier ``travel-sample`` in FROM clause, but the correlation is self-referencing to the same document.
...
Note that, in ``travel-sample`` keyspace data model, the `"route"` documents refer the `"airline"` documents using the attribute `airlineid`.
