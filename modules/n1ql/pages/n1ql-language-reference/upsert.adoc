...
To execute the following statement, user must have the _Query Update_ and _Query Insert_ privileges on ``travel-sample``.
...
To execute the following statement, user must have the _Query Update_ and _Query Insert_ privileges on the ``travel-sample`` bucket and _Query Select_ privilege on ``travel-sample`` (for RETURNING clause).
...
To execute the following statement, user must have the _Query Update_ and _Query Insert_ privileges on the ``travel-sample`` bucket and _Query Select_ privilege on ``beer-sample``.
