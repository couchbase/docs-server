...
* To execute the following statement, you must have the _Query Update_ and _Query Insert_ privileges on the `pass:c[`travel-sample`.inventory.landmark]` bucket and _Query Select_ privilege on `pass:c[`beer-sample`]`.
+
[source,n1ql]
----
