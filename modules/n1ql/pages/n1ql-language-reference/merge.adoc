...
[subs="normal"]
----
merge-source-expr ::= <<ansi-generic-args,expr>> [ [ AS ] <<ansi-generic-alias,alias>> ]
----

image::n1ql-language-reference/merge-source-expr.png["expr ( 'AS'? alias )?"]

[id="ansi-generic-args"]
...
[subs="normal"]
----
set-clause ::= SET <<ansi-path,path>> '=' expr [ <<ansi-update-for,update-for>> ] [ ',' <<ansi-path,path>> '=' expr [ <<ansi-update-for,update-for>> ] ]*
----
...
[subs="normal"]
----
update-for ::= ( FOR [ name-var ':' ] var ( IN | WITHIN ) <<ansi-path,path>> [ ',' [ name-var ':' ] var ( IN | WITHIN ) <<ansi-path,path>> ]* )+ [ WHEN cond ] END
----
...
[subs="normal"]
----
path::= identifier [ '[' expr ']' ]* [ '.' <<ansi-path,path>> ]
----
