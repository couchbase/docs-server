...
:description: {sqlpp} Pushdowns optimize the performance of {sqlpp} queries by supporting grouping and aggregate expressions.
...
Grouping and aggregate pushdown improves the performance of {sqlpp} queries with groupings and aggregations.
...
[plantuml,GBAP_pre55execution,svg]
....
@startuml
...
The {sqlpp} indexer will do 2nd level of aggregation and compute the final result.
* The {sqlpp} indexer can pushdown only if the leading key has a predicate.
