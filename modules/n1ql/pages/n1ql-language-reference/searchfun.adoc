...
For more information, refer to xref:fts:fts-query-types.adoc#preparing-for-full-text-searches[Preparing for Full Text Searches].
...
The search functions are useful when you need to combine the a full text search with the power of a N1QL query; for example, combining joins and natural-language search in the same query.
