...
For more details, refer to xref:fts-supported-queries.adoc[Supported Queries].
...
[NOTE]
--
If an "index" option within the options argument of the _SEARCH_ function isn't explicitly set, N1QL can pick any FTS index available in the system that will qualify for the query. If a number of FTS indexes qualify, then one that is defined most precise will be chosen.
--
