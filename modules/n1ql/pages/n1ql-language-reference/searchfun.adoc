...
You can use the `indexUUID` field as an alternative to the `index` field.
You can also specify both the `indexUUID` field and the `index` field together.
If you do this, the `indexUUID` field and the `index` field must both identify the same full text index.
