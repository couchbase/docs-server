...
For information on how to use prepared statements with various SDKs, refer to xref:sdk:n1ql-query.adoc#prepare-stmts[Querying with N1QL] and xref:sdk:nodejs/n1ql-queries-with-sdk.adoc#topic_esq_3td_5v[N1QL from the SDK].
...
To execute the following statement, user must have the _Query Select_ privilege on both keyspaces ``travel-sample`` and ``beer-sample``.
...
To execute the following statement, user must have the _Query Update_ and _Query Select_ privileges on ``travel-sample``.
