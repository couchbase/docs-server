...
For information on how to use prepared statements with various SDKs, refer to xref:java-sdk:concept-docs:n1ql-query.adoc#prepared-statements-for-query-optimization[Querying with {sqlpp}] and xref:java-sdk:howtos:n1ql-queries-with-sdk.adoc#parameterized-queries[{sqlpp} from the SDK].
