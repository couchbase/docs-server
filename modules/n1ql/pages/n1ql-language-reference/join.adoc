...
In Couchbase Server 7.1 and later, you may also use comma-separated Cartesian joins.
...
[subs="normal"]
----
join-clause ::= <<section_ek1_jnx_1db,ansi-join-clause>> | <<lookup-join-clause,lookup-join-clause>> | <<index-join-clause,index-join-clause>>
----
...
* You may chain ANSI joins with comma-separated Cartesian joins; however, the comma-separated joins must come after any JOIN, NEST, or UNNEST clauses.
...
[subs="normal"]
----
rhs-keyspace ::= <<ansi-keyspace-ref,keyspace-ref>> [ [ AS ] <<ansi-keyspace-alias,alias>> ] [ <<ansi-join-hints,ansi-join-hints>> ] 
----
...
Lookup joins can be chained with other lookup joins or nests and index joins or nests, but they cannot be mixed with ANSI joins, ANSI nests, or comma-separated Cartesian joins.
...
Index joins can be chained with other index joins or nests and lookup joins or nests, but they cannot be mixed with ANSI joins, ANSI nests, or comma-separated Cartesian joins.
