...
[subs="normal"]
----
ansi-join-clause ::= [ <<ansi-join-type,ansi-join-type>> ] JOIN <<ansi-keyspace-ref,keyspace-ref>> [ [ AS ] <<ansi-as-alias,alias>> ] [ <<ansi-join-hints,ansi-join-hints>> ] <<ansi-join-predicate,ansi-join-predicate>>
----
...
`expr`:: Boolean expression representing the join condition between the left-hand side <<section_nkd_3nx_1db>> and the right-hand side <<ansi-keyspace-ref>>.
