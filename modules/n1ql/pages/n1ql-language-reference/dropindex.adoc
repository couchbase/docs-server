...
The DROP INDEX statement provides two possible syntaxes for specifying the index and the keyspace on which the index is built.
...
Refer to <<keyspace-ref>>.

index-using:: (Optional) <<index-using>>.

=== Checking for an Existing Index

The optional `IF EXISTS` modifier enables you to check whether the specified index exists before dropping it.
...
[[keyspace-full-index,full keyspace]]
