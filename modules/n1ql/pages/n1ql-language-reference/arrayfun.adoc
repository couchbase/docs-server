...
This function returns a new array of numbers, from [.in]`start` until the largest number less than [.in]`end`.
Successive numbers are incremented by [.in]`step`.
If [.in]`step` is not specified, the default value of 1 is used.
If [.in]`step` is negative, the function decrements until the smallest number greater than [.in]`end`.
...
This function returns a new array with the specified value repeated [.in]`n` times.
...
If n is specified, at most [.in]`n` replacements are performed.
