...
image::learn:services-and-indexes/indexes/query_execution.png["1. Client submits the query over the REST API; 2. Query Service parses, analyzes, and creates plan; 3. Query Service sends request and index filters to Index Service; 4. Index Service returns qualified doc keys to Query Service; 5. Query Service sends fetch request with doc keys to Data Service; 6. Data Service returns documents to the Query Service; 7. Query Service evaluates the documents; 8. Query Service returns result to client",670]
...
image::learn:services-and-indexes/indexes/query_service.svg["Parse, Plan, Scan, Fetch, Join, Filter, Pre-Aggregate, Aggregate, Sort, Offset, Limit, Project",670]
...
together {
  package "Route documents" as route {
...
together {
  package "Airline documents" as airline {
  ...
  package "Airport documents" as airport {
  ...
  package "Landmark documents" as landmark {
...
<.> The query selects the `image_direct_url` field, which is MISSING for many documents in the `landmark` keyspace.
...
<.> Therefore the optimizer falls back on the `def_inventory_landmark_primary` index.
====
...
<.> The WHERE clause filters out documents where `image_direct_url` is not MISSING.
...
<.> The optimizer correctly chooses the `idx_image_direct_url` index.
====
