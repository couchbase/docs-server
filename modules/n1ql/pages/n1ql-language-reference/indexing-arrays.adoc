...
The array expression can be either a <<full-array-expr,full array expression>>, which includes an `ARRAY` operator, or a <<simple-array-expr,simple array expression>>, which does not.
...
This query performs a covering UNNEST IndexScan, by applying all the predicates, using `ixf_sched`.
...
This query performs a covering UNNEST IndexScan, by applying the predicates on both levels of the ARRAY, using `ixf_sched_nested`.
...
This query performs a covering UNNEST IndexScan, by applying the predicates on both levels of the ARRAY, using `ixf_sched_nested`; and uses index aggregation.
...
This query performs a covering IndexScan, by applying all the predicates, using `ixf_sched`.
