...
.Q2: Find the list of scheduled 'UA' flights on day 1
[[Q2,Q2]]
[source,N1QL]
----
SELECT * FROM `travel-sample`
...
.Q3: Find the list of scheduled 'FL' flights that have one or more stops
[[Q3]]
[source,N1QL]
----
SELECT * FROM `travel-sample`
...
.C4: Create a partial index on a nested array [.var]`special_flights`
[[C4,C4]]
[source,N1QL]
----
...
.Q4: Use nested ANY operator to use the index
[[Q4,Q4]]
[source,N1QL]
----
...
.Q4A: Use UNNEST operators to use the index
[[Q4A,Q4A]]
[source,N1QL]
----
SELECT count(*) FROM `travel-sample`
...
.Q5: Find the list of scheduled 'US681' flights on day 2
[[Q5,Q5]]
[source,N1QL]
----
...
.Q6: Find details of all route documents matching a specific schedule
[[Q6,Q6]]
[source,N1QL]
----
SELECT * FROM `travel-sample`
...
.Q6A: Find details of all route documents matching a specific schedule
[[Q6A,Q6A]]
[source,N1QL]
----
...
For example, the following query <<Q9A>> can be covered with the ALL index [.var]`idx_sched_covered_simple_all` in <<C9>>, but <<Q9B>> is not covered when using the DISTINCT [.var]`index idx_sched_covered_simple` defined in <<C8>>.

.C9: UNNEST covered with the ALL index
[[C9,C9]]
[source,N1QL]
----
...
.Q9A: UNNEST covered with the ALL index
[[Q9A,Q9A]]
[source,N1QL]
----
EXPLAIN SELECT meta(t).id FROM `travel-sample` t
...
.Q9B: UNNEST not covered when using the DISTINCT index
[[Q9B,Q9B]]
[source,N1QL]
----
EXPLAIN SELECT meta(t).id FROM `travel-sample` t
