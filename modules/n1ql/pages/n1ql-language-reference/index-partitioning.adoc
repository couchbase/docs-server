...
[subs="normal"]
----
create-index ::= CREATE INDEX __index-name__ ON <<named-keyspace-ref>> '(' <<index-key>> [ ',' <<index-key>> ] * ')' [ <<index-partition>> ] [ <<index-with>> ]
----

image::n1ql-language-reference/create-partitioned-index-syntax.png["'CREATE' 'INDEX' index-name 'ON' named-keyspace-ref '(' index-key ( ',' index-key )* ')' index-partition ? index-with?"]
...
The name of the keyspace or bucket to create an index on.
