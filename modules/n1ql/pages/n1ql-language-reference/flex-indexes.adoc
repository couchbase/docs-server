...
[abstract]
You can use a full-text index when running a N1QL query.
For a N1QL query to be able to use a full-text index, the full-text index and the N1QL query must both meet certain requirements.

In prior versions of Couchbase Query it was possible to perform an FTS search within a N1QL query using {searchfun}[search functions].
Starting with Couchbase Server 6.6, it is possible to use a full-text index when running a N1QL query, even if the query does not not contain a search function.
A full-text index may offer advantages over a GSI index when performing text-based or language-aware queries.
...
Because of the semantic differences described above, there are several restrictions on the way the query engine can make use of full-text indexes.

* A full-text index cannot be used as a covering index.
* LIMIT and OFFSET clauses cannot be pushed down to a full-text index.
* Aggregate functions and Window functions cannot be pushed down to a full-text index.
...
* Use the {index-type}[USE FTS] hint in the N1QL query
* Set the `use_fts` {query-settings}[request-level parameter] to `true`

When using an index hint, it is possible to specify mixed index types for a query -- `FTS` and `GSI`.

[[ex-usage-1]]
.Specifying any FTS index
====
[source,n1ql]
----
SELECT META(d).id
...
[[ex-usage-2]]
.Specifying any FTS or GSI index
====
[source,n1ql]
----
SELECT META(d).id
...
[[ex-usage-3]]
.Specifying named FTS and GSI indexes
====
[source,n1ql]
----
SELECT META(d).id
...
[NOTE]
However, if the query predicate contains a search function, none of this applies -- instead, an index is selected for the query according to the method described on the {searchfun}[Search Functions] page.
...
The full-text index _must_ use the keyword analyzer.
...
====
In the following index definition, the `id` and `reviews.review.author` fields may be searched within a query.
...
NOTE: The "type" field in a custom type mapped index is not searchable.

[[query-requirements]]
== Query Requirements

In order to use a full-text index with a N1QL query, the query must meet certain requirements.
...
The following query can be used with the full-text index:
...
The following queries are unambiguous and can be used with the full-text index defined in <<ex-conditional-1>>:
...
The following expressions can be used with the full-text index:
...
The following expressions cannot be used with the full-text index:
...
[source,n1ql]
----
type = "X" AND name = "abc" AND age = 10 -- <.>
----
...
[source,n1ql]
----
type = "X" OR name = "abc" AND age = 10 -- <.>
type = "X" AND name = "abc" OR age = 10 -- <.>
----
...
a >= 10 AND a <= 20
b >= "hot" AND b <= "hotel"
c >= "2020-03-01" AND c <= "2020-04-01" -- <.>

type = "xyz" AND a >= 10 AND a <= 20
a >= 10 AND a <= 20 AND type = "xyz"
type = "xyz" AND (a >= 10 AND a <= 20) -- <.>
----
...
[source,n1ql]
----
ISSTRING(name) AND name >= "abhi" -- <.>
ISNUMBER(age) AND age > 30 -- <.>
----
...
====
Assume that a full-text index definition contains the following type mapping over documents of type `"hotel"`.
...
The following predicates may be used with the above full-text index:

[source,n1ql]
----
type = "hotel" AND ANY r in reviews SATISFIES r.author = "xyz" END

type = "hotel" AND ANY r in reviews SATISFIES r.ratings.Cleanliness = 5 OR r.ratings.Overall = 4 END

type = "hotel" AND ANY r in reviews SATISFIES r.ratings.Cleanliness = 5 OR r.ratings.Overall = 4 END AND ANY p in public_likes SATISFIES p LIKE "xyz" END
----
====
...
====
The following predicate may be used with the full-text index whose definition is given under <<any-in-satisfies>>:

[source,n1ql]
----
EVERY r IN reviews SATISFIES r.ratings.Cleanliness = 5 END
----
====
...
[source,n1ql]
----
ANY AND EVERY r IN reviews SATISFIES r.ratings.Cleanliness = 5 END
----
====
