...
include::include::ROOT:partial$query-context.adoc[tag=statement]

Consider the following indexes, which are included with the `travel-sample` data that is shipped with the product.
To install sample data, see {install-sample-buckets}[].
...
include::include::ROOT:partial$query-context.adoc[tag=section]
