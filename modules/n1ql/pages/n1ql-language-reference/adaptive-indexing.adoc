...
In this case, you must ensure that any fields filtered by a WHERE clause in the index definition are also referenced by the {pairs}[PAIRS()] function.
For example, the following query <<Q9>> cannot select the adaptive index defined in <<C9A>>.
...
However, the same query <<Q9>> can select the index defined in <<C9B>>.
...
Alternatively, you could use the `SELF` keyword to ensure that the fields used in the WHERE clause are included in the {pairs}[PAIRS()] function.
Refer to <<C5>> for an example.

An IntersectScan does not eliminate redundant queries, and may impact performance.
