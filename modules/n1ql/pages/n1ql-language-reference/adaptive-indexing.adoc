...
[subs="normal"]
----
create-index ::= CREATE INDEX _index-name_ ON <<named-keyspace-ref>> '(' <<index-key>> [ ',' <<index-key>> ]* ')' [ <<where-clause>> ] [ <<index-using>> ] [ <<index-with>> ]
----
...
To create an adaptive index, the index key must be a simple array expression containing a xref:n1ql-language-reference/metafun.adoc#pairs[PAIRS()] function.
...
_expr_::
A N1QL expression that is allowed in xref:n1ql-language-reference/createindex.adoc[CREATE INDEX].
...
EXPLAIN SELECT * FROM `travel-sample` WHERE TYPE="hotel"; <3>
----
...
[source,json]
----
include::example$n1ql-language-reference/adaptive-idx-7-explain.jsonc[tag=excerpt]
...
----
====
+
...
[source,json]
----
include::example$n1ql-language-reference/adaptive-idx-8-explain.jsonc[tag=excerpt]
...
----
====
...
For example, the following query uses the index [.var]`def_city`, instead of the specified adaptive index [.var]`ai_city1`:
+
====
[source,n1ql]
----
CREATE INDEX `ai_city1` ON `travel-sample`(pairs({"city" : lower(city)}));
----
...
====
+
<1> This query uses the [.var]`def_city` index instead of our specified [.var]`ai_city1` index because it’s a functional index expression on the field city.
...
For example, the following query uses a default index, such as [.var]`def_city`, instead of the specified adaptive index [.var]`ai_city`.
