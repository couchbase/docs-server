...
For details, see xref:n1ql-language-reference/selectclause.adoc[SELECT Clause].

expr;;
[Optional] A string expression of input objects.
For details, see xref:n1ql-language-reference/index.adoc[N1QL Expressions].

from-term;;
[Optional] The [.var]`from-term` defines the input objects for the query and it can be either a keyspace identifier or an expression.
+
...
Couchbase Server version 4.6.2 adds support for generic xref:n1ql-language-reference/index.adoc[expressions] in the `from-term`.
...
----
+
See xref:n1ql-language-reference/subqueries.adoc[Subqueries] for details and examples.
