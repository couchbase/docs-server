...
Assuming the FTS index `fts_travel` is created on the keyspace `pass:c[`travel-sample`]`, running the following {sqlpp} query finds all documents that have "sanfrancisco" anywhere in the document.
