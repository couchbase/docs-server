...
* The {durability_level}[durability_level] request-level specifies the transactional durability level.
...
In order to meet transaction durability requirements, the bucket must be replicated to at least one other Data node.
If the bucket is not replicated to any other data nodes, you must turn off replication for this bucket to be able to commit the transaction.
...
+
--
. In the Couchbase Web Console, click the *Buckets* menu title.

. Click *travel-sample* to expand the options for the `travel-sample` bucket.

. In the *travel-sample* bucket options, click btn:[Edit].

. When the Edit Bucket Settings window appears, click *Advanced bucket settings* to expand the options.

. Under *Replicas*, make sure the *Enable* check box is empty.
  If it isn't, click the check box to clear it.

. Choose btn:[Save Changes] to save the changes.

. You now need to rebalance the servers.
  Click the *Servers* menu title.

. Click btn:[Rebalance] and wait for the rebalance to finish.
--
====
...
To specify the durability level, you could enter it as a named parameter.
For this worked example, make sure there is no named parameter called `durability_level`, so that the durability level defaults to `"majority"`.
--
