...
:atrcollection: xref:settings:query-settings.adoc#atrcollection
:numatrs: xref:settings:query-settings.adoc#numatrs
...
* The {atrcollection}[atrcollection] parameter specifies where the active transaction record is stored.

* The {numatrs}[numatrs] parameter specifies the total number of active transaction records.
...
* Alternatively, you can use the btn:[Execute as Transaction] button to run a single statement as a transaction, or a complete set of statements as a transaction all at once.
