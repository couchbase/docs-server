...
| {kvtimeout}[kvtimeout] request-level parametereditor
...
| Sets where to store the active transaction record.
...
For more information about how to create a transaction with each tool, see the following sections.
...
NOTE: You must have cbq shell version 2.0 or later to use the automatic transaction ID feature.
