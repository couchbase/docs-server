...
:description: The WINDOW clause defines named windows for window functions.
...
The *window reference* enables you to reuse the specification of an existing named window, and extend that specification if necessary.
...
The existing named window must be within the scope of the current window definition.
Furthermore, when one named window refers to another existing named window, the other existing named window must be declared earlier in the current query block.
...
* The current window definition may only include a <<window-order-clause,window order clause>> if the existing named window does not specify a window order clause.

* The existing named window may not specify a <<window-frame-clause,window frame clause>>.
