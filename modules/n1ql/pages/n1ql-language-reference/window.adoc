...
This page gives the generic syntax of window functions, window options, and the window clause.
See the xref:n1ql-language-reference/aggregatefun.adoc[Aggregate Functions] page or xref:n1ql-language-reference/windowfun.adoc[Window Functions] page for details of the syntax of individual functions.
...
image::n1ql-language-reference/window-function.png[]
...
See the xref:n1ql-language-reference/aggregatefun.adoc[Aggregate Functions] page for a list of aggregate functions.

[[window-functions,window-functions]]
==== Window Functions

See the xref:n1ql-language-reference/windowfun.adoc[Window Functions] page for a list of window functions.
...
[subs="normal"]
----
window-function-arguments ::= ( xref:n1ql-language-reference/aggregatefun.adoc#aggregate-quantifier[aggregate-quantifier]? xref:n1ql-language-reference/index.adoc[expr] ( ',' xref:n1ql-language-reference/index.adoc[expr] ( ',' xref:n1ql-language-reference/index.adoc[expr] )? )? )?
----

image::n1ql-language-reference/window-function-arguments.png[]

See the xref:n1ql-language-reference/aggregatefun.adoc[Aggregate Functions] page or xref:n1ql-language-reference/windowfun.adoc[Window Functions] page for details of the arguments for individual functions.
...
This clause can only be used with the xref:n1ql-language-reference/windowfun.adoc#fn-window-nth-value[NTH_VALUE()] function.
...
This clause can only be used with the xref:n1ql-language-reference/windowfun.adoc#fn-window-first-value[FIRST_VALUE()], xref:n1ql-language-reference/windowfun.adoc#fn-window-last-value[LAST_VALUE()], xref:n1ql-language-reference/windowfun.adoc#fn-window-nth-value[NTH_VALUE()], xref:n1ql-language-reference/windowfun.adoc#fn-window-lag[LAG()], and xref:n1ql-language-reference/windowfun.adoc#fn-window-lead[LEAD()] functions.
...
The *window clause* specifies the partitioning, ordering, and framing for window functions.
...
[subs="normal"]
----
window-partition-clause ::= 'PARTITION' 'BY' xref:n1ql-language-reference/index.adoc[expr] ( ',' xref:n1ql-language-reference/index.adoc[expr] )*
----
...
* The xref:n1ql-language-reference/windowfun.adoc#fn-window-row-number[ROW_NUMBER()] function returns a distinct number for each object.
If objects are tied, the results may be unpredictable.

* The xref:n1ql-language-reference/windowfun.adoc#fn-window-rank[RANK()], xref:n1ql-language-reference/windowfun.adoc#fn-window-dense-rank[DENSE_RANK()], xref:n1ql-language-reference/windowfun.adoc#fn-window-percent-rank[PERCENT_RANK()], and xref:n1ql-language-reference/windowfun.adoc#fn-window-cume-dist[CUME_DIST()] functions return the same result for each object.
...
To guarantee the order of the final results, use the query xref:n1ql-language-reference/orderby.adoc[ORDER BY clause].
====
...
See the xref:n1ql-language-reference/orderby.adoc[ORDER BY clause] page for details.
...
[subs="normal"]
----
window-frame-clause ::= ( 'ROWS' | 'RANGE' | 'GROUPS' ) ( ('UNBOUNDED' 'PRECEDING' | 'CURRENT' 'ROW' | xref:n1ql-language-reference/index.adoc[valexpr] 'FOLLOWING') | 'BETWEEN' ('UNBOUNDED' 'PRECEDING' | 'CURRENT' 'ROW' | xref:n1ql-language-reference/index.adoc[valexpr] ('PRECEDING' | 'FOLLOWING')) 'AND' ('UNBOUNDED' 'FOLLOWING' | 'CURRENT' 'ROW' | xref:n1ql-language-reference/index.adoc[valexpr] ('PRECEDING' | 'FOLLOWING')) )
----
...
If the ordering term expression does not evaluate to a number, the window frame will be empty, which means the window function will return its default value: in most cases this is NULL, except for xref:n1ql-language-reference/aggregatefun.adoc#count[COUNT()] or xref:n1ql-language-reference/aggregatefun.adoc#countn[COUNTN()], whose default value is 0.
...
If you want to use `RANGE` with either `_valexpr_ PRECEDING` or `_valexpr_ FOLLOWING`, and you want to use an ordering expression based on date or time, use the appropriate xref:n1ql-language-reference/datefun.adoc[date or time function] to convert the date or time into milliseconds, then use the resulting number in the ordering expression.
====

The `BETWEEN ... AND ...` clause specifies the start point and end point of the window frame.
...
If it violates this restriction implicitly, the window frame will be empty, which means the window function will return its default value: in most cases this is NULL, except for xref:n1ql-language-reference/aggregatefun.adoc#count[COUNT()] or xref:n1ql-language-reference/aggregatefun.adoc#countn[COUNTN()], whose default value is 0.

Window frames that result in an explicit violation are:
...
Window frames that result in an implicit violation are:
...
See the xref:n1ql-language-reference/windowfun.adoc[Window Functions] page for examples.
