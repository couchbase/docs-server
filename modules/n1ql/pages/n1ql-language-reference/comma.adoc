...
image::n1ql-language-reference/comma-separated-join.png["',' ( rhs-keyspace | rhs-subquery | rhs-generic )"]
...
[subs="normal"]
----
rhs-keyspace ::= <<rhs-keyspace-ref,keyspace-ref>> [ [ AS ] <<rhs-keyspace-alias,alias>> ] [ <<rhs-keyspace-hints,ansi-join-hints>> ]
----
