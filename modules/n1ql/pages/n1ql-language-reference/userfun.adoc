...
The following query uses the `geohash` and `adjacent` functions to find the 6-figure geohash of the selected hotel, and the geohashes immediately to the north, south, west, and east.
...
LET area = SUBSTR(geohash(geo.lat, geo.lon), 0, 6)
...
(At the latitude of the selected hotel, each geohash represents an area of approximately 700 𐄂 600 meters.)
====
