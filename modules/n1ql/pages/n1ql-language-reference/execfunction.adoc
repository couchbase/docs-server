...
Similarly, to execute a scope function in a particular scope, the function name must be a qualified identifier with the full path to a scope, such as `default:{backtick}travel-sample{backtick}.modes.func1`.
Refer to xref:n1ql-language-reference/createfunction.adoc#context[Global and Scope Functions] for more information.
