...
[subs="normal"]
----
execute-function ::= EXECUTE FUNCTION <<name,function>> '(' [ <<expression,expression>> ( ',' <<expression,expression>> )* ] ')'
----

image::n1ql-language-reference/execute-function.png["'EXECUTE' 'FUNCTION' function '(' (expression (',' expression)* )? ')'"]
