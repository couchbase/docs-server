...
[source,sqlpp]
----
SELECT URLENCODE("SELECT name FROM hotel LIMIT 1;") AS encoded;
----
...
[
  {
    "encoded": "SELECT%20name%20FROM%20hotel%20LIMIT%201%3B"
  }
]
----
====
...
=== Arguments
encoded_string:: A string, or any valid xref:n1ql-language-reference/index.adoc[expression] which evaluates to a string, that is the string to URL-decode.
...
[source,sqlpp]
----
SELECT URLDECODE("SELECT%20name%20FROM%20hotel%20LIMIT%201%3B") AS decoded;
----
...
[
  {
    "decoded": "SELECT name FROM hotel LIMIT 1;"
  }
]
----
====
