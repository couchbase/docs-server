...
These statistics are used by the cost-based optimizer when choosing the optimal index to execute a query.
...
[subs="normal"]
----
update-statistics ::= UPDATE STATISTICS [ FOR ] <<named-keyspace-ref>> '(' <<index-key>> [ ',' <<index-key>> ]* ')' [ WITH <<options>> ]
----

image::n1ql-language-reference/update-statistics.png["'UPDATE' 'STATISTICS' 'FOR'? named-keyspace-ref '(' index-key ( ',' index-key )* ')' 'WITH' options"]
...
Specifies the name of the keyspace for which you want to update statistics.
--

[[index-key,index-key]]
=== Index Key

This may be any of the expressions used to create an index, including, but not limited to:
...
* An {adaptive-expr}[expression with the PAIRS() function], as used in an adaptive index.
...
The minimum resolution is `0.02` (5000 distribution bins) and the maximum is `5.0` (20 ditribution bins).
...
Cardinality is a measure of the uniqueness of the data values.
...
.Multiple predicates
====
This example uses the same indexes and statistics as <<ex1>>.

[source,N1QL]
----
EXPLAIN SELECT COUNT(*) FROM `travel-sample` WHERE country = 'United States' AND type = 'hotel';
----
...
[
  {
    "plan": {
      ...
      "~children": [
        {
          "#operator": "IndexScan3",
          "cardinality": 740.8095238095237, // <1>
          "cost": 417.07576190476186,
          "index": "idx_country_city",
          "index_id": "ebcdefa561f1e3f6",
          "index_projection": {
            "primary_key": true
        ...
        {
          "#operator": "Fetch",
          "cardinality": 740.8095238095237, // <2>
          "cost": 4047.042428571428,
          "keyspace": "travel-sample",
          "namespace": "default"
        },
        {
          ...
          "~child": {
            ...
            "~children": [
              {
                "#operator": "Filter",
                "cardinality": 16.542774045435976, // <3>
                "condition": "(((`travel-sample`.`country`) = \"United States\") and ((`travel-sample`.`type`) = \"hotel\"))",
                "cost": 4049.2648571428567
              },
              {
    ...
    "text": "select count(1) from `travel-sample` where country = 'United States' and type = 'hotel'"
  }
]
----
====
