...
For example, to execute <<ex-1>> below, you must have the _Query Select_ privilege on the `pass:c[`travel-sample`.inventory.route]` keyspace.
...
[horizontal]
*keyspace-ref*:: Refer to <<keyspace-ref>> below.
*options*:: Refer to <<infer-parameters>> below.
