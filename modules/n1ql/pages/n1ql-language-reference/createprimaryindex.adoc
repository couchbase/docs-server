...
Alternatively, if the keyspace is a named collection, the keyspace reference may be just an {identifiers}[identifier] that refers to the {logical-hierarchy}[collection name] of the keyspace.
...
After the index creation statement comes back, query `system:indexes` for the status of the index.
...
The output from `system:indexes` shows the `travel-sample-primary-index` in the pending state (`"state": "pending"`).
====
