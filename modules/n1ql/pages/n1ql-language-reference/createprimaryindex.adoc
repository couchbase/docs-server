...
[source,sqlpp]
----
CREATE PRIMARY INDEX ON airport USING GSI
...
====
This example creates a named primary index on the default collection in the default scope within the `travel-sample` bucket.
