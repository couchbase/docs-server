...
keyspace-ref:: [Required] Refer to <<keyspace-ref>>.

index-using:: [Optional] Refer to <<index-using>>.

index-with:: [Optional] Refer to <<index-with>>.

=== Checking for an Existing Index

The optional `IF NOT EXISTS` modifier enables you to check whether the specified primary index exists before creating it.
