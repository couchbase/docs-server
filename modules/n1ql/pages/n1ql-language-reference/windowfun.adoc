...
Window functions can only appear in the xref:n1ql-language-reference/selectclause.adoc[SELECT] projection clause or query xref:n1ql-language-reference/orderby.adoc[ORDER BY] clause.
If the query contains the xref:n1ql-language-reference/groupby.adoc[GROUP BY] clause or xref:n1ql-language-reference/aggregatefun.adoc[aggregate functions], any expressions that occur in window functions in the query must only depend on GROUP BY expressions or aggregate functions.
...
If required, use a xref:n1ql-language-reference/let.adoc[LET clause], a xref:n1ql-language-reference/with.adoc[WITH clause], or an intervening xref:n1ql-language-reference/subqueries.adoc[subquery] to avoid repeated evaluation.

NOTE: An expression within the window function may not contain another, nested window function.
If necessary, you can specify one window function in a xref:n1ql-language-reference/subqueries.adoc[subquery], and another window function in the parent query.

Window functions are processed after xref:n1ql-language-reference/from.adoc#section_ek1_jnx_1db[joins], the xref:n1ql-language-reference/let.adoc[LET] clause, the xref:n1ql-language-reference/where.adoc[WHERE] clause, and the xref:n1ql-language-reference/groupby.adoc[GROUP BY], xref:n1ql-language-reference/groupby.adoc[LETTING], and xref:n1ql-language-reference/groupby.adoc[HAVING] clauses.
...
[subs="normal"]
----
cume-dist-function ::= CUME_DIST '()' OVER '(' [ xref:n1ql-language-reference/window.adoc#window-partition-clause[window-partition-clause] ] xref:n1ql-language-reference/window.adoc#window-order-clause[window-order-clause] ')'
----
...
The objects are ordered by the xref:n1ql-language-reference/window.adoc#window-order-clause[window order clause].
...
[subs="normal"]
----
dense-rank-function ::= DENSE_RANK '()' OVER '(' [ xref:n1ql-language-reference/window.adoc#window-partition-clause[window-partition-clause] ] xref:n1ql-language-reference/window.adoc#window-order-clause[window-order-clause] ')'
----
...
=== Description
Returns the requested value from the first object in the current window frame, where the window frame is specified by  the xref:n1ql-language-reference/window.adoc#window-clause[window clause].

=== Syntax

[subs="normal"]
----
first-value-function ::= FIRST_VALUE '(' <<fn-window-first-value-args,expr>> ')' [ <<fn-window-first-value-nulls>> ] OVER '(' xref:n1ql-language-reference/window.adoc#window-clause[window-clause] ')'
----

image::n1ql-language-reference/fn-window-first-value.png[]

[[fn-window-first-value-args]]
=== Arguments
expr:: [Required] The value that you want to return from the first object in the window frame.

[[fn-window-first-value-nulls,nulls-treatment]]
==== Nulls Treatment
The xref:n1ql-language-reference/window.adoc#nulls-treatment[nulls treatment] clause determines how NULL or MISSING values are treated when finding the first object in the window frame:
...
If the xref:n1ql-language-reference/window.adoc#nulls-treatment[nulls treatment] clause is omitted, the default is `RESPECT NULLS`.
...
* If the xref:n1ql-language-reference/window.adoc#window-order-clause[window order clause] is omitted.

* If the window frame is defined by `ROWS`, and there are tied objects in the window frame.

To make the function return deterministic results, add a xref:n1ql-language-reference/window.adoc#window-order-clause[window order clause], or add further ordering terms to the xref:n1ql-language-reference/window.adoc#window-order-clause[window order clause] so that no objects are tied.
...
[subs="normal"]
----
lag-function ::= LAG '(' <<fn-window-lag-args,expr>> [ ',' <<fn-window-lag-args,offset>> [ ',' <<fn-window-lag-args,default>> ] ] ')' [ <<fn-window-lag-nulls>> ] OVER '(' [ xref:n1ql-language-reference/window.adoc#window-partition-clause[window-partition-clause] ] xref:n1ql-language-reference/window.adoc#window-order-clause[window-order-clause] ')'
----
...
[[fn-window-lag-args]]
=== Arguments
expr:: [Required] The value that you want to return from the offset object.

offset:: [Optional] A positive integer greater than 0.
If omitted, the default is 1.

default:: [Optional] The value to returned when the offset goes out of window scope.
If omitted, the default is NULL.

[[fn-window-lag-nulls,nulls-treatment]]
==== Nulls Treatment
The xref:n1ql-language-reference/window.adoc#nulls-treatment[nulls treatment] clause determines how NULL or MISSING values are treated when counting the offset:
...
=== Description
Returns the requested value from the last object in the current window frame, where the window frame is specified by  the xref:n1ql-language-reference/window.adoc#window-clause[window clause].

=== Syntax

[subs="normal"]
----
last-value-function ::= LAST_VALUE '(' <<fn-window-last-value-args,expr>> ')' [ <<fn-window-last-value-nulls>> ] OVER '(' xref:n1ql-language-reference/window.adoc#window-clause[window-clause] ')'
----

image::n1ql-language-reference/fn-window-last-value.png[]

[[fn-window-last-value-args]]
=== Arguments
expr:: [Required] The value that you want to return from the last object in the window frame.

[[fn-window-last-value-nulls,nulls-treatment]]
==== Nulls Treatment
The xref:n1ql-language-reference/window.adoc#nulls-treatment[nulls treatment] clause determines how NULL or MISSING values are treated when finding the last object in the window frame:
...
* If the xref:n1ql-language-reference/window.adoc#window-frame-clause[window frame clause] is omitted.
...
[subs="normal"]
----
lead-function ::= LEAD '(' <<fn-window-lead-args,expr>> [ ',' <<fn-window-lead-args,offset>> [ ',' <<fn-window-lead-args,default>> ] ] ')' [ <<fn-window-lead-nulls>> ] OVER '(' [ xref:n1ql-language-reference/window.adoc#window-partition-clause[window-partition-clause] ] xref:n1ql-language-reference/window.adoc#window-order-clause[window-order-clause] ')'
----
...
=== Description
Returns the requested value from an object in the current window frame, where the window frame is specified by  the xref:n1ql-language-reference/window.adoc#window-clause[window clause].

=== Syntax

[subs="normal"]
----
nth-value-function ::= NTH_VALUE '(' <<fn-window-nth-value-args,expr>> ',' <<fn-window-nth-value-args,offset>> ')' [ <<fn-window-nth-value-from>> ] [ <<fn-window-nth-value-nulls>> ] OVER '(' xref:n1ql-language-reference/window.adoc#window-clause[window-clause] ')'
----

image::n1ql-language-reference/fn-window-nth-value.png[]

[[fn-window-nth-value-args]]
=== Arguments
expr:: [Required] The value that you want to return from the offset object in the window frame.
...
==== Nth Val From
The xref:n1ql-language-reference/window.adoc#nthval-from[nth val from] clause determines where the function starts counting the offset.
...
[subs="normal"]
----
ntile-function ::= NTILE '(' <<fn-window-ntile-args,num_tiles>> ')' OVER '(' [ xref:n1ql-language-reference/window.adoc#window-partition-clause[window-partition-clause] ] xref:n1ql-language-reference/window.adoc#window-order-clause[window-order-clause] ')'
----
...
=== Example
====
For each country, allocate each airport to one of ten tiles.

[source,n1ql]
----
SELECT a.airportname, a.country, NTILE(10) OVER (
  PARTITION BY a.country
  ORDER BY a.faa
) AS `ntile`
FROM `travel-sample` AS a
...
[subs="normal"]
----
percent-rank-function ::= PERCENT_RANK '()' OVER '(' [ xref:n1ql-language-reference/window.adoc#window-partition-clause[window-partition-clause] ] xref:n1ql-language-reference/window.adoc#window-order-clause[window-order-clause] ')'
----
...
[subs="normal"]
----
rank-function ::= RANK '()' OVER '(' [ xref:n1ql-language-reference/window.adoc#window-partition-clause[window-partition-clause] ] xref:n1ql-language-reference/window.adoc#window-order-clause[window-order-clause] ')'
----
...
If the xref:n1ql-language-reference/window.adoc#window-frame-clause[window frame clause] is not specified, the fractional ratio is calculated for the whole window partition.

=== Syntax

[subs="normal"]
----
ratio-to-report-function ::= RATIO_TO_REPORT '(' <<fn-window-ratio-to-report-args,expr>> ')' OVER '(' xref:n1ql-language-reference/window.adoc#window-clause[window-clause] ')'
----

image::n1ql-language-reference/fn-window-ratio-to-report.png[]

[[fn-window-ratio-to-report-args]]
=== Arguments
expr:: [Required] The value for which you want to calculate the fractional ratio.
...
The xref:n1ql-language-reference/window.adoc#window-order-clause[window order clause] determines the sort order of the objects.
If the xref:n1ql-language-reference/window.adoc#window-order-clause[window order clause] is omitted, the return values may be unpredictable.

=== Syntax

[subs="normal"]
----
row-number-function ::= ROW_NUMBER '()'  OVER '(' [ xref:n1ql-language-reference/window.adoc#window-partition-clause[window-partition-clause] ] [ xref:n1ql-language-reference/window.adoc#window-order-clause[window-order-clause] ] ')'
----
