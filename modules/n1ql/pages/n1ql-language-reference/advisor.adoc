= ADVISOR Function
:description: The ADVISOR function provides index recommendations to optimize query response time.
...
|**index** +
__required__
|The {sqlpp} command used to define the index.
|string

|**statements** +
__required__
|An array of Statement objects, each giving information about one of the {sqlpp} input queries associated with this index.
...
|**run_count** +
__required__
|When the function is used with a single {sqlpp} input query, this is always 1.

When the function is used with an array of queries, or a collection of queries from a session, this is the number of times that this {sqlpp} input query occurs in the input array or session.
|integer

|**statement** +
__required__
|The {sqlpp} input query.
|string
|===
...
[source,sqlpp]
----
include::example$functions/advisor-purge.n1ql[]
----

.Result
[source,json]
----
include::example$functions/advisor-purge.jsonc[]
----
====
