...
When altering the number of replicas in this manner, the index service attempts to distribute the replicas based on the server groups in use in the cluster where possible.
If the value of this property is not less than the number of index nodes in the cluster, then the operation will fail.

nodes::
[Required] An array of strings, specifying a list of nodes.
If `action` is set to `move`, the node list determines the new destination nodes for the index and its replicas.
If `action` is set to `replica_count`, the node list restricts the set of nodes available for placement of replicas.

[NOTE]
.Moving an index
====
* When moving an index or index replica, the number of destination nodes must be the same as the number of nodes on which the index and any replicas are currently placed.
...
====
* When changing the number of replicas, the number of destination nodes must be greater than the replica count + 1.

* The range of destination nodes must include all of the nodes on which the index or index partitions and any index replicas are currently placed.
You cannot move an index or index replica and change the number of replicas at the same time.
The planner decides where to place any new index replicas on the nodes that you have specified, based on the server load.
====
...
====
Create an index on node 192.168.10.10 with replicas on nodes 192.168.10.*11* and 192.168.10.*12*, then increase the number of replicas to 4 including the extra nodes 192.168.10.*13* and 192.168.10.*14*.
