...
[subs="normal"]
----
alter-index ::= ALTER INDEX <<index-path>> [ <<index-using>> ] WITH <<options>>
----

image::n1ql-language-reference/alter-index.png["'ALTER' 'INDEX' index-path index-using? 'WITH' options"]
