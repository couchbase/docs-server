...
When this action is specified, you must use the `num_replica` property to specify the required number of replicas.
...
If `action` is set to `replica_count`, the node list restricts the set of nodes available for placement of the index and its replicas.
...
After all unhealthy replicas are dropped, the index service will if necessary drop replicas with the highest replicaId.
An unhealthy replica may not have the highest replicaId, so after an index reduction there may be "gaps" in the sequence of replicaIds -- for example, 1, 2, 4, where replicaId 3 was dropped.
...
ALTER INDEX `travel-sample`.country_idx
WITH {"action": "replica_count", "num_replica": 1, "nodes": ["192.168.10.10:8091", "192.168.10.11:8091", "192.168.10.12:8091"]}
----
====

.Reducing the number of nodes available for index and replica placement
====
...
If you then decide that you want to remove one node from consideration for index and replica placement, you need to remove the index (which removes all replicas) and then recreate the index with a different node set.
...
CREATE INDEX country_idx ON `travel-sample`(country, city)
WHERE type="route" USING GSI
WITH {"num_replica": 2, "nodes": ["192.168.10.10:8091", "192.168.10.11:8091", "192.168.10.12:8091"]};
----
