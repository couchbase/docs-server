...
You can use the `ALTER INDEX` statement to change the placement of an existing index or replica among different GSI nodes, to increase or decrease the number of replicas, or to drop a specified index replica.
...
nodes::
[Required if `action` is set to `move`; optional if `action` is set to `num_replica`] An array of strings, specifying a list of nodes.
