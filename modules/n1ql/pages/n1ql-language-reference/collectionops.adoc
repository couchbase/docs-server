...
[source,n1ql]
----
SELECT ARRAY v FOR v IN schedule, w IN schedule WHEN v.utc > "19:00" AND w.day = 5 END
...
[source,n1ql]
----
SELECT ARRAY v FOR v IN schedule WHEN v.utc > "19:00" AND v.day = 5 END
  AS fri_evening_flights
FROM `travel-sample`.inventory.route
WHERE airline="KL"
  AND sourceairport="ABQ"
...
[source,n1ql]
----
SELECT OBJECT "num_" || TOSTRING(i):v FOR i:v IN schedule WHEN v.day = 5 END
  AS fri_flights
FROM `travel-sample`.inventory.route
WHERE airline="KL"
  AND sourceairport="ABQ"
...
Existence tests enable you to test whether an array contains any elements.
