...
Range predicates (<<collection-op-any>> and <<collection-op-every>>), also known as _range conditions_ or _quantified expressions_, enable you to test a Boolean condition over elements in an array.
...
This condition may make use of the `var` identifier.
...
NOTE: `SOME` is a synonym for `ANY`.
...
Range transformations (<<array>> and <<first>>) enable you to map and filter elements and attributes from an input array.
They use the `IN` and `WITHIN` operators to range through the array.
...
This expression may make use of the `var` identifier.
