...
create-function-inline ::= CREATE [ OR REPLACE ] FUNCTION <<{section}-name,function>> '(' <<{section}-parameter,params>> ')'
                         [ IF NOT EXISTS ] ( '{' <<inline-expression,expr>> '}' | LANGUAGE INLINE AS <<inline-expression,expr>> )
----

image::n1ql-language-reference/create-function-inline.png["'CREATE' ( 'OR' 'REPLACE' )? 'FUNCTION' function '(' params? ')' ( 'IF' 'NOT' 'EXISTS' )? ( '{' expr '}' | 'LANGUAGE' 'INLINE' 'AS' expr )"]
