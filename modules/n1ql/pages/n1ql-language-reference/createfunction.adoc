...
* [def]_Inline functions_ are defined using {sqlpp} expressions, including subqueries.
...
function:: (Required) Refer to <<inline-name>> below.

params:: (Optional) Refer to <<inline-parameter>> below.

body:: (Required) Refer to <<inline-expression>> below.
...
function:: (Required) Refer to <<external-name>> below.

params:: (Optional) Refer to <<external-parameter>> below.

obj:: (Required) Refer to <<external-object>> below.

library:: (Required) Refer to <<external-library>> below.
...
// FIXME:
External functions in {sqlpp} _only_ support plain JavaScript, without any of the xref:eventing:eventing-language-constructs.adoc#added-lang-features[added language features] supported by functions in the Eventing Service.
...
This parameter is a string and must be wrapped in quotes.
You must create the JavaScript library and the JavaScript function using the {sqlpp} Functions REST API.
