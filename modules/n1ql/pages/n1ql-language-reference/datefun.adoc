...
| iso_year
| The ISO-8601 year.
The `iso_year` starts on a Monday and contains the first Thursday of the Gregorian calendar.
`iso_year` and `iso_week` are _not_ aligned with the Gregorian calendar.

Use `iso_week` and `iso_year` together for consistent results -- do not mix them with `week` and `year`.
| -
| -
| 2006
...
The first `iso_week` (number 1) of an `iso_year` starts on Monday and contains the first Thursday of the Gregorian calendar.
`iso_year` and `iso_week` are _not_ aligned with the Gregorian calendar.
