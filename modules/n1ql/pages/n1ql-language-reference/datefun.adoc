...
While Epoch/UNIX time can be represented in either seconds or milliseconds, _all N1QL date functions specifically treat Epoch/UNIX timestamps as milliseconds_.
...
N1QL accepts format strings following several conventions:
...
It is important for applications to be able to extract and manipulate particular components of a date, so that these can be used in N1QL queries.
