...
----
index-ref ::= ( <<index-type,index-type>> | <<use-index-args,index-name>> [ <<index-type,index-type>> ] )
----
...
or for any other query which takes advantage of the search service's natural-language capabilities.
For more information, refer to xref:n1ql-language-reference/flex-indexes.adoc[Full Text Search Indexes in N1QL].

When the index name is omitted, this clause is required.
When the index name is specified, this clause is optional; if this clause omitted, the default is `USING GSI`.
...
To qualify for this query, there must be an FTS index on state, city, and type, using the keyword analyzer.
