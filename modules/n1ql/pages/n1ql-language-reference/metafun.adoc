...
+
In Couchbase Server version 7.1.2 and later, the first argument may be also modified with `IGNORE MISSING`.
...
+
When the `IGNORE MISSING` modifier is used alongside the `ASC` or `DESC` modifier, the order of the modifiers does not matter.
