...
Prior to version 7.1.2, if the field denoted by `index-key` does not exist in the document,
then the document was not indexed.

For versions 7.1.2{plus}, the `INCLUDE MISSING` clause will ensure that documents which do not hold the `index-key` field are indexed regardless.
Documents without the `index-key` field will not be indexed
(The same behaviour for Couchbase Server versions prior to 7.1.2)

NOTE: The `INCLUDE` part of the clause can be left out.
