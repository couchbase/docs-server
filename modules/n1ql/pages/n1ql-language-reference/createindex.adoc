...
index-key:: [Required] Refer to <<index-key>> below.

index-order:: [Optional] Refer to <<index-order>> below.

index-partition:: [Optional] Refer to <<index-partition>> below.

where-clause:: [Optional] Refer to <<where-clause>> below.
...
The optional `IF NOT EXISTS` modifier enables you to check whether an index exists before creating it.
...
image::n1ql-language-reference/keyspace-ref.png["Refer to EBNF", align=left]
