...
In Couchbase Server versions prior to 7.1.2, if the field denoted by `index-key` does not exist in the document,
then the document is not indexed.

For Couchbase Server version 7.1.2 or later, the optional `INCLUDE MISSING` clause ensures that documents which do not hold the `index-key` field are indexed regardless.
If this clause is not present, then documents without the `index-key` field are not indexed.

The `INCLUDE` keyword is optional.
Omitting this keyword makes no difference to the operation of the `INCLUDE MISSING` clause.
...
For details, refer to <<index-partitioning>> below.
