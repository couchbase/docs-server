...
index-order:: [Optional] Specifies the sort order of the index key.
...
array-expr::
An array expression.
For details about array expressions, refer to <<array-indexing>> below.
...
.Create Index with Missing Leading Key [.status]#Couchbase Server 7.1.2#
====
Create an index with the `INCLUDE MISSING` clause.

[source, n1ql]
----
include::learn:example$services-and-indexes/indexes/create-idx-index-with-missing-field.n1ql[]
----

Documents will be indexed, even if the `district` field is not included in the document.
====
