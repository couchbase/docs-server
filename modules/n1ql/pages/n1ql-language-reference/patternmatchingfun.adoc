...
<1> The `REGEXP_SPLIT` function returns any zero-length matches that occur at the start of the _expression_ string, except when the split pattern is zero-length. Otherwise, it returns any zero-length matches immediately after a previous match.
====

<1> The `REGEXP_SPLIT` function returns any zero-length matches that occur at the start of the _expression_ string, or immediately after a previous match.
