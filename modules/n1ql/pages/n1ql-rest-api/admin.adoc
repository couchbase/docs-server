...
|Name|Description|Schema
|**auto-prepare** +
__optional__|Specifies whether the query engine should create a prepared statement every time a N1QL request is submitted, whether the PREPARE statement is included or not.
Refer to xref:n1ql-language-reference/prepare.adoc#auto-prepare[Auto-Prepare] for more information. +
**Default** : `false`|boolean
|**completed** +
__optional__|A nested object that sets the parameters for the completed requests catalog.
All completed requests that match these parameters are tracked in the completed requests catalog.
Refer to xref:manage:monitor/monitoring-n1ql-query.adoc#sys-completed-req[Configuring the Completed Requests] for more information and examples.|<<_completed_requests_logging_parameters,Completed requests logging parameters>>
|**completed-limit** +
__optional__|Sets the number of requests to be logged in the completed requests catalog.
...
|**completed-threshold** +
__optional__|All completed queries lasting longer than this threshold are logged in the completed requests catalog.
Refer to xref:manage:monitor/monitoring-n1ql-query.adoc#sys-completed-req[Configuring the Completed Requests] for more information and examples.|integer (ms)
|**controls** +
...
**Default** : `false`|boolean
|**distribute** +
__optional__|This field is only available with the POST method.
...
The actual value of this field is ignored.|boolean
|**keep-alive-length** +
...
This setting is provided for technical support only.|number
|**max-parallelism** +
...
[[_completed_requests_logging_parameters]]
**Completed requests logging parameters**

[options="header", cols=".^3,.^11,.^4"]
|===
|Name|Description|Schema
|**aborted** +
__optional__|If true, all requests that generate a panic are logged.|boolean
|**client** +
__optional__|The IP address of the client.
If specified, all completed requests from this IP address are logged.|string
|**context** +
__optional__|The opaque ID or context provided by the client.
...
If specified, all completed queries returning this error number are logged.|number
|**tag** +
__optional__|A unique string which tags a set of qualifiers.|string
|**threshold** +
__optional__|A duration.
If specified, all completed queries lasting longer than this threshold are logged.
This is another way of specifying the `completed-threshold` setting, as described in <<_settings>>.|number (ms)
|**user** +
__optional__|A user name, as given in the request credentials.
If specified, all completed queries with this user name are logged.|string
|===
