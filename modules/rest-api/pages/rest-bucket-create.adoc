...
This example enables `durabilityImpossibleFallback` for the bucket `testBucket` allowing durable writes to succeed even if there are not enough nodes for a majority.
...
This example restores the default behavior for the bucket `testBucket`, so that durable writes fail if there are not enough nodes to fulfill the majority requirement.
