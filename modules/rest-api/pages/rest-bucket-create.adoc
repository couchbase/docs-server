...
Sets the bucket's _maximum time to live_. The default value is `0`, which does not have documents automatically expire. It also does not change how change how any expiration values you directly set on a document.

Setting this parameter to a non-zero value has two effects:

* It sets a default expiration time for all documents contained in the bucket. When you create or mutate documents in the bucket, Couchbase Server sets them to expire after the number of seconds you set in `maxTTL`. The bucket's `maxTTL` value can be overridden by a `maxTTL` parameter set on the collection containing a document, or by directly setting an expiration on the document itself (except as explained in the next point). 

* It sets an upper limit on the expiration time you can directly set on a document. If you set a document to expire to a number of seconds greater than the bucket's `maxTTL` setting, Couchbase Server uses the `maxTTL` setting. As with setting a default expiration, a non-zero `maxTTL` setting on a document's collection overrides the bucket's `maxTTL` setting.
