...
Sets the number of threads for the bucket.
This parameter can be set to `3` (the default) or `8`.

If you provide a value other than `3` or `8`, Couchbase Server returns an error notification such as: `{"threadsNumber":"The number of threads must be an integer between 2 and 8"}`.
Couchbase Server ignores all values except `3` and `8`.

You can modify this parameter after creating the bucket.
When you change this setting, Couchbase Server restarts the bucket with the new value.
During the bucket's warm-up period, data may be inaccessible.
