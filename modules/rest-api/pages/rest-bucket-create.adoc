...
Note that parameters can be considered in three groups, as follows:

* Parameters that _must_ be specified on bucket creation.
This group includes:

** A parameter that _can_ be edited subsequently, which is xref:rest-api:rest-bucket-create.adoc#ramquotamb[ramQuotaMB].

** A parameter that _cannot_ be edited subsequently, which is xref:rest-api:rest-bucket-create.adoc#name[name].

* Parameters that _can_ be specified on bucket creation, but if not specified, acquire a default value.
This group includes:

** Parameters that _can_ be edited subsequently.

** Parameters that _cannot_ be edited subsequently.

* Parameters that relate specifically to _auto-compaction_.
These are _not_ required for _memory-optimized_ storage.
This group includes:

** Parameters that are valid for Couchbase Server Enterprise and Community Editions alike.

** Parameters that are valid for Couchbase Server Community Edition only.
