...
Parameters that support the creation and editing of buckets can be broken two groups: Genera and Auto-compaction.
...
Sets the lifetime of the bucket's data encryption keys (DEKs) in seconds.
Once this period passes after a DEK expires, Couchbase Server re-encrypts any data that's still encrypted the data using the active DEK.
...
Depending on how often you rotate the DEKs, this can lead to a large number of DEKs being kept. 
