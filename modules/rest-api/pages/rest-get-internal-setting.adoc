...
WARNING: Couchbase intends the settings on this page for performance tuning.
...
(optional) Sets the number of simultaneous transfers a node can participate in when performing a data service rebalance file-based backfill.
This limit applies to vBucket file transfers where the node is the source or target of the transfer.
See xref:learn:clusters-and-availability/rebalance.adoc#limiting-concurrent-vbucket-moves[Limiting Concurrent vBucket Moves].

`<internal-setting>`::
(optional) The name of any general internal setting, set to its new value.
For example, `rebalanceMovesPerNode`.
See xref:rest-api:rest-limit-rebalance-moves.adoc[Limiting Concurrent vBucket Moves] for more information about `rebalanceMovesPerNode`.
...
The following example sets `rebalanceMovesPerNode` to `4`:
...
Use the `GET` method to check the result:

[source,bash]
----
curl -X GET -u Administrator:password \
http://node1.example.com:8091/internalSettings | jq '.' | grep rebalanceMovesPer
----

This returns the following output, which confirms the change:
