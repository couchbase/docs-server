...
A pruning task is supported, which allows the removal of backups that are no longer required.
...
* The third, `PruneEveryWeek`, specifies that a prune occur every week, and that all builds that have occurred from the current day back to 7 days ago be included.
