...
The `doFlush` operation succeeds only if flush was configured either during the initial xref:clustersetup:bucket-setup.adoc#topic_jbt_4jn_vs[bucket setup] or after the xref:clustersetup:change-settings-bucket.adoc#topic_mfl_rmn_vs[bucket settings] have been changed.
