...
Internal users can only be specified on Couchbase Server Version 5.5.6 and later.
...
http://10.143.186.101:8091/settings/audit \
-d auditdEnabled=true \
-d disabled=20490,20491,20492 \
-d disabledUsers=jsmith/local,@eventing/local,@cbq-engine/local \
...
It specifies a list of filterable-event ids as `disabled`; and specifies one local user and two _internal users_ as `disabledUser`, ensuring filterable events from these users will not be audited.
(Note that _internal users_, such as `@cbq-engine`, cannot be specified on Couchbase Server 5.5 versions earlier than 5.5.6.)
It also specifies values for `rotateSize`, `rotateInterval`, and `logPath`.

== See Also

Instructions on managing auditing from the UI of Couchbase Web Console is provided in xref:security:security-auditing.adoc[Auditing].
A complete list of auditable events, in tabular form, is provided in xref:security:security-audit-events.adoc[Audit Events].
