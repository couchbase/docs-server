...
+
If `auditdEnabled` is `true`, and `disabled` is not specified, all filterable events are disabled.
...
It specifies a list of filterable-event ids as `disabled`; and specifies one local user and two internal users as `disabledUser`, ensuring that all but the default list of most important events from these users will not be audited.
