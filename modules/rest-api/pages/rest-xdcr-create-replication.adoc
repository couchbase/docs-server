...
The `conflictLogging` setting enables or disables logging true conflicts for the replication.
When enabled (`disabled=false`), you can specify the target bucket, scope, and collection for logging conflicts, and source collections of the replication.
This helps track and resolve document conflicts manually during replication.
...
For an example and explanation about using `conflictLogging` and `loggingRules`, see xref:xref:learn:clusters-and-availability/xdcr-conflict-logging-feature.adoc#example-special-loggingrules[Example for Special Logging Rules].
