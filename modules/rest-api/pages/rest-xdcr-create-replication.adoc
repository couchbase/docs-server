...
curl -v -X POST -u [admin]:[password]
  ...
  -d cLogPauseReplThreshold=[Integer]
  -d cLogMonitorDuration=[Integer]
...
The `conflictLogging` setting is used to configure and enable or disable logging conflicts for the replication.
For configuration information and examples, see xref:learn:clusters-and-availability/xdcr-conflict-logging-feature.adoc#configure-conflictlogging-settings[Enabling and Configuring Conflict Logging].

NOTE: To configure and enable Conflict Logging, you must enable the bucket property `enableCrossClusterVersioning` on all buckets of the XDCR topology.
This bucket property cannot be disabled after it is enabled.
...
The `conflictLogging` setting is used to configure and enable or disable logging conflicts for the replication.
For configuration information and examples, see xref:learn:clusters-and-availability/xdcr-conflict-logging.adoc[Enabling and Configuring Conflict Logging].

To configure and enable Conflict Logging, you must enable the bucket property `enableCrossClusterVersioning` on all buckets of the XDCR topology.
This bucket property cannot be disabled after it is enabled.
For information about the bucket property `enableCrossClusterVersioning`, see xref:rest-api:rest-bucket-create.adoc#xdcr-enablecrossclusterversioning[XDCR enableCrossClusterVersioning].
...
=== Autopause on Conflict Rate

The following example creates a continuous replication from travel-sample to ts on remote cluster 10.144.210.102, configuring XDCR to pause replication if 100 or more conflicts occur on any source node within a 60-second window:
...
-d toCluster=10.144.210.102 
-d toBucket=ts 
-d cLogPauseReplThreshold=100 
-d cLogMonitorDuration=60
----

If the command succeeds, a response containing the replication ID is returned:
