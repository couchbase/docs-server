...
|One or more certificate fingerprints that Couchbase Server can use to authenticate messages from the IdP. It uses these fingerprints instead of using a certificate embedded in the IdP's metadata. 
