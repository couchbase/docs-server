...
[[g-api-index]]GET /api/index:: Returns all index definitions as JSON.
+
...
[[g-api-index-name]]GET /api/index/\{indexName}:: Returns the definition of an index as JSON.
+
*Permission Required *: cluster.bucket[[.var]`bucket_name`].fts!read
+
*Parameters*
...
[[p-api-index-name]]PUT /api/index/\{indexName}:: Creates/updates an index definition.
 ...
 ** For indexType [.input]`alias`, an example [.param]`indexParams` JSON:
+
----
{
    "targets": {
 ...
 ** For indexType [.input]`blackhole`, the [.param]`indexParams` can be null.
 ** For indexType [.input]`bleve`, an example [.param]`indexParams` JSON:
+
----
{
    "mapping": {
...
For sourceType [.input]`couchbase`, an example sourceParams JSON:
...
[[d-api-index-name]]DELETE /api/index/\{indexName}:: Deletes an index definition.
...
[[p-api-idx-name-ingestcontrol]]POST /api/index/\{indexName}/ingestControl/\{op}:: Pause index updates and maintenance (no more ingesting document mutations).
+
...
[[p-api-idx-name-planfreezecontrol]]POST /api/index/\{indexName}/planFreezeControl/\{op}:: Freeze the assignment of index partitions to nodes.
...
[[p-api-idx-name-querycontrol]]POST /api/index/\{indexName}/queryControl/\{op}:: Disallow queries on an index.
...
[[g-api-stats]]GET /api/stats:: Returns indexing and data related metrics, timings and counters from the node as JSON.
+
...
[[g-api-stats-index-name]]GET /api/stats/index/\{indexName}:: Returns metrics, timings and counters for a single index from the node as JSON.
+
*Permission Required*: cluster.bucket[[.var]`bucket_name`].stats!read
+
.Sample response
...
[[g-api-index-name-count]]GET /api/index/\{indexName}/count:: Returns the count of indexed documents.
+
*Permission Required *: cluster.bucket[[.var]`bucket_name`].fts!read
+
*Parameters*
...
[[p-api-index-name-query]]POST /api/index/\{indexName}/query:: Queries an index.
