...
| clientTLSCert
| Path to a certificate to be used by Couchbase Server as a client, in authentication with LDAP.
The `serverCertValidation` flag must be set to `true`, and a value must be specified for the `clientTLSkey` flag.

| clientTLSkey
| Path to the private key that corresponds to Couchbase Server's client certificate, for use in authentication with LDAP.
The `serverCertValidation` flag must be set to `true`, and a value must be specified for the `clientTLSCert` flag.
