...
* `false` (default value): Index Servers rebuild any index that are newly assigned to them during a rebalance.
* `true`: Couchbase Server moves a reassigned index's files between Index Servers.  

// Remove after the 7.x series.
ifeval::['{page-component-version}' == '7.6'] 
NOTE: In versions 7.6.0 and 7.6.1, setting this parameter to `true` prevented you from using the `WITH <node>` clause of the `CREATE  INDEX` or `ALTER INDEX` statements to choose which Index Service node or nodes contains the index. 
Couchbase Server version 7.6.2 and later removes this restriction.
endif::[] 
| boolean
