...
The output from running the previous command on a database with one internal and one external user is:
...
Excludes users and groups from the backup. The backup skips any user or group that matches the `filterExpression`.  The `filterExpression` can be one of the following:
+
* `*`: excludes everything. 
* `admin`: excludes the Full Administrator
* `group:<groupWildcard>`: excludes group definitions whose name matches the `groupWildcard` wildcard expression. For example, `exclude=group:admin*` excludes any groups whose name starts with the string `admin`.
...
* `permission:<permissionExpression>`: excludes groups and users who have the permission matching `permissionExpression`. For example, `exclude=permission:cluster!backup_admin` excludes any user or group that has the backup admin permission at the database level.

include=<filterExression>::
Includes matching users and groups in the backup. Only users or groups that match the `filterExpression` are included. All users and groups that do not match the expression are excluded. The `filterExpression` can be one of the following:
+
* `*`: includes everything. 
...
* `permission:<permissionExpression>`: includes groups and users who have the permission in the `permissionExpression`. For example, `include=permission:cluster.bucket[travel-sample]!data_writer` includes any user or group that has data writer permissions on the `travel-sample` bucket.

The `exclude` and `include` parameters are mutually exclusive. You cannot use both in the same method call. However, you can have multiple instances of one of the parameters in the method. For example, you use two instances of the `include` parameter in a single method call. The resulting backup contains the users and groups that match either `include` parameter. 
...
Required parameter containing the URL encoded JSON backup data. 
canOverwrite=<true|false>::
Boolean value that controls whether the restore overwrites any existing users or groups in the database that also exist in the backup. Defaults to false, which means the restore does not overwrite any already-existing users or groups in the database.

.Example

The following example backs up all users and groups by calling the `GET` method on the `/settings/rbac/backup` endpoint. It saves the backup to a file. 
