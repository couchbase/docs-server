...
NOTE: In Couchbase Server 7.1.1 and later versions, if an existing user's password is to be changed, and their existing role-assignments are to be kept unchanged, the `/settings/rbac/users/local` URI can be used with the `PATCH` method: this allows the `password` parameter to be used, specifying a new password; and the `username` and `roles` parameters to be omitted.
...
[#create-a-local-user-and-assign-temporary-password]
==== Create a Local User, and Assign a Temporary Password
...
* Make sure that `password` and `temporaryPassword` conform to the settings in xref:rest-api:rest-set-password-policy.adoc[Set Password Policy].
...
* Specify the necessary roles in the `roles` flag.
Separate multiple instances of `role` with commas.
...
If the call is successful, it returns `200 OK`.
No object is returned.

For more information about forcing password change, see xref:manage:manage-security/manage-native-passwords.adoc#force-password-update[Force Password Update].

===== Edit a Local User, and Force Password Change
...
For more information about locking and unlocking user accounts, see the section xref:learn:security/authentication-overview.adoc#blocking-user-authentication[Blocking User Authentication] and xref:manage:manage-security/manage-users-and-roles.adoc#manage-user-account-locking-using-api[Manage User Account Locking Using the REST API].

===== View Locked or Unlocked User Accounts
...
[source,console]
----
include::rest-api:example$rbac-backup-users-groups.sh[tag=backup-all-but-admin]  
----
...
[source,console]
----
include::rest-api:example$rbac-backup-users-groups.sh[tag=backup-only-groups]  
----
...
[source,console]
----
include::rest-api:example$rbac-backup-users-groups.sh[tag=backup-local-user-and-admin]  
----
