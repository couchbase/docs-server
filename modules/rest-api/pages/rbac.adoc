...
It is useful if you want to transfer or synchronize users between different database clusters. 

=== Backup Users and Groups

Use the `GET` method with the  `/settings/rbac/backup` endpoint to backup all users and groups in the database, including the Full Administrator account. 
...
When called without parameters, the backup endpoint returns all of the users and groups defined in the database. 

.Example
The following example backs up all users and groups. 
----
include::rest-api:example$rbac-backup-users-groups.sh[tag=backup-everything]  
----

The next example shows the result of running the previous command on a database with one internal and one external user:

[source,json]
----
include::rest-api:example$rbac-backup-users-groups.sh[tag=backup-everything-output]  
----

IMPORTANT: The backup contains the hashed passwords for users defined in the local authentication domain. Be sure to keep the contents of the backup secure.

.Parameters

Use the `include` or `exclude` parameters to control what's included in the backup. 
You can include multiple parameters in the same method call. However, they must be all `include` or `exclude`. The two types of parameters are mutually exclusive. 

The `filterExpression` defines what the parameter includes or excludes from the backup. It can be one of the following:

* `*`: includes or excludes everything. 
* `admin`: includes or excludes the Full Administrator
* `group:<groupWildcard>`: includes or excludes group definitions whose name matches the `groupWildcard` wildcard expression. For example, `include=group:analyst*` includes only groups whose name starts with the string `analyst`.
* `user:<local|external|\*>:<userWildcard>`: includes or excludes users. The `local`, `external`, or `*` expression sets the authentication domain to which the argument applies. The `userWildcard` wildcard expression determines which usernames the backup includes or excludes. For example, `exclude=user:*:john*` excludes any user whose username starts with the string `john` in both the internal and external authentication domain.
* `permission:<permissionExpression>`: includes or excludes groups and users who have the permission in the `permissionExpression`. For example, `exclude=permission:cluster!backup_admin` excludes any user or group that has the backup admin permission at the database level.

The following example backs up all users defined in the local authentication domain and all groups.

[source,console]
----
include::rest-api:example$rbac-backup-users-groups.sh[tag=backup-local-users-groups]  
----

Running the previous command on the same database as the earlier example produces this output:

[source,json]
----
include::rest-api:example$rbac-backup-users-groups.sh[tag=backup-local-users-groups-output]  
----

=== Restore Users and Groups

Use the `PUT` method with the  `/settings/rbac/backup` endpoint to restore users and groups from a backup. If the call is successful, it returns a JSON report indicating the number of users and groups restored whether it skipped any.
...
----
 curl -X PUT -u <administrator>:<password> 
    http://<host>:<port>/settings/rbac/backup
    --data-urlencode 'backup=<backupJSONData>''
    -d canOverwrite=<true|false>
----

.Parameters

* `backup=<backupJSONData>`: the URL encoded JSON backup data. This parameter is required.
* `canOverwrite=<true|false>`: whether to overwrite existing users or groups. Defaults to false, which means the restore does not overwrite any already-existing users or groups in the database.

.Example

The following example calls the `GET` method on the `/settings/rbac/backup` endpoint, saving it to a file. 
Then it deletes the user with the username `user2` by calling the DELETE method (see  xref:##delete-users-and-groups[Delete Users and Groups]). 
Finally, it restores `user2` by calling the `PUT` method on the `/settings/rbac/backup`, passing it the backup file.

[source,console]
----
include::rest-api:example$rbac-backup-users-groups.sh[tag=backup-delete-restore-user]
----

The next example shows the output from running the previous commands:

[source,json]
----
include::rest-api:example$rbac-backup-users-groups.sh[tag=backup-delete-restore-user-output]
----

The following example demonstrates overwriting any user accounts amd groups in the database that are in the backup.

[source,console]
----
include::rest-api:example$rbac-backup-users-groups.sh[tag=restore-overwrite]
----
...
[source,json]
----
include::rest-api:example$rbac-backup-users-groups.sh[tag=restore-overwrite-output]
----
