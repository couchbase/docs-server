...
A _passphrase_ can be established, to control access to a node certificate's private key, by means of the `-d <json-passphrase-definition>` argument.
The value of `json-passphrase-definition` must be a JSON object.
...
The following example shows how to register a script in association with the private key, so that the passphrase is requested from the user:
