...
| `DATE`
| The parameter `first` returns only those tasks that started after the supplied date. 
| A datetime string in https://www.rfc-editor.org/rfc/rfc3339[RFC-3339] format

| `COUNT`
| The parameter `limit` returns the number of most recent tasks to return.
| Integer

| `TASK_NAME`
| The parameter `taskName` returns only those tasks where the name matches `TASK_NAME` including case.
