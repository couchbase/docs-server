...
* For information about using the `POST /controller/rebalance` REST API to redistribute services data and processing after failovers or node additions and removal, see xref:rest-api:rest-cluster-rebalance.adoc[Rebalancing the Cluster].
====
...
After the topology change and rebalance, two nodes that previously had no services using the index disk storage path will now have services using it.
