...
If this setting is `true`, a node is _not_ failed over if this would result in the loss of durably written data -- for example, if data has been durably written to two nodes, a maximum of one of those nodes can be automatically failed over.
