...
The remote cluster's update is recorded in a _version-vector_.
If the specified time-window expires without further modification being made by the remote cluster, information about the remote cluster's update is removed from the version-vector.
