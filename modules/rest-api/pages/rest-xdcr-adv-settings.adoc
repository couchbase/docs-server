...
----
curl -X POST -u Administrator:password http://localhost:8091/settings/replications/<settings_URI> -d mobile=Active | jq '.'
----
...
XDCR optimistic replication is applicable only when the bucket property `enableCrossClusterVersioning` is disabled.
