...
| `conflictLogging`
| disabled (true/false)
| Configuration settings for logging true conflicts.

This configuration setting defines objects/parameters and options used to control how conflicts are logged within the application.

This parameter uses a JSON object named `loggingRules` to define its values.
For more  information, see xref:learn:clusters-and-availability/xdcr-conflict-logging-feature.adoc#use-conflictlogging-and-loggingrules[Use conflictLogging and loggingRules].
