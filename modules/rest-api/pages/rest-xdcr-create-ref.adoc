...
To create a reference to a Cloud Native Gateway (CNG) target cluster, prefix the hostname with `couchbase2://` (for example, `couchbase2://<cng-hostname>`).
...
remoteType::
The connection type for the remote reference.
Returns "cng" for Cloud Native Gateway references or "cbcluster" for direct, node-to-node cluster references.

NOTE: The REST API response output does not retain the `couchbase2://` prefix in the `hostname` field. 
The connection type is identified by `"remoteType": "cng"`.
The Couchbase Web Console UI retains the `couchbase2://` prefix in the hostname to indicate that the reference connects via CNG.
...
The example supplies 3 URL-encoded values: the root certificate of the target cluster and a certificate and key for the client connection.
...
=== Create a Remote Reference to a Cloud Native Gateway (CNG) Target Cluster

The following example creates a fully secure remote reference to a Cloud Native Gateway (CNG) target cluster using the `couchbase2://` prefix in the hostname.
...
curl -u Administrator:password -X POST \
  ...
  -d "secureType=full" \
  --data-urlencode "certificate=$(cat ./cng-ca.pem)"
----
