...
When an issue arises, an alert is generated, and 1 or both of the following can be configured to occur:
...
[[maxdatadiskusedperc]]
* `maxDataDiskUsedPerc`. 
The percentage of disk space used that triggers an alert on the filesystem containing the data service storage path.
This alert warns you that the filesystem is becoming full.
...
It defaults to `75`.
If you enable the data disk usage limit setting, Couchbase Server ignores the value you set here.
Instead, it begins alerting you when disk use reaches within 10% of the disk use limit.
...
+
NOTE: If the node exceeds 90% of the available system connections, then contact https://support.couchbase.com[Couchbase Support].
...
If this period elapses and no progress has been made, Couchbase Server triggers an alert.
...
NOTE: For security reasons, the `pass` field within the `emailServer` sub-document is always returned as blank, irrespective of its actual setting.
...
== Configure Alert Settings via REST API


*Endpoint:* `POST /settings/alerts`

Configure email and Web Console alert notifications for cluster events, including XDCR replication deletions.
...
| Boolean
| `false`
| Whether email alerts are globally enabled on the cluster.

| `alerts`
| String (comma-separated)
| -
| Comma-separated list of alert identifiers to enable. Include `xdcr_replication_deleted` to receive alerts when an XDCR replication is deleted.
...
=== Example: Enable Alert for Deleted XDCR Replications via cURL

[source,bash]
----
curl -X POST -u Administrator:password \

 http://127.0.0.1:8091/settings/alerts \

 -d "enabled=true" \

 -d "emailHost=smtp.example.com" \

 -d "emailPort=25" \

 -d "emailUser=admin@example.com" \

 -d "recipients=devops@example.com,admin@example.com" \

 -d "alerts=auto_failover_node,xdcr_replication_deleted,disk_space_low"
----
