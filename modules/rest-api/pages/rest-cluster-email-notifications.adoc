...
curl -X POST http://<ip-address-or-domain-name:8091>/settings/alerts/limits
  ...
  -d crlExpirationDays=<integer>
  -d crlWarningValidityFraction=<integer>
...
* `crlExpirationDays`.
The number of days before a loaded Certificate Revocation List's (CRL's) `nextUpdate` that Couchbase Server starts warning that it will expire.
This parameter is optional.
The default value is `3`.
See xref:learn:security/certificate-revocation.adoc[Certificate Revocation] for more information.

* `crlWarningValidityFraction`.
Limits the CRL-expiration warning period to `1/N` of each CRL's own validity period, so short-lived CRLs (for example, those issued daily) do not trigger constant warnings.
A value of `0` removes the limit.
This parameter is optional.
The default value is `4`.
For example, a CRL valid for 24 hours, with the defaults, raises the warning only in its last 6 hours (24 / 4), not for its whole lifetime.
