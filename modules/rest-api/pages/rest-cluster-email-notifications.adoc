...
+
NOTE: If the node exceeds 90% of the available system connections, then contact xref:https://support.couchbase.com[Couchbase Support].
...
Note that for security reasons, the `pass` field within the `emailServer` sub-document is always returned as blank, irrespective of its actual setting.
