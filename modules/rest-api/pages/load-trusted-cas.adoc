...
Loads trusted certificates that reside on the accessed node into the Couchbase-Server cluster.
...
Loaded _intermediate_ certificates are provided to clients when secure client-access is attempted, so that the node's chain of authority can be inspected.
