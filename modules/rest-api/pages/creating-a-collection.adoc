...
This establishes whether a record of changes made to documents within the collection should be created and written to disk continuously.
This parameter is ignored unless the value of `storageBackend` is specified as `magma`, when the bucket is created (see xref:rest-api:rest-bucket-create.adoc[Creating and Editing Buckets]).
...
The following call modifies the definition of the collection `my_collection_in_my_scope`; specifying that a change-record should be maintained:
