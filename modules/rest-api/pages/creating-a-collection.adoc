...
This establishes whether a _change history_ is to be created for the collection.
If the value is `true`, a change history _is_ created; and if the value is `false`, a change history is _not_ created.
This parameter is ignored unless the following settings have been made for the bucket within which the collection resides:
...
This parameter can be used to override the bucket parameter `historyRetentionCollectionDefault`, which establishes a default setting for all collections in the bucket.
Thus, if the value of `historyRetentionCollectionDefault` is `false`, and therefore, by default, no change history will be created for any collection; an exception is made for any collection whose `history` setting is `true`.
