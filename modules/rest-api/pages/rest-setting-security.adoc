...
Time and memory parameters for `argon2id`, used only if `argon2id` is specified for Data-Service password hashing, as the value of `passwordHashAlg`.
For information on these parameters, see https://www.ory.sh/choose-recommended-argon2-parameters-password-hashing/#argon2s-cryptographic-password-hashing-parameters[Choose Argon2 Parameters for Secure Password Hashing and Login^].
...
* `scramShaIterations`.
Iterations parameter of `scram-sha1` hashes, used only if `scramSha256Enabled` is specified for Data-Service password hashing, as the value of `passwordHashAlg`.
Specified in seconds.
Default is `15000`
...
Specified in seconds.
Default is `1800000` (20 days and 20 hours).

* `validateNodeCertSan`.
Whether _Subject Alternative Name_ should be used in cluster-certificate validation.
Value can be `true` (the default) or `false.`
Only to be set globally.
