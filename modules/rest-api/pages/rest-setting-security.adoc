...
* Pause Eventing functions on the node (see xref:eventing:eventing-lifecycle.adoc#exercise-the-eventing-lifecycle[Exercise the Eventing Lifecycle]).
...
* Ensure XDCR replications are configured as fully secure (see xref:manage:manage-xdcr/enable-full-secure-replication.adoc[Enable Fully Secure Replications]).
