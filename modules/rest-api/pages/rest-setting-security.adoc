...
The `honorCipherOrder` parameter is _not_ specified, meaning that it retains its default of `true`; which ensures that the cluster's own cipher-suites preference is used, rather than the client's.
...
If the call is successful, and is made prior to any explicit settings made by the administrator, the output is as follows:
