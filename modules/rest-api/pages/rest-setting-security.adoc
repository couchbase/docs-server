...
This parameter, which is available only in 7.1.1 and later versions, is described separately, in xref:rest-api:rest-specify-node-addition-conventions.adoc[Restrict Node-Addition].
This parameter can only be set globally.
...
+
Note that for Couchbase Server Version 7.6 and later, disablement of all three SCRAM/SHA protocols is _recommended_: see xref:learn:security/native-authentication.adoc#authentication-for-applications[Native Authentication for Applications].
...
See xref:learn:security/native-authentication.adoc#password-hash-migration[Automatic Password Hash Migration] for more information.
+
