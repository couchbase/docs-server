...
Once a node is added, a user can optionally configure the services that are running on that node as described in <<concept-mds>>.
It should be noted that configuring those services does negate the behavior described in the previous paragraph.
...
image::architecture/server-cluster-single-node-type.png[,570]
...
The managed caching layer within the indexing service is built into the underlying storage engine.
...
For more information on buckets, see the blog https://blog.couchbase.com/multi-tenancy-couchbase-server[Multi-tenancy with Couchbase Server^].
...
Through the Couchbase SDKs the application automatically and transparently distributes the data and workload across these vBuckets.

For more information on vBuckets, see the technical white paper available http://www.couchbase.com/binaries/content/assets/website/docs/whitepapers/technical-whitepaper-couchbase-server-vbuckets.pdf[here^].

== Database Change Protocol

Database Change Protocol (DCP) is a high-performance streaming protocol that communicates the state of the data using an ordered change log with sequence numbers.
...
DCP also feeds most connectors, which are integrations with external systems such as Hadoop, Kafka or Elasticsearch.
