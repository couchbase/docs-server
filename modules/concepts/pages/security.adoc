...
xref:security:concepts-rba.adoc[RBAC for Administrators] requires xref:security:security-ldap-new.adoc[configuration with LDAP]so that the administrator's user ID in LDAP can be mapped by the Full Administrator to one or more fixed administrative roles.
