...
Line three indicates whether changes made to documents in the collection are _deduplicated_ when a historical record is maintained: if the value is `true` (the default), deduplication is occurring (note that the value can only be `true` when _Magma_ has been configured as the storage engine for the bucket, and when `historyRetentionCollectionDefault` for the bucket is `true`: see xref:rest-api:rest-bucket-create.adoc[Creating and Editing Buckets]).
