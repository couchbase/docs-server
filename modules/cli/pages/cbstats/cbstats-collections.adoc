...
Line one displays, in _bytes_, the total memory used by this collection (here, `22137956`).
Line two displays the total disk space occupied by this collection for the specified bucket, on the specified node: this total (here, `2161511`) is, again, expressed in _bytes_.

Line three indicates whether changes made to documents in the collection are _deduplicated_ when a historical record is maintained: if the value is `true`, deduplication is _off_.
(Note that the value can only be `true` when _Magma_ has been configured as the storage engine for the bucket, and when `historyRetentionCollectionDefault` for the bucket is `true`: see xref:rest-api:rest-bucket-create.adoc[Creating and Editing Buckets].
...
Line four displays the number of items within the collection (here, `3273`).
