...
You can collect diagnostics through the command-line interface by using either the xref:cli:cbcli/couchbase-cli.adoc#cbcli-intro[[.cmd]`couchbase-cli`] couchbase-cli tool or the xref:cbcollect-info-tool.adoc#cbcollect-info-tool[[.cmd]`cbcollect_info`] cbcollect_info tool.
