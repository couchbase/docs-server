...
For convenience, Couchbase provides Server developer tools package and Server admin tools package.

=== Server Developer Tools Packages

Server developer tools package contains the following utilities:
...
* xref:cbcli:couchbase-cli.adoc[`couchbase-cli`]
* cbstats
* mctimings
* mcstat

This package lets you install the following Server developer tools on systems where you have not installed Couchbase Server.
...
=== Server Admin Tools Packages

Server admin tools package contains the following utilities:
...
This package lets you install the following Server admin tools on systems where you have not installed Couchbase Server.
...
=== For Server Developer Tools Packages

The {tools-ver} version of `cbimport`, `cbexport`, `cbbackupmgr`, `cbq`, `couchbase-cli`, `mctimings`, `cbstats`, and `mcstat` utilities are compatible with the following Couchbase Server versions:
...
The 7.6.2 version of `cbimport`, `cbexport`, `cbbackupmgr`, and `cbq` utilities are compatible with the following Couchbase Server versions:
...
So, you can download and use the latest version of these utilities (`cbimport`, `cbexport`, `cbbackupmgr`, `cbq`, `couchbase-cli`, `mctimings`, `cbstats`, and `mcstat`) with the previous Couchbase Server versions.

=== For Server Admin Tools Packages

The {tools-ver} version of `cbimport`, `cbexport`, `cbbackupmgr`, `cbq`, `couchbase-cli`, `cbstats`, `cbdatarecovery`, `mctimings`, `mcstat`, and `mctestauth` utilities are compatible with the following Couchbase Server versions:
...
NOTE: Determine the version of any Server admin tools package utility by using the `--version` option, except for `cbc`. Determine the `cbc` utility version by using `cbc version` because `couchbase-cli` is a utility for managing/administering the Couchbase cluster. So, the version of `couchbase-cli` and the Couchbase cluster must match. For all the Server admin tools package utilities, any existing feature in a higher Server version is not supported by a lower version utility.
