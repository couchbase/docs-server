...
The conflict resolution behavior is the same as cross datacenter replication (XDCR), which is detailed in
xref:learn:clusters-and-availability/replication-architecture.adoc#xdcr-conflict-resolution[Availability].
...
To restore from compressed backups, use
`cbbackupmgr`,
which is available for Couchbase Server Enterprise Edition only.
See
