...
Couchbase Server has an optimized xref:architecture:db-engine-architecture.adoc[disk warmup].
...
Couchbase Server does lazy xref:concepts:data-management.adoc[expiration], that is, expired items are flagged as deleted rather than being immediately erased.
...
xref:architecture:db-engine-architecture.adoc[Ejection] means that documents are removed from RAM but the key and metadata remain.
