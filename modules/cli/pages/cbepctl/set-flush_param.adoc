...
Note also that the scanner shares its thread pool with the Index Service; and may therefore adversely affect the performance of any simultaneous index-updates: if instances of this problem occur, the scanner should be either run with less frequency, or disabled.
