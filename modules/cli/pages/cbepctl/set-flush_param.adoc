...
Couchbase Server has an optimized xref:architecture:db-engine-architecture.adoc#concept_b5n_bwn_vs[disk warmup].
...
Couchbase Server does lazy xref:concepts:data-management.adoc#concept_ukb_wgg_ps[expiration], that is, expired items are flagged as deleted rather than being immediately erased.
...
xref:architecture:db-engine-architecture.adoc#concept_b5n_bwn_vs[Ejection] means that documents are removed from RAM but the key and metadata remain.
