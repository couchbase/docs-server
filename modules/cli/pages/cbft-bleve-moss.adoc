...
* `-c`, `--count` `int`.
The number of terms within the index partition that are to be checked.
The default is `100`.

* `-f`, `--field` `string`.
A field to which command-execution should be restricted.
The default is that the command is executed on _all_ fields.
...
Thus, the list of fields to be checked is printed first; then, each field and the number of terms it contains is listed, along with the number of problems encountered with the field during checking.
