= mctimings
:description: pass:q[The [.cmd]`mctimings` tool displays end-to-end timing information for all operations.]
...
If this flag is used, one or more operation names or associated hexadecimals can be specified; so as to limit output to that which corresponds to those operations.
Hexadecimals must be specified within _back-ticks_.
If this flag is not used, then only the total number of operations are displayed.
|===
...
To get timing histograms for all operations on host _localhost_ for the bucket `travel-sample`:

----
mctimings -h localhost:11210 -u Administrator -P password -b travel-sample -v
----
...
To get timing histograms for just GET operations in the bucket `travel-sample` by specifying the operation name:

----
mctimings -h localhost:11210 -u Administrator -P password -b travel-sample -v GET
----

The same can also be achieved by specifying the hexadecimal opcode of the operation.
Note that the hexadecimal must be specified within _back-ticks_:

----
mctimings -h localhost:11210 -u Administrator -P password -b travel-sample -v `0x00`
----
...
An example of getting the timings of GETs and SETs from the bucket `travel-sample`:

----
mctimings -h localhost:11210 -u Administrator -P password -b travel-sample -v GET SET
----
