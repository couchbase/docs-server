...
If both the `-v` and the `-j` options are specified, the `-v` option is ignored, and the `-j` option is used.

If run with the `-v` option, and with no operations explicitly specified, `mctimings` returns a timing-histogram for all  operations performed during the sampling period.
If run with the `-v` option, and with one or more operations explicitly specified, `mctimings` returns a timing-histogram for each of the explicitly specified operations, as performed during the sampling period.

If run with the `-j` option, and with no operations explicitly specified, `mctimings` returns JSON output for all  operations performed during the sampling period.
If run with the `-j` option, and with one or more operations explicitly specified, `mctimings` returns JSON output for each of the explicitly specified operations, as performed during the sampling period.
...
| `-v` or `--verbose`
| Specifies that verbose output should be displayed: meaning that operation timings are displayed as histograms, in addition to totals.
If specific operation names are provided, output is limited to these operations, as they were performed during the sample period.
Otherwise, output covers all operations so performed.
...
If the `-j` (or `--json`) option is used in addition to `-v`, the `-v` option is ignored; and output is provided as JSON, rather than as histograms.
...
| `-f` or `--file path.json`
| Display as histogram data the contents of a JSON file created by prior use of the `-j` (or `--json`) option.
...
-j -a DCP_OPEN > /home/vagrant/dcpopen.json
----

The JSON contents of the file can now be inspected as follows.
Note that the https://stedolan.github.io[jq^] command is used, to facilitate readability.

----
more /home/vagrant/dcpopen.json | jq
----
