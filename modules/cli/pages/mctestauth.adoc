...
The `mctestauth` tool allows you to troubleshoot authentication issues in data services. Data services use the [Simple Authentication and Security Layer](https://en.wikipedia.org/wiki/Simple_Authentication_and_Security_Layer) (SASL) framework for authentication. In addition to its own SASL mechanisms, the data services allow the usage of any underlying mechanisms supported by SASL. For example, SCRAM-SHA512 or PLAIN. 
