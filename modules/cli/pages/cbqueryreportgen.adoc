...
The `-k, --keyspace` flag specifies the keyspace. The keyspace is the AWR repository in the format `bucket.scope.collection`. Example: `travel-sample._default.awr`.

The `-t1` and `-t2` flags specify the start-time and end-time for the report. You can enter both start-time and end-time as `t1,t2` in the `YYYY-MM-DDTHH:MM:SS` format. Example: `2025-09-01T00:00:00,2025-09-02T00:00:00`.
